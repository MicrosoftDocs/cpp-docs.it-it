...
Se, inoltre, un carattere contenuto nell'argomento richiede in genere una sequenza di escape quando viene usato in un valore letterale stringa, ad esempio`"`, il carattere virgolette () o barra rovesciata (`\`), la barra rovesciata di escape necessaria è automaticamente inserito prima del carattere.

L'operatore C++ Microsoft per non si comporta correttamente quando viene usato con stringhe che includono sequenze di escape. In questa situazione, il compilatore genera l' [errore del compilatore C2017](../error-messages/compiler-errors-1/compiler-error-c2017.md).
