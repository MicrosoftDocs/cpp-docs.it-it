...
Per la funzione `__isb` intrinseca, l'unica restrizione attualmente valida è _ARM64_BARRIER_SY; tutti gli altri valori sono riservati dall'architettura.
...
## <a name="arm64-support-for-intrinsics-from-other-architectures"></a><a name="I"></a> Supporto di ARM64 per le funzioni intrinseche di altre architetture
...
|_bittest64|_bittest64 char senza segno (__int64 const \* , __int64)|
...
|_disable|void __cdecl _disable (void) **Nota:**  sulle piattaforme arm64 questa funzione genera l'istruzione, ma è `MSR DAIFCLR,#2` disponibile solo come funzione intrinseca.|
|_enable|void __cdecl _enable (void) **Nota:**  sulle piattaforme arm64 questa funzione genera l'istruzione, ma è `MSR DAIFSET,#2` disponibile solo come funzione intrinseca.|
