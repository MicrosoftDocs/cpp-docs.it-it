...
Il formato del nome delle impostazioni locali è preferenziale, ad esempio usare `en-US` per la lingua inglese (Stati Uniti) o `bs-Cyrl-BA` per la lingua bosniaca (Cirillico, Bosnia ed Erzegovina). Il set dei nomi delle impostazioni locali è descritto nell'articolo relativo ai [nomi delle impostazioni locali](/windows/desktop/Intl/locale-names). Per un elenco dei nomi di impostazioni locali supportate in base alla versione del sistema operativo Windows, vedere la colonna [Language tag](https://msdn.microsoft.com/library/cc233982.aspx) (Tag di lingua) della tabella in **Appendix A: Product Behavior** (Appendice A: comportamento del prodotto) in "[MS-LCID]: Windows Language Code Identifier (LCID) Reference" (Informazioni di riferimento sugli identificatori di lingua (LCID) di Windows). Sono elencate le parti lingua, script e paese supportate dei nomi delle impostazioni locali. Per informazioni sui nomi delle impostazioni locali supportati che presentano ordinamenti non predefiniti, vedere la colonna dei **nomi delle impostazioni locali** nell'articolo relativo agli [identificatori di ordinamento](/windows/desktop/Intl/sort-order-identifiers). In Windows 10 o versioni successive, sono consentiti i nomi delle impostazioni locali che corrispondono a tag di lingua BCP-47 validi. Ad esempio, `jp-US` è un tag BCP-47 valido, ma in effetti è solo `US` per le funzionalità delle impostazioni locali.
