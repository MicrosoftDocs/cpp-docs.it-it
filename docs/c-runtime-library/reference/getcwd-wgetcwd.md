...
**wgetcwd** è una versione a caratteri wide **getcwd**; il *buffer* argomento e il valore restituito di **wgetcwd** sono stringhe a caratteri "wide". **wgetcwd** e **getcwd** si comportano in modo identico in caso contrario.

Quando si **debug** e **CRTDBG_MAP_ALLOC** sono definiti, le chiamate a **getcwd** e **wgetcwd** vengono sostituiti dalle chiamate a **_ getcwd_dbg** e **wgetcwd_dbg** per consentire il debug delle allocazioni di memoria. Per altre informazioni, vedere [_getcwd_dbg, _wgetcwd_dbg](getcwd-dbg-wgetcwd-dbg.md).
