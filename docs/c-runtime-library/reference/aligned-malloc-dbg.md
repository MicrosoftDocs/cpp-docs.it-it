...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "_aligned_malloc_dbg (funzione)"
  - "aligned_malloc_dbg (funzione)"
...
## Note  
 `_aligned_malloc_dbg` è una versione di debug della funzione [\_aligned\_malloc](../../c-runtime-library/reference/aligned-malloc.md).  Quando [\_DEBUG](../../c-runtime-library/debug.md) non è definito, ogni chiamata a `_aligned_malloc_dbg` viene ridotta a una chiamata a `_aligned_malloc`.  Sia `_aligned_malloc` che `_aligned_malloc_dbg` allocano un blocco di memoria nell'heap di base, ma `_aligned_malloc_dbg` contiene diverse funzionalità di debug: buffer presenti a entrambi i lati della porzione utente del blocco in cui verificare la presenza di perdite e informazioni su `filename`\/`linenumber` per determinare l'origine delle richieste di allocazione.  
