...
Il **dup** e **_dup2** funzioni associare un secondo descrittore del file a un file attualmente aperto. Queste funzioni possono essere utilizzate per associare un descrittore di file predefiniti, ad esempio quelle per **stdout**, con un altro file. Le operazioni sul file possono essere eseguite utilizzando qualsiasi descrittore del file. Il tipo di accesso consentito per il file non viene influenzato dalla creazione di un nuovo descrittore. **dup** restituisce il descrittore di file disponibili successivo per il file specificato. **_dup2** forza *fd2* per fare riferimento allo stesso file come *fd1*. Se *fd2* è associato a un file aperto al momento della chiamata, tale file viene chiuso.

Entrambi **dup** e **_dup2** accettano descrittori del file come parametri. Per passare un flusso (**FILE \*** ) a una di queste funzioni, utilizzare [fileno](fileno.md). Il **fileno** routine restituisce il descrittore di file attualmente associato al flusso specificato. Nell'esempio seguente viene illustrato come associare **stderr** (definito come **FILE \***  in stdio. h) con un descrittore di file:
