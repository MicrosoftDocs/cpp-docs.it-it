...
ms.locfileid: "32403960"
---
# <a name="gmtime-gmtime32-gmtime64"></a>gmtime, _gmtime32, _gmtime64

Converte un **time_t** tempo valore a un **tm** struttura. Sono disponibili versioni più sicure di queste funzioni. Vedere [gmtime_s, _gmtime32_s, _gmtime64_s](gmtime-s-gmtime32-s-gmtime64-s.md).
...
Puntatore a una struttura di tipo [tm](../../c-runtime-library/standard-types.md). I campi della struttura restituita presentano il valore valutato del *sourceTime* argomento in formato UTC anziché nell'ora locale. Ognuno dei campi della struttura è di tipo **int**, come segue:
...
|**tm_isdst**|Restituisce sempre 0 per **gmtime**.|
...
**_gmtime64**, che usa la **__time64_t** struttura, abilita le date relativamente a 23:59:59, 31 dicembre 3000 UTC, mentre **_gmtime32** rappresentano solo una data a 23:59:59 18 gennaio 2038, UTC. La mezzanotte del 1 gennaio 1970 è il limite inferiore dell'intervallo di date per entrambe le funzioni.

**gmtime** è una funzione inline che restituisce **_gmtime64**, e **time_t** equivale a **__time64_t** a meno che non **_USE_32BIT_TIME_ T** è definito. Se è necessario forzare il compilatore a interpretare **time_t** come precedente 32 bit **time_t**, è possibile definire **_USE_32BIT_TIME_T**, ma determina **gmtime** essere allineati al **_gmtime32** e **time_t** deve essere definito come **__time32_t**. Questa operazione non è consigliabile perché non è consentita nelle piattaforme a 64 bit e in ogni caso potrebbero verificarsi errori per l'applicazione dopo il 18 gennaio 2038.
...
Il **_gmtime32** funzione suddivide il *sourceTime* valore e lo archivia in una struttura allocata in modo statico di tipo **tm**, definito in fase di. H. Il valore di *sourceTime* in genere viene ottenuto da una chiamata ai [tempo](time-time32-time64.md) (funzione).
