...
> [!IMPORTANT]
> **_mbsncpy** e **_mbsncpy_l** non possono essere utilizzati nelle applicazioni eseguite in Windows Runtime. Per altre informazioni, vedere [Funzioni CRT non supportate nelle app della piattaforma UWP (Universal Windows Platform)](../../cppcx/crt-functions-not-supported-in-universal-windows-platform-apps.md).
...
La funzione **strncpy** copia i caratteri di *conteggio* iniziale di *strSource* in *strDest* e restituisce *strDest*. Se *count* è minore o uguale alla lunghezza di *strSource*, un carattere null non viene aggiunto automaticamente alla stringa copiata. Se *count* è maggiore della lunghezza di *strSource*, la stringa di destinazione viene riempita con caratteri null fino al *numero*di lunghezza . Il comportamento di **strncpy** non è definito se le stringhe di origine e di destinazione si sovrappongono.

> [!IMPORTANT]
> **strncpy** non verifica la presenza di spazio sufficiente in *strDest*; questo lo rende una potenziale causa di sovraccarichi del buffer. L'argomento *count* limita il numero di caratteri copiati; non è un limite alla dimensione di *strDest*. Vedere l'esempio seguente. Per altre informazioni, vedere [Evitare sovraccarichi del buffer](/windows/win32/SecBP/avoiding-buffer-overruns).
...
**wcsncpy** e **_mbsncpy** sono versioni a caratteri wide e a caratteri multibyte di **strncpy**. Gli argomenti e il valore restituito di **wcsncpy** e **_mbsncpy** variano di conseguenza. In alternativa queste sei funzioni si comportano in modo identico.
...
> [!NOTE]
> **_strncpy_l** e **_wcsncpy_l** non hanno alcuna dipendenza delle impostazioni locali; sono forniti solo per **_tcsncpy_l** e non sono destinati ad essere chiamati direttamente.
...
Nell'esempio seguente viene illustrato l'utilizzo di **strncpy** e come può essere utilizzato in modo improprio per causare bug di programma e problemi di sicurezza. Il compilatore genera un avviso per ogni chiamata a **strncpy** simile a **crt_strncpy_x86.c(15) : avviso C4996: 'strncpy': questa funzione o variabile potrebbe non essere sicura. Considerare invece l'utilizzo di strncpy_s. Per disabilitare la deprecazione, utilizzare _CRT_SECURE_NO_WARNINGS. Per informazioni dettagliate, vedere** la Guida in linea.
