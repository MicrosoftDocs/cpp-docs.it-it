...
Ognuna di queste funzioni restituisce un puntatore al file aperto. Un valore di puntatore Null indica un errore. Se *filename* oppure *modalità* è **NULL** o una stringa vuota, queste funzioni attivano il gestore di parametri non validi, come descritto in [parametro Convalida](../../c-runtime-library/parameter-validation.md). Se l'esecuzione può continuare, queste funzioni restituiscono **NULL** e impostare **errno** al **EINVAL**.
...
Il **fopen** funzione si apre il file specificato da *filename*. Per impostazione predefinita, un "narrow" *filename* stringa viene interpreta usando la codepage ANSI (CP_ACP). Nelle applicazioni per Windows Desktop può essere modificata nella codepage OEM (CP_OEMCP) usando la funzione [SetFileApisToOEM](https://msdn.microsoft.com/library/windows/desktop/aa365534\(v=vs.85\).aspx) . È possibile usare il [AreFileApisANSI](https://msdn.microsoft.com/library/windows/desktop/aa363781\(v=vs.85\).aspx) funzione per determinare se *filename* viene interpretato con ANSI o la tabella codici OEM predefinita di sistema. **wfopen** è una versione a caratteri wide **fopen**; gli argomenti da **wfopen** sono stringhe a caratteri "wide". In caso contrario, **wfopen** e **fopen** si comportano in modo identico. Usando solo **wfopen** non influisce sul set di caratteri codificati usato nel flusso di file.

**fopen** accetta percorsi validi nel file system al momento dell'esecuzione; **fopen** accetta percorsi UNC e percorsi che coinvolgono le unità di rete mappate fino a quando il sistema che esegue il codice abbia accesso alla condivisione o un'unità mappata al momento dell'esecuzione. Quando si creano i percorsi per **fopen**, verificare che le unità, i percorsi o le condivisioni di rete saranno disponibili nell'ambiente di esecuzione. È possibile usare barre (/) o barre rovesciate (\\) come separatori di directory in un percorso.

Controllare sempre il valore restituito per verificare se il puntatore è NULL prima di eseguire qualsiasi altra operazione sul file. Se si verifica un errore, la variabile globale **errno** è impostata e può essere usata per ottenere informazioni specifiche sull'errore. Per altre informazioni, vedere [errno, _doserrno, _sys_errlist, and _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md) (errno, _doserrno, _sys_errlist e _sys_nerr).
...
> **FILE *fp = fopen ("nuovofile", "rt + ccs =**_codifica_**");**
...
Se il file esiste già e viene aperto per la lettura o l'aggiunta, l'indicatore ordine byte (BOM), se presente nel file, determina la codifica. La codifica di indicatore ordine byte ha la precedenza sulla codifica specificata dal **ccs** flag. Il **ccs** codifica viene utilizzata solo quando nessun indicatore ordine byte è presente o il file è un nuovo file.

> [!NOTE]
> Rilevamento indicatore ordine byte si applica solo ai file aperti in modalità Unicode (ovvero passando il **ccs** flag).

Nella tabella seguente vengono riepilogate le modalità usate per vari **ccs** flag assegnato **fopen** e Byte Order Mark nel file.
...
Se *modalità* viene **", a ccs =**_codifica_**"**, **fopen** tenta innanzitutto di aprire il file utilizzando entrambe lettura e accesso in scrittura. Se questa operazione ha esito positivo, la funzione legge l'indicatore ordine byte per determinare la codifica del file. Se l'operazione non riesce, la funzione usa la codifica predefinita per il file. In entrambi i casi **fopen** verrà quindi riaprire il file utilizzando l'accesso di sola scrittura. (Si applica a **"a"** modalità unico, non **"+"** modalità.)
...
Quando il **"r +"**, **"w +"**, o **"a +"** viene specificato il tipo di accesso, sono abilitate sia la lettura e scrittura (il file viene definito aperto per "aggiornamento"). Tuttavia, quando si passa dalla lettura alla scrittura, l'operazione di input deve rilevare un marcatore EOF. Se non esiste alcun EOF, è necessario usare una nuova chiamata a una funzione di posizionamento dei file. Le funzioni di posizionamento di file sono **fsetpos**, [fseek](fseek-fseeki64.md), e [rewind](rewind.md). Quando si passa dalla scrittura alla lettura, è necessario utilizzare una nuova chiamata a **fflush** o a un funzione di posizionamento dei file.

Oltre ai valori precedenti, è possano aggiungere i caratteri seguenti a *modalità* per specificare la modalità di conversione per caratteri di nuova riga.
...
Le opzioni seguenti possono essere aggiunto alla *modalità* per specificare i comportamenti aggiuntivi.
...
Caratteri validi per il *modalità* stringa usata nel **fopen** e **fdopen** corrispondono a *oflag* argomenti che vengono usati in [Open](open-wopen.md) e [sopen](sopen-wsopen.md), come indicato di seguito.
...
Se si utilizza **rb** modalità, è necessario convertire il codice, e se si prevede di leggere la maggior parte di un file di grandi dimensioni o non sono massimizzare le prestazioni di rete, è inoltre consigliabile se utilizzare la memoria file Win32 mappati come opzione.
...
Il **c**, **n**, **t**, **S**, **R**, **T**, e **1!d**  *modalità* sono estensioni Microsoft per le opzioni **fopen** e **fdopen** e non deve essere utilizzata dove si desidera la portabilità ANSI.
