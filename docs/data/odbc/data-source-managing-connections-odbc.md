...
2.  Nel **DSN su File** scheda, fare clic su **Add**.  
  
3.  Nel **Crea nuova origine dati** finestra di dialogo, selezionare un driver per Excel e quindi fare clic su **successivo**.  
...
2.  Nel **il programma di installazione di ODBC Microsoft Excel** finestra di dialogo, selezionare il database versione e una cartella di lavoro.  
...
2.  Chiamare relativi `OpenEx` o `Open` funzione membro.  
...
2.  Eliminare definitivamente l'oggetto a meno che non si desidera usarlo di nuovo.  
...
2.  L'eliminazione dell'oggetto, invece di chiamare relativi `OpenEx` o `Open` nuovamente la funzione membro.  
