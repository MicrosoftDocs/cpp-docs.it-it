...
OLE DB usa set di righe dello schema per restituire dati relativi ai dati in un provider. Questi dati vengono spesso definiti "metadati". Per impostazione predefinita, un provider deve sempre `DBSCHEMA_TABLES`supportare `DBSCHEMA_COLUMNS`, e `DBSCHEMA_PROVIDER_TYPES`, come descritto in [IDBSchemaRowset](/previous-versions/windows/desktop/ms713686(v=vs.85)) in *OLE DB Programmer ' s Reference*. I set di righe vengono definiti in una mappa di schema. Per informazioni sulle voci della mappa di schema, vedere [SCHEMA_ENTRY](../../data/oledb/schema-entry.md).
...
*riid*<br/>
in Interfaccia per [QueryInterface](../../atl/queryinterface.md) nell'output `IUnknown`.
...
*ppRowset*<br/>
out In uscita `IUnknown` richiesto da *riid*. Si `IUnknown` tratta di un'interfaccia sull'oggetto set di righe dello schema.
...
## <a name="setrestrictions"></a>IDBSchemaRowsetImpl:: sele restrizioni
...
Il metodo richiede che l'utente disponga di una mappa di schema nella classe di sessione. Utilizzando le informazioni sulla mappa dello `GetRowset` schema, crea un oggetto set di righe specificato se il parametro *rguidSchema* è uguale a uno dei GUID delle voci della mappa. Per una descrizione della voce di mapping, vedere [SCHEMA_ENTRY](../../data/oledb/schema-entry.md) .
