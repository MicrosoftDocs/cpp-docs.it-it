...
Usa Visual Studio, è possibile compilare DLL Win32 in C o C++ che non usano la libreria di classi MFC (Microsoft Foundation). È possibile creare un progetto di DLL non MFC con la creazione guidata applicazione Win32.

La libreria MFC stessa è disponibile, in librerie a collegamento statico o in alcune DLL, con la creazione guidata DLL MFC. Se la DLL utilizza MFC, Visual Studio supporta tre diversi scenari di sviluppo delle DLL:
...
Se la DLL non utilizza MFC, è possibile usare Visual Studio per compilare una DLL Win32 non MFC. Il collegamento della DLL a MFC (statico o dinamico) occupa dalla memoria e spazio su disco significativo. Non effettuare questo collegamento a MFC, a meno che la DLL non utilizzi realmente MFC.
