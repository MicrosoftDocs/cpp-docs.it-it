...
Per impostazione predefinita, il comando **Aggiorna** elenca solo le librerie che non sono aggiornate. non li aggiorna. Per aggiornare effettivamente le librerie, usare l'opzione **--No-Dry-Run** .
...
|**vcpkg Esporta \<> pkg... \[opt]...**|Esporta un pacchetto|
|**vcpkg edit \<pacchetto>**|Apre un port per la modifica (usa %EDITOR%, impostazione predefinita 'code')|
|**vcpkg creare \<l'URL \<> pkg \[> ArchiveName]**|Crea un nuovo pacchetto|
