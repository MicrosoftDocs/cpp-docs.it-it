---
title: "Gestione delle eccezioni ARM | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
### Presupposti  
 Le immagini eseguibili per Windows su ARM usano il formato Portable Executable \(PE\).  Per altre informazioni, vedere l'argomento relativo alla [specifica Microsoft PE e COFF](http://go.microsoft.com/fwlink/p/?linkid=84140).  Le informazioni sulla gestione delle eccezioni sono memorizzate nelle sezioni .pdata e .xdata dell'immagine.  
...
|-------------------|---------|-----------|  
|0|0\-31|`Function Start RVA` è l'indirizzo RVA a 32 bit dell'inizio della funzione.  Se la funzione contiene codice Thumb, è necessario impostare il bit inferiore di questo indirizzo.|  
|1|0\-1|`Flag` è un campo a 2 bit che indica come interpretare i restanti 30 bit della seconda parola .pdata.  Se `Flag` è 0, i bit restanti formano un `Exception Information RVA` \(con i due bit inferiori implicitamente 0\).  Se `Flag` è diverso da zero, i bit restanti formano una struttura `Packed Unwind Data`.|  
|1|2\-31|`Exception Information RVA` o `Packed Unwind Data`.<br /><br /> `Exception Information RVA` è l'indirizzo della struttura di informazioni di eccezione a lunghezza variabile archiviata nella sezione .xdata.  Questi dati devono essere allineati a 4 byte.<br /><br /> `Packed Unwind Data` è una descrizione compressa delle operazioni necessarie per la rimozione da una funzione, supponendo una forma canonica.  In questo caso non sono necessari record .xdata.|  
...
|-------------------|---------|-----------|  
|0|0\-31|`Function Start RVA` è l'indirizzo RVA a 32 bit dell'inizio della funzione.  Se la funzione contiene codice Thumb, è necessario impostare il bit inferiore di questo indirizzo.|  
|1|0\-1|`Flag` è un campo a 2 bit con i significati seguenti:<br /><br /> -   00 \= i dati di rimozione compressi non sono usati; i bit rimanenti puntano al record .xdata.<br />-   01 \= dati di rimozione compressi.<br />-   10 \= dati di rimozione compressi in cui si suppone che la funzione non abbia un prologo.  Si rivela utile per la descrizione dei frammenti di funzione non contigui all'inizio della funzione.<br />-   11 \= riservato.|  
|1|2\-12|`Function Length` è un campo a 11 bit che indica la lunghezza dell'intera funzione in byte diviso 2.  Se la funzione è superiore a 4000 byte, è invece necessario usare un record .xdata completo.|  
|1|13\-14|`Ret` è un campo a 2 bit che indica la modalità di restituzione della funzione:<br /><br /> -   00 \= restituzione tramite pop {pc} \(in questo caso il bit del flag `L` deve essere impostato su 1\).<br />-   01 \= restituzione tramite un branch a 16 bit.<br />-   10 \= restituzione tramite un branch a 32 bit.<br />-   11 \= nessun epilogo.  Si rivela utile per descrivere un frammento di funzione non contiguo che può solo contenere un prologo, ma il cui epilogo si trova altrove.|  
|1|15|`H` è un flag a 1 bit che indica se la funzione "ospita" i registri di parametri Integer \(r0\-r3\) eseguendone il push all'inizio della funzione e dealloca i 16 byte di stack prima di restituire il risultato.  \(0 \= non ospita i registri, 1 \= ospita i registri.\)|  
|1|16\-18|`Reg` è un campo a 3 bit che indica l'indice del registro non volatile salvato per ultimo.  Se il bit `R` è 0, vengono salvati solo i registri Integer, che si presumono inclusi nell'intervallo r4\-rN, dove N è pari a 4 \+ `Reg`.  Se il bit `R` è 1, vengono salvati solo i registri a virgola mobile, che si presumono inclusi nell'intervallo d8\-dN, dove N è pari a 8 \+ `Reg`.  La combinazione speciale di `R` \= 1 e `Reg` \= 7 indica che non viene salvato alcun registro.|  
|1|19|`R` è un flag a 1 bit che indica se i registri non volatili salvati sono registri Integer \(0\) o a virgola mobile \(1\).  Se `R` è impostato su 1 e il campo `Reg` è impostato su 7, non viene eseguito il push di registri non volatili.|  
|1|20|`L` è un flag a 1 bit che indica se la funzione salva\/ripristina registri LR, oltre agli altri registri indicati dal campo `Reg` \(0 \= non salva\/ripristina, 1 \= salva\/ripristina\).|  
|1|21|`C` è un flag a 1 bit che indica se la funzione include istruzioni aggiuntive per impostare una catena di frame per l'analisi rapida dello stack \(1\) o meno \(0\).  Se questo bit è impostato, r11 viene aggiunto implicitamente all'elenco di registri Integer non volatili salvati.  Se viene usato il flag `C`, vedere le limitazioni indicate di seguito.|  
|1|22\-31|`Stack Adjust` è un campo a 10 bit che indica il numero di byte di stack allocati per la funzione, diviso 4.  Tuttavia, solo i valori compresi tra 0x000 e 0x3F3 possono essere codificati direttamente.  Le funzioni che allocano più di 4044 byte di stack devono usare un record .xdata completo.  Se il campo `Stack Adjust` è 0x3F4 o maggiore, i 4 bit inferiori hanno un significato particolare:<br /><br /> -   I bit 0\-1 indicano il numero di parole di regolazione dello stack \(1\-4\), meno 1.<br />-   Il bit 2 è impostato su 1 se il prologo ha combinato questa regolazione nella propria operazione push.<br />-   Il bit 3 è impostato su 1 se l'epilogo ha combinato questa regolazione nella propria operazione pop.|  
...
-   Se il flag `C` è impostato su 1:  
    ...
    -   Anche il flag `L` deve essere impostato su 1, perché la concatenazione di frame ha richiesto sia r11 che LR.  
  
    -   r11 non deve essere incluso nel set di registri descritti da `Reg`.  In altri termini, se viene eseguito il push di r4\-r11, `Reg` deve descrivere solo r4\-r10 perché il flag `C` presuppone r11.  
...
-   Se il campo `Ret` è impostato su 0, il flag `L` deve essere impostato su 1.  
 ...
 Per la descrizione che segue, vengono tratti due pseudo flag da `Stack Adjust`:  
  
-   `PF`, o "riduzione prologo", indica che `Stack Adjust` è 0x3F4 o superiore e il bit 2 è impostato.  
  
-   `EF`, o "riduzione epilogo", indica che `Stack Adjust` è 0x3F4 o superiore e il bit 3 è impostato.  
...
|5|`Stack Adjust` \!\= 0 e `PF`\=\=0|16\/32|`sub sp,sp,#xx`|00\-7F\/E8\-EB|  
 ...
 Per impostare la concatenazione di frame, è presente l'istruzione 3a o 3b se il bit `C` è impostato.  È un `mov` a 16 bit se non viene eseguito il push di registri diversi da r11 e LR; in caso contrario, è un `add` a 32 bit.  
  
 Se viene specificata una regolazione non ridotta, l'istruzione 5 è la regolazione dello stack esplicita.  
  
 Le istruzioni 2 e 4 sono impostate in base alla necessità o meno di un'operazione push.  In questa tabella vengono riepilogati i registri che vengono salvati in base ai campi `C`, `L`, `R` e `PF`.  In tutti i casi, `N` è uguale a `Reg` \+ 4, `E` è uguale a `Reg` \+ 8 e `S` è uguale a \(~`Stack Adjust`\) & 3.  
...
|10b|`Ret`\=\=2|32|`b     address`|  
 ...
 L'istruzione 6 è la regolazione dello stack esplicita se viene specificata una regolazione non ridotta.  Dato che `PF` è indipendente da `EF`, è possibile che sia presente l'istruzione 5 senza l'istruzione 6 o viceversa.  
  
 Le istruzioni 7 e 8 usano la stessa logica del prologo per stabilire quali registri vengono ripristinati dallo stack, ma con le seguenti due modifiche: `EF` viene usato al posto di `PF` e, se `Ret` \= 0, LR viene sostituito da PC nell'elenco di registri e l'epilogo termina immediatamente.  
  
 Se `H` è impostato, l'istruzione 9a o 9b è presente.  L'istruzione 9a viene usata quando `L` è 0, per indicare che LR non è nello stack.  In questo caso, lo stack viene regolato manualmente e `Ret` deve essere 1 o 2 per specificare una restituzione esplicita.  L'istruzione 9b viene usata quando `L` è 1, per indicare una fine precoce all'epilogo e per restituire e regolare lo stack contemporaneamente.  
  
 Se l'epilogo non è ancora terminato, è presente l'istruzione 10a o 10b, per indicare un ramo a 16 o 32 bit, in base al valore di `Ret`.  
...
1.  Un'intestazione da 1 o 2 parole che descrive la dimensione complessiva della struttura .xdata e fornisce dati di funzione chiave.  La seconda parola è presente solo se i campi `Epilogue Count` e `Code Words` sono impostati entrambi su 0.  I campi sono illustrati in dettaglio in questa tabella:  
    ...
    |0|20|`X` è un campo a 1 bit che indica la presenza \(1\) o l'assenza \(0\) di dati di eccezione.|  
    |0|21|`E` è un campo a 1 bit che indica che le informazioni che descrivono un singolo epilogo sono compresse nell'intestazione \(1\) anziché richiedere parole di ambito aggiuntive in un secondo momento \(0\).|  
    ...
    |0|23\-27|`Epilogue Count` è un campo a 5 bit che ha due significati a seconda dello stato del bit `E`:<br /><br /> -   Se `E` è 0, il campo indica il numero totale degli ambiti di eccezione descritti nella sezione 3.  Se sono presenti più di 31 ambiti, questo campo e il campo `Code Words` devono essere impostati entrambi su 0 per indicare che è necessaria una parola di estensione.<br />-   Se `E` è 1, questo campo specifica l'indice del primo codice di rimozione che descrive l'unico epilogo.|  
    |0|28\-31|`Code Words` è un campo a 4 che specifica il numero di parole a 32 bit necessarie per contenere tutti i codici di rimozione nella sezione 4.  Se sono necessarie più di 15 parole per più di 63 byte di codice di rimozione, questo campo e il campo `Epilogue Count` devono essere impostati entrambi su 0 per indicare che è necessaria una parola di estensione.|  
    |1|0\-15|`Extended Epilogue Count` è un campo a 16 bit che fornisce spazio aggiuntivo per codificare un numero di epiloghi insolitamente ampio.  La parola di estensione che contiene questo campo è presente solo se i campi `Epilogue Count` e `Code Words` nella prima parola di intestazione sono impostati entrambi su 0.|  
    |1|16\-23|`Extended Code Words` è un campo a 8 bit che fornisce spazio aggiuntivo per codificare un numero di parole di codice di rimozione insolitamente ampio.  La parola di estensione che contiene questo campo è presente solo se i campi `Epilogue Count` e `Code Words` nella prima parola di intestazione sono impostati entrambi su 0.|  
    |1|24\-31|Riservata|  
  
2.  Se il bit `E` nell'intestazione è stato impostato su 0, i dati di intestazione sono seguiti da un elenco di informazioni relative agli ambiti di epilogo, compresse una per parola e archiviate in ordine di inizio offset crescente.  Ogni ambito contiene i campi seguenti:  
  
    |Bit|Scopo|  
    |---------|-----------|  
    |0\-17|`Epilogue Start Offset` è un campo a 18 bit che descrive l'offset dell'epilogo, in byte diviso 2, rispetto all'inizio della funzione.|  
    |18\-19|`Res` è un campo a 2 bit riservato per l'espansione futura.  Il suo valore deve essere 0.|  
    |20\-23|`Condition` è un campo a 4 bit che indica la condizione in base alla quale viene eseguito l'epilogo.  Per gli epiloghi non condizionali, deve essere impostato su 0xE, che significa "sempre".  Si noti che un epilogo deve essere interamente condizionale o interamente non condizionale e, in modalità Thumb\-2, l'epilogo inizia con la prima istruzione dopo l'opcode IT.|  
    |24\-31|`Epilogue Start Index` è un campo a 8 bit che indica l'indice di byte del primo codice di rimozione che descrive questo epilogo.|  
...
4.  Se il campo `X` nell'intestazione è 1, i byte di codice di rimozione sono seguiti dalle informazioni del gestore di eccezioni.  Si tratta di un `Exception Handler RVA` che contiene l'indirizzo del gestore di eccezioni, seguito immediatamente dalla quantità \(a lunghezza variabile\) dei dati richiesti dal gestore di eccezioni.  
...
|FF||||\-|end|  
 ...
 Mostra l'intervallo di valori esadecimali per ogni byte in un codice di rimozione `Code`, insieme alla dimensione opcode `Opsize` e all'interpretazione dell'istruzione originale corrispondente.  Le celle vuote indicano codici di rimozione più brevi.  Nelle istruzioni con valori ampi che coprono più byte, i bit più significativi sono archiviati per primi.  Il campo `Opsize` mostra la dimensione opcode implicita associata a ogni operazione Thumb\-2.  Le voci apparentemente duplicate nella tabella con diverse codifiche consentono di distinguere le varie dimensioni di opcode.  
...
-   Solo epiloghi; prologo e altri eventuali epiloghi in altri frammenti.  
 ...
 Nel primo caso, deve essere descritto solo il prologo.  È possibile usare il formato .pdata compatto descrivendo normalmente il prologo e specificando un valore `Ret` pari a 3 per indicare l'assenza di epiloghi.  Nel formato .xdata completo, è possibile specificare i codici di rimozione del prologo all'indice 0 come di consueto e specificare un numero di epiloghi pari a 0.  
 ...
 Nel terzo e quarto caso, la presenza di uno pseudo\-prologo è specificata impostando il campo `Flag` del record .pdata compatto su 2 oppure impostando il flag `F` nell'intestazione .xdata su 1.  In entrambi i casi, il controllo della presenza di una rimozione epilogo parziale viene ignorato e tutte le rimozioni non epilogo vengono considerate complete.  
...
#### Wrapping di riduzione  
 Un caso più complesso di frammenti di funzione è il *wrapping di riduzione*, una tecnica per posticipare il salvataggio dei registri dall'inizio della funzione a un punto successivo della funzione, allo scopo di ottimizzare il procedimento per i casi semplici che non richiedono il salvataggio dei registri.  Può essere descritto come un'area esterna che alloca lo spazio stack ma salva un set minimo di registri e un'area interna che salva e ripristina ulteriori registri.  
...
```  
 ...
 L'elemento chiave in questo scenario è che a ogni limite di istruzione lo stack è pienamente coerente con i codici di rimozione per l'area.  Se si verifica una rimozione prima del push interno in questo esempio, è considerata parte dell'area A e verrà rimosso solo il prologo dell'area A.  Se la rimozione si verifica dopo il push interno, è considerata parte dell'area B, che è priva di prologo ma contiene codici di rimozione che descrivono il push interno e il prologo originale dall'area A.  Una logica analoga vale per il pop interno.  
...
-   Parola 1  
    ...
    -   `Ret` \= 1, che indica una restituzione tramite branch a 16 bit  
  
    -   `H` \= 0, che indica che i parametri non erano ospitati  
  
    -   `R`\=0 e `Reg` \= 1, che indicano il push\/pop di r4\-r5  
  
    -   `L` \= 0, che indica che il registro LR non viene salvato\/ripristinato  
  
    -   `C` \= 0, che indica assenza di concatenazione dei frame  
  
    -   `Stack Adjust` \= 0, che indica assenza di regolazione dello stack  
...
-   Parola 1  
    ...
    -   `L` \= 1, che indica che il registro LR è stato salvato\/ripristinato  
...
-   Parola 1  
    ...
    -   `Ret` \= 0, che indica una restituzione di tipo pop {pc} \(in questo caso restituzione di tipo ldr pc,\[sp\],\#0x14\)  
  
    -   `H` \= 1, che indica che i parametri erano ospitati  
...
-   Parola 1  
    ...
    -   `Flag` \= 0, che indica la presenza del record .xdata \(necessario data la presenza di più epiloghi\)  
...
-   Parola 0  
    ...
    -   `Epilogue Count` \= 0x04, che indica 4 ambiti di epilogo totali  
...
-   Parola 1  
    ...
    -   `Flag` \= 0, che indica la presenza del record .xdata \(necessario vista la presenza di più epiloghi\)  
...
-   Parola 0  
    ...
    -   `Epilogue Count` \= 0x001, che indica 1 ambito di epilogo totale  
...
-   Parola 0  
    ...
    -   `X` \= 1, che indica la presenza di dati di eccezione  
  
    -   `E` \= 1, che indica un unico epilogo  
  
    -   `F` \= 0, che indica una descrizione di funzione completa, incluso il prologo  
  
    -   `Epilogue Count` \= 0x00, che indica che i codici di eccezione dell'epilogo iniziano all'offset 0x00  
...
-   La parola 3 specifica un gestione di eccezioni \= 0x0019A7ED \(\= 0x0059A7ED – 0x00400000\)  
...
-   Parola 1  
    ...
    -   `R`\=0 e `Reg` \= 7, che indicano che non è stato salvato\/ripristinato alcun registro  
  
    -   `L` \= 1, che indica che il registro LR è stato salvato\/ripristinato  
  
    -   `C` \= 0, che indica assenza di concatenazione dei frame  
  
    -   `Stack Adjust` \= 1, che indica una regolazione dello stack 1 × 4 byte  
