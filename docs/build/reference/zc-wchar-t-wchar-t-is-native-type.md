...
Non è consigliabile **/Zc:wchar_t-** perché il C++ standard richiede che `wchar_t` sia un tipo incorporato. L'uso della versione `typedef` può causare problemi di portabilità. Se esegue l'aggiornamento da versioni precedenti di Visual C++ e si verifica l'errore del compilatore [C2664](../../error-messages/compiler-errors-2/compiler-error-c2664.md) perché il codice sta cercando di convertire in modo implicito una `wchar_t` a `unsigned short`, è consigliabile modificare il codice per correggere l'errore, anziché impostare **/Zc:wchar_t-**.
