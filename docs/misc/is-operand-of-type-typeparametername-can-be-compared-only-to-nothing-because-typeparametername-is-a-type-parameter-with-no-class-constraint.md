...
Un parametro di tipo viene usato come operando per [Is Operator](../Topic/Is%20Operator%20\(Visual%20Basic\).md) quando il parametro di tipo viene definito senza la parola chiave [Class \(Visual Basic\)](http://msdn.microsoft.com/it-it/0777c6e6-46bc-451b-ad70-57b49d4ef4f7) o un nome della classe specifico nell'elenco di vincoli.  
  
 `Is` confronta due tipi riferimento per determinare se puntano alla stessa istanza di oggetti in memoria. Non può accettare un operando che non è un tipo riferimento, a meno che l'altro operando sia [Nothing](../Topic/Nothing%20\(Visual%20Basic\).md).  
