...
```  
 ...
 In alcuni casi, la posizione di un set di regole può dipendere dalla proprietà [!INCLUDE[vstecmsbuild](../build/includes/vstecmsbuild_md.md)].  Le proprietà [!INCLUDE[vstecmsbuild](../build/includes/vstecmsbuild_md.md)] non possono essere referenziate da un set di regole.  Per risolvere questo problema, specificare i percorsi di ricerca aggiuntivi tramite la proprietà **CodeAnalysisRuleSetDirectories** di [!INCLUDE[vstecmsbuild](../build/includes/vstecmsbuild_md.md)].  In questo scenario, specificare solo il nome del set di regole nell'attributo **Path** dell'elemento **Include**.  
