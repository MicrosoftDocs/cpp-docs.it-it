...
- Quando si potrebbe desiderare di suddividere un progetto in più file di risorse e/o file di intestazione e come procedere

- Come si condivide un'intestazione comune. File H tra due. File RC

- Come si dividono le risorse del progetto in più. File RC
...
- **Panoramica del modo in cui Visual C++ gestisce i file di risorse e i file di intestazione** fornisce una panoramica del modo in cui il comando Inclusioni risorsa in Visual C++ consente di usare più file di risorse e i file di intestazione nello stesso progetto.
...
- **Inclusione di file di intestazione aggiuntivi** descrive dove si potrebbe voler includere più file di intestazione e vengono fornite informazioni dettagliate per eseguire questa operazione.

- **Condivisione di un File di intestazione tra due. File RC** viene illustrato come è possibile condividere un file di intestazione tra più. File RC in progetti diversi o anche nello stesso progetto.

- **Uso di più file di risorse nello stesso progetto** descrive dove si potrebbe voler suddividere il progetto in più. RC di file e vengono fornite informazioni dettagliate per eseguire questa operazione.

- **Applicazione dei file di Visual C++ Non modificabili** descrive come è possibile assicurarsi che Visual C++ non modifichi e riformatti accidentalmente una risorsa personalizzata.

- **Gestione di simboli condivisi da più di Visual C++ modificate. File RC** viene descritto come condividere gli stessi simboli tra più. File RC e su come evitare di assegnare valori numerici di ID duplicati.

- **La gestione delle dipendenze tra. RC. CPP, e. File H** viene descritto come Visual C++ consente di evitare la ricompilazione non necessari. File CPP dipendenti da file di simboli di risorsa.

- **Informazioni su come Visual C++ gestisce Set include** fornisce dettagli tecnici sul modo in cui Visual C++ tiene traccia di più (annidati). File RC e più file di intestazione sono # include da una. File RC.
...
MYAPP. RC il file di risorse dell'applicazione che viene modificato tramite Visual C++.
...
RES\MYAPP. RC2 contiene le risorse non può essere modificate da Visual C++, ma verranno incluso nell'ultima compilata. File EXE. La Creazione guidata applicazioni non crea tali risorse per impostazione predefinita, poiché Visual C++ può modificare tutte le risorse standard, inclusa la risorsa di versione (una nuova funzionalità di questa versione). Tramite la Creazione guidata applicazioni viene generato un file vuoto qualora si volessero aggiungere a questo file risorse in formato personalizzato.
...
**Condivisione di un File di intestazione tra due. File RC**
...
Mediante le Direttive in fase di compilazione è possibile organizzare le risorse modificabili e non modificabili di Visual C++ in più file RC, in cui MYAPP.RC "master" non esegue alcuna operazione tranne includere (tramite #include) gli altri file RC. Se si usa Visual Studio C++ project. File di codice MAK, quindi si deve includere "master". RC di file nel progetto in modo che tutte le # include le risorse vengono compilati con l'applicazione.

**Imposizione dei file Visual C++ non modificabili**

RES\MYAPP creato dalla creazione guidata applicazioni. File RC2 è un esempio di un file che contiene le risorse che è eseguire *non* desidera accidentalmente lette in Visual C++ e quindi lo scriverà nuovo con perdita di informazioni di formattazione. Per evitare questo problema, inserire le seguenti righe all'inizio del file RES\MYAPP.RC2:
...
**Gestione di simboli condivisi da più di Visual C++ modificate. File RC**
...
`_APS_NEXT_RESOURCE_VALUE` è il valore di simbolo successivo che verrà usato per una risorsa finestra di dialogo, risorsa di menu e così via. L'intervallo valido per i valori dei simboli risorsa è compreso tra 1 e 0x6FFF.

`_APS_NEXT_COMMAND_VALUE` è il valore di simbolo successivo che verrà usato per l'identificazione di un comando. L'intervallo valido per i valori dei simboli di comando è compreso tra 0x8000 e 0xDFFF.

`_APS_NEXT_CONTROL_VALUE` è il valore di simbolo successivo che verrà usato per un controllo di finestra di dialogo. L'intervallo valido per i valori dei simboli di controllo finestra di dialogo è compreso tra 8 e 0xDFFF.

`_APS_NEXT_SYMED_VALUE` è il valore di simbolo successivo che verrà generato manualmente quando si assegna un valore di simbolo utilizzando il comando nuovo nel Visualizzatore simboli.

Quando viene creato un nuovo file RC, Visual C++ inizia con valori leggermente più elevati del valore valido più basso. Questi valori verranno inizializzati anche dalla Creazione guidata applicazioni in valori più appropriati per le applicazioni MFC. Per altre informazioni sugli intervalli di valori di ID, vedere [Nota tecnica 20](../mfc/tn020-id-naming-and-numbering-conventions.md).
...
Per evitare questo problema, è necessario riservare un intervallo numerico separato per ognuno dei quattro domini di ID nei rispettivi file RC. Eseguire questa operazione manualmente aggiornando il `_APS_NEXT` in ognuno dei valori di. File RC **prima di** iniziare ad aggiungere le risorse. Ad esempio, se il primo. File RC utilizza il valore predefinito `_APS_NEXT` valori, quindi è possibile assegnare i seguenti `_APS_NEXT` valori al secondo. File RC:
...
**La gestione delle dipendenze tra. RC. CPP, e. File H**
