 ...
 *bDisableIfNoHandler* è diverso da zero per disabilitare l'oggetto dell'interfaccia utente, se non è né un `ON_UPDATE_COMMAND_UI` né un `ON_COMMAND` gestore.  
  
 **WM_EXITHELPMODE**  
  
 Questo messaggio viene inserito un `CFrameWnd` che per uscire dall'installazione sensibile al contesto modalità Guida in linea. La ricezione di questo messaggio viene terminato il ciclo modale avviato da **CFrameWnd::OnContextHelp.**  
