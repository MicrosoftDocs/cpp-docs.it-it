...
Oltre al tipo di immagine standard, sono disponibili anche tipi di carattere e colore. Per altre informazioni sull'uso del tipo carattere nel controllo ActiveX, vedere l'articolo [controlli ActiveX MFC: Usando i tipi di carattere](../mfc/mfc-activex-controls-using-fonts.md).
...
Per aggiungere l'ID di pagina proprietà per la pagina delle proprietà immagine standard, inserire la riga seguente dopo il BEGIN_PROPPAGEIDS (macro) nel file di implementazione del controllo (. CPP):
...
1. Nel **tipo di proprietà** , quindi selezionare **IPictureDisp** <strong>\*</strong> per il tipo di proprietà.
