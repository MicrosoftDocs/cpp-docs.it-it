...
|KeyDown|**void FireKeyDown (breve** `nChar` **, short**`nShiftState`**)** |Generato quando un `WM_SYSKEYDOWN` o `WM_KEYDOWN` messaggio viene ricevuto.<br /><br /> Voce della mappa eventi: **EVENT_STOCK_KEYDOWN)**|  
|KeyPress|**void FireKeyPress (breve\***`pnChar`**)** |Generato quando un `WM_CHAR` messaggio viene ricevuto.<br /><br /> Voce della mappa eventi: **EVENT_STOCK_KEYPRESS)**|  
|KeyUp|**void FireKeyUp (breve** `nChar` **, short**`nShiftState`**)** |Generato quando un `WM_SYSKEYUP` o `WM_KEYUP` messaggio viene ricevuto.<br /><br /> Voce della mappa eventi: **EVENT_STOCK_KEYUP)**|  
|MouseDown|**void FireMouseDown (breve** `nButton` **, short** `nShiftState` **, float***x* **, float** *y***)** |Viene generato se qualsiasi **BUTTONDOWN** (sinistro, centrale o a destra) viene ricevuto. Immediatamente prima di questo evento viene generato, viene acquisito il mouse.<br /><br /> Voce della mappa eventi: **EVENT_STOCK_MOUSEDOWN)**|  
|MouseMove|**void FireMouseMove (breve** `nButton` **, short** `nShiftState` **, float***x* **, float** *y***)** |Generato quando un `WM_MOUSEMOVE` messaggio viene ricevuto.<br /><br /> Voce della mappa eventi: **EVENT_STOCK_MOUSEMOVE)**|  
|MouseUp|**void FireMouseUp (breve** `nButton` **, short** `nShiftState` **, float***x* **, float** *y***)** |Viene generato se qualsiasi **BUTTONUP** (sinistro, centrale o a destra) viene ricevuto. Prima di questo evento viene generato, viene rilasciato il mouse capture.<br /><br /> Voce della mappa eventi: **EVENT_STOCK_MOUSEUP)**|  
...
##  <a name="_core_classwizard_changes_for_stock_events"></a>Modifiche di aggiunta evento guidata per gli eventi predefiniti  
