...
ms.locfileid: "36955456"
---
# <a name="settings-for-the-cstatusbarctrl"></a>Impostazioni per CStatusBarCtrl
La posizione predefinita di un [CStatusBarCtrl](../mfc/reference/cstatusbarctrl-class.md) finestra stato è nella parte inferiore della finestra padre, ma è possibile specificare lo stile CCS_TOP affinché venga visualizzato nella parte superiore dell'area client della finestra padre.  
  
 È possibile specificare lo stile SBARS_SIZEGRIP per includere un riquadro di ridimensionamento in fondo a destra il `CStatusBarCtrl` finestra stato. Un riquadro di ridimensionamento è simile a un bordo di ridimensionamento. è un'area rettangolare che l'utente può fare clic e trascinare per ridimensionare la finestra padre.  
...
>  Se si combinano stili CCS_TOP e SBARS_SIZEGRIP, il riquadro di ridimensionamento non funziona anche se il sistema lo disegna nella finestra di stato.  
 ...
 La procedura della finestra Modifica automaticamente le dimensioni della finestra di stato ogni volta che viene ricevuto un messaggio WM_SIZE. In genere, quando cambiano le dimensioni della finestra padre, l'elemento padre invia un messaggio WM_SIZE nella finestra di stato.  
 ...
 Per recuperare la larghezza dei bordi di una finestra stato chiamando [GetBorders](../mfc/reference/cstatusbarctrl-class.md#getborders). Questa funzione membro include il puntatore a una matrice di tre elementi che riceve la larghezza del bordo orizzontale, il bordo verticale e il bordo tra i rettangoli.  
