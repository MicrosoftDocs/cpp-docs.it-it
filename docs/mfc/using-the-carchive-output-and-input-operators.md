---
title: Utilizzando il CArchive &lt; &lt; e &gt; &gt; operatori | Documenti Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
ms.locfileid: "36953285"
---
# <a name="using-the-carchive-ltlt-and-gtgt-operators"></a>Utilizzando il CArchive &lt; &lt; e &gt; &gt; operatori
...
1.  Nell'esempio seguente viene illustrato come caricare un oggetto da un valore archiviato in precedenza in un file:  
 ...
 In genere, archiviare e caricare i dati da e verso un file tramite un archivio nel `Serialize` le funzioni di `CObject`-classi derivate, che devono essere dichiarato con la macro DECLARE_SERIAL. Un riferimento a un `CArchive` oggetto viene passato per il `Serialize` (funzione). Si chiama il `IsLoading` funzione del `CArchive` oggetto per determinare se il `Serialize` funzione è stata chiamata per caricare i dati dal file o archiviare dati nel file.  
  
 Il `Serialize` funzione di un serializzabile `CObject`-classe derivata ha in genere il formato seguente:  
  
 [!code-cpp[NVC_MFCSerialization#9](../mfc/codesnippet/cpp/using-the-carchive-output-and-input-operators_3.cpp)]  
  
 Il modello di codice precedente è esattamente uguale a quella AppWizard crea per il `Serialize` funzione del documento (una classe derivata da `CDocument`). Questo modello di codice consente di scrivere codice che è più facile da esaminare, perché il codice di archiviazione e il codice di caricamento deve essere sempre paralleli, come nell'esempio seguente:  
...
> [!NOTE]
>  Memorizzazione e caricamento `CObject`s tramite un archivio richiede attenzione aggiuntiva. Per altre informazioni, vedere [memorizzazione e caricamento di CObjects tramite un archivio](../mfc/storing-and-loading-cobjects-via-an-archive.md).  
