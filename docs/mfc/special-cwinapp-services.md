...
Per abilitare il trascinamento della selezione di file nell'applicazione, la creazione guidata applicazione MFC scrive una chiamata alla funzione membro [CWnd](../mfc/reference/cwnd-class.md) [DragAcceptFiles](../mfc/reference/cwnd-class.md#dragacceptfiles) per la `InitInstance`finestra cornice principale in. È possibile rimuovere tale chiamata se non si desidera implementare la funzionalità di trascinamento della selezione.
