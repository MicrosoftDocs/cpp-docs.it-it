...
[Esempio scribble 1,0 MFC](https://download.microsoft.com/download/4/0/9/40946FEC-EE5C-48C2-8750-B0F8DA1C99A8/MFC/general/Scribble.zip.exe). Per informazioni sulla conversione in Visual Studio 2017 o versione successiva, [vedere Guida al porting: Scribble](../porting/porting-guide-mfc-scribble.md)MFC.

##  <a name="top"></a> Sezioni

In questa parte della procedura dettagliata sono incluse le seguenti sezioni:

- [Sostituzione delle classi di base](#replaceclass)
...
##  <a name="replaceclass"></a>Sostituzione delle classi di base
...
1. In Scribble. h modificare la definizione `CScribbleApp` della classe in modo che sia derivata dalla [classe CWinAppEx](../mfc/reference/cwinappex-class.md).
...
Questa procedura dettagliata copia i file di risorse dall'esempio [creato in procedura dettagliata: Creazione di un'applicazione Ribbon utilizzando MFC](../mfc/walkthrough-creating-a-ribbon-application-by-using-mfc.md).

### <a name="to-add-bitmaps-to-the-project"></a>Per aggiungere bitmap al progetto

1. Usare Esplora file per copiare i file con estensione bmp seguenti dalla directory delle risorse`res`() dell'esempio della barra multifunzione alla directory`res`delle risorse () del progetto Scribble:
...
1. Importare le bitmap per il menu file collegato al pulsante **dell'applicazione** .
   ...
   1. Importare il formato filelarge. bmp, che contiene nove immagini 32x32 (32x288). Modificare l'ID della bitmap in `IDB_RIBBON_FILELARGE`.
...
1. Importare le bitmap per le categorie e i pannelli della barra multifunzione. Ogni scheda della barra multifunzione è una categoria e consiste in un'etichetta di testo e in un'immagine facoltativa.
   ...
   1. Importare la bitmap homelarge. bmp, che contiene nove immagini 32x32 per bitmap di pulsanti di grandi dimensioni. Modificare l'ID della bitmap in `IDB_RIBBON_HOMELARGE`.
...
1. Nella finestra di dialogo **Aggiungi risorsa** selezionare **barra multifunzione** e quindi fare clic su **nuovo**.
   ...
   In Visual Studio verrà creata una risorsa barra multifunzione che verrà aperta nella visualizzazione Progettazione. L'ID della risorsa della `IDR_RIBBON1`barra multifunzione è, che viene visualizzato in **visualizzazione risorse**. La barra multifunzione contiene una categoria e un pannello.
...
1. Le modifiche seguenti creano il menu visualizzato quando l'utente fa clic sul pulsante dell' **applicazione** . Fare clic sui puntini di sospensione ( **...** ) accanto a **elementi principali** per aprire l' **Editor elementi**.
   ...
   1. Fare clic su **Aggiungi** per aggiungere un pulsante. Modificare **didascalia** `&Save`in , ID `ID_FILE_SAVE`in , immagine `2`in e **immagine di grandi dimensioni** in `2`.

   1. Fare clic su **Aggiungi** per aggiungere un pulsante. Modificare **didascalia** `Save &As`in , ID `ID_FILE_SAVE_AS`in , immagine `3`in e **immagine di grandi dimensioni** in `3`.

   1. Fare clic su **Aggiungi** per aggiungere un pulsante. Modificare **didascalia** `&Print`in , ID `ID_FILE_PRINT`in , immagine `4`in e **immagine di grandi dimensioni** in `4`.

   1. Modificare il tipo di **elemento** in **separatore** , quindi fare clic su **Aggiungi**.

   1. Modificare il tipo di **elemento** in **Button**. Fare clic su **Aggiungi** per aggiungere un quinto pulsante. Modificare **didascalia** `&Close`in , ID `ID_FILE_CLOSE`in , immagine `5`in e **immagine di grandi dimensioni** in `5`.
...
1. Le modifiche seguenti consentono di creare un sottomenu sotto il pulsante **stampa** creato nel passaggio precedente.
   ...
   1. Fare clic sul pulsante **stampa** , modificare il tipo di **elemento** in **pulsante**, quindi fare clic su **Inserisci**. Modificare **didascalia** `&Print`in , ID `ID_FILE_PRINT`in , immagine `4`in e **immagine di grandi dimensioni** in `4`.

   1. Fare clic sul pulsante **stampa** , quindi fare clic su **Inserisci** per aggiungere un pulsante. Modificare **didascalia** `&Quick Print`in , ID `ID_FILE_PRINT_DIRECT`in , immagine `7`in e **immagine di grandi dimensioni** in `7`.

   1. Fare clic sul pulsante **stampa** , quindi fare clic su **Inserisci** per aggiungere un altro pulsante. Modificare **didascalia** `Print Pre&view`in , ID `ID_FILE_PRINT_PREVIEW`in , immagine `6`in e **immagine di grandi dimensioni** in `6`.
...
2. Nel file mainfrm.cpp aggiungere il seguente codice prima dell'istruzione finale `return` alla fine della funzione `CMainFrame::OnCreate`. Viene creata un'istanza della barra multifunzione.
...
##  <a name="addcategory"></a>Personalizzazione della risorsa della barra multifunzione
...
1. Nel pannello **modifica** aggiungere un pulsante responsabile della cancellazione del contenuto del documento. L'ID del messaggio per questo pulsante è già stato definito nella `IDR_SCRIBBTYPE` risorsa di menu. Specificare `Clear All` come testo del pulsante e l'indice della bitmap che decora il pulsante. Aprire la **casella degli strumenti**, quindi trascinare un **pulsante** nel pannello **modifica** . Fare clic sul pulsante, quindi impostare Caption `Clear All`su , ID `ID_EDIT_CLEAR_ALL`su, **Image index** su `0`, **large image index** su `0`.
...
1. Nella funzione, digitare il codice seguente prima dell' `return 0;` istruzione per modificare lo stile e la gestione visiva predefiniti. `CMainFrame::OnCreate`
