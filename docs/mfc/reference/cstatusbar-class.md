---
description: 'Altre informazioni su: classe CStatusBar'
...
|[CStatusBar:: CreateEx](#createex)|Crea un `CStatusBar` oggetto con stili aggiuntivi per l' `CStatusBarCtrl` oggetto incorporato.|
...
Per ulteriori informazioni sull'utilizzo di `CStatusBar` , vedere l'articolo relativo all' [implementazione della barra di stato in MFC](../../mfc/status-bar-implementation-in-mfc.md) e alla [Nota tecnica 31: barre di controllo](../../mfc/tn031-control-bars.md).
...
Crea una barra di stato (una finestra figlio) e la associa all' `CStatusBar` oggetto.
...
- La barra di controllo CBRS_BOTTOM è alla fine della finestra cornice.
...
Chiamare questa funzione per creare una barra di stato (finestra figlio) e associarla all' `CStatusBar` oggetto.
...
Utilizzare `CreateEx` , anziché [create](#create), quando è necessario che determinati stili siano presenti durante la creazione del controllo barra di stato incorporato. Impostare ad esempio *dwCtrlStyle* su SBT_TOOLTIPS per visualizzare le descrizioni comandi in un oggetto della barra di stato.
...
Il `itemAction` membro della `DRAWITEMSTRUCT` struttura definisce l'azione di disegno da eseguire. Eseguire l'override di questa funzione membro per implementare il disegno per un oggetto disegnato dal proprietario `CStatusBar` . L'applicazione deve ripristinare tutti gli oggetti GDI (Graphics Device Interface) selezionati per il contesto di visualizzazione fornito in *lpDrawItemStruct* prima della chiusura di questa funzione membro.
...
## <a name="cstatusbarsetindicators"></a><a name="setindicators"></a> CStatusBar:: sespies
...
- Non SBPS_NOBORDERS bordo 3D intorno al riquadro.

- SBPS_POPOUT bordo inverso in modo che il testo "fuoriesca".

- SBPS_DISABLED non viene disegnato il testo.

- SBPS_STRETCH riquadro esteso per riempire lo spazio inutilizzato. Questo stile può essere presente solo in un riquadro per barra di stato.

- Non SBPS_NORMAL alcun tratto, bordi o pop-out.
