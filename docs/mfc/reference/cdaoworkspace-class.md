...
3.  Facoltativamente è possibile chiamare [Append](#append) se si desidera aggiungere l'area di lavoro alla raccolta di aree di lavoro del motore di database. È possibile lavorare con l'area di lavoro senza accodarlo.  
...
2.  Chiamare `SetDefaultPassword` e, se vuoi [SetDefaultUser](#setdefaultuser).  
  
3.  Chiamare `Create` per questo oggetto dell'area di lavoro o quelle successive, senza specificare una password.  
...
2.  Chiamare `SetDefaultUser` e, se vuoi [SetDefaultPassword](#setdefaultpassword).  
  
3.  Chiamare `Create` per questo oggetto dell'area di lavoro o quelle successive, senza specificare un nome utente.  
