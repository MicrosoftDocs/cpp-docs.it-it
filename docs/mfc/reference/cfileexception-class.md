---
title: Classe CFileException | Documenti di Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|[CFileException::ThrowOsError](#throwoserror)|Genera un'eccezione di file basato su un numero di errore del sistema operativo.|  
...
### <a name="parameters"></a>Parametri  
 ...
 `lOsError`  
 Un motivo specifici del sistema operativo per l'eccezione, se disponibile. Il `lOsError` parametro fornisce più informazioni rispetto `cause` viene.  
  
 `lpszArchiveName`  
 Punta a una stringa contenente il nome di `CFile` oggetto che ha causato l'eccezione.  
...
##  <a name="errnotoexception"></a>CFileException::ErrnoToException  
 Converte un valore di errore libreria run-time specificato in un `CFileException` errore valore enumerato.  
...
### <a name="parameters"></a>Parametri  
 ...
 [in] `nMaxError`  
 Numero massimo di caratteri che può contenere il buffer specificato. Ciò include il carattere di terminazione null.  
  
 [in, out] `pnHelpContext`  
 Puntatore a un intero senza segno che riceve l'ID del contesto della Guida in linea. Se `NULL`, non viene restituito alcun ID.  
...
- `CFileException::fileNotFound`2: Impossibile trova il file.  
...
- `CFileException::accessDenied`5: il file potrebbe non essere accessibile.  
...
- `CFileException::sharingViolation`11: SHARE. EXE non è stato caricato o un'area condivisa era bloccata.  
...
### <a name="remarks"></a>Note  
 Vedere il manuale tecnico del sistema operativo per un elenco dei codici di errore. Questo membro dati è una variabile pubblica di tipo **lungo**.  
...
### <a name="example"></a>Esempio  
 [!code-cpp[28 NVC_MFCFiles](../../atl-mfc-shared/reference/codesnippet/cpp/cfileexception-class_5.cpp)]  
