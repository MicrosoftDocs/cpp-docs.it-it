...
|[CWinApp::Unregister](#unregister)|Annulla la registrazione di tutto il noto per la registrazione `CWinApp` da parte dell'oggetto.|
...
|[CWinApp::SetAppID](#setappid)|Imposta in modo esplicito l'ID modello utente dell'applicazione per l'applicazione. Questo metodo deve essere chiamato prima che un'interfaccia utente venga presentata all'utente (il posto migliore è il costruttore dell'applicazione).|
...
Ogni applicazione che usa le classi Microsoft Foundation può contenere un solo oggetto derivato da `CWinApp`. Questo oggetto viene costruito quando vengono C++ costruiti altri oggetti globali ed è già disponibile quando Windows chiama la `WinMain` funzione, fornita dalla libreria Microsoft Foundation Class. Dichiarare l'oggetto `CWinApp` derivato a livello globale.
...
Oltre alle `CWinApp` funzioni membro, il libreria Microsoft Foundation Class fornisce le funzioni globali seguenti per `CWinApp` accedere all'oggetto e ad altre informazioni globali:
...
Vedere [CWinApp: Classe](../../mfc/cwinapp-the-application-class.md) dell'applicazione per ulteriori informazioni `CWinApp` sulla classe, inclusa una panoramica dei seguenti elementi:
...
L'implementazione predefinita di `ApplicationRecoveryCallback` `CDataRecoveryHandler` utilizza per salvare l'elenco dei documenti attualmente aperti nel registro di sistema. Questo metodo non salva i file in modo automatico.
...
Chiamare [HideApplication](#hideapplication) prima di `CloseAllDocuments`chiamare.
...
Eseguire l'override di questa funzione membro per personalizzare l'elaborazione delle `AfxMessageBox` chiamate a livello di applicazione.
...
Per un modo più semplice e semplificato per implementare un cursore di attesa, `CWaitCursor`usare.
...
Chiamata eseguita dal Framework all'interno della `Run` funzione membro per uscire da questa istanza dell'applicazione.
...
Non chiamare questa funzione membro da un punto qualsiasi, ma `Run` all'interno della funzione membro.
...
La durata del periodo di attesa del Framework per la restituzione della funzione di callback di ripristino è l'intervallo di ping. È possibile personalizzare l'intervallo di ping eseguendo l' `CWinApp::GetApplicationRecoveryPingInterval` override di o fornendo un valore personalizzato `RegisterWithRestartManager`a.
...
È possibile usare `GetNextDocTemplate` in un ciclo di iterazione in diretta se si stabilisce la posizione iniziale con `GetFirstDocTemplatePosition`una chiamata a.
...
*lpszResourceName*<br/>
Punta a una stringa con terminazione null che contiene il nome della risorsa icona. È anche possibile usare un `CString` oggetto per questo argomento.
...
*nIDCursor*<br/>
Identificatore di costante del manifesto **OCR_** che specifica un cursore di Windows predefinito. È necessario disporre `#define OEMRESOURCE` di `#include \<afxwin.h>` prima di per accedere alle costanti **OCR_** in Windows. H.
...
Utilizzare la `LoadOEMCursor` funzione membro o [LoadStandardCursor](#loadstandardcursor) per accedere ai cursori Windows predefiniti.
...
*nIDIcon*<br/>
Identificatore di costante del manifesto **OIC_** che specifica un'icona di Windows predefinita. È necessario disporre `#define OEMRESOURCE` di `#include \<afxwin.h>` before per accedere alle costanti **OIC_** in Windows. H.
...
- Cursore di inserimento testo standard IDC_IBEAM

- Cursore della clessidra IDC_WAIT usato quando Windows esegue un'attività che richiede molto tempo
...
È necessario aggiungere un' `ON_COMMAND( ID_CONTEXT_HELP, OnContextHelp )` istruzione `CWinApp` alla mappa messaggi della classe e aggiungere anche una voce della tabella di tasti di scelta rapida, in genere Shift + F1, per abilitare questa funzione membro.
...
Per abilitare questa funzione `ON_COMMAND( ID_FILE_NEW, OnFileNew )` membro, è `CWinApp` necessario aggiungere un'istruzione alla mappa messaggi della classe. Se abilitata, questa funzione gestisce l'esecuzione del comando file nuovo.
...
Per abilitare questa funzione `ON_COMMAND( ID_FILE_OPEN, OnFileOpen )` membro, è `CWinApp` necessario aggiungere un'istruzione alla mappa messaggi della classe. Se abilitata, questa funzione gestisce l'esecuzione del comando file Apri.
...
Per abilitare questa funzione `ON_COMMAND( ID_FILE_PRINT_SETUP, OnFilePrintSetup )` membro, è `CWinApp` necessario aggiungere un'istruzione alla mappa messaggi della classe. Se abilitata, questa funzione gestisce l'esecuzione del comando di stampa del file.
...
Per abilitare questa funzione `ON_COMMAND( ID_HELP, OnHelp )` membro, è `CWinApp` necessario aggiungere un'istruzione alla mappa messaggi della classe. Se abilitata, chiamata dal framework quando l'utente preme il tasto F1.
...
Per abilitare questa funzione `ON_COMMAND( ID_HELP_FINDER, OnHelpFinder )` membro, è `CWinApp` necessario aggiungere un'istruzione alla mappa messaggi della classe. Se abilitata, il Framework chiama questa funzione del gestore di messaggi quando l'utente dell'applicazione seleziona il comando Help Finder da `WinHelp` richiamare con l'argomento **HELP_FINDER** standard.
...
Per abilitare questa funzione `ON_COMMAND( ID_HELP_INDEX, OnHelpIndex )` membro, è `CWinApp` necessario aggiungere un'istruzione alla mappa messaggi della classe. Se abilitata, il Framework chiama questa funzione del gestore di messaggi quando l'utente dell'applicazione seleziona il comando help index da `WinHelp` richiamare con l'argomento **HELP_INDEX** standard.
...
Per abilitare questa funzione `ON_COMMAND( ID_HELP_USING, OnHelpUsing )` membro, è `CWinApp` necessario aggiungere un'istruzione alla mappa messaggi della classe. Il Framework chiama questa funzione del gestore di messaggi quando l'utente dell'applicazione seleziona la guida utilizzando il comando per richiamare `WinHelp` l'applicazione con l'argomento **HELP_HELPONHELP** standard.
...
*lCount*<br/>
Il contatore viene incrementato ogni `OnIdle` volta che viene chiamato quando la coda di messaggi dell'applicazione è vuota. Questo conteggio viene reimpostato su 0 ogni volta che viene elaborato un nuovo messaggio. È possibile utilizzare il parametro *lCount* per determinare la durata relativa del periodo di inattività dell'applicazione senza elaborare un messaggio.
...
1. Se il ciclo di messaggi nell'libreria Microsoft Foundation Class controlla la coda di messaggi e non trova alcun messaggio in sospeso, `OnIdle` chiama per l'oggetto applicazione e fornisce 0 come argomento *lCount* .
...
3. Il ciclo di messaggi verifica nuovamente la coda dei messaggi. Se nessun messaggio è in sospeso, chiama `OnIdle` di nuovo, incrementando l'argomento *lCount* .

4. Infine, `OnIdle` termina l'elaborazione di tutte le attività inattive e restituisce 0. Ciò indica al ciclo di messaggi di arrestare `OnIdle` la chiamata fino a quando non viene ricevuto il messaggio successivo dalla coda di messaggi, a quel punto il ciclo inattivo viene riavviato con l'argomento impostato su 0.

Non eseguire attività di lunga durata perché `OnIdle` l'applicazione non è in grado di elaborare `OnIdle` l'input dell'utente finché non viene restituito.

> [!NOTE]
> L'implementazione predefinita di `OnIdle` aggiorna i comandi degli oggetti dell'interfaccia utente, ad esempio voci di menu e pulsanti della barra degli strumenti, ed esegue la pulizia interna della struttura dei dati. Pertanto, se si esegue `OnIdle`l'override di, `CWinApp::OnIdle` è necessario `lCount` chiamare con la nella versione sottoposta a override. Chiamare prima di tutto l'elaborazione inattiva della classe di base, ovvero fino a `OnIdle` quando la classe di base non restituisce 0. Se è necessario eseguire il lavoro prima del completamento dell'elaborazione della classe di base, esaminare l'implementazione della classe di base per selezionare il *lCount* appropriato durante il quale eseguire il lavoro.

Se non si desidera `OnIdle` chiamare ogni volta che un messaggio viene recuperato dalla coda di messaggi, è possibile eseguire l'override di [CWinThreadIsIdleMessage](../../mfc/reference/cwinthread-class.md#isidlemessage). Se un'applicazione ha impostato un timer molto breve o se il sistema sta inviando il messaggio WM_SYSTIMER, `OnIdle` viene chiamato ripetutamente e diminuisce le prestazioni.

### <a name="example"></a>Esempio

I due esempi seguenti illustrano come usare `OnIdle`. Il primo esempio elabora due attività inattive usando l'argomento *lCount* per definire la priorità delle attività. La prima attività è con priorità alta ed è consigliabile farlo quando possibile. La seconda attività è meno importante e deve essere eseguita solo quando si verifica una pausa lungo nell'input dell'utente. Si noti la chiamata alla versione della classe di base `OnIdle`di. Il secondo esempio gestisce un gruppo di attività inattive con priorità diverse.
...
Puntatore a un oggetto `CDocument` in caso di esito positivo; in caso contrario null.
...
Quando si avvia un nuovo progetto MFC utilizzando la creazione guidata applicazione, la creazione guidata applicazione creerà un'istanza locale `CCommandLineInfo`di, quindi chiamerà `ProcessShellCommand` e `ParseCommandLine` nella funzione membro [InitInstance](#initinstance) . Una riga di comando segue la route descritta di seguito:

1. Dopo essere stato creato `InitInstance`in, `CCommandLineInfo` l'oggetto viene passato `ParseCommandLine`a.

2. `ParseCommandLine`chiama `CCommandLineInfo::ParseParam` quindi ripetutamente, una volta per ogni parametro.
...
Si noti che è possibile `ParseCommandLine` chiamare direttamente in base alle esigenze.
...
Diverso da zero se il messaggio è stato elaborato `PreTranslateMessage` completamente in e non deve essere ulteriormente elaborato. Zero se il messaggio deve essere elaborato in modo normale.
...
Quando si avvia un nuovo progetto MFC utilizzando la creazione guidata applicazione, tramite la creazione guidata applicazione viene creata un'istanza locale di `CCommandLineInfo`, `ProcessShellCommand` quindi viene chiamato `ProcessShellCommand` e [ParseCommandLine](#parsecommandline) nella funzione membro `InitInstance`. Una riga di comando segue la route descritta di seguito:
...
Esegue tutte le attività di registrazione non `RegisterShellFileTypes`gestite da.
...
Se il database di registrazione associa già un'estensione di file specificata a un altro tipo di file, non viene creata alcuna nuova associazione. Per la `CDocTemplate` registrazione di queste informazioni, vedere la classe per il formato delle stringhe necessarie.
...
|*pRecoveryCallback*|in Funzione di callback di ripristino. Questa funzione deve prendere un parametro LPVOID come input e restituire un valore DWORD. La funzione di callback di recupero `CWinApp::ApplicationRecoveryCallback`predefinita è.|
...
`Run`acquisisce e invia i messaggi di Windows fino a quando l'applicazione non riceve un messaggio WM_QUIT. Se la coda di messaggi dell'applicazione non contiene attualmente alcun messaggio, `Run` chiama [OnIdle](#onidle) per eseguire l'elaborazione del tempo di inattività. I messaggi in arrivo passano alla funzione membro [PreTranslateMessage](#pretranslatemessage) per un'elaborazione speciale, quindi alla funzione `TranslateMessage` Windows per la conversione della tastiera standard; infine, viene chiamata la `DispatchMessage` funzione di Windows.

`Run`viene ignorato raramente, ma è possibile eseguirne l'override per fornire un comportamento speciale.
...
Se presente, l'opzione viene rimossa dalla riga di comando. Per ulteriori informazioni sull'incorporamento, vedere l'articolo [server: Implementazione di un](../../mfc/servers-implementing-a-server.md)server.

##  <a name="saveallmodified"></a>  CWinApp::SaveAllModified

Chiamata eseguita dal Framework per salvare tutti i documenti quando la finestra cornice principale dell'applicazione deve essere chiusa o tramite un messaggio WM_QUERYENDSESSION.
...
Se *hDevMode* e *hDevNames* sono entrambi null, `SelectPrinter` usa la stampante predefinita corrente.
...
La `Unregister` funzione Annulla la registrazione eseguita dall'oggetto applicazione e la funzione [Register](#register) . In genere, entrambe le funzioni vengono chiamate in modo implicito da MFC e pertanto non verranno visualizzate nel codice.
...
In questo esempio `CWinApp* pApp = AfxGetApp();` viene utilizzato per ottenere la classe CWinApp che illustra un modo `WriteProfileBinary` che `GetProfileBinary` e può essere utilizzato da qualsiasi funzione in un'applicazione MFC.
...
Questo esempio usa `CWinApp* pApp = AfxGetApp();` per ottenere la classe CWinApp che illustra un modo in cui `WriteProfileString`, `WriteProfileInt` `GetProfileString`, e `GetProfileInt` può essere usato da qualsiasi funzione in un'applicazione MFC.
