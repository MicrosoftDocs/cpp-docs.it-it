...
ms.locfileid: "37079851"
---
# <a name="cricheditview-class"></a>CRichEditView (classe)
Con [CRichEditDoc](../../mfc/reference/cricheditdoc-class.md) e [CRichEditCntrItem](../../mfc/reference/cricheditcntritem-class.md), fornisce la funzionalità del controllo rich edit nel contesto dell'architettura di visualizzazione di documenti di MFC.  
...
|[CRichEditView::GetInPlaceActiveItem](#getinplaceactiveitem)|Recupera l'elemento OLE che è attualmente attiva in visualizzazione rich edit.|  
...
|[CRichEditView::GetTextLengthEx](#gettextlengthex)|Recupera il numero di caratteri o byte in visualizzazione rich edit. Elenco di flag espanso per metodo per determinare la lunghezza.|  
...
|[CRichEditView::OnUpdateCharEffect](#onupdatechareffect)|Aggiorna l'interfaccia utente di comando per le funzioni membro pubblico di tipo carattere.|  
...
|[CRichEditView::TextNotFound](#textnotfound)|Reimposta lo stato di ricerca interna del controllo.|  
...
|[CRichEditView::OnPrinterChanged](#onprinterchanged)|Imposta le caratteristiche di stampate sul dispositivo specificato.|  
...
|Nome|Descrizione|  
|----------|-----------------|  
|[CRichEditView::m_nBulletIndent](#m_nbulletindent)|Indica la quantità di rientro per gli elenchi di elenco puntato.|  
|[CRichEditView::m_nWordWrap](#m_nwordwrap)|Indica i vincoli di incapsulamento di word.|  
  
## <a name="remarks"></a>Note  
 Un "controllo rich edit" è una finestra in cui l'utente può immettere e modificare il testo. Il testo può essere assegnato formattazione di carattere e paragrafo e può includere oggetti OLE incorporati. Controlli rich edit forniscono un'interfaccia di programmazione per la formattazione del testo. Tuttavia, un'applicazione deve implementare qualsiasi componenti dell'interfaccia utente necessari per rendere disponibili le operazioni di formattazione per l'utente.  
 ...
 Per un esempio dell'utilizzo di una visualizzazione rich edit in un'applicazione MFC, vedere la [WORDPAD](../../visual-cpp-samples.md) applicazione di esempio.  
...
### <a name="remarks"></a>Note  
 ...
 Questa funzione determina il tipo di Incolla in base ai risultati del gestore per Incolla speciale. Se *cf* è 0, il nuovo elemento Usa la rappresentazione sotto forma di icona corrente. Se *cf* è diverso da zero e *hMetaPict* non **NULL**, il nuovo elemento Usa *hMetaPict* per la relativa rappresentazione.  
...
##  <a name="getcharformatselection"></a>  CRichEditView::GetCharFormatSelection  
 Chiamare questa funzione per ottenere il carattere degli attributi della selezione corrente di formattazione.  
...
### <a name="return-value"></a>Valore restituito  
 Un [CHARFORMAT2](http://msdn.microsoft.com/library/windows/desktop/bb787883) struttura che contiene il carattere degli attributi della selezione corrente di formattazione.  
  
### <a name="remarks"></a>Note  
 Per altre informazioni, vedere la [EM_GETCHARFORMAT](http://msdn.microsoft.com/library/windows/desktop/bb788026) messaggio e il [CHARFORMAT2](http://msdn.microsoft.com/library/windows/desktop/bb787883) struttura in Windows SDK.  
...
### <a name="parameters"></a>Parametri  
 *lpchrg*  
 Puntatore ai [struttura CHARRANGE](http://msdn.microsoft.com/library/windows/desktop/bb787885) struttura che specifica l'intervallo di caratteri (e gli elementi OLE) per copiare in un oggetto dati specificato da *lplpdataobj*.  
 ...
 *lpRichDataObj*  
 Puntatore a un [IDataObject](http://msdn.microsoft.com/library/windows/desktop/ms688421) controllo oggetto contenente i dati negli Appunti da rich edit ( [IRichEditOle::GetClipboardData](http://msdn.microsoft.com/library/windows/desktop/bb774341)).  
  
 *lplpdataobj*  
 Puntatore alla variabile che riceve l'indirizzo del puntatore il `IDataObject` oggetto che rappresenta l'intervallo specificato nella *lpchrg* parametro. Il valore di *lplpdataobj* viene ignorata se viene restituito un errore.  
  
### <a name="return-value"></a>Valore restituito  
 Un `HRESULT` valore reporting la riuscita dell'operazione. Per ulteriori informazioni sul `HRESULT`, vedere [struttura of COM Error Codes](http://msdn.microsoft.com/library/windows/desktop/ms690088) in Windows SDK.  
  
### <a name="remarks"></a>Note  
 Se il valore restituito indica l'esito positivo, **IRichEditOleCallback::GetClipboardData** restituisce il `IDataObject` accede *lplpdataobj*; in caso contrario, viene restituito quello a cui accede  *lpRichDataObj*. Eseguire l'override di questa funzione per fornire i propri dati negli Appunti. L'implementazione predefinita di questa funzione restituisce **E_NOTIMPL**.  
  
 Si tratta di un'avanzata sottoponibile a override.  
  
 Per altre informazioni, vedere [IRichEditOle::GetClipboardData](http://msdn.microsoft.com/library/windows/desktop/bb774341), [IRichEditOleCallback::GetClipboardData](http://msdn.microsoft.com/library/windows/desktop/bb774315), e [struttura CHARRANGE](http://msdn.microsoft.com/library/windows/desktop/bb787885) in Windows SDK e vedere [IDataObject](http://msdn.microsoft.com/library/windows/desktop/ms688421) in Windows SDK.  
 ...
 *lpoleobj*  
 Puntatore a un **OLEOBJECT** struttura che specifica il primo oggetto OLE selezionato se la selezione contiene uno o più elementi OLE. Se la selezione non contiene elementi, *lpoleobj* viene **NULL**. Il **OLEOBJECT** struttura contiene un puntatore a un oggetto OLE v-table.  
...
- `SEL_MULTIOBJECT` Indica che la selezione corrente contiene più di un oggetto OLE.  
 ...
 Per altre informazioni, vedere [IRichEditOleCallback::GetContextMenu](http://msdn.microsoft.com/library/windows/desktop/bb774317) e [struttura CHARRANGE](http://msdn.microsoft.com/library/windows/desktop/bb787885) in Windows SDK.  
  
 Per ulteriori informazioni sul **OLEOBJECT** digitare, vedere l'articolo di strutture di dati OLE e allocazione di struttura nel *OLE Knowledge Base*.  
...
##  <a name="getinplaceactiveitem"></a>  CRichEditView::GetInPlaceActiveItem  
 Chiamata di questa funzione per ottenere OLE elemento che è attualmente attivato sul posto in questa `CRichEditView` oggetto.  
...
### <a name="return-value"></a>Valore restituito  
 Un puntatore allo stato attivo sul posto, single [CRichEditCntrItem](../../mfc/reference/cricheditcntritem-class.md) oggetto in questa visualizzazione rich edit; **NULL** se non vi è alcun elemento OLE attualmente nello stato attivo sul posto.  
...
### <a name="return-value"></a>Valore restituito  
 I margini utilizzati per la stampa, misurata in `MM_TWIPS`.  
...
### <a name="return-value"></a>Valore restituito  
 Il formato della carta utilizzato per la stampa, misurata in `MM_TWIPS`.  
...
### <a name="return-value"></a>Valore restituito  
 Un [PARAFORMAT2](http://msdn.microsoft.com/library/windows/desktop/bb787942) struttura che contiene gli attributi della selezione corrente di formattazione di paragrafo.  
  
### <a name="remarks"></a>Note  
 Per altre informazioni, vedere [EM_GETPARAFORMAT](http://msdn.microsoft.com/library/windows/desktop/bb774182) messaggio e [PARAFORMAT2](http://msdn.microsoft.com/library/windows/desktop/bb787942) struttura in Windows SDK.  
...
### <a name="return-value"></a>Valore restituito  
 I limiti dell'area dell'immagine utilizzata per la stampa, misurata in `MM_TWIPS`.  
...
### <a name="return-value"></a>Valore restituito  
 Puntatore a un [CRichEditCntrItem](../../mfc/reference/cricheditcntritem-class.md) oggetto selezionato nella `CRichEditView` dell'oggetto; **NULL** se in questa vista è selezionato alcun elemento.  
...
### <a name="parameters"></a>Parametri  
 *dwFlags*  
 Valore che specifica il metodo da utilizzare per determinare la lunghezza del testo. Questo membro può essere uno o più dei valori elencati nel membro flag della [GETTEXTLENGTHEX](http://msdn.microsoft.com/library/windows/desktop/bb787915) descritte in SDK.  
...
### <a name="remarks"></a>Note  
 `GetTextLengthEx` offre ulteriori metodi per determinare la lunghezza del testo. Supporta la funzionalità Rich Edit 2.0. Per altre informazioni, vedere [sui controlli Rich Edit](http://msdn.microsoft.com/library/windows/desktop/bb787873) in Windows SDK.  
...
### <a name="return-value"></a>Valore restituito  
 Un `HRESULT` valore che indica l'esito positivo dell'inserimento.  
  
### <a name="remarks"></a>Note  
 Per ulteriori informazioni sul `HRESULT`, vedere [struttura of COM Error Codes](http://msdn.microsoft.com/library/windows/desktop/ms690088) in Windows SDK.  
  
##  <a name="isricheditformat"></a>  CRichEditView::IsRichEditFormat  
 Chiamare questa funzione per determinare se *cf* è un formato degli Appunti è testo, testo RTF o RTF con elementi OLE.  
...
##  <a name="m_nbulletindent"></a>  CRichEditView::m_nBulletIndent  
 Il rientro per gli elementi punto elenco in un elenco. Per impostazione predefinita, 720 unità, ovvero 1/2 pollici.  
 ...
 *dwEffect*  
 L'elenco desiderato formattazione effetti per attivare/disattivare dei caratteri.  
  
### <a name="remarks"></a>Note  
 Ogni chiamata a questa funzione attiva o disattiva gli effetti di formattazione specificati per la selezione corrente.  
  
 Per ulteriori informazioni sul *dwMask* e *dwEffect* parametri e i relativi valori possibili, vedere i membri dei dati corrispondente [CHARFORMAT](http://msdn.microsoft.com/library/windows/desktop/bb787881) in Windows SDK.  
  
### <a name="example"></a>Esempio  
 [!code-cpp[NVC_MFCDocView#155](../../mfc/codesnippet/cpp/cricheditview-class_5.cpp)]  
  
##  <a name="onfindnext"></a>  CRichEditView::OnFindNext  
 Chiamato dal framework durante l'elaborazione comandi dalla finestra di dialogo ricerca/sostituzione.  
 ...
 *bAvanti*  
 La direzione di ricerca: **TRUE** indica; **FALSE**verso l'alto.  
  
 *bCase*  
 Indica se la ricerca deve essere tra maiuscole e minuscole.  
  
 *bWord*  
 Indica se la ricerca è per corrispondere a parole intere solo o non.  
...
### <a name="remarks"></a>Note  
 L'implementazione predefinita di questa funzione chiama la [CView::OnUpdate](../../mfc/reference/cview-class.md#onupdate) funzione membro senza informazioni hint (vale a dire usando i valori predefiniti pari a 0 per il *lHint* parametro e **NULL**  per il *pHint* parametro). Eseguire l'override di questa funzione per eseguire qualsiasi inizializzazione che richiede informazioni sul documento. Ad esempio, se l'applicazione presenta dimensioni fisse documenti, è possibile utilizzare questa funzione per inizializzare i limiti di scorrimento della visualizzazione in base alle dimensioni del documento. Se l'applicazione supporta i documenti e di dimensioni variabili, utilizzare `OnUpdate` per aggiornare lo scorrimento Limita ogni volta che il documento viene modificato.  
...
### <a name="remarks"></a>Note  
 In genere, questa operazione viene mediante la creazione di un [COleStreamFile](../../mfc/reference/colestreamfile-class.md) intorno il `IStorage`. Il `COleStreamFile` può essere collegato a un archivio e [CObject:: Serialize](../../mfc/reference/cobject-class.md#serialize) chiamata per caricare i dati.  
  
 Si tratta di un'avanzata sottoponibile a override.  
  
 Per altre informazioni, vedere [IStorage](http://msdn.microsoft.com/library/windows/desktop/aa380015) in Windows SDK.  
  
##  <a name="onparaalign"></a>  CRichEditView::OnParaAlign  
 Chiamare questa funzione per modificare l'allineamento del paragrafo per paragrafi selezionati.  
...
### <a name="parameters"></a>Parametri  
 *dcPrinter*  
 Un [CDC](../../mfc/reference/cdc-class.md) oggetto per la nuova stampante.  
  
### <a name="remarks"></a>Note  
 L'implementazione predefinita imposta il formato carta fisico altezza e alla larghezza per il dispositivo di output (stampante). Se è presente alcun contesto di dispositivo non associati *dcPrinter*, l'implementazione predefinita imposta il formato carta per 8,5 per 11 pollici.  
 ...
 *bWord*  
 Indica se la ricerca deve selezionare parole intere o non.  
...
##  <a name="onreplacesel"></a>  CRichEditView::OnReplaceSel  
 Chiamato dal framework durante l'elaborazione di comandi di sostituzione dalla finestra di dialogo Sostituisci.  
 ...
 *bAvanti*  
 Indica la direzione della ricerca: **TRUE** è inattivo; **FALSE**verso l'alto.  
...
### <a name="remarks"></a>Note  
 ...
 Per altre informazioni, vedere [MessageBeep](http://msdn.microsoft.com/library/windows/desktop/ms680356) in Windows SDK.  
...
### <a name="remarks"></a>Note  
 La maschera *dwMask* specifica quale carattere gli attributi di formattazione da controllare. Il flag *dwEffect* elencare il carattere degli attributi a set, la cancellazione di formattazione.  
 ...
 *wAlign*  
 L'allineamento del paragrafo da controllare. Uno dei valori seguenti:  
...
### <a name="parameters"></a>Parametri  
 *pDC*  
 Puntatore a un contesto di dispositivo di output delle pagine.  
...
### <a name="return-value"></a>Valore restituito  
 L'indice dell'ultimo carattere che si adatta alla pagina più uno.  
...
### <a name="remarks"></a>Note  
 ...
 Si noti che i margini sono relativi alla pagina fisica, non la pagina logica. Di conseguenza, i margini pari a zero spesso ridurrà il testo poiché molte stampanti dispongono di aree della pagina. Per evitare di taglio del testo, è necessario chiamare [SetMargins](#setmargins) e impostare i margini ragionevoli prima della stampa.  
  
##  <a name="queryacceptdata"></a>  CRichEditView::QueryAcceptData  
 Chiamato dal framework per incollare un oggetto in modifica avanzata.  
...
### <a name="parameters"></a>Parametri  
 *lpdataobj*  
 Puntatore ai [IDataObject](http://msdn.microsoft.com/library/windows/desktop/ms688421) alla query.  
  
 *lpcfFormat*  
 Puntatore per il formato di dati accettabili.  
  
 *dwReco*  
 Non usato.  
  
 *bReally*  
 Indica se l'operazione Incolla deve continuare o No.  
...
### <a name="remarks"></a>Note  
 Eseguire l'override della funzione per gestire diversi organizzazione degli elementi COM nella classe derivata di documento. Si tratta di un'avanzata sottoponibile a override.  
  
 Per ulteriori informazioni sul `HRESULT` e `IDataObject`, vedere [struttura of COM Error Codes](http://msdn.microsoft.com/library/windows/desktop/ms690088) e [IDataObject](http://msdn.microsoft.com/library/windows/desktop/ms688421), rispettivamente, in Windows SDK.  
...
### <a name="remarks"></a>Note  
 Solo gli attributi specificati dal **dwMask** appartenente *cf* modificata da questa funzione.  
  
 Per altre informazioni, vedere [EM_SETCHARFORMAT](http://msdn.microsoft.com/library/windows/desktop/bb774230) messaggio e [CHARFORMAT2](http://msdn.microsoft.com/library/windows/desktop/bb787883) struttura in Windows SDK.  
...
### <a name="parameters"></a>Parametri  
 *rectMargin*  
 I nuovi valori dei margini per la stampa, misurata in `MM_TWIPS`.  
  
### <a name="remarks"></a>Note  
 Se [m_nWordWrap](#m_nwordwrap) viene `WrapToTargetDevice`, è necessario chiamare [WrapChanged](#wrapchanged) dopo l'utilizzo di questa funzione per modificare le caratteristiche di stampa.  
  
 Si noti che i margini utilizzati da [PrintPage](#printpage) sono relativi alla pagina fisica, non la pagina logica. Di conseguenza, i margini pari a zero spesso ridurrà il testo poiché molte stampanti dispongono di aree della pagina. Per evitare di taglio del testo, è necessario chiamare utilizzare `SetMargins` per impostare i margini di stampa ragionevole prima della stampa.  
...
### <a name="parameters"></a>Parametri  
 *sizePaper*  
 I nuovi valori di dimensioni carta per la stampa, misurata in `MM_TWIPS`.  
...
### <a name="remarks"></a>Note  
 Solo gli attributi specificati dal **dwMask** appartenente *pf* modificata da questa funzione.  
  
 Per altre informazioni, vedere [EM_SETPARAFORMAT](http://msdn.microsoft.com/library/windows/desktop/bb774276) messaggio e [PARAFORMAT2](http://msdn.microsoft.com/library/windows/desktop/bb787942) struttura in Windows SDK.  
  
### <a name="example"></a>Esempio  
 [!code-cpp[NVC_MFCDocView#162](../../mfc/codesnippet/cpp/cricheditview-class_12.cpp)]  
  
##  <a name="textnotfound"></a>  CRichEditView::TextNotFound  
 Chiamare questa funzione per reimpostare lo stato interno di ricerca del [CRichEditView](../../mfc/reference/cricheditview-class.md) controllo dopo una chiamata a [FindText](#findtext).  
...
### <a name="remarks"></a>Note  
 È consigliabile che questo metodo deve essere chiamato immediatamente dopo le chiamate non riuscite per [FindText](#findtext) in modo che lo stato di ricerca interna del controllo viene reimpostato in modo corretto.  
  
 Il *lpszFind* parametro deve includere lo stesso contenuto la stringa fornita per [FindText](#findtext). Dopo aver reimpostato lo stato di ricerca interno, questo metodo chiama il metodo di [OnTextNotFound](#ontextnotfound) metodo con la stringa di ricerca specificato.  
  
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo [CRichEditView::FindText](#findtext).  
  
##  <a name="wrapchanged"></a>  CRichEditView::WrapChanged  
 Chiamare questa funzione quando cambiano le caratteristiche di stampa ( [SetMargins](#setmargins) oppure [SetPaperSize](#setpapersize)).  
...
### <a name="remarks"></a>Note  
 Override di questa funzione per modificare il modo in cui il rich edit Vista risponde alle modifiche [m_nWordWrap](#m_nwordwrap) o caratteristiche di stampa ( [OnPrinterChanged](#onprinterchanged)).  
