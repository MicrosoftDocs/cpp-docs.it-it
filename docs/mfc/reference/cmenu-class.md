...
|[CMenu::Attach](#attach)|Connette un handle di menu di Windows a `CMenu` un oggetto.|
...
|[CMenu::CreatePopupMenu](#createpopupmenu)|Crea un menu popup vuoto e lo collega a un `CMenu` oggetto.|
|[CMenu::DeleteMenu](#deletemenu)|Elimina un elemento specificato dal menu. Se la voce di menu dispone di un menu di scelta rapida associato, Elimina l'handle dal menu a comparsa e libera la memoria usata.|
|[CMenu::DeleteTempMap](#deletetempmap)|Elimina tutti gli `CMenu` oggetti temporanei creati `FromHandle` dalla funzione membro.|
|[CMenu::DestroyMenu](#destroymenu)|Elimina il menu associato a un `CMenu` oggetto e libera la memoria occupata dal menu.|
|[CMenu::Detach](#detach)|Scollega un handle di menu di Windows da `CMenu` un oggetto e restituisce l'handle.|
...
|[CMenu::GetSafeHmenu](#getsafehmenu)|Restituisce l' `m_hMenu` `CMenu` oggetto di cui è stato eseguito il wrapper.|
|[CMenu::GetSubMenu](#getsubmenu)|Recupera un puntatore a un menu di scelta rapida.|
...
|[CMenu::LoadMenu](#loadmenu)|Carica una risorsa di menu dal file eseguibile e la collega a un `CMenu` oggetto.|
...
Creare un `CMenu` oggetto nel stack frame come locale, quindi chiamare `CMenu`le funzioni membro per modificare il nuovo menu in base alle esigenze. Chiamare quindi [CWnd:: semenu](../../mfc/reference/cwnd-class.md#setmenu) per impostare il menu su una finestra, seguito immediatamente da una chiamata alla funzione membro `CMenu` [Detach](#detach) dell'oggetto. La `CWnd::SetMenu` funzione membro imposta il menu della finestra sul nuovo menu, fa in modo che la finestra venga ridisegnato per riflettere la modifica del menu e passa anche la proprietà del menu alla finestra. La chiamata a `Detach` scollega HMENU `CMenu` dall'oggetto, in modo che quando la variabile locale `CMenu` passa fuori dall'ambito, il `CMenu` distruttore di oggetti non tenta di eliminare definitivamente un menu che non ne è più proprietario. Il menu stesso viene eliminato automaticamente quando viene eliminata definitivamente la finestra.
...
*nIDNewItem*<br/>
Specifica l'ID di comando della nuova voce di menu o, se *nFlags* è impostato su MF_POPUP, il menu `HMENU`di scelta rapida del menu a comparsa. Il parametro *nIDNewItem* viene ignorato (non necessario) se *nFlags* è impostato su MF_SEPARATOR.
...
- MF_UNCHECKED funge da interruttore con MF_CHECKED per rimuovere un segno di spunta accanto all'elemento. Quando l'applicazione fornisce bitmap di segno di spunta (vedere la `SetMenuItemBitmaps` funzione membro), viene visualizzata la bitmap "segno di spunta fuori".
...
- MF_MENUBARBREAK posiziona l'elemento in una nuova riga in menu statici o in una nuova colonna nei menu a comparsa. La nuova colonna del menu popup sarà separata dalla colonna precedente mediante una linea di divisione verticale.

- MF_MENUBREAK posiziona l'elemento in una nuova riga in menu statici o in una nuova colonna nei menu a comparsa. Non viene posizionata alcuna linea di separazione tra le colonne.

- MF_OWNERDRAW specifica che l'elemento è un elemento di traccia del proprietario. Quando il menu viene visualizzato per la prima volta, la finestra che possiede il menu riceve un messaggio WM_MEASUREITEM, che recupera l'altezza e la larghezza della voce di menu. Il messaggio WM_DRAWITEM è quello inviato ogni volta che il proprietario deve aggiornare l'aspetto visivo della voce di menu. Questa opzione non è valida per una voce di menu di primo livello.
...
*nCheck*<br/>
Specifica come controllare la voce di menu e come determinare la posizione dell'elemento nel menu. Il parametro *nPer* può essere una combinazione di MF_CHECKED o MF_UNCHECKED con flag MF_BYPOSITION o MF_BYCOMMAND. Questi flag possono essere combinati utilizzando l'operatore OR bit per bit. Hanno i significati seguenti:

- MF_BYCOMMAND specifica che il parametro restituisce l'ID del comando della voce di menu esistente. Questa è l'impostazione predefinita.
...
|------------|--------------------|
|MF_BYCOMMAND|Specifica che il parametro fornisce l'ID del comando della voce di menu esistente. Si tratta dell'impostazione predefinita se non è impostato né MF_BYCOMMAND né MF_BYPOSITION.|
|MF_BYPOSITION|Specifica che il parametro assegna la posizione della voce di menu esistente. Il primo elemento si trova nella posizione 0.|
...
Il menu è inizialmente vuoto. È possibile aggiungere voci di menu tramite la `AppendMenu` funzione `InsertMenu` membro o.
...
Se la voce di menu dispone di un menu di scelta rapida `DeleteMenu` associato, Elimina l'handle dal menu a comparsa e libera la memoria usata dal menu a comparsa.
...
Chiamato automaticamente dal gestore `CWinApp` del tempo di inattività, Elimina tutti `CMenu` gli oggetti temporanei creati dalla funzione membro [FromHandle](#fromhandle) .
...
`DeleteTempMap`Scollega l'oggetto menu di Windows collegato a un oggetto `CMenu` temporaneo prima di eliminare `CMenu` l'oggetto.
...
Il menu viene scollegato dall' `CMenu` oggetto prima che venga eliminato definitivamente. La funzione `DestroyMenu` Windows viene chiamata automaticamente `CMenu` nel distruttore.
...
Il `itemAction` membro`DRAWITEMSTRUCT` della struttura definisce l'azione di disegno da eseguire. Eseguire l'override di questa funzione membro per implementare il disegno per `CMenu` un oggetto disegnato dal proprietario. L'applicazione deve ripristinare tutti gli oggetti GDI (Graphics Device Interface) selezionati per il contesto di visualizzazione fornito in *lpDrawItemStruct* prima della chiusura di questa funzione membro.
...
L'utilizzo del valore MF_BYPOSITION richiede che un'applicazione utilizzi l' `CMenu`oggetto corretto. Se viene `CMenu` utilizzata la proprietà della barra dei menu, viene applicata una voce di menu di primo livello (un elemento nella barra dei menu). Per impostare lo stato di un elemento in un popup o un menu popup annidato in base alla posizione, un'applicazione deve specificare l' `CMenu` oggetto del menu a comparsa.

Quando in un'applicazione viene specificato il flag MF_BYCOMMAND, in Windows vengono controllate tutte le voci di menu popup subordinate a `CMenu`. Pertanto, a meno che non siano presenti voci di menu duplicate, è sufficiente utilizzare la `CMenu` della barra dei menu.
...
Se un `CMenu` oggetto non è già collegato all'oggetto menu di Windows, viene creato `CMenu` e collegato un oggetto temporaneo.

Questo oggetto `CMenu` temporaneo è valido solo fino alla successiva esecuzione del tempo di inattività nel ciclo di eventi dell'applicazione, al momento dell'eliminazione di tutti gli oggetti temporanei.
...
Recupera l'ID della guida del contesto `CMenu`associato a.
...
ID della guida del contesto attualmente associato `CMenu` a se ne esiste uno; in caso contrario, zero.
...
*fByPos*<br/>
Valore che specifica il significato `nIDItem`di. Per impostazione predefinita `ByPos` , è false, che indica che uItem è un identificatore della voce di menu. Se `ByPos` non è impostato su false, indica una posizione della voce di menu.
...
Restituisce l'HMENU di cui è `CMenu` stato eseguito il wrapper da`CMenu` questo oggetto o un puntatore null.
...
*nIDNewItem*<br/>
Specifica l'ID di comando della nuova voce di menu o, se *nFlags* è impostato su MF_POPUP, il menu di scelta rapida (HMENU) del menu a comparsa. Il parametro *nIDNewItem* viene ignorato (non necessario) se *nFlags* è impostato su MF_SEPARATOR.
...
|nFlags|Interpretazione di lpszNewItem|
|------------|-----------------------------------|
|MF_OWNERDRAW|Contiene un valore a 32 bit fornito dall'applicazione che può essere utilizzato dall'applicazione per gestire dati aggiuntivi associati alla voce di menu. Questo valore a 32 bit è disponibile per l'applicazione nel `itemData` membro della struttura fornita dai messaggi [WM_MEASUREITEM](/windows/win32/Controls/wm-measureitem) e [WM_DRAWITEM](/windows/win32/Controls/wm-drawitem) . Questi messaggi vengono inviati quando la voce di menu viene inizialmente visualizzata o modificata.|
...
Ogni volta che un menu che risiede in una finestra viene modificato (indipendentemente dal fatto che venga visualizzata la finestra), l'applicazione `CWnd::DrawMenuBar`deve chiamare.
...
*lpMenuItemInfo*<br/>
Vedere la descrizione di lpmii `InsertMenuItem` in nel Windows SDK.

*fByPos*<br/>
Vedere la descrizione di fByPosition `InsertMenuItem` in nel Windows SDK.
...
Carica una risorsa da un modello di menu in memoria e la `CMenu` collega all'oggetto.
...
I `mtOption` flag devono includere MF_END per l'ultimo elemento in un elenco popup e per l'ultimo elemento nell'elenco principale. Vedere la `AppendMenu` funzione membro per altri flag. Il `mtId` membro deve essere omesso dalla struttura MENUITEMTEMPLATE quando MF_POPUP è specificato in `mtOption`.

Lo spazio allocato per la struttura MENUITEMTEMPLATE deve essere sufficientemente `mtString` grande da contenere il nome della voce di menu come stringa con terminazione null.
...
Per impostazione predefinita, questa funzione membro non esegue alcuna operazione. Eseguire l'override di questa funzione membro e `MEASUREITEMSTRUCT` compilare la struttura per informare le finestre delle dimensioni del menu.
...
|MF_STRING|Contiene un puntatore long a una stringa con terminazione null o a `CString`un oggetto.|
|MF_SEPARATOR|Il parametro *lpszNewItem* viene ignorato (non necessario).|
...
Ogni volta che un menu che risiede in una finestra viene modificato (indipendentemente dal fatto che venga visualizzata la finestra), l'applicazione `CWnd::DrawMenuBar`deve chiamare. Per modificare gli attributi delle voci di menu esistenti, è molto più veloce usare le `CheckMenuItem` funzioni `EnableMenuItem` membro e.
...
Se ha esito positivo, l' `CMenu` handle dell'oggetto; in caso contrario, null.
...
Non elimina l'handle per un menu di scelta rapida, quindi il menu può essere riutilizzato. Prima di chiamare questa funzione, l'applicazione può chiamare `GetSubMenu` la funzione membro per recuperare l' `CMenu` oggetto popup per il riutilizzo.
...
Associa un ID di guida del `CMenu`contesto a.
...
*dwContextHelpId*<br/>
ID della guida del contesto da `CMenu`associare a.
...
*lpMenuItemInfo*<br/>
Vedere la descrizione di lpmii `SetMenuItemInfo` in nel Windows SDK.

*fByPos*<br/>
Vedere la descrizione di fByPosition `SetMenuItemInfo` in nel Windows SDK.
...
*pWnd*<br/>
Identifica la finestra proprietaria del menu a comparsa. Questo parametro non può essere NULL, anche se è specificato il flag TPM_NONOTIFY. Questa finestra riceve tutti i messaggi WM_COMMAND dal menu. Nelle versioni di Windows 3,1 e successive la finestra non riceve messaggi WM_COMMAND fino a `TrackPopupMenu` quando non viene restituito. In Windows 3,0, la finestra riceve i messaggi WM_COMMAND `TrackPopupMenu` prima della restituzione.
...
*pWnd*<br/>
Puntatore alla finestra proprietaria del menu a comparsa e ricezione dei messaggi dal menu creato. Questa finestra può essere qualsiasi finestra dell'applicazione corrente, ma non può essere NULL. Se si specifica TPM_NONOTIFY nel parametro *fuFlags* , la funzione non invierà alcun messaggio a *pWnd*. La funzione deve restituire per la finestra a cui punta *pWnd* per ricevere il messaggio WM_COMMAND.
