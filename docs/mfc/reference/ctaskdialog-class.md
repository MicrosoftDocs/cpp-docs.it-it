...
|[CTaskDialog::GetCommonButtonFlag](#getcommonbuttonflag)|Converte un standard pulsante di Windows per il tipo di pulsante comune associato il `CTaskDialog` classe.|  
|[CTaskDialog::GetCommonButtonId](#getcommonbuttonid)|Converte uno dei comuni tipi di pulsanti associati con la `CTaskDialog` classe a un pulsante standard di Windows.|  
|[CTaskDialog::GetOptions](#getoptions)|Restituisce i flag di opzione per questo `CTaskDialog`.|  
...
|[CTaskDialog::LoadCommandControls](#loadcommandcontrols)|Aggiunge controlli pulsante di comando utilizzando i dati dalla tabella di stringhe.|  
...
|[CTaskDialog::OnNavigatePage](#onnavigatepage)|Il framework chiama questo metodo quando l'utente sposta lo stato attivo in relazione a controlli `CTaskDialog`.|  
...
|[CTaskDialog::SetCommonButtons](#setcommonbuttons)|Consente di aggiungere pulsanti comuni per la `CTaskDialog`.|  
...
|||  
|-|-|  
|`m_aButtons`|La matrice di controlli pulsanti di comando per il `CTaskDialog`.|  
|`m_aRadioButtons`|Matrice di controlli pulsanti di opzione per il `CTaskDialog`.|  
|`m_bVerified`|`TRUE` indica che la casella di controllo di verifica è selezionata; `FALSE` indica non.|  
|`m_footerIcon`|L'icona nel piè di pagina il `CTaskDialog`.|  
...
|`m_nButtonElevation`|Una maschera che indica quale dei pulsanti comuni richiedono l'elevazione dei privilegi di controllo dell'account utente.|  
...
|`m_nDefaultRadioButton`|L'ID del pulsante di opzione di controllo che sia selezionata quando il `CTaskDialog` viene visualizzato.|  
...
|`m_strCollapse`|La stringa di `CTaskDialog` viene visualizzato a destra della casella di espansione quando sono occultate le informazioni espanse.|  
...
## <a name="remarks"></a>Note  
 Il `CTaskDialog` classe sostituisce la finestra di messaggio standard di Windows e dispone di funzionalità aggiuntive, ad esempio nuovi controlli per raccogliere informazioni dall'utente. Questa classe è nella libreria MFC in [!INCLUDE[vs_dev10_long](../../build/includes/vs_dev10_long_md.md)]. Il `CTaskDialog` è disponibile a partire da Windows Vista. Le versioni precedenti di Windows non è possibile visualizzare il `CTaskDialog` oggetto. Utilizzare `CTaskDialog::IsSupported` per determinare in fase di esecuzione se l'utente corrente può visualizzare la finestra di dialogo delle attività. La finestra di messaggio standard di Windows è ancora supportata in [!INCLUDE[vs_dev10_long](../../build/includes/vs_dev10_long_md.md)].  
 ...
 La figura seguente mostra un esempio `CTaskDialog` per illustrare il percorso di alcuni dei controlli.  
...
### <a name="parameters"></a>Parametri  
 ...
 [in] *strCaption*  
 La stringa che la `CTaskDialog` viene visualizzato all'utente. Questa stringa può essere utilizzata per illustrare lo scopo del comando.  
...
### <a name="remarks"></a>Note  
 I pulsanti di opzione per la [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md) consentono di raccogliere informazioni dall'utente. Utilizzare la funzione [CTaskDialog::GetSelectedRadioButtonID](#getselectedradiobuttonid) per determinare quale pulsante di opzione è selezionato.  
...
### <a name="parameters"></a>Parametri  
 [in] *strContent*  
 La stringa da usare per il contenuto del `CTaskDialog`.  
...
### <a name="remarks"></a>Note  
 Esistono due modi che è possibile aggiungere un `CTaskDialog` all'applicazione. Il primo modo consiste nell'utilizzare uno dei costruttori per creare una `CTaskDialog` e visualizzarli mediante [CTaskDialog::DoModal](#domodal). Il secondo modo consiste nell'utilizzare la funzione statica [CTaskDialog::ShowDialog](#showdialog), che consente di visualizzare un `CTaskDialog` senza creare esplicitamente un `CTaskDialog` oggetto.  
  
 Il secondo costruttore crea controlli pulsante di comando utilizzando i dati dal file di risorse dell'applicazione. Tabella di stringhe nel file di risorse dispone di più stringhe con ID di stringa associato. Questo metodo aggiunge un controllo pulsante di comando per ogni voce valido nella tabella di stringhe tra *nIDCommandControlsFirst* e *nCommandControlsLast*, estremi inclusi. Per questi controlli pulsante di comando, la stringa nella tabella di stringhe è la didascalia del controllo e l'ID di stringa è il relativo ID.  
...
### <a name="remarks"></a>Note  
 Visualizza questa istanza di [CTaskDialog](../../mfc/reference/ctaskdialog-class.md). Quindi, l'applicazione è in attesa per l'utente chiudere la finestra di dialogo.  
  
 Il `CTaskDialog` si chiude quando l'utente seleziona un pulsante comune, un controllo di collegamento di comando, o chiude il `CTaskDialog`. Il valore restituito è l'identificatore che indica come viene chiusa la finestra di dialogo.  
...
### <a name="remarks"></a>Note  
 ...
 Viene fornito l'elenco enumerato dei pulsanti in commctrl.  
...
##  <a name="getcommonbuttonid"></a>  CTaskDialog::GetCommonButtonId  
 Converte uno dei comuni tipi di pulsanti associati con la [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md) a un pulsante standard di Windows.  
...
### <a name="remarks"></a>Note  
 Non è necessario utilizzare questo metodo per recuperare l'ID del pulsante di comando selezionato dall'utente. Tale ID viene restituito da uno degli identificatori [CTaskDialog::DoModal](#domodal) oppure [CTaskDialog::ShowDialog](#showdialog).  
...
### <a name="remarks"></a>Note  
 È possibile utilizzare questo metodo per determinare la disponibilità di entrambi i controlli pulsante comando e i pulsanti comuni del `CTaskDialog` classe *.  
  
 Se *nCommandControlID* tratta non è un identificatore valido per uno di un comune `CTaskDialog` pulsante o un controllo pulsante di comando, questo metodo genera un'eccezione.  
...
### <a name="remarks"></a>Note  
 Utilizzare questa funzione per determinare in fase di esecuzione se il computer che esegue l'applicazione supporta il `CTaskDialog` classe. Se il computer non supporta il `CTaskDialog`, è necessario fornire un altro metodo per la comunicazione di informazioni all'utente. L'applicazione verificherà un arresto anomalo se si tenta di utilizzare un `CTaskDialog` in un computer che non supporta la `CTaskDialog` classe.  
  
### <a name="example"></a>Esempio  
 [!code-cpp[NVC_MFC_CTaskDialog#1](../../mfc/reference/codesnippet/cpp/ctaskdialog-class_5.cpp)]  
  
##  <a name="loadcommandcontrols"></a>  CTaskDialog::LoadCommandControls  
 Aggiunge controlli pulsante di comando utilizzando i dati dalla tabella di stringhe.  
...
##  <a name="loadradiobuttons"></a>  CTaskDialog::LoadRadioButtons  
 Aggiunge controlli pulsante di opzione utilizzando i dati dalla tabella di stringhe.  
...
### <a name="remarks"></a>Note  
 Questo metodo crea pulsanti di opzione utilizzando i dati dal file di risorse dell'applicazione. Tabella di stringhe nel file di risorse dispone di più stringhe con ID di stringa associato. Nuovi pulsanti di opzione aggiunti utilizzando questo metodo per utilizzano la stringa per la didascalia del pulsante di opzione e l'ID di stringa per l'ID. del pulsante di opzione L'intervallo di stringhe selezionato viene fornito da *nIDRadioButtonsFirst* e *nRadioButtonsLast*, estremi inclusi. Se è presente una voce vuota nell'intervallo, il metodo non aggiunge un pulsante di opzione per tale voce.  
  
 Per impostazione predefinita, sono abilitati nuovi pulsanti di opzione.  
...
### <a name="parameters"></a>Parametri  
 [in] *bExpanded*  
 Un valore diverso da zero indica che vengono visualizzate le informazioni supplementari; 0 indica che le informazioni aggiuntive sono nascosto.  
...
### <a name="remarks"></a>Note  
 Questo metodo chiama [ShellExecute](http://msdn.microsoft.com/library/windows/desktop/bb762153) prima viene restituito S_OK.  
...
##  <a name="onnavigatepage"></a>  CTaskDialog::OnNavigatePage  
 Il framework chiama questo metodo in risposta ai [CTaskDialog::NavigateTo](#navigateto) metodo.  
...
### <a name="parameters"></a>Parametri  
 [in] *bChecked*  
 TRUE indica che è selezionata la casella di controllo verifica; FALSE indica che non è.  
...
### <a name="remarks"></a>Note  
 Utilizzare questo metodo per modificare la modalità di un controllo pulsante di comando è abilitato o richiede l'elevazione dei privilegi dopo che è stato aggiunto il `CTaskDialog` classe.  
...
### <a name="remarks"></a>Note  
 È possibile impostare i pulsanti comuni disponibili a un'istanza del [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md) utilizzando il costruttore [CTaskDialog::CTaskDialog](#ctaskdialog) e il metodo [CTaskDialog::SetCommonButtons ](#setcommonbuttons). `CTaskDialog::SetCommonButtonOptions` non supporta l'aggiunta di nuovi pulsanti comuni.  
  
 Se si utilizza questo metodo per disabilitare o elevare il livello di un pulsante comune che non è disponibile per questo `CTaskDialog`, questo metodo genera un'eccezione utilizzando il [verificare](diagnostic-services.md#ensure) macro.  
  
 Questo metodo consente a qualsiasi pulsante è disponibile per il `CTaskDialog` ma non è inclusa nella *nDisabledButtonMask*, anche se era disabilitata. Questo metodo considera l'elevazione dei privilegi in modo analogo: registra comuni pulsanti che non richiedono l'elevazione dei privilegi, se il pulsante comune è disponibile ma non è incluso nel *nElevationButtonMask*.  
  
### <a name="example"></a>Esempio  
 [!code-cpp[NVC_MFC_CTaskDialog#6](../../mfc/reference/codesnippet/cpp/ctaskdialog-class_6.cpp)]  
  
##  <a name="setcommonbuttons"></a>  CTaskDialog::SetCommonButtons  
 Consente di aggiungere pulsanti comuni per la `CTaskDialog`.  
...
### <a name="remarks"></a>Note  
 ...
 I pulsanti indicato da *nButtonMask* eseguire l'override dei pulsanti comuni aggiunti in precedenza per il `CTaskDialog`. Solo i pulsanti indicati in *nButtonMask* sono disponibili.  
  
 Se entrambi *nDisabledButtonMask* oppure *nElevationButtonMask* contiene un pulsante che non si trova nello *nButtonMask*, questo metodo genera un'eccezione utilizzando il [Verificare](diagnostic-services.md#ensure) macro.  
...
### <a name="remarks"></a>Note  
 Il contenuto del `CTaskDialog` classe è il testo che viene visualizzato all'utente nella sezione principale della finestra di dialogo.  
...
### <a name="remarks"></a>Note  
 Il controllo pulsante di comando predefinito è il controllo viene selezionata quando il `CTaskDialog` viene prima visualizzato all'utente.  
...
### <a name="remarks"></a>Note  
 L'area di espansione del `CTaskDialog` classe consente di fornire informazioni aggiuntive all'utente. L'area di espansione è nella parte principale del `CTaskDialog`, che si trova immediatamente sotto il titolo e il contenuto di stringa.  
  
 Quando il `CTaskDialog` viene visualizzato, non sono visualizzati informazioni espanse e inserisce `strCollapsedLabel` accanto al pulsante di espansione. Quando l'utente fa clic sul pulsante di espansione, il `CTaskDialog` consente di visualizzare *strExpandedInformation* e le modifiche nell'etichetta *strExpandedLabel*.  
...
### <a name="remarks"></a>Note  
 L'icona di piè di pagina viene visualizzata nella parte inferiore della [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md). È possibile associare testo nel piè di pagina. È possibile modificare il testo del piè di pagina con [CTaskDialog::SetFooterText](#setfootertext).  
  
 Questo metodo genera un'eccezione con il [verificare che](diagnostic-services.md#ensure) macro se la `CTaskDialog` viene visualizzato o il parametro di input è NULL.  
  
 Un `CTaskDialog` può accettare solo un `HICON` o `LPCWSTR` come un'icona di piè di pagina. Questo viene configurato impostando l'opzione TDF_USE_HICON_FOOTER nel costruttore oppure [CTaskDialog::SetOptions](#setoptions). Per impostazione predefinita, il `CTaskDialog` è configurato per utilizzare `LPCWSTR` come tipo di input per l'icona di piè di pagina. Questo metodo genera un'eccezione se si prova a impostare l'icona con il tipo appropriato.  
...
### <a name="remarks"></a>Note  
 ...
 Un `CTaskDialog` può accettare solo un `HICON` o `LPCWSTR` come icona principale. È possibile configurare questa impostando l'opzione TDF_USE_HICON_MAIN nel costruttore o nel [CTaskDialog::SetOptions](#setoptions) metodo. Per impostazione predefinita, il `CTaskDialog` è configurato per utilizzare `LPCWSTR` come tipo di input per l'icona principale. Questo metodo genera un'eccezione se si prova a impostare l'icona con il tipo appropriato.  
...
### <a name="remarks"></a>Note  
 L'istruzione principale del `CTaskDialog` classe è testo visualizzato all'utente in un tipo di carattere grassetto grandi dimensioni. Si trova nella finestra di dialogo sotto la barra del titolo.  
...
### <a name="remarks"></a>Note  
 ...
 TDF_USE_HICON_MAIN  
 Consente di configurare il `CTaskDialog` da utilizzare un `HICON` per l'icona principale. L'alternativa consiste nell'usare un `LPCWSTR`.  
  
 TDF_USE_HICON_FOOTER  
 Consente di configurare il `CTaskDialog` da utilizzare un `HICON` per l'icona di piè di pagina. L'alternativa consiste nell'usare un `LPCWSTR`.  
  
 TDF_ALLOW_DIALOG_CANCELLATION  
 Consente all'utente chiudere la `CTaskDialog` usando la tastiera o utilizzando l'icona nell'angolo superiore destro della finestra di dialogo, anche se il **Annulla** pulsante non è abilitato. Se questo flag non è impostato e il **annullare** pulsante non è abilitato, l'utente non è possibile chiudere la finestra di dialogo utilizzando Alt + F4, il tasto ESC, o pulsante di chiusura della barra del titolo.  
  
 TDF_USE_COMMAND_LINKS  
 Configura il `CTaskDialog` usare i controlli pulsante di comando.  
  
 TDF_USE_COMMAND_LINKS_NO_ICON  
 Configura il `CTaskDialog` usare i controlli pulsante di comando senza visualizzare un'icona accanto al controllo. TDF_USE_COMMAND_LINKS esegue l'override TDF_USE_COMMAND_LINKS_NO_ICON.  
 ...
 Configura l'indicatore di stato per un indicatore di stato di selezione. Se si abilita questa opzione, è necessario impostare TDF_SHOW_PROGRESS_BAR a si comportano nel modo previsto.  
  
 TDF_CALLBACK_TIMER  
 Indica che il `CTaskDialog` callback intervallo è impostato su circa 200 millisecondi.  
  
 TDF_POSITION_RELATIVE_TO_WINDOW  
 Configura il `CTaskDialog` da allineare al centro rispetto alla finestra padre. Se questo flag non è abilitato, il `CTaskDialog` centrata relativo al monitoraggio.  
  
 TDF_RTL_LAYOUT  
 Configura il `CTaskDialog` per un layout di lettura da destra a sinistra.  
  
 TDF_NO_DEFAULT_RADIO_BUTTON  
 Indica che nessun pulsante di opzione viene selezionata quando il `CTaskDialog` viene visualizzata.  
  
 TDF_CAN_BE_MINIMIZED  
 Consente all'utente di ridurre al minimo il `CTaskDialog`. Per supportare questa opzione, il `CTaskDialog` non può essere modale. MFC non supporta questa opzione perché MFC non supporta un modale `CTaskDialog`.  
...
### <a name="remarks"></a>Note  
 ...
 Uso *nMarqueeSpeed* per impostare la velocità della barra del testo scorrevole; i valori superiori indicano una velocità inferiore. Un valore pari a 0 per *nMarqueeSpeed* rende la barra di selezione di spostare la velocità predefinita per Windows.  
...
### <a name="remarks"></a>Note  
 Questo metodo genera un'eccezione con il [verificare che](diagnostic-services.md#ensure) macro se *nProgressPos* non è compreso nell'intervallo a barre lo stato di avanzamento. È possibile modificare l'intervallo di barra di stato di avanzamento con [CTaskDialog::SetProgressBarRange](#setprogressbarrange).  
...
### <a name="remarks"></a>Note  
 La posizione dell'indicatore di stato è relativo alla *nRangeMin* e *nRangeMax*. Ad esempio, se *nRangeMin* è 50 e *nRangeMax* è 100, una posizione di 75 si trova a metà tra l'indicatore di stato. Uso [CTaskDialog::SetProgressBarPosition](#setprogressbarposition) per impostare la posizione dell'indicatore di stato.  
  
 Per visualizzare l'indicatore di stato, l'opzione che tdf_show_progress_bar deve essere abilitato e TDF_SHOW_MARQUEE_PROGRESS_BAR non deve essere attivato. Questo metodo imposta TDF_SHOW_PROGRESS_BAR automaticamente e Cancella TDF_SHOW_MARQUEE_PROGRESS_BAR. Uso [CTaskDialog::SetOptions](#setoptions) per modificare manualmente le opzioni per questa istanza del [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md).  
  
 Questo metodo genera un'eccezione con il [verificare che](diagnostic-services.md#ensure) macro se *nRangeMin* è non è minore *nRangeMax*. Questo metodo genera un'eccezione anche se il `CTaskDialog` è già visualizzato e dispone di un indicatore di stato di selezione.  
...
### <a name="remarks"></a>Note  
 Questo metodo genera un'eccezione con il [verificare che](diagnostic-services.md#ensure) macro se la `CTaskDialog` è già visualizzato e dispone di un indicatore di stato di selezione.  
  
 Nella tabella seguente sono elencati i possibili valori per *nState*. In tutti questi casi, l'indicatore di stato il riempimento con colore regolari finché non raggiunge il punto di interruzione designato. A questo punto verrà modificata in base allo stato di colore.  
  
 PBST_NORMAL  
 Dopo lo stato di avanzamento barra riempie la `CTaskDialog` non modifica il colore della barra. Per impostazione predefinita, il colore regolare è verde.  
  
 PBST_ERROR  
 Dopo lo stato di avanzamento barra riempie la `CTaskDialog` di modificare il colore della barra di errore. Per impostazione predefinita, questo è rosso.  
  
 PBST_PAUSED  
 Dopo lo stato di avanzamento barra riempie la `CTaskDialog` modificare il colore della barra di colore in pausa. Per impostazione predefinita, questo è giallo.  
  
 È possibile impostare in cui l'indicatore di stato deve essere arrestata dopo [CTaskDialog::SetProgressBarPosition](#setprogressbarposition).  
...
### <a name="parameters"></a>Parametri  
 [in] *nRadioButtonID*  
 L'ID del controllo pulsante di opzione.  
...
### <a name="parameters"></a>Parametri  
 [in] *bChecked*  
 True per la casella di controllo verifica selezionate quando il `CTaskDialog` viene visualizzato. FALSE per la casella di controllo verifica deselezionati quando il `CTaskDialog` viene visualizzato.  
...
### <a name="remarks"></a>Note  
 Questo metodo genera un'eccezione con il [assicurarsi che](diagnostic-services.md#ensure) macro se questa istanza di `CTaskDialog` classe è già visualizzata.  
...
### <a name="remarks"></a>Note  
 Questo metodo statico consente di creare un'istanza di `CTaskDialog` classe senza creare esplicitamente un `CTaskDialog` oggetto nel codice. Poiché è presente alcun `CTaskDialog` dell'oggetto, non è possibile chiamare altri metodi del `CTaskDialog` se si utilizza questo metodo per mostrare un `CTaskDialog` all'utente.  
...
### <a name="return-value"></a>Valore restituito  
 A seconda dei casi il codice di notifica specifica. Per altre informazioni, vedere la sezione Osservazioni.  
  
### <a name="remarks"></a>Note  
 L'implementazione predefinita di `TaskDialogCallback` gestisce il messaggio specifico e quindi chiama appropriato nel metodo per il [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md). Ad esempio, in risposta al messaggio TDN_BUTTON_CLICKED `TaskDialogCallback` chiamate [CTaskDialog::OnCommandControlClick](#oncommandcontrolclick).  
  
 I valori per *wParam* e *lParam* dipendono dal messaggio generato specifico. È possibile che uno o entrambi questi valori sia vuoto. La tabella seguente elenca le notifiche predefiniti supportati e quali i valori delle *wParam* e *lParam* rappresentano. Se si esegue l'override di questo metodo in una classe derivata, è necessario implementare il codice di callback per ogni messaggio nella tabella seguente.  
...
|TDN_HYPERLINK_CLICKED|Non usato.|Un [LPCWSTR](http://msdn.microsoft.com/library/windows/desktop/aa383751) struttura che contiene il collegamento.|  
