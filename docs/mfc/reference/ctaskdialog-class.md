...
### <a name="remarks"></a>Note  
 Questo metodo chiama [ShellExecute](http://msdn.microsoft.com/library/windows/desktop/bb762153) prima che venga restituito S_OK.  
...
### <a name="remarks"></a>Note  
 ...
 Abilita i collegamenti ipertestuali di `CTaskDialog`.  
  
 TDF_USE_HICON_MAIN  
 Consente di configurare il `CTaskDialog` usare un `HICON` dell'icona principale. L'alternativa consiste nell'usare un `LPCWSTR`.  
  
 TDF_USE_HICON_FOOTER  
 Consente di configurare il `CTaskDialog` usare un `HICON` per l'icona del piè di pagina. L'alternativa consiste nell'usare un `LPCWSTR`.  
 ...
 Configura il `CTaskDialog` usare controlli pulsante di comando senza visualizzare un'icona accanto al controllo. TDF_USE_COMMAND_LINKS sostituzioni TDF_USE_COMMAND_LINKS_NO_ICON.  
  
 TDF_EXPAND_FOOTER_AREA  
 Indica che l'area di espansione è correntemente espanso.  
  
 TDF_EXPANDED_BY_DEFAULT  
 Determina se l'area di espansione è espanso per impostazione predefinita.  
 ...
 Configura l'indicatore di stato da un indicatore di stato di selezione. Se si abilita questa opzione, è necessario impostare TDF_SHOW_PROGRESS_BAR affinché il comportamento previsto.  
  
 TDF_CALLBACK_TIMER  
 Indica che il `CTaskDialog` callback interval è impostato su circa 200 millisecondi.  
 ...
 TDF_RTL_LAYOUT  
 Configura il `CTaskDialog` un layout di lettura da destra a sinistra.  
  
 TDF_NO_DEFAULT_RADIO_BUTTON  
 Indica che nessun pulsante di opzione è selezionato quando la `CTaskDialog` viene visualizzata.  
  
 TDF_CAN_BE_MINIMIZED  
 Consente all'utente per ridurre al minimo il `CTaskDialog`. Per supportare questa opzione, il `CTaskDialog` non può essere modale. MFC non supporta questa opzione perché MFC non supporta un non modale `CTaskDialog`.  
...
### <a name="remarks"></a>Note  
 ...
 PBST_NORMAL  
 Dopo lo stato di avanzamento viene compilato a barre, il `CTaskDialog` non modifica il colore della barra. Per impostazione predefinita, il normale colore è verde.  
  
 PBST_ERROR  
 Dopo lo stato di avanzamento viene compilato a barre, il `CTaskDialog` di modificare il colore della barra di errore. Per impostazione predefinita, questo è rosso.  
  
 PBST_PAUSED  
 Dopo lo stato di avanzamento viene compilato a barre, il `CTaskDialog` modificare il colore della barra di colore in pausa. Per impostazione predefinita, questo è giallo.  
