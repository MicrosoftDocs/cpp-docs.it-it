...
ms.locfileid: "79420232"
---
# <a name="cmfctoolbarimages-class"></a>Classe CMFCToolBarImages

Immagini su una barra degli strumenti. La classe `CMFCToolBarImages` gestisce le immagini della barra degli strumenti caricate da risorse dell'applicazione o da file.
...
|[CMFCToolBarImages:: CreateFromImageList](#createfromimagelist)|Inizializza le immagini della barra degli strumenti da un elenco di immagini ( [classe CImageList](../../mfc/reference/cimagelist-class.md)).|
|[CMFCToolBarImages:: CreateRegionFromImage](#createregionfromimage)||
|[CMFCToolBarImages::D eleteImage](#deleteimage)|Elimina l'immagine che dispone di un indice specificato dalle immagini della barra degli strumenti se questo set di immagini della barra degli strumenti contiene immagini definite dall'utente.|
|[CMFCToolBarImages::D RAW](#draw)|Disegna una singola immagine della barra degli strumenti (pulsante).|
|[CMFCToolBarImages::D rawEx](#drawex)||
|[CMFCToolBarImages:: EnableRTL](#enablertl)||
|[CMFCToolBarImages:: EndDrawImage](#enddrawimage)|Libera le risorse di sistema dopo che è stata disegnata un'immagine della barra degli strumenti.|
|[CMFCToolBarImages:: ExtractIcon](#extracticon)|Restituisce l'icona con un indice di immagine specificato dalle immagini della barra degli strumenti.|
...
|[CMFCToolBarImages:: GetImageSize](#getimagesize)|Recupera la dimensione delle immagini della barra degli strumenti archiviate in memoria (dimensioni di origine) o la dimensione delle immagini della barra degli strumenti disegnate sullo schermo (dimensioni di destinazione).|
...
|[CMFCToolBarImages:: GrayImages](#grayimages)|Grigio Visualizza le immagini della barra degli strumenti per renderle visualizzate disabilitate.|
...
La bitmap completa delle immagini della barra degli strumenti gestite da `CMFCToolbarImages` è costituita da una o più piccole immagini della barra degli strumenti (pulsanti) di dimensioni fisse.

## <a name="example"></a>Esempio

Nell'esempio seguente viene illustrato come configurare un oggetto `CMFCToolBarImages` usando vari metodi nella classe `CMFCToolBarImages`. Nell'esempio viene illustrato come impostare la dimensione dell'immagine della barra degli strumenti, caricare un'immagine e impostare il colore trasparente dell'immagine. Questo frammento di codice fa parte dell' [esempio demo di Visual Studio](../../overview/visual-cpp-samples.md).
...
*bSetBitPerPixel*<br/>
in TRUE se l'oggetto `CMFCToolBarImages` utilizza la profondità di colore (bit per pixel) della nuova immagine; FALSE se l'oggetto `CMFCToolbarImages` mantiene la profondità del colore corrente.

*imageList*<br/>
in Riferimento a un oggetto `CMFCToolbarImages` che contiene l'immagine da aggiungere.
...
Il numero di immagini della barra degli strumenti che l'oggetto `CMFCToolBarImages` gestisce dopo l'aggiunta della nuova bitmap; -1 se l'operazione non è riuscita.
...
Costruisce un oggetto `CMFCToolBarImages`, Inizializza il motore di rendering e imposta la dimensione dell'immagine sul valore predefinito 16 x 15 pixel. Usare [CMFCToolBarImages:: SetImageSize](#setimagesize) per modificare le dimensioni dell'immagine prima di aggiungere immagini.
...
*DS*<br/>
in Riferimento all'oggetto `CAfxDrawState` passato al metodo `PrepareDrawImage`.
...
Il numero di immagini nell'oggetto `CMFCToolBarImages`.
...
Struttura `SIZE`, che specifica le dimensioni in pixel di un'immagine.
...
L'oggetto `CMFCToolbarImages` è di sola lettura quando la bitmap con le immagini della barra degli strumenti è stata caricata da un file di sola lettura o quando la bitmap è stata copiata usando il metodo `CMFCToolBarImages::CopyTemp`.
...
TRUE se un oggetto `CMFCToolBarImages` è valido. in caso contrario, FALSE.
...
*DS*<br/>
in Riferimento alla struttura `CAfxDrawState` che archivia le risorse allocate tra le fasi di rendering dell'immagine.
...
TRUE se le risorse necessarie per creare l'immagine della barra degli strumenti sono state allocate correttamente; in caso contrario, FALSE.
...
##  <a name="isscaled"></a>CMFCToolBarImages:: Scaled
