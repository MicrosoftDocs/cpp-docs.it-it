...
Crea la finestra con mini-cornice di Windows e la `CMiniFrameWnd` collega all'oggetto.
...
- Imposta il bit di stile CS_DBLCLKS, che invia messaggi di doppio clic alla routine della finestra quando l'utente fa doppio clic con il mouse.

- Imposta i bit di stile CS_HREDRAW e CS_VREDRAW, che indirizzano il contenuto dell'area client da ricreare quando la finestra cambia dimensione.
...
- MFS_THICKFRAME consente di ridimensionare la finestra con mini-cornice quando il contenuto dell'area client consente.
...
Per una descrizione dei possibili valori dello stile della finestra, vedere [CWnd:: create](../../mfc/reference/cwnd-class.md#create) . La combinazione tipica usata per le finestre con mini-cornice&#124;è&#124;WS_POPUP WS_CAPTION WS_SYSMENU.
...
*dwExStyle*<br/>
Specifica lo stile esteso dell'oggetto `CMiniFrameWnd` da creare. Applicare uno degli [stili della finestra estesa](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles) alla finestra.
...
Quando `CreateEx` viene eseguito, Windows invia i messaggi [WM_GETMINMAXINFO](../../mfc/reference/cwnd-class.md#ongetminmaxinfo), [WM_NCCREATE](../../mfc/reference/cwnd-class.md#onnccreate), [WM_NCCALCSIZE](../../mfc/reference/cwnd-class.md#onnccalcsize)e [WM_CREATE](../../mfc/reference/cwnd-class.md#oncreate) alla finestra.

Per estendere la gestione dei messaggi predefinita, derivare una `CMiniFrameWnd`classe da, aggiungere una mappa messaggi alla nuova classe e fornire funzioni membro per i messaggi precedenti. Eseguire `OnCreate`l'override, ad esempio, per eseguire l'inizializzazione necessaria per una nuova classe.

Eseguire l' `On`override di altri gestori di messaggi di *messaggi* per aggiungere ulteriori funzionalità alla classe derivata.
...
Le finestre della casella degli strumenti della tavolozza precedente non sono più supportate. Lo stile precedente, che non disponeva di un pulsante Chiudi "X", era supportato durante l'esecuzione di un'applicazione MFC nelle versioni precedenti di Windows, ma non è più C++supportato in Visual .NET. È ora supportato solo il nuovo stile WS_EX_TOOLWINDOW. per una descrizione di questo stile, vedere [stili finestra estesi](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles).
