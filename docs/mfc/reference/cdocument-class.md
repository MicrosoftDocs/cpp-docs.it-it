...
|[CDocument::BeginReadChunks](#beginreadchunks)|Consente di inizializzare blocco lettura.|  
|[CDocument::CanCloseFrame](#cancloseframe)|Advanced sottoponibile a override; chiamato prima di chiudere una finestra cornice di visualizzazione di questo documento.|  
...
|[CDocument::FindChunk](#findchunk)|Esegue la ricerca di un blocco con GUID specificato.|  
...
|[CDocument::GetFirstViewPosition](#getfirstviewposition)|Restituisce la posizione del primo nell'elenco delle viste; usare all'inizio dell'iterazione.|  
|[CDocument::GetNextView](#getnextview)|Scorrere l'elenco delle viste associate al documento.|  
|[CDocument::GetPathName](#getpathname)|Restituisce il percorso del file di dati del documento.|  
|[CDocument::GetThumbnail](#getthumbnail)|Chiamata eseguita per creare una bitmap da utilizzare dal provider di anteprima per visualizzare l'anteprima.|  
...
|[CDocument::IsSearchAndOrganizeHandler](#issearchandorganizehandler)|Indica se questa istanza di `CDocument` oggetto è stato creato per il gestore di ricerca & Organizza.|  
...
|[CDocument::OnNewDocument](#onnewdocument)|Chiamata eseguita per creare un nuovo documento.|  
|[CDocument:: OnOpenDocument](#onopendocument)|Chiamata eseguita per aprire un documento esistente.|  
|[CDocument::OnPreviewHandlerQueryFocus](#onpreviewhandlerqueryfocus)|Indirizza il gestore di anteprima per restituire l'HWND non potrà chiamare la funzione GetFocus.|  
|[CDocument::OnPreviewHandlerTranslateAccelerator](#onpreviewhandlertranslateaccelerator)|Indirizza il gestore di anteprima per gestire una sequenza di tasti passata ai livelli superiori dal message pump del processo in cui è in esecuzione il gestore di anteprima.|  
...
|[CDocument::OnRichPreviewSiteChanged](#onrichpreviewsitechanged)|Chiamata eseguita quando l'anteprima avanzata sito è cambiato.|  
...
|[CDocument::ReportSaveLoadException](#reportsaveloadexception)|Advanced sottoponibile a override; chiamata eseguita quando un oggetto aperto o operazione di salvataggio non può essere completata a causa di un'eccezione.|  
...
|[CDocument::SetModifiedFlag](#setmodifiedflag)|Imposta un flag che indica che hanno modificato il documento dall'ultimo salvataggio.|  
...
|[CDocument:: OnUpdateFileSendMail](#onupdatefilesendmail)|Abilita il comando Invia messaggi se il supporto di posta elettronica è presentano.|  
...
|[CDocument::m_bSearchMode](#m_bsearchmode)|Specifica che `CDocument` oggetto è stato creato dall'indicizzatore o un'altra applicazione di ricerca.|  
...
|[CDocument::m_lfRichPreviewFont](#m_lfrichpreviewfont)|Specifica il carattere del testo per la finestra di anteprima avanzata. Queste informazioni di carattere viene impostate dall'host.|  
  
## <a name="remarks"></a>Note  
 Un documento rappresenta l'unità di dati che l'utente in genere viene aperto con il comando File Apri e salvato con il comando Salva.  
  
 `CDocument` supporta le operazioni standard, ad esempio la creazione di un documento, il caricamento e averlo salvato. Il framework consente di modificare documenti tramite l'interfaccia definita da `CDocument`.  
  
 Un'applicazione può supportare più di un tipo di documento. ad esempio, un'applicazione potrebbe supportare sia fogli di calcolo e documenti di testo. Ogni tipo di documento dispone di un modello di documento associato. il modello di documento specifica quali risorse (ad esempio, menu, icona o tasto di scelta rapida tabella) vengono utilizzate per il tipo di documento. Ogni documento contiene un puntatore a esso associata `CDocTemplate` oggetto.  
  
 Gli utenti interagiscono con un documento tramite il [CView](../../mfc/reference/cview-class.md) oggetti associati. Una vista esegue il rendering di un'immagine del documento in una finestra cornice e interpreta l'input dell'utente come operazioni sul documento. Un documento può disporre di più visualizzazioni associate. Quando l'utente apre una finestra in un documento, il framework crea una visualizzazione e la collega al documento. Il modello di documento specifica il tipo di visualizzazione e la finestra cornice vengono utilizzati per visualizzare ogni tipo di documento.  
  
 Documenti fanno parte dello standard del framework di routing di comandi e di conseguenza ricevono comandi da componenti dell'interfaccia utente standard (ad esempio, la voce di menu Salva). Un documento riceve comandi inoltrati dalla visualizzazione attiva. Se il documento non gestisce un comando specifico, inoltra il comando per il modello di documento che lo gestisce.  
  
 Quando vengono modificati i dati del documento, ognuna delle visualizzazioni deve riflettere tali modifiche. `CDocument` fornisce la [UpdateAllViews](#updateallviews) funzione membro per la notifica le visualizzazioni di tali modifiche, in modo le viste possono ridisegnarsi in base alle esigenze. Il framework viene inoltre richiesto all'utente di salvare un file modificato prima della chiusura.  
...
##  <a name="addview"></a>  CDocument:: AddView  
 Chiamare questa funzione per collegare una vista al documento.  
...
### <a name="remarks"></a>Note  
 Questa funzione consente di aggiungere la visualizzazione specificata all'elenco di viste associate al documento; la funzione imposta inoltre il proprio puntatore documento al documento. Il framework chiama questa funzione quando ci si connette un oggetto vista appena creata in un documento. Questo errore si verifica in risposta a un comando File nuovo, Apri o una nuova finestra o quando viene diviso in una finestra con separatore.  
  
 Chiamare questa funzione solo se si creano manualmente e collegamento di una vista. In genere si consentirà il framework connettersi documenti e visualizzazioni definendo un [CDocTemplate](../../mfc/reference/cdoctemplate-class.md) oggetto per associare una classe di documento, classe di visualizzazione e classe finestra cornice.  
  
### <a name="example"></a>Esempio  
 [!code-cpp[NVC_MFCDocViewSDI#12](../../mfc/codesnippet/cpp/cdocument-class_1.cpp)]  
  
##  <a name="beginreadchunks"></a>  CDocument::BeginReadChunks  
 Consente di inizializzare blocco lettura.  
...
### <a name="remarks"></a>Note  
 Il framework gestisce la creazione di documenti per l'utente. Eseguire l'override di [OnNewDocument](#onnewdocument) funzione membro per eseguire l'inizializzazione in base al documento; ciò è particolarmente importante in applicazioni single document interface (SDI).  
...
### <a name="remarks"></a>Note  
 Cancellare il percorso da un `CDocument` oggetto fa sì che l'applicazione richiedere all'utente quando il documento viene quindi salvato. In questo modo, un **salvare** comando si comportano come una **Salva con nome** comando.  
...
### <a name="return-value"></a>Valore restituito  
 Posizione nell'elenco di blocco interne se ha esito positivo. In caso contrario `NULL`.  
 ...
 *nOpenFlags*  
 La condivisione e modalità di accesso. Specifica l'azione da intraprendere quando si apre il file. È possibile combinare le opzioni elencate nel costruttore CFile [CFile::CFile](../../mfc/reference/cfile-class.md#cfile) usando l'operatore OR (&#124;) (operatore). L'autorizzazione di un accesso e una condivisione di opzione sono obbligatori; il **modeCreate** e **modeNoInherit** modalità sono facoltative.  
...
### <a name="return-value"></a>Valore restituito  
 Un **posizione** valore utilizzabili per iterazione con il [GetNextView](#getnextview) funzione membro.  
...
### <a name="parameters"></a>Parametri  
 *rPosition.*  
 Un riferimento a un **posizione** valore restituito da una precedente chiamata ai `GetNextView` o [GetFirstViewPosition](#getfirstviewposition) funzioni membro. Questo valore non deve essere **NULL**.  
  
### <a name="return-value"></a>Valore restituito  
 Un puntatore alla visualizzazione identificata *rPosition*.  
  
### <a name="remarks"></a>Note  
 La funzione restituisce la vista identificata *rPosition* e quindi imposta *rPosition* per il **posizione** valore nella visualizzazione successiva nell'elenco. Se la vista recuperata è l'ultimo nell'elenco, quindi *rPosition* è impostata su **NULL**.  
  
### <a name="example"></a>Esempio  
 [!code-cpp[NVC_MFCDocView#59](../../mfc/codesnippet/cpp/cdocument-class_4.cpp)]  
  
##  <a name="getpathname"></a>  CDocument::GetPathName  
 Chiamare questa funzione per ottenere il percorso completo del file di disco del documento.  
...
### <a name="return-value"></a>Valore restituito  
 Percorso completo del documento. Questa stringa è vuota se il documento non è stato salvato oppure non dispone di un file di disco associato.  
 ...
 *phbmp*  
 Contiene un handle a una bitmap, quando la funzione ha esito positivo.  
  
 *pdwAlpha*  
 Contiene un valore DWORD che specifica il valore del canale alfa, quando la funzione ha esito positivo.  
  
### <a name="return-value"></a>Valore restituito  
 Restituisce `TRUE` se una mappa di bit per l'anteprima è stato creato correttamente; in caso contrario `FALSE`.  
...
### <a name="remarks"></a>Note  
 Eseguire l'override di questo metodo in una classe derivata per inizializzare il contenuto della ricerca. Il contenuto deve essere una stringa con parti delimitate da ";". Selezionare, ad esempio,"; rettangolo; elemento OLE".  
...
### <a name="parameters"></a>Parametri  
 *pStream*  
 Puntatore a un flusso. Questo flusso è fornito dalla Shell.  
...
### <a name="remarks"></a>Note  
 L'implementazione predefinita di questa funzione controlla se l'ultima visualizzazione viene rimosso e, in tal caso, Elimina il documento. Eseguire l'override di questa funzione se si desidera eseguire un'elaborazione speciale quando il framework aggiunge o rimuove una vista. Ad esempio, se si desidera un documento su cursori rimangono aperti anche quando nessuna vista collegata a esso, l'override della funzione.  
...
### <a name="remarks"></a>Note  
 ...
 Eseguire l'override di questa funzione se si desidera eseguire l'elaborazione di particolari operazioni di pulitura quando il framework chiude un documento. Ad esempio, se il documento rappresenta un record in un database, è possibile eseguire l'override di questa funzione per chiudere il database. È necessario chiamare la versione della classe base di questa funzione dalla sostituzione.  
...
### <a name="remarks"></a>Note  
 Gli eventi del documento potrebbero influire sulla più classi. Questo metodo è responsabile della gestione degli eventi di documento che interessano diverso da classi di [CDocument (classe)](../../mfc/reference/cdocument-class.md). Attualmente, l'unica classe che deve rispondere a eventi del documento è il [CDataRecoveryHandler classe](../../mfc/reference/cdatarecoveryhandler-class.md). Il `CDocument` classe dispone di altri metodi sostituibile responsabile della gestione l'effetto sul `CDocument`.  
  
 Nella tabella seguente sono elencati i possibili valori per *deEvent* e gli eventi che corrispondono alla.  
 ...
 *lprcBounds*  
 Specifica un rettangolo di delimitazione dell'area in cui deve essere disegnata l'immagine di anteprima.  
  
### <a name="remarks"></a>Note  
  
##  <a name="onfilesendmail"></a>  CDocument:: OnFileSendMail  
 Invia un messaggio tramite l'host della posta elettronica che si trovano (se presente) con il documento come un allegato.  
 ...
 **CDocument** supporta l'invio al documento tramite posta elettronica se è presente il supporto di posta elettronica (MAPI). Vedere gli articoli [negli argomenti di MAPI](../../mfc/mapi.md) e [supporto MAPI in MFC](../../mfc/mapi-support-in-mfc.md).  
...
### <a name="remarks"></a>Note  
 L'implementazione predefinita di questa funzione chiama la [DeleteContents](#deletecontents) funzione membro per assicurarsi che il documento è vuoto e quindi contrassegna il nuovo documento siano puliti. Eseguire l'override di questa funzione per inizializzare la struttura dei dati per un nuovo documento. È necessario chiamare la versione della classe base di questa funzione dalla sostituzione.  
  
 Se l'utente sceglie il comando File nuovo in un'applicazione SDI, il framework utilizza questa funzione per reinizializzare il documento esistente anziché crearne uno nuovo. Se l'utente sceglie il nuovo File in un'applicazione più document interface (MDI), il framework crea un nuovo documento ogni volta e quindi chiama questa funzione per inizializzarla. In questa funzione anziché nel costruttore per il comando File nuovo per essere efficace per le applicazioni SDI, è necessario inserire il codice di inizializzazione.  
  
 Si noti che esistono casi in `OnNewDocument` viene chiamato due volte. Questo errore si verifica quando il documento incorporato come un Server di documenti ActiveX. Viene innanzitutto chiamata la funzione il `CreateInstance` (metodo) (esposti dal `COleObjectFactory`-classe derivata) e una seconda volta dal `InitNew` (metodo) (esposti dal `COleServerDoc`-classe derivata).  
...
### <a name="remarks"></a>Note  
 L'implementazione predefinita di questa funzione consente di aprire il file specificato, viene chiamato il [DeleteContents](#deletecontents) funzione membro per assicurarsi che il documento è vuoto, chiama [CObject:: Serialize](../../mfc/reference/cobject-class.md#serialize) per leggere il file il contenuto e quindi contrassegna come di pulizia. Eseguire l'override di questa funzione se si desidera utilizzare un nome diverso dal meccanismo di archiviazione o il meccanismo di file. Ad esempio, è possibile scrivere un'applicazione in cui i documenti rappresentano i record in un database anziché in file separati.  
  
 Se l'utente sceglie il comando Apri File in un'applicazione SDI, il framework utilizza questa funzione per reinizializzare esistenti **CDocument** oggetto, anziché crearne uno nuovo. Se l'utente sceglie Apri File in un'applicazione MDI, il framework crea un nuovo **CDocument** oggetto ogni volta e quindi chiama questa funzione per inizializzarla. In questa funzione anziché nel costruttore per il comando Apri File per essere efficace per le applicazioni SDI, è necessario inserire il codice di inizializzazione.  
...
##  <a name="onpreviewhandlerqueryfocus"></a>  CDocument::OnPreviewHandlerQueryFocus  
 Indirizza il gestore di anteprima per restituire l'HWND recuperati dalla chiamata di `GetFocus` (funzione).  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce S_OK se l'operazione riesce; o un valore di errore in caso contrario.  
  
### <a name="remarks"></a>Note  
  
##  <a name="onpreviewhandlertranslateaccelerator"></a>  CDocument::OnPreviewHandlerTranslateAccelerator  
 Indirizza il gestore di anteprima per gestire una sequenza di tasti passata ai livelli superiori dal message pump del processo in cui è in esecuzione il gestore di anteprima.  
...
### <a name="return-value"></a>Valore restituito  
 Se il messaggio di sequenza di tasti può essere elaborato dal gestore di anteprima, il gestore di processi e restituisce S_OK. Se il gestore di anteprima non è in grado di elaborare il messaggio di sequenza di tasti, verrà chiesto all'host tramite `IPreviewHandlerFrame::TranslateAccelerator`. Se l'host elabora il messaggio, questo metodo restituisce S_OK. Se l'host non in grado di elaborare il messaggio, questo metodo restituirà S_FALSE.  
...
##  <a name="onrichpreviewsitechanged"></a>  CDocument::OnRichPreviewSiteChanged  
 Chiamata eseguita quando il sito di anteprima avanzata è cambiato.  
...
### <a name="parameters"></a>Parametri  
 *lpszPathName*  
 Punta al percorso completo in cui deve essere salvato il file.  
  
### <a name="return-value"></a>Valore restituito  
 Diverso da zero se il documento è stato salvato; in caso contrario 0.  
  
### <a name="remarks"></a>Note  
 L'implementazione predefinita di questa funzione consente di aprire il file specificato, le chiamate [CObject:: Serialize](../../mfc/reference/cobject-class.md#serialize) per scrivere i dati del documento per il file e quindi segni il documento come eseguire la pulizia. Eseguire l'override di questa funzione se si desidera eseguire un'elaborazione speciale quando il framework consente di salvare un documento. Ad esempio, è possibile scrivere un'applicazione in cui i documenti rappresentano i record in un database anziché in file separati.  
...
##  <a name="onupdatefilesendmail"></a>  CDocument:: OnUpdateFileSendMail  
 Abilita il **ID_FILE_SEND_MAIL** comando se è presente il supporto di posta elettronica (MAPI).  
...
### <a name="parameters"></a>Parametri  
 *pCmdUI*  
 Un puntatore per il [CCmdUI](../../mfc/reference/ccmdui-class.md) oggetto associato il **ID_FILE_SEND_MAIL** comando.  
  
### <a name="remarks"></a>Note  
 In caso contrario, la funzione rimuove il **ID_FILE_SEND_MAIL** comando dal menu, inclusi i separatori sopra o sotto il menu elementi a seconda dei casi. MAPI è abilitata se MAPI32. DLL è presente nel percorso e, nella sezione [posta elettronica] del file WIN. File INI, MAPI = 1. La maggior parte delle applicazioni inserire questo comando dal menu File.  
...
### <a name="parameters"></a>Parametri  
 *pFrame*  
 Puntatore ai [CFrameWnd](../../mfc/reference/cframewnd-class.md) che contiene l'oggetto associato `CDocument` oggetto.  
  
### <a name="remarks"></a>Note  
 È possibile eseguire l'override per fornire pulizia personalizzata, ma è necessario verificare di chiamare anche la classe di base.  
...
### <a name="parameters"></a>Parametri  
 *ppValue*  
 [out] Quando termina, la funzione *ppValue* contiene il valore che è stato letto.  
 ...
 *bAbort*  
 Specifica se il file deve essere rilasciata mediante `CFile::Close` o `CFile::Abort`. **FALSE** se il file deve essere liberata usando [CFile::Close](../../mfc/reference/cfile-class.md#close); **TRUE** se il file deve essere liberata usando [CFile:: Abort](../../mfc/reference/cfile-class.md#abort).  
  
### <a name="remarks"></a>Note  
 Se *bAbort* viene **TRUE**, `ReleaseFile` chiamate `CFile::Abort`, mentre il file viene rilasciato. `CFile::Abort` non genererà un'eccezione.  
  
 Se *bAbort* viene **FALSE**, `ReleaseFile` chiamate `CFile::Close` e il file viene rilasciato.  
...
### <a name="remarks"></a>Note  
 Questa funzione rimuove la visualizzazione specificata dall'elenco di viste associate al documento; Imposta inoltre il proprio puntatore documento su **NULL**. Questa funzione viene chiamata dal framework quando viene chiusa una finestra cornice o viene chiuso un riquadro di una finestra con separatore.  
 ...
 Vedere l'esempio nella [AddView](#addview) per un esempio di implementazione.  
  
##  <a name="reportsaveloadexception"></a>  CDocument::ReportSaveLoadException  
 Chiamata eseguita se viene generata un'eccezione (in genere un [CFileException](../../mfc/reference/cfileexception-class.md) oppure [eccezione CArchiveException](../../mfc/reference/carchiveexception-class.md)) durante il salvataggio o il caricamento del documento.  
 ...
 *e*  
 Punta all'eccezione che è stata generata. Potrebbe essere **NULL**.  
 ...
 *nIDPDefault*  
 Identificatore del messaggio di errore da visualizzare se la funzione non viene specificato uno più specifico.  
  
### <a name="remarks"></a>Note  
 L'implementazione predefinita esamina l'oggetto eccezione e cerca un messaggio di errore che descrive nel dettaglio la causa. Se non viene trovato un messaggio specifico o se *espulsione* viene **NULL**, il messaggio generale specificato il *nIDPDefault* parametro viene utilizzato. La funzione viene quindi visualizzato un messaggio contenente il messaggio di errore. Eseguire l'override di questa funzione se si desidera fornire messaggi di errore aggiuntive e personalizzate. Si tratta di un'avanzata sottoponibile a override.  
...
### <a name="remarks"></a>Note  
 Chiamare questa funzione in modo coerente, consente di garantire che il framework chiede di salvare le modifiche prima di chiudere un documento. In genere è consigliabile utilizzare il valore predefinito di **TRUE** per il *bModified* parametro. Per contrassegnare un documento come eseguire la pulizia (non modificato), chiamare questa funzione con un valore di **FALSE**.  
 ...
 *bAddToMRU*  
 Determina se il nome del file viene aggiunto a utilizzati più di recente elenco file (MRU). Se **impostato su TRUE,** viene aggiunto il nome del file; se **FALSE**, non è aggiunto.  
...
### <a name="remarks"></a>Note  
 Chiamare questa funzione consente di aggiornare i titoli di tutte le finestre cornice che visualizzano il documento.  
...
### <a name="remarks"></a>Note  
 È necessario chiamare questa funzione dopo aver chiamato il [SetModifiedFlag](#setmodifiedflag) funzione membro. Questa funzione informa ogni visualizzazione associata al documento, ad eccezione di vista specificata dalla *pSender*, che il documento è stato modificato. In genere chiamare questa funzione dalla classe di visualizzazione dopo che l'utente ha modificato il documento tramite una vista.  
  
 Questa funzione chiama la [CView::OnUpdate](../../mfc/reference/cview-class.md#onupdate) visualizzare funzione membro per ognuna delle visualizzazioni del documento, ad eccezione di invio, il passaggio *pHint* e *lHint*. Usare questi parametri per passare informazioni alle visualizzazioni sulle modifiche apportate al documento. È possibile codificare informazioni utilizzando *lHint* e/o è possibile definire un [CObject](../../mfc/reference/cobject-class.md)-derivata per archiviare informazioni sulle modifiche e passare un oggetto di tale classe mediante *pHint*. Eseguire l'override di `CView::OnUpdate` funzione del membro nel [CView](../../mfc/reference/cview-class.md)-derivata per ottimizzare l'aggiornamento della visualizzazione della vista in base alle informazioni passate.  
