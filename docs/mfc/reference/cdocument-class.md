...
|[CDocument:: GetAdapter](#getadapter)|Restituisce un puntatore a un oggetto che implementa `IDocument` interfaccia.|
...
|[CDocument:: GetFile](#getfile)|Restituisce un puntatore all'oggetto `CFile` desiderato.|
...
|[CDocument:: IsSearchAndOrganizeHandler](#issearchandorganizehandler)|Indica se questa istanza di `CDocument` oggetto è stata creata per la ricerca & gestore di organizzazione.|
...
|Nome|Descrizione|
|----------|-----------------|
|[CDocument:: m_bGetThumbnailMode](#m_bgetthumbnailmode)|Specifica che `CDocument` oggetto è stato creato da dllhost per le anteprime. Deve essere archiviato `CView::OnDraw`.|
|[CDocument:: m_bPreviewHandlerMode](#m_bpreviewhandlermode)|Specifica che `CDocument` oggetto è stato creato da prevhost per `Rich Preview`. Deve essere archiviato `CView::OnDraw`.|
|[CDocument:: m_bSearchMode](#m_bsearchmode)|Specifica che `CDocument` oggetto è stato creato dall'indicizzatore o da un'altra applicazione di ricerca.|
...
Per ulteriori informazioni su `CDocument`, vedere argomenti relativi alla [serializzazione](../../mfc/serialization-in-mfc.md), all' [architettura documento/visualizzazione](../../mfc/document-view-architecture.md)e alla [creazione di documenti/visualizzazioni](../../mfc/document-view-creation.md).
...
Se si deseleziona il percorso da un oggetto `CDocument`, l'applicazione chiederà all'utente quando il documento viene salvato successivamente. In questo modo un comando **Salva** si comporta come un comando **Salva con nome** .

##  <a name="deletecontents"></a>CDocument::D eleteContents

Chiamata eseguita dal Framework per eliminare i dati del documento senza eliminare definitivamente l'oggetto `CDocument` stesso.
...
Chiamare questa funzione membro per ottenere un puntatore a un oggetto `CFile`.
...
*nOpenFlags*<br/>
Modalità di condivisione e accesso. Specifica l'azione da intraprendere quando si apre il file. È possibile combinare le opzioni elencate nel costruttore CFile [CFile:: CFile](../../mfc/reference/cfile-class.md#cfile) usando l'operatore OR bit per&#124;bit (). Sono necessarie un'autorizzazione di accesso e un'opzione di condivisione; le modalità `modeCreate` e `modeNoInherit` sono facoltative.
...
*rPosition*<br/>
Riferimento a un valore di posizione restituito da una chiamata precedente alle funzioni membro `GetNextView` o [GetFirstViewPosition](#getfirstviewposition) . Questo valore non può essere NULL.
...
Restituisce TRUE se questa istanza di `CDocument` è stata creata per il gestore di ricerca & organizzare.
...
Specifica che l'oggetto `CDocument` è stato creato da dllhost per le anteprime. Deve essere archiviato `CView::OnDraw`.
...
Specifica che l'oggetto `CDocument` è stato creato da prevhost per l'anteprima avanzata. Deve essere archiviato `CView::OnDraw`.
...
Si noti che in alcuni casi `OnNewDocument` viene chiamato due volte. Questo errore si verifica quando il documento viene incorporato come server di documenti ActiveX. La funzione viene chiamata prima dal metodo di `CreateInstance` (esposto dalla classe derivata da `COleObjectFactory`) e una seconda volta dal metodo `InitNew` (esposto dalla classe derivata da `COleServerDoc`).
...
Se l'utente sceglie il comando Apri file in un'applicazione SDI, il Framework usa questa funzione per reinizializzare l'oggetto `CDocument` esistente, anziché crearne uno nuovo. Se l'utente sceglie file aperto in un'applicazione MDI, il Framework costruisce ogni volta un nuovo oggetto `CDocument`, quindi chiama questa funzione per inizializzarla. È necessario inserire il codice di inizializzazione in questa funzione anziché nel costruttore affinché il comando Apri file sia efficace nelle applicazioni SDI.
...
Indica al gestore di anteprime di restituire l'HWND recuperato dalla chiamata alla funzione `GetFocus`.
...
*phwnd*<br/>
out Quando termina, questo metodo contiene un puntatore a HWND restituito dalla chiamata della funzione `GetFocus` dal thread in primo piano del gestore di anteprime.
...
In caso contrario, la funzione rimuove il comando ID_FILE_SEND_MAIL dal menu, inclusi i separatori sopra o sotto la voce di menu nel modo appropriato. MAPI è abilitato se MAPI32. La DLL è presente nel percorso e nella sezione [mail] della WIN. File INI, MAPI = 1. La maggior parte delle applicazioni inserisce questo comando nel menu file.
...
*pFrame*<br/>
Puntatore a [CFrameWnd](../../mfc/reference/cframewnd-class.md) che include l'oggetto `CDocument` associato.
...
Il valore predefinito di `PreCloseFrame` non esegue alcuna operazione nell'`CDocument`. Le classi derivate da `CDocument`[COleDocument](../../mfc/reference/coledocument-class.md) e [CRichEditDoc](../../mfc/reference/cricheditdoc-class.md) utilizzano questa funzione membro.
...
##  <a name="setpathname"></a>CDocument:: sepathname
...
##  <a name="settitle"></a>CDocument:: toTitle
