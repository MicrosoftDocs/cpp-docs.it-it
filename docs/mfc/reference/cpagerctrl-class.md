...
|[CPagerCtrl::CreateEx](#createex)|Crea un controllo pager con gli stili estesi specificati e lo associa all'oggetto `CPagerCtrl` corrente.|
|[CPagerCtrl::ForwardMouse](#forwardmouse)|Abilita o Disabilita l'invio di messaggi [WM_MOUSEMOVE](/windows/win32/inputdev/wm-mousemove) alla finestra che è contenuta nel controllo pager corrente.|
...
|---------------|-----------------|
|*dwStyle*|in Combinazione bit per bit (o) degli stili della [finestra](../../mfc/reference/styles-used-by-mfc.md#window-styles) e degli [stili del controllo pager](/windows/win32/Controls/pager-control-styles) da applicare al controllo.|
|*rect*|in Riferimento a una struttura [Rect](/previous-versions/dd162897\(v=vs.85\)) che contiene la posizione e le dimensioni del controllo nelle coordinate del client.|
|*pParentWnd*|in Puntatore a un oggetto [CWnd](../../mfc/reference/cwnd-class.md) che rappresenta la finestra padre del controllo. Questo parametro non può essere NULL.|
...
Crea un controllo pager con gli stili estesi specificati e lo associa all'oggetto `CPagerCtrl` corrente.
...
|Parametro|Descrizione|
|---------------|-----------------|
|*dwExStyle*|in Combinazione bit per bit di stili estesi da applicare al controllo. Per ulteriori informazioni, vedere il parametro *dwExStyle* della funzione [CreateWindowEx](/windows/win32/api/winuser/nf-winuser-createwindowexw) .|
|*dwStyle*|in Combinazione bit per bit (o) degli stili della [finestra](../../mfc/reference/styles-used-by-mfc.md#window-styles) e degli [stili del controllo pager](/windows/win32/Controls/pager-control-styles) da applicare al controllo.|
|*rect*|in Riferimento a una struttura [Rect](/previous-versions/dd162897\(v=vs.85\)) che contiene la posizione e le dimensioni del controllo nelle coordinate del client.|
|*pParentWnd*|in Puntatore a un oggetto [CWnd](../../mfc/reference/cwnd-class.md) che rappresenta la finestra padre del controllo. Questo parametro non può essere NULL.|
|*nID*|in ID del controllo.|
...
Abilita o Disabilita l'invio di messaggi [WM_MOUSEMOVE](/windows/win32/inputdev/wm-mousemove) alla finestra che è contenuta nel controllo pager corrente.
...
|Parametro|Descrizione|
|---------------|-----------------|
|*bForward*|in TRUE per l'invio dei messaggi del mouse oppure FALSE per non inviare i messaggi del mouse.|

### <a name="remarks"></a>Note

Questo metodo invia il messaggio [PGM_FORWARDMOUSE](/windows/win32/Controls/pgm-forwardmouse) , descritto nella Windows SDK.
...
Questo metodo invia il messaggio [PGM_GETBORDER](/windows/win32/Controls/pgm-getborder) , descritto nella Windows SDK.
...
Questo metodo invia il messaggio [PGM_GETBKCOLOR](/windows/win32/Controls/pgm-getbkcolor) , descritto nella Windows SDK.
...
Questo metodo invia il messaggio [PGM_GETBUTTONSIZE](/windows/win32/Controls/pgm-getbuttonsize) , descritto nella Windows SDK.

Se il controllo pager ha lo stile PGS_HORZ, le dimensioni del pulsante determinano la larghezza dei pulsanti del pager e se il controllo pager ha lo stile PGS_VERT, la dimensione del pulsante determina l'altezza dei pulsanti del cercapersone. Per altre informazioni, vedere [stili del controllo pager](/windows/win32/Controls/pager-control-styles).
...
|Parametro|DESCRIZIONE|
|---------------|-----------------|
|*iButton*|in Indica il pulsante per il quale viene recuperato lo stato. Se lo stile del controllo pager è PGS_HORZ, specificare PGB_TOPORLEFT per il pulsante sinistro e PGB_BOTTOMORRIGHT per il pulsante destro. Se lo stile del controllo pager è PGS_VERT, specificare PGB_TOPORLEFT per il pulsante in alto e PGB_BOTTOMORRIGHT per il pulsante in basso. Per altre informazioni, vedere [stili del controllo pager](/windows/win32/Controls/pager-control-styles).|
...
Questo metodo invia il messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) , descritto nella Windows SDK.
...
Questo metodo invia il messaggio [PGM_GETPOS](/windows/win32/Controls/pgm-getpos) , descritto nella Windows SDK.
...
Questo metodo invia il messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) , descritto nella Windows SDK. Viene quindi verificato se lo stato restituito è PGF_DEPRESSED. Per ulteriori informazioni, vedere la sezione valore restituito del messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) .
...
Questo metodo invia il messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) , descritto nella Windows SDK. Viene quindi verificato se lo stato restituito è PGF_GRAYED. Per ulteriori informazioni, vedere la sezione valore restituito del messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) .
...
Questo metodo invia il messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) , descritto nella Windows SDK. Viene quindi verificato se lo stato restituito è PGF_HOT. Per ulteriori informazioni, vedere la sezione valore restituito del messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) .
...
Questo metodo invia il messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) , descritto nella Windows SDK. Viene quindi verificato se lo stato restituito è PGF_INVISIBLE. Per ulteriori informazioni, vedere la sezione valore restituito del messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) .
...
Questo metodo invia il messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) , descritto nella Windows SDK. Viene quindi verificato se lo stato restituito è PGF_NORMAL. Per ulteriori informazioni, vedere la sezione valore restituito del messaggio [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) .
...
Questo metodo invia il messaggio [PGM_RECALCSIZE](/windows/win32/Controls/pgm-recalcsize) , descritto nella Windows SDK. Di conseguenza, il controllo pager Invia la notifica [PGN_CALCSIZE](/windows/win32/Controls/pgn-calcsize) per ottenere le dimensioni di scorrimento della finestra contenuta.
...
|Parametro|Descrizione|
|---------------|-----------------|
|*clrBk*|in Valore [COLORREF](/windows/win32/gdi/colorref) che contiene il nuovo colore di sfondo del controllo pager.|
...
Questo metodo invia il messaggio [PGM_SETBKCOLOR](/windows/win32/Controls/pgm-setbkcolor) , descritto nella Windows SDK.
...
|Parametro|Descrizione|
|---------------|-----------------|
|*iBorder*|in Dimensioni del nuovo bordo, misurate in pixel. Se il parametro *iBorder* è negativo, le dimensioni del bordo sono impostate su zero.|
...
Questo metodo invia il messaggio [PGM_SETBORDER](/windows/win32/Controls/pgm-setborder) , descritto nella Windows SDK.
...
|Parametro|Descrizione|
|---------------|-----------------|
|*iButtonSize*|in Dimensioni del nuovo pulsante, misurate in pixel.|
...
Questo metodo invia il messaggio [PGM_SETBUTTONSIZE](/windows/win32/Controls/pgm-setpos) , descritto nella Windows SDK.
...
|Parametro|Descrizione|
|---------------|-----------------|
|*hwndChild*|in Handle per la finestra da contenere.|

### <a name="remarks"></a>Note

Questo metodo invia il messaggio [PGM_SETCHILD](/windows/win32/Controls/pgm-setchild) , descritto nella Windows SDK.
...
Questo metodo invia il messaggio [PGM_SETPOS](/windows/win32/Controls/pgm-setpos) , descritto nella Windows SDK.
