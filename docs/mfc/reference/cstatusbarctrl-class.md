---
title: CStatusBarCtrl (classe) | Documenti di Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|[CStatusBarCtrl::GetBorders](#getborders)|Recupera la larghezza dei bordi orizzontali e verticali di un controllo barra di stato corrente.|  
...
|[CStatusBarCtrl::SetMinHeight](#setminheight)|Imposta l'altezza minima di uno stato barra area di disegno del controllo.|  
|[CStatusBarCtrl::SetParts](#setparts)|Imposta il numero di parti in uno stato della barra di controllo e la coordinata del bordo destro di ogni parte.|  
|[CStatusBarCtrl::SetSimple](#setsimple)|Specifica se un controllo barra di stato Visualizza testo semplice o tutte le parti di controllo impostate da una precedente chiamata a `SetParts`.|  
...
## <a name="remarks"></a>Note  
 Un "controllo della barra dello stato" è una finestra orizzontale, in genere visualizzata nella parte inferiore di una finestra padre, in cui un'applicazione possono essere visualizzati diversi tipi di informazioni sullo stato. Il controllo barra di stato può essere diviso in parti per visualizzare più di un tipo di informazioni.  
 ...
 Per ulteriori informazioni sull'utilizzo di `CStatusBarCtrl`, vedere [controlli](../../mfc/controls-mfc.md) e [CStatusBarCtrl utilizzando](../../mfc/using-cstatusbarctrl.md).  
...
### <a name="parameters"></a>Parametri  
 `dwStyle`  
 Specifica lo stile del controllo barra di stato. Applicare qualsiasi combinazione di stili di controllo della barra di stato [comuni degli stili del controllo](http://msdn.microsoft.com/library/windows/desktop/bb775498) nel [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Questo parametro deve includere il **WS_CHILD** stile. Deve includere inoltre il **WS_VISIBLE** stile.  
  
 `rect`  
 Specifica le dimensioni e posizione del controllo barra di stato. Può essere un [CRect](../../atl-mfc-shared/reference/crect-class.md) oggetto o un [RECT](http://msdn.microsoft.com/library/windows/desktop/dd162897) struttura.  
...
### <a name="remarks"></a>Note  
 Costruire un `CStatusBarCtrl` in due passaggi. In primo luogo, chiamare il costruttore e quindi chiamare **crea**, che crea il controllo barra di stato e lo collega a di `CStatusBarCtrl` oggetto.  
  
 È la posizione predefinita di una finestra di stato nella parte inferiore della finestra padre, ma è possibile specificare il `CCS_TOP` stile affinché venga visualizzato nella parte superiore dell'area client della finestra padre. È possibile specificare il **SBARS_SIZEGRIP** stile per includere un riquadro di ridimensionamento all'estremità destra della finestra di stato. La combinazione di `CCS_TOP` e **SBARS_SIZEGRIP** stili non è consigliabile, perché il riquadro di ridimensionamento non funziona anche se viene disegnato nella finestra di stato.  
...
##  <a name="createex"></a>CStatusBarCtrl::CreateEx  
 Crea un controllo (una finestra figlio) e lo associa a di `CStatusBarCtrl` oggetto.  
...
### <a name="parameters"></a>Parametri  
 `lpDrawItemStruct`  
 Un puntatore long a un [DRAWITEMSTRUCT](http://msdn.microsoft.com/library/windows/desktop/bb775802) struttura che contiene informazioni sul tipo di disegno necessari.  
  
### <a name="remarks"></a>Note  
 Il **itemAction** membro del `DRAWITEMSTRUCT` struttura definisce l'operazione di disegno che deve essere eseguita.  
  
 Per impostazione predefinita, questa funzione membro non esegue alcuna operazione. Eseguire l'override di questa funzione membro per implementare la creazione per un proprietario `CStatusBarCtrl` oggetto.  
...
### <a name="parameters"></a>Parametri  
 *pBorders*  
 Indirizzo di una matrice di interi con tre elementi. Il primo elemento riceve lo spessore del bordo orizzontale, il secondo riceve la larghezza del bordo verticale e il terzo riceve la larghezza del bordo tra i rettangoli.  
...
### <a name="remarks"></a>Note  
 Questo metodo invia il [SB_GETICON](http://msdn.microsoft.com/library/windows/desktop/bb760744) messaggio, che è descritta nel [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
 ...
 *pParts*  
 Indirizzo di una matrice di interi con lo stesso numero di elementi come il numero di parti specificato da `nParts`. Ogni elemento nella matrice riceve le coordinate client del bordo destro della parte corrispondente. Se un elemento è impostato su-1, la posizione del bordo destro per tale parte estende al bordo destro della barra di stato.  
  
### <a name="return-value"></a>Valore restituito  
 Il numero di parti del controllo se ha esito positivo o zero in caso contrario.  
  
### <a name="remarks"></a>Note  
 Questa funzione membro recupera inoltre la coordinata del bordo destro del numero specificato di parti.  
...
- **0** del testo disegnato con un bordo in basso rispetto al piano della barra di stato.  
...
- `SBT_OWNERDRAW`Se il testo ha la `SBT_OWNERDRAW` , tipo di disegno `pType` riceve questo messaggio e restituisce il valore a 32 bit associato al testo anziché al tipo di lunghezza e l'operazione.  
  
### <a name="return-value"></a>Valore restituito  
 La lunghezza in caratteri, del testo o un [CString](../../atl-mfc-shared/reference/cstringt-class.md) contenente il testo corrente.  
...
### <a name="example"></a>Esempio  
 [!code-cpp[6 NVC_MFC_CStatusBarCtrl](../../mfc/reference/codesnippet/cpp/cstatusbarctrl-class_8.cpp)]  
...
### <a name="return-value"></a>Valore restituito  
 Oggetto [CString](../../atl-mfc-shared/reference/cstringt-class.md) oggetto contenente il testo da utilizzare nella descrizione comandi.  
  
### <a name="remarks"></a>Note  
 Questa funzione membro implementa il comportamento del messaggio Win32 [SB_GETTIPTEXT](http://msdn.microsoft.com/library/windows/desktop/bb760751), come descritto nel [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Note  
 Questa funzione membro implementa il comportamento del messaggio Win32 [SB_ISSIMPLE](http://msdn.microsoft.com/library/windows/desktop/bb760753), come descritto nel [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Parametri  
 `cr`  
 **COLORREF** valore che specifica il nuovo colore di sfondo. Specificare il `CLR_DEFAULT` valore affinché la barra di stato utilizzare il colore di sfondo predefinito.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro implementa il comportamento del messaggio Win32 [SB_SETBKCOLOR](http://msdn.microsoft.com/library/windows/desktop/bb760754), come descritto nel [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Note  
 Questa funzione membro implementa il comportamento del messaggio Win32 [SB_SETICON](http://msdn.microsoft.com/library/windows/desktop/bb760755), come descritto nel [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
### <a name="example"></a>Esempio  
  Vedere l'esempio per [CStatusBarCtrl::SetBkColor](#setbkcolor).  
  
##  <a name="setminheight"></a>CStatusBarCtrl::SetMinHeight  
 Imposta l'altezza minima di uno stato barra area di disegno del controllo.  
...
### <a name="example"></a>Esempio  
 [!code-cpp[9 NVC_MFC_CStatusBarCtrl](../../mfc/reference/codesnippet/cpp/cstatusbarctrl-class_11.cpp)]  
  
##  <a name="setparts"></a>CStatusBarCtrl::SetParts  
 Imposta il numero di parti in uno stato della barra di controllo e la coordinata del bordo destro di ogni parte.  
 ...
 *pWidths*  
 Indirizzo di una matrice di interi con lo stesso numero di elementi come parti specificate dal `nParts`. Ogni elemento nella matrice specifica la posizione, nelle coordinate client, del bordo destro della parte corrispondente. Se un elemento è – 1, la posizione del bordo destro per tale parte si estende al bordo destro del controllo.  
...
##  <a name="setsimple"></a>CStatusBarCtrl::SetSimple  
 Specifica se un controllo barra di stato Visualizza testo semplice o tutte le parti di controllo impostate da una precedente chiamata a [SetParts](#setparts).  
...
### <a name="parameters"></a>Parametri  
 [in] `bSimple`  
 Flag di tipo di visualizzazione. Se questo parametro è `TRUE`, il controllo Visualizza testo semplice; in caso `FALSE`, viene visualizzato più parti.  
  
### <a name="return-value"></a>Valore restituito  
 Restituisce sempre 0.  
  
### <a name="remarks"></a>Note  
 Se l'applicazione passa il controllo barra di stato da non semplice a semplice o viceversa, il sistema ridisegna immediatamente il controllo.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro implementa il comportamento del messaggio Win32 [SB_SETTIPTEXT](http://msdn.microsoft.com/library/windows/desktop/bb760759), come descritto nel [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
