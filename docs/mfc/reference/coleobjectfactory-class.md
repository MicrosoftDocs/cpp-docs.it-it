...
|[COleObjectFactory::Register](#register)|Registra l'object factory con il sistema OLE DLL.|  
...
|[COleObjectFactory::Revoke](#revoke)|Revoca la registrazione della factory di questo oggetto con le DLL di sistema OLE.|  
...
|[COleObjectFactory::VerifyUserLicense](#verifyuserlicense)|Verifica che il controllo è concesso in licenza per l'utilizzo in fase di progettazione.|  
...
-   Aggiornare il Registro di sistema OLE, nonché la registrazione in fase di esecuzione che informano OLE che gli oggetti sono in esecuzione e pronta a ricevere messaggi.  
  
-   Applicazione di gestione delle licenze, limitando l'utilizzo del controllo per gli sviluppatori con licenza in fase di progettazione e le applicazioni con licenza in fase di esecuzione.  
...
##  <a name="coleobjectfactory"></a>COleObjectFactory::COleObjectFactory  
 Costruisce un `COleObjectFactory` oggetto, viene inizializzato come una factory di annullare la registrazione di oggetti e lo aggiunge all'elenco di factory.  
...
### <a name="parameters"></a>Parametri  
 `clsid`  
 Riferimento all'ID classe OLE rappresenta questo oggetto factory.  
  
 `pRuntimeClass`  
 Puntatore alla classe in fase di esecuzione di questa factory è possibile creare gli oggetti di C++.  
...
- **afxRegDefault** imposta il modello di threading ThreadingModel = Apartment.  
...
##  <a name="getclassid"></a>COleObjectFactory::GetClassID  
 Restituisce un riferimento all'ID classe OLE rappresenta questa factory.  
...
### <a name="return-value"></a>Valore restituito  
 Riferimento all'ID classe OLE questa factory rappresenta.  
  
### <a name="remarks"></a>Note  
 Per ulteriori informazioni, vedere [chiave CLSID](http://msdn.microsoft.com/library/windows/desktop/ms691424) in Windows SDK.  
  
##  <a name="getlicensekey"></a>COleObjectFactory::GetLicenseKey  
 Richiede una chiave di licenza univoca di DLL del controllo e la archivia nel `BSTR` a cui puntava `pbstrKey`.  
...
### <a name="remarks"></a>Note  
 L'implementazione predefinita di questa funzione restituisce 0 e memorizzato alcun valore nel `BSTR`. Se si utilizza la creazione guidata controllo ActiveX di MFC per creare il progetto, la creazione guidata controllo fornisce un override che recupera il codice di licenza del controllo.  
...
##  <a name="register"></a>COleObjectFactory::Register  
 Registra l'object factory con il sistema OLE DLL.  
...
### <a name="return-value"></a>Valore restituito  
 Diverso da zero se la factory registrata. in caso contrario 0.  
  
### <a name="remarks"></a>Note  
 Questa funzione viene chiamata generalmente dal [:: InitInstance](../../mfc/reference/cwinapp-class.md#initinstance) quando viene avviata l'applicazione.  
  
##  <a name="revoke"></a>COleObjectFactory::Revoke  
 Revoca la registrazione della factory di questo oggetto con le DLL di sistema OLE.  
...
### <a name="parameters"></a>Parametri  
 `lpszProgID`  
 Puntatore a una stringa contenente l'ID di programma leggibile, ad esempio "Excel.Document.5".  
  
 `bRegister`  
 Determina se la factory di oggetto della classe del controllo da registrare.  
  
### <a name="remarks"></a>Note  
 Seguono le discussioni breve delle due forme per questa funzione:  
...
- **UpdateRegistry (** `bRegister` **)** questa forma della funzione è sottoponibile a override. Se `bRegister` è **TRUE**, questa funzione registra la classe del controllo al Registro di sistema. In caso contrario, Annulla la registrazione della classe.  
  
     Se si utilizza la creazione guidata controllo ActiveX di MFC per creare il progetto, la creazione guidata controllo fornisce un override di questa funzione virtuale pura.  
...
### <a name="remarks"></a>Note  
 Le chiamate di versione predefinito [GetLicenseKey](#getlicensekey) per ottenere una copia del controllo della stringa di licenza e lo confronta con la stringa in `bstrKey`. Se le due stringhe corrispondono, la funzione restituisce un valore diverso da zero. in caso contrario viene restituito 0.  
...
### <a name="return-value"></a>Valore restituito  
 Diverso da zero se la licenza in fase di progettazione non è valida. in caso contrario 0.  
