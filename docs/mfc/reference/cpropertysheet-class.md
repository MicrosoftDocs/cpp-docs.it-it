...
|[CPropertySheet::GetActiveIndex](#getactiveindex)|Recupera l'indice della pagina active della finestra delle proprietà.|  
...
## <a name="remarks"></a>Note  
 ...
 Quando si costruisce una `CPropertySheet` dell'oggetto, alcune [stili finestra](../../mfc/reference/styles-used-by-mfc.md#window-styles) può provocare un'eccezione first-chance si verificano. Ciò è dovuto il sistema tenta di modificare lo stile della finestra delle proprietà prima che venga creato il foglio. Per evitare questa eccezione, verificare che i seguenti stili è impostato quando si crea il `CPropertySheet`:  
...
-   WS_CLIPCHILDREN  
 ...
 Scambio di dati tra un `CPropertySheet` oggetto e un oggetto esterno è simile per lo scambio di dati con un `CDialog` oggetto. La differenza importante è che le impostazioni di una finestra delle proprietà sono in genere variabili membro del `CPropertyPage` oggetti anziché del `CPropertySheet` oggetto stesso.  
  
 È possibile creare un tipo di scheda della finestra di dialogo chiamato una procedura guidata, è costituito da una finestra delle proprietà con una sequenza di pagine delle proprietà che guidano l'utente tramite i passaggi di un'operazione, ad esempio configurare un dispositivo o creando un notiziario. In una finestra di dialogo scheda associata alla procedura guidata, le pagine delle proprietà non dispone di schede e pagina solo una proprietà è visibile alla volta. Inoltre, invece di dover **OK** e **applica** pulsanti, una finestra di dialogo scheda associata alla procedura guidata ha un **nuovamente** pulsante, un **Avanti** o  **Fine** pulsante, una **annullare** pulsante e un **Guida** pulsante.  
...
##  <a name="addpage"></a>  CPropertySheet:: addPage  
 Aggiunge la pagina fornita con la scheda più a destra nella pagina delle proprietà.  
...
### <a name="remarks"></a>Note  
 Aggiungere le pagine delle proprietà nell'ordine da sinistra a destra che si desidera visualizzarle.  
 ...
 Quando si aggiunge una pagina delle proprietà mediante `AddPage`, il `CPropertySheet` è l'elemento padre del `CPropertyPage`. Per accedere alla finestra delle proprietà dalla pagina delle proprietà, chiamare [CWnd::GetParent](../../mfc/reference/cwnd-class.md#getparent).  
 ...
 *iSelectPage*  
 L'indice della pagina che sarà inizialmente in primo piano. Valore predefinito è la prima pagina aggiunta al foglio.  
...
### <a name="remarks"></a>Note  
 ...
 È possibile visualizzare le immagini della filigrana e/o intestazione automaticamente se si utilizzano i terzo o quarto prototipi della `Construct`, elencate in precedenza e si passano i valori validi per il *hbmWatermark*, *hpalWatermark* , e/o *hbmHeader* parametri.  
...
### <a name="remarks"></a>Note  
 ...
 È possibile visualizzare le immagini della filigrana e/o intestazione automaticamente se si utilizzano i terzo o quarto prototipi della `CPropertySheet`precedente, e si passano i valori validi il *hbmWatermark*, *hpalWatermark*, e / o *hbmHeader* parametri.  
...
### <a name="remarks"></a>Note  
 La chiamata a `Create` può essere all'interno del costruttore, oppure è possibile chiamare dopo che viene chiamato il costruttore.  
  
 Lo stile predefinito, espresso mediante il passaggio di -1 come *dwStyle*, viene effettivamente **WS_SYSMENU&#124;**`WS_POPUP`**&#124;WS_CAPTION&#124;DS_MODALFRAME&#124;DS_ CONTEXTHELP&#124;WS_VISIBLE**. Il valore predefinito è esteso lo stile di finestra, espresso, passare 0 come *dwExStyle*, viene effettivamente **WS_EX_DLGMODALFRAME**.  
  
 Il `Create` funzione membro restituisce immediatamente dopo la creazione delle proprietà. Per eliminare la finestra delle proprietà, chiamare [DestroyWindow](../../mfc/reference/cwnd-class.md#destroywindow).  
  
 Finestre delle proprietà non modale visualizzate con una chiamata a `Create` siano privi di pulsanti OK, Annulla, applica e Guida come le finestre delle proprietà modale. Pulsanti desiderati devono essere creati dall'utente.  
...
### <a name="return-value"></a>Valore restituito  
 `IDOK` o `IDCANCEL` se la funzione esito positivo; in caso contrario, 0 o -1. Se la finestra delle proprietà è stata stabilita come una procedura guidata (vedere [SetWizardMode](#setwizardmode)), `DoModal` restituisce `ID_WIZFINISH` o `IDCANCEL`.  
  
### <a name="remarks"></a>Note  
 Il valore restituito corrisponde all'ID del controllo che chiuso la finestra delle proprietà. Dopo che questa funzione restituisce un risultato, le finestre corrispondente per la finestra delle proprietà e tutte le pagine verranno sono stati eliminati definitivamente. Gli oggetti stessi saranno ancora disponibile. In genere, si recupererà i dati dal [CPropertyPage](../../mfc/reference/cpropertypage-class.md) oggetti dopo `DoModal` restituisce `IDOK`.  
  
 Per visualizzare una finestra delle proprietà non modale, chiamare [crea](#create) invece.  
  
 Creazione di una pagina delle proprietà dalla relativa risorsa finestra di dialogo corrispondente, può causare un'eccezione first-chance. Ciò è dovuto alla pagina delle proprietà modificare lo stile della risorsa finestra di dialogo per lo stile desiderato prima che venga creata la pagina. Poiché le risorse sono in genere di sola lettura, verrà generata un'eccezione. Il sistema gestisce l'eccezione e crea una copia della risorsa modificata. Le eccezioni first-chance possono pertanto essere ignorato.  
  
> [!NOTE]
>  Questa eccezione deve essere gestita dal sistema operativo se si esegue la compilazione con il modello di gestione delle eccezioni asincrone. Per ulteriori informazioni sui modelli di gestione delle eccezioni, vedere [/EH (modello di gestione delle eccezioni)](../../build/reference/eh-exception-handling-model.md). In questo caso, non eseguire il wrapping di chiamate a `CPropertySheet::DoModal` con un blocco try-catch C++ in cui catch gestisce tutte le eccezioni, ad esempio, `catch (...)`. Questo blocco gestisce l'eccezione concepito per il sistema operativo e causa un comportamento imprevedibile. Tuttavia, è possibile utilizzare in modo sicuro C++ delle eccezioni con tipi di eccezione specifici o la gestione delle eccezioni strutturata in cui viene passata l'eccezione di violazione di accesso al sistema operativo.  
  
 Per evitare la generazione di questa eccezione first-chance, è possibile manualmente garantisce che la finestra delle proprietà disponga di corrette [stili finestra](../../mfc/reference/styles-used-by-mfc.md#window-styles). È necessario impostare gli stili per una finestra delle proprietà seguenti:  
...
-   WS_CLIPCHILDREN  
 ...
 Disabilitare tutti gli altri stili di Windows perché non sono compatibili con finestre delle proprietà. Questa raccomandazione non è applicabile agli stili estesi. Impostazione in modo appropriato gli stili standard garantisce che la finestra delle proprietà non deve essere modificato e si eviterà la generazione di eccezioni first-chance.  
  
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo [:: addPage](#addpage).  
...
### <a name="parameters"></a>Parametri  
 *bStacked*  
 Indica se le schede in pila sono abilitate nella finestra delle proprietà. Disabilitare le righe in pila dei tag impostando *bStacked* alla **FALSE**.  
  
### <a name="remarks"></a>Note  
 Per impostazione predefinita, se una finestra delle proprietà dispone di più schede di quelle presenti in una singola riga nella larghezza della finestra delle proprietà, le schede vengono impilate su più righe. Per utilizzare le schede di scorrimento anziché sovrapposizione schede, chiamare `EnableStackedTabs` con *bStacked* impostata su **FALSE** prima di chiamare [DoModal](#domodal) o [crea](#create).  
...
##  <a name="getpageindex"></a>  CPropertySheet::GetPageIndex  
 Recupera il numero di indice della pagina specificata nella pagina delle proprietà.  
...
##  <a name="gettabcontrol"></a>  CPropertySheet:: GetTabControl  
 Recupera un puntatore a un controllo tab esegua operazioni specifiche del controllo scheda (vale a dire, per usare una qualsiasi delle API in [CTabCtrl](../../mfc/reference/ctabctrl-class.md)).  
...
### <a name="remarks"></a>Note  
 Usare questa struttura per inizializzare l'aspetto della finestra delle proprietà dopo la sua creazione, ma prima che venga visualizzato con il [DoModal](#domodal) funzione membro. Ad esempio, impostare il *dwSize* appartenente `m_psh` le dimensioni si desidera utilizzare la finestra delle proprietà di.  
...
### <a name="parameters"></a>Parametri  
 *lpRect*  
 Punta a un [RECT](http://msdn.microsoft.com/library/windows/desktop/dd162897) struttura oppure [CRect](../../atl-mfc-shared/reference/crect-class.md) oggetto che contiene la finestra di dialogo coordinate da convertire.  
...
### <a name="remarks"></a>Note  
 ...
 Il [GetDialogBaseUnits](http://msdn.microsoft.com/library/windows/desktop/ms645475) funzione Windows restituisce informazioni sulle dimensioni di carattere del sistema, ma è possibile specificare un carattere diverso per ogni finestra delle proprietà se si utilizza il **DS_SETFONT** stile il file di definizione di risorsa. Il [MapDialogRect](http://msdn.microsoft.com/library/windows/desktop/ms645502) per questa finestra di dialogo funzione di Windows, descritte in SDK, utilizza il tipo di carattere appropriato.  
...
### <a name="return-value"></a>Valore restituito  
 Specifica se l'applicazione ha impostato lo stato attivo di input a uno dei controlli nella finestra delle proprietà. Se `OnInitDialog` restituisce un valore diverso da zero, Windows imposta lo stato attivo di input del primo controllo nella finestra delle proprietà. L'applicazione può restituire 0 solo se è impostata in modo esplicito lo stato attivo di input a uno dei controlli nella finestra delle proprietà.  
  
### <a name="remarks"></a>Note  
 Questa funzione membro viene chiamata in risposta al messaggio WM_INITDIALOG. Questo messaggio viene inviato alla finestra delle proprietà durante la [Create](#create) oppure [DoModal](#domodal) chiamate, che si verificano subito prima che venga visualizzata la finestra delle proprietà.  
  
 Eseguire l'override di questa funzione membro se è necessario eseguire un'elaborazione speciale quando viene inizializzata delle proprietà. Nella versione sottoposta a override, chiamare innanzitutto la classe di base `OnInitDialog` ma ignorare il relativo valore restituito. In genere si tornerà **TRUE** dalla funzione membro sottoposto a override.  
...
##  <a name="removepage"></a>  CPropertySheet::RemovePage  
 Rimuove una pagina delle proprietà ed Elimina definitivamente la finestra associata.  
...
### <a name="remarks"></a>Note  
 Il [CPropertyPage](../../mfc/reference/cpropertypage-class.md) oggetto stesso non viene eliminato finché il proprietario del `CPropertySheet` finestra è chiusa.  
...
### <a name="remarks"></a>Note  
 ...
 Se si desidera modificare il testo sul pulsante Fine o nascondere la successiva e Back pulsante una volta l'utente ha completato la procedura guidata, chiamata [SetFinishText](#setfinishtext). Si noti che per la fine e Avanti sia condivisa con lo stesso pulsante. È possibile visualizzare una fine o in una sola volta un pulsante Avanti, ma non entrambi.  
  
### <a name="example"></a>Esempio  
 Un `CPropertySheet` ha tre pagine della procedura guidata proprietà: `CStylePage`, `CColorPage`, e `CShapePage`.  Nel frammento di codice riportato di seguito viene illustrato come abilitare e disabilitare il **Indietro** e **successivo** pulsanti nella pagina delle proprietà della procedura guidata.  
...
##  <a name="setwizardmode"></a>  CPropertySheet:: SetWizardMode  
 Stabilisce una pagina delle proprietà come una procedura guidata.  
...
### <a name="remarks"></a>Note  
 Una caratteristica chiave una creazione guidata pagina delle proprietà è che l'utente si sposta con accanto o fine, eseguire il backup e annullare pulsanti invece di tabulazioni.  
  
 Chiamare `SetWizardMode` prima di chiamare [DoModal](#domodal). Dopo aver chiamato `SetWizardMode`, `DoModal` restituirà uno **ID_WIZFINISH** (se l'utente chiude con il pulsante Fine) o **IDCANCEL**.  
