...
|Nome|DESCRIZIONE|
|----------|-----------------|
|[CFont::CreateFont](#createfont)|Inizializza un `CFont` oggetto con le caratteristiche specificate.|
...
|[CFont::CreatePointFont](#createpointfont)|Inizializza un `CFont` oggetto con l'altezza specificata, misurata in decimi di un punto e carattere tipografico.|
...
|[CFont::GetLogFont](#getlogfont)|Compila un `LOGFONT` `CFont` oggetto con le informazioni sul tipo di carattere logico associato all'oggetto.|

### <a name="public-operators"></a>Operatori pubblici

|Nome|DESCRIZIONE|
|----------|-----------------|
|[CFont:: operator HFONT](#operator_hfont)|Restituisce l'handle del tipo di carattere GDI di `CFont` Windows collegato all'oggetto.|

## <a name="remarks"></a>Note

Per usare un `CFont` oggetto, costruire un `CFont` oggetto e alleghirvi un tipo di carattere di Windows con [CreateFont](#createfont), [CreateFontIndirect](#createfontindirect), [CreatePointFont](#createpointfont)o [CreatePointFontIndirect](#createpointfontindirect)e quindi usare il membro dell'oggetto. funzioni per modificare il tipo di carattere.

Le `CreatePointFont` funzioni `CreatePointFontIndirect` e sono spesso più semplici da usare `CreateFont` rispetto `CreateFontIndirect` a o perché eseguono automaticamente la conversione per l'altezza del tipo di carattere da una dimensione punto a unità logiche.
...
L'oggetto risultante deve essere inizializzato `CreateFont`con `CreateFontIndirect`, `CreatePointFont`, o `CreatePointFontIndirect` prima di poter essere utilizzato.
...
*nHeight*<br/>
Specifica l'altezza desiderata (in unità logiche) del tipo di carattere. Per una `lfHeight` descrizione, vedere il membro della struttura [LOGFONT](/windows/win32/api/wingdi/ns-wingdi-logfontw)nell'Windows SDK. Il valore assoluto di *nHeight* non deve superare 16.384 unità del dispositivo dopo la conversione. Per tutti i confronti di altezza, il mapper dei tipi di carattere cerca il tipo di carattere più grande che non supera la dimensione richiesta o il tipo di carattere più piccolo se tutti i tipi di carattere superano la dimensione richiesta.
...
*nEscapement*<br/>
Specifica l'angolo (in unità di 0,1 gradi) tra il vettore di scappamento e l'asse x della superficie di visualizzazione. Il vettore di scappamento è la riga attraverso le origini del primo e dell'ultimo carattere su una riga. L'angolo viene misurato in senso antiorario dall'asse x. Per ulteriori `lfEscapement` informazioni, vedere `LOGFONT` il membro nella struttura del Windows SDK.
...
*nWeight*<br/>
Specifica lo spessore del carattere (in pixel inchiostrati per 1000). Per ulteriori informazioni, vedere il `LOGFONT` membro lfWeight nella struttura del Windows SDK. I valori descritti sono approssimativi. l'aspetto effettivo dipende dal carattere tipografico. Alcuni tipi di carattere hanno solo FW_NORMAL, FW_REGULAR e FW_BOLD weights. Se viene specificato FW_DONTCARE, viene utilizzato un peso predefinito.
...
*nCharSet*<br/>
Specifica il set di caratteri del tipo di `lfCharSet` carattere del membro `LOGFONT` nella struttura nel Windows SDK per un elenco di valori.
...
*nOutPrecision*<br/>
Specifica la precisione di output desiderata. La precisione di output definisce il livello di corrispondenza tra l'output e l'altezza, la larghezza, l'orientamento, lo scappamento e il passo del tipo di carattere richiesto. Per un `lfOutPrecision` elenco di valori `LOGFONT` e altre informazioni, vedere il membro nella struttura del Windows SDK.

*nClipPrecision*<br/>
Specifica la precisione di ritaglio desiderata. La precisione di ritaglio definisce la modalità di ritaglio dei caratteri che sono parzialmente all'esterno dell'area di ridimensionamento. Per un `lfClipPrecision` elenco di valori `LOGFONT` , vedere il membro nella struttura del Windows SDK.

Per usare un tipo di carattere di sola lettura incorporato, un'applicazione deve specificare CLIP_ENCAPSULATE.

Per ottenere una rotazione coerente dei tipi di carattere del dispositivo, TrueType e vettoriali, un'applicazione può usare l'operatore OR per combinare il valore CLIP_LH_ANGLES con uno degli altri valori di *nClipPrecision* . Se il bit CLIP_LH_ANGLES è impostato, la rotazione di tutti i tipi di carattere varia a seconda che l'orientamento del sistema di coordinate sia a sinistra o a destra. Per ulteriori informazioni sull'orientamento dei sistemi di coordinate, vedere la descrizione del parametro *nOrientation* . Se CLIP_LH_ANGLES non è impostato, i tipi di carattere del dispositivo ruotano sempre in senso antiorario, ma la rotazione di altri tipi di carattere dipende dall'orientamento del sistema di coordinate.

*nQuality*<br/>
Specifica la qualità di output del tipo di carattere, che definisce la precisione con cui GDI deve tentare di trovare la corrispondenza con gli attributi del tipo di carattere logico con quelli di un tipo di carattere fisico effettivo. Per un `lfQuality` elenco di valori `LOGFONT` , vedere il membro nella struttura del Windows SDK.

*nPitchAndFamily*<br/>
Specifica il pitch e la famiglia del tipo di carattere. Per un `lfPitchAndFamily` elenco di valori `LOGFONT` e altre informazioni, vedere il membro nella struttura del Windows SDK.

*lpszFacename*<br/>
Puntatore `CString` o a una stringa con terminazione null che specifica il nome tipografico del tipo di carattere. La lunghezza di questa stringa non deve superare i 30 caratteri. La funzione [EnumFontFamilies](/windows/win32/api/wingdi/nf-wingdi-enumfontfamiliesw) di Windows può essere utilizzata per enumerare tutti i tipi di carattere attualmente disponibili. Se *lpszFacename* è null, GDI usa un carattere tipografico indipendente dal dispositivo.
...
Al termine dell' `CFont` oggetto creato `CreateFont` dalla funzione, usare `CDC::SelectObject` per selezionare un tipo di carattere diverso nel contesto di dispositivo, quindi eliminare l' `CFont` oggetto che non è più necessario.
...
Quando l' `CFont` oggetto creato `CreateFontIndirect` dalla funzione non è più necessario, usare `CDC::SelectObject` per selezionare un tipo di carattere diverso nel contesto di dispositivo, quindi eliminare `CFont` l'oggetto che non è più necessario.
...
*lpszFaceName*<br/>
Puntatore `CString` o a una stringa con terminazione null che specifica il nome tipografico del tipo di carattere. La lunghezza di questa stringa non deve superare i 30 caratteri. La funzione EnumFontFamilies di Windows può essere utilizzata per enumerare tutti i tipi di carattere attualmente disponibili. Se *lpszFaceName* è null, GDI usa un carattere tipografico indipendente dal dispositivo.
...
Al termine dell' `CFont` oggetto creato `CreatePointFont` dalla funzione, selezionare innanzitutto il tipo di carattere al di fuori del contesto di dispositivo, quindi eliminare `CFont` l'oggetto.
...
Questa funzione è identica a [CreateFontIndirect](#createfontindirect) , con la differenza `lfHeight` che il membro `LOGFONT` di viene interpretato in decimi di un punto anziché in unità di dispositivo.
...
*lpLogFont*<br/>
Punta a una struttura [LOGFONT](/windows/win32/api/wingdi/ns-wingdi-logfontw) che definisce le caratteristiche del tipo di carattere logico. Il `lfHeight` membro`LOGFONT` della struttura viene misurato in decimi di un punto anziché di unità logiche. Ad esempio, impostare `lfHeight` su 120 per richiedere un tipo di carattere a 12 punti.
...
Al termine dell' `CFont` oggetto creato `CreatePointFontIndirect` dalla funzione, selezionare innanzitutto il tipo di carattere al di fuori del contesto di dispositivo, quindi eliminare `CFont` l'oggetto.
...
Se un `CFont` oggetto non è già collegato all'handle, viene creato e `CFont` collegato un oggetto temporaneo. Questo oggetto `CFont` temporaneo è valido solo fino alla successiva esecuzione del tempo di inattività dell'applicazione nel ciclo di eventi, quindi vengono eliminati tutti gli oggetti grafici temporanei. Un altro modo per indicare che l'oggetto temporaneo è valido solo durante l'elaborazione di un messaggio di finestra.
...
Utilizzare questo operatore per ottenere l'handle GDI di Windows del tipo di carattere associato `CFont` all'oggetto.
...
Handle dell'oggetto del tipo di carattere GDI Windows collegato `CFont` a in caso di esito positivo; in caso contrario, null.

### <a name="remarks"></a>Note

Poiché questo operatore viene usato automaticamente per le conversioni da `CFont` a [tipi di carattere e testo](/windows/win32/gdi/fonts-and-text), è `CFont` possibile passare gli oggetti alle funzioni che prevedono HFONTS.
