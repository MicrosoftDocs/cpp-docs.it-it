...
È possibile accedere agli oggetti eccezione nell'ambito di un'espressione [catch](../../mfc/reference/exception-processing.md#catch) . È anche possibile generare oggetti `CDaoException` dal codice personalizzato con la funzione globale [AfxThrowDaoException](../../mfc/reference/exception-processing.md#afxthrowdaoexception) .
...
1. Costruisce un oggetto `CDaoException`.
...
1. Esaminare la struttura [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) corrente che `GetErrorInfo` restituisce nel membro dati [m_pErrorInfo](#m_perrorinfo) . I membri forniscono informazioni sull'errore DAO.
...
`GetErrorInfo` archivia le informazioni nel membro dati `m_pErrorInfo` dell'oggetto eccezione. Per una breve descrizione delle informazioni restituite, vedere [m_pErrorInfo](#m_perrorinfo). Se si intercetta un'eccezione di tipo `CDaoException` generata da MFC, il membro del `m_pErrorInfo` sarà già compilato. Se si sceglie di chiamare direttamente DAO, è necessario chiamare manualmente la funzione membro `GetErrorInfo` dell'oggetto eccezione per riempire `m_pErrorInfo`. Per una descrizione più dettagliata, vedere la struttura [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) .
...
Per dettagli completi sulle informazioni contenute nell'oggetto `CDaoErrorInfo`, vedere la struttura [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) .
