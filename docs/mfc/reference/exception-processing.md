---
title: Elaborazione delle eccezioni | Documenti di Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
Quando viene eseguito un programma, può verificarsi un numero di errori chiamati "eccezioni" e le condizioni anormali. Ad esempio esaurimento di memoria, errori di allocazione di risorse e impossibilità di trovare i file.  
  
 La libreria Microsoft Foundation Class utilizza uno schema di gestione delle eccezioni è modellato da vicino quella proposta del comitato degli standard ANSI per C++. Prima di chiamare una funzione che potrebbe verificarsi una situazione anomala, è necessario impostare un gestore di eccezioni. Se la funzione rileva una condizione anomala, viene generata un'eccezione e il controllo viene passato al gestore di eccezioni.  
  
 Configureranno i gestori di eccezioni più macro incluse con la libreria Microsoft Foundation Class. Un numero di altre funzioni globali consigliabile di generare eccezioni specializzate e terminare i programmi, se necessario. Queste macro e funzioni globali rientrano nelle categorie seguenti:  
...
- Funzioni di terminazione, determinando la chiusura del programma.  
...
|[CATCH_ALL](#catch_all)|Definisce un blocco di codice per il rilevamento di tutte le eccezioni dal precedente **PROVARE** blocco.|  
|[AND_CATCH](#and_catch)|Definisce un blocco di codice per il rilevamento di tipi di eccezione aggiuntivi dal precedente **PROVARE** blocco.|  
|[AND_CATCH_ALL](#and_catch_all)|Definisce un blocco di codice per il rilevamento di tutti gli altri tipi aggiuntivi di eccezione generate in un precedente **PROVARE** blocco.|  
...
|[GENERAZIONE DI ECCEZIONI](#throw)|Genera un'eccezione specificata.|  
...
|[AfxThrowFileException](#afxthrowfileexception)|Genera un'eccezione del file.|  
|[AfxThrowMemoryException](#afxthrowmemoryexception)|Genera un'eccezione di memoria.|  
|[AfxThrowNotSupportedException](#afxthrownotsupportedexception)|Genera un'eccezione non è supportato.|  
|[AfxThrowResourceException](#afxthrowresourceexception)|Genera un'eccezione di Windows risorsa non trovata.|  
|[AfxThrowUserException](#afxthrowuserexception)|Genera un'eccezione in un'azione viene avviata dall'utente.|  
...
|||  
|-|-|  
|[AfxAbort](#afxabort)|Chiamato per terminare un'applicazione quando un errore irreversibile si verifica.|  
  
##  <a name="a-nametrya--try"></a><a name="try"></a>PROVARE A  
...
### <a name="remarks"></a>Note  
 Oggetto **PROVARE** blocco identifica un blocco di codice che potrebbe generare eccezioni. Nell'esempio seguente vengono gestite le eccezioni **CATCH** e `AND_CATCH` blocchi. Recursion è consentito: le eccezioni possono essere passate a un **PROVARE** blocco, ignorando le o utilizzando il `THROW_LAST` (macro). Fine di **PROVARE** blocco con un `END_CATCH` o `END_CATCH_ALL` (macro).  
...
### <a name="remarks"></a>Note  
 ...
 Se *classe_eccezione* è la classe `CException`, quindi tutti i tipi di eccezione verranno intercettati. È possibile utilizzare il [CObject:: IsKindOf](../../mfc/reference/cobject-class.md#iskindof) funzione membro per determinare l'eccezione specifica che è stata generata. Un modo migliore per rilevare i diversi tipi di eccezioni è utilizzare sequenziale `AND_CATCH` istruzioni, ognuna con un altro tipo di eccezione.  
  
 Il puntatore all'oggetto eccezione viene creato dalla macro. Non è necessario dichiarare manualmente.  
  
> [!NOTE]
>  Il **CATCH** blocco viene definito come ambito C++ definito da parentesi graffe. Se si dichiarano delle variabili in questo ambito, è possibile accedere a queste variabili solo all'interno di tale ambito. Questo vale anche per *nome_puntatore_oggetto_eccezione*.  
...
##  <a name="a-namecatchalla--catchall"></a><a name="catch_all"></a>CATCH_ALL  
 Definisce un blocco di codice che rileva tutti i tipi di eccezione generati nel passaggio precedente **PROVARE** blocco.  
...
##  <a name="a-nameandcatcha--andcatch"></a><a name="and_catch"></a>AND_CATCH  
 Definisce un blocco di codice per il rilevamento di tipi di eccezione aggiuntivi generati in un precedente **PROVARE** blocco.  
 ...
 *nome_puntatore_oggetto_eccezione*  
 Un nome per un puntatore all'oggetto eccezione che verrà creato dalla macro. È possibile utilizzare il nome del puntatore per accedere all'oggetto eccezione all'interno del blocco `AND_CATCH`. Questa variabile viene dichiarata automaticamente.  
  
### <a name="remarks"></a>Note  
 Utilizzare il **CATCH** macro per intercettare un tipo di eccezione, il `AND_CATCH` (macro) per rilevare ogni tipo successive. Fine di **PROVARE** blocco con un `END_CATCH` (macro).  
  
 Il codice di elaborazione delle eccezioni può interrogare l'oggetto eccezione, se necessario, per ottenere ulteriori informazioni sulla causa specifica dell'eccezione. Chiamare il `THROW_LAST` (macro) all'interno di `AND_CATCH` bloccare per spostare l'elaborazione sul frame esterno dell'eccezione. `AND_CATCH`Contrassegna la fine della precedente **CATCH** o `AND_CATCH` blocco.  
  
> [!NOTE]
>  Il `AND_CATCH` blocco viene definito come ambito C++ (racchiusi tra parentesi graffe). Se si dichiarano variabili nell'ambito corrente, tenere presente che sono accessibili solo all'interno di tale ambito. Questo vale anche per il *nome_puntatore_oggetto_eccezione* variabile.  
...
### <a name="parameters"></a>Parametri  
 *nome_puntatore_oggetto_eccezione*  
 Un nome per un puntatore all'oggetto eccezione che verrà creato dalla macro. È possibile utilizzare il nome del puntatore per accedere all'oggetto eccezione all'interno del blocco `AND_CATCH_ALL`. Questa variabile viene dichiarata automaticamente.  
  
### <a name="remarks"></a>Note  
 Utilizzare il **CATCH** macro per intercettare un tipo di eccezione, il `AND_CATCH_ALL` macro per intercettare tutti gli altri tipi successive. Se si utilizza `AND_CATCH_ALL`, fine il **PROVARE** blocco con un `END_CATCH_ALL` (macro).  
  
 Il codice di elaborazione delle eccezioni può interrogare l'oggetto eccezione, se necessario, per ottenere ulteriori informazioni sulla causa specifica dell'eccezione. Chiamare il `THROW_LAST` (macro) all'interno di `AND_CATCH_ALL` bloccare per spostare l'elaborazione sul frame esterno dell'eccezione. `AND_CATCH_ALL`Contrassegna la fine della precedente **CATCH** o `AND_CATCH_ALL` blocco.  
  
> [!NOTE]
>  Il `AND_CATCH_ALL` blocco viene definito come ambito C++ (definito da parentesi graffe). Se si dichiarano variabili nell'ambito corrente, tenere presente che sono accessibili solo all'interno di tale ambito.  
...
### <a name="remarks"></a>Note  
 Per ulteriori informazioni sui `END_CATCH` (macro), vedere l'articolo [eccezioni](../../mfc/exception-handling-in-mfc.md).  
...
##  <a name="a-namethrowa--throw-mfc"></a><a name="throw"></a>GENERAZIONE DI ECCEZIONI (MFC)  
...
### <a name="remarks"></a>Note  
 **GENERARE** interrupt l'esecuzione, passare il controllo associato del programma **CATCH** blocchi nel programma. Se non è stato fornito il **CATCH** bloccare, quindi il controllo viene passato a un modulo di libreria Microsoft Foundation Class che visualizza un errore del messaggio e viene chiuso.  
...
##  <a name="a-namethrowlasta--throwlast"></a><a name="throw_last"></a>THROW_LAST  
 Genera l'eccezione nuovamente al successivo esterno **CATCH** blocco.  
...
### <a name="remarks"></a>Note  
 Questa macro consente di generare un'eccezione creata localmente. Se si tenta di generare un'eccezione che si è appena rilevata, in genere uscirà dall'ambito e da eliminare. Con `THROW_LAST`, l'eccezione viene passata correttamente alla successiva **CATCH** gestore.  
...
### <a name="remarks"></a>Note  
 È responsabilità dell'utente per determinare la causa in base al codice di errore del sistema operativo.  
...
##  <a name="a-nameafxthrowuserexceptiona--afxthrowuserexception"></a><a name="afxthrowuserexception"></a>AfxThrowUserException  
 Genera un'eccezione per interrompere un'operazione dell'utente finale.  
...
### <a name="remarks"></a>Note  
 Questa funzione viene in genere chiamata immediatamente dopo `AfxMessageBox` ha riscontrato un errore all'utente.  
...
### <a name="remarks"></a>Note  
 Le informazioni fornite a questa funzione possono essere visualizzate dall'applicazione Guida (Microsoft Visual Basic o un'altra applicazione client di automazione OLE).  
...
### <a name="remarks"></a>Note  
 La versione che accetta un `HRESULT` come argomento converte il codice risultato nella corrispondente `SCODE`. Per ulteriori informazioni su `HRESULT` e `SCODE`, vedere [struttura di codici di errore COM](http://msdn.microsoft.com/library/windows/desktop/ms690088) nel [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Parametri  
 `nAfxDaoError`  
 Valore intero che rappresenta un codice di errore esteso oggetto DAO, che è uno dei valori elencato in [CDaoException::m_nAfxDaoError](../../mfc/reference/cdaoexception-class.md#m_nafxdaoerror).  
...
### <a name="remarks"></a>Note  
 Il framework chiama anche `AfxThrowDaoException`. Nella chiamata, è possibile passare un parametro o entrambi. Ad esempio, se si desidera generare uno degli errori definiti **CDaoException::nAfxDaoError** ma non importante il *scode* parametro, passare un codice valido nel `nAfxDaoError` parametro e accettare il valore predefinito per *scode*.  
  
 Per informazioni sulle eccezioni relativi alle classi DAO MFC, vedere la classe `CDaoException` in questo libro e l'articolo [eccezioni: eccezioni di Database](../../mfc/exceptions-database-exceptions.md).  
...
### <a name="parameters"></a>Parametri  
 ...
 `pdb`  
 Un puntatore per il `CDatabase` oggetto che rappresenta la connessione all'origine dati a cui è associato all'eccezione.  
...
### <a name="remarks"></a>Note  
 ...
 Per informazioni di **RETCODE** valori definiti da ODBC, vedere il capitolo 8, "Il recupero dello stato e informazioni sull'errore," il [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Per informazioni sulle estensioni MFC per questi codici, vedere la classe [CDBException](../../mfc/reference/cdbexception-class.md).  
  
### <a name="requirements"></a>Requisiti  
  **Intestazione** AFX. h  
  
##  <a name="a-nameafxaborta--afxabort"></a><a name="afxabort"></a>AfxAbort  
 La funzione di terminazione predefinito fornita da MFC.  
...
### <a name="remarks"></a>Note  
 `AfxAbort`viene chiamato internamente dalle funzioni membro MFC quando si verifica un errore irreversibile, ad esempio un'eccezione non rilevata che non può essere gestito. È possibile chiamare `AfxAbort` nel raro caso quando si verifica un errore irreversibile da cui è possibile ripristinare.  
