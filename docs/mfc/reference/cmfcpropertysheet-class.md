...
### <a name="remarks"></a>Note  
 Questo metodo aggiunge la pagina delle proprietà specificata come scheda a destra nella finestra delle proprietà. Pertanto, è possibile utilizzare questo metodo per aggiungere le pagine in ordine da sinistra a destra.  
...
### <a name="parameters"></a>Parametri  
 [in] `pCategory`  
 Puntatore a un nodo dell'albero padre, o `NULL` per associare la pagina specificata con il nodo di livello superiore. Chiamare il [CMFCPropertySheet::AddTreeCategory](#addtreecategory) per ottenere l'indicatore di misura.  
  
 [in] `pPage`  
 Puntatore a un oggetto pagina delle proprietà.  
  
 [in] `nIconNum`  
 Indice in base zero di un'icona o -1 se non viene utilizzata alcuna icona. Quando la pagina non è selezionata, l'icona viene visualizzata accanto a pagina delle proprietà del controllo struttura ad albero. Il valore predefinito è -1.  
  
 [in] `nSelIconNum`  
 Indice in base zero di un'icona o -1 se non viene utilizzata alcuna icona. L'icona viene visualizzata accanto a pagina delle proprietà del controllo struttura ad albero quando è selezionata la pagina. Il valore predefinito è -1.  
...
### <a name="remarks"></a>Note  
 Per utilizzare il valore del `nHeaderHeight` parametro per creare un'intestazione personalizzata, eseguire l'override di [CMFCPropertySheet::OnDrawPageHeader](#ondrawpageheader) metodo.  
...
### <a name="remarks"></a>Note  
 È possibile impostare una finestra delle proprietà in modo che appaia in stili diversi, ad esempio un controllo struttura ad albero, un elenco di pulsanti di spostamento o un set di pagine a schede.  
 ...
 Questo metodo effettua un'asserzione in modalità di debug, se il controllo della finestra delle proprietà non è impostato sullo stile di Microsoft OneNote.  
...
### <a name="remarks"></a>Note  
 Un controllo di finestra delle proprietà può essere visualizzati in diversi formati diversi, ad esempio un set di pagine a schede, un controllo albero o un elenco di pulsanti di navigazione. Utilizzare il [CMFCPropertySheet:: Setlook](#setlook) per specificare l'aspetto del controllo finestra delle proprietà.  
...
### <a name="parameters"></a>Parametri  
 [in] `pPage`  
 Puntatore a un oggetto che rappresenta la proprietà enabled di pagina pagina delle proprietà.  
...
##  <a name="ondrawpageheader"></a>  CMFCPropertySheet::OnDrawPageHeader  
 Chiamato dal framework per disegnare l'intestazione per una pagina delle proprietà personalizzate.  
...
### <a name="remarks"></a>Note  
 Utilizzare questo metodo per rimuovere un nodo, che è anche definito come una categoria, da un controllo struttura ad albero. Utilizzare il [CMFCPropertySheet::AddTreeCategory](#addtreecategory) metodo per aggiungere un nodo a un controllo struttura ad albero.  
...
### <a name="remarks"></a>Note  
 Questo metodo rimuove la pagina delle proprietà specificato ed elimina la finestra associata. La pagina delle proprietà dell'oggetto che il `pPage` parametro specifica fino a quando non viene eliminato il [CMFCPropertySheet](../../mfc/reference/cmfcpropertysheet-class.md) finestra è chiusa.  
...
### <a name="return-value"></a>Valore restituito  
 Il primo metodo consiste nel rapporto di overload, sintassi `TRUE` se questo metodo dà esito positivo; in caso contrario, `FALSE`.  
  
### <a name="remarks"></a>Note  
 Se la finestra delle proprietà nello stile di Microsoft Outlook, il framework visualizza un elenco di pulsanti di navigazione, chiamati il controllo del riquadro di Outlook a sinistra della finestra delle proprietà. Utilizzare questo metodo per impostare l'elenco di immagini da utilizzare per il controllo riquadro di Outlook.  
...
### <a name="remarks"></a>Note  
 Per visualizzare una finestra delle proprietà in uno stile diverso da quello predefinito, è possibile chiamare questo metodo prima di creare una finestra delle proprietà.  
  
 Nella tabella seguente sono elencati i valori di enumerazione che possono essere specificati nel `look` parametro.  
  
|Valore|Descrizione|  
|-----------|-----------------|  
|`CMFCPropertySheet::PropSheetLook_Tabs`|(Impostazione predefinita) Visualizza una scheda per ogni pagina delle proprietà. Le schede vengono visualizzate nella parte superiore della finestra delle proprietà e sono in pila se sono presenti più schede di quelle rientri in una singola riga.|  
|`CMFCPropertySheet::PropSheetLook_OutlookBar`|Visualizza un elenco di pulsanti di navigazione, lo stile della barra di Microsoft Outlook, sul lato sinistro della finestra delle proprietà. Ogni pulsante nell'elenco corrisponde a una pagina delle proprietà. Il framework Visualizza frecce di scorrimento, se sono presenti più pulsanti di quelle presenti l'area visibile dell'elenco.|  
|`CMFCPropertySheet::PropSheetLook_Tree`|Visualizza un controllo albero sul lato sinistro della finestra delle proprietà. Ogni nodo padre o figlio del controllo struttura ad albero corrisponde a una pagina delle proprietà. Se sono presenti altri nodi di quelle presenti l'area visibile del controllo struttura ad albero, il framework Visualizza frecce di scorrimento.|  
|`CMFCPropertySheet::PropSheetLook_OneNoteTabs`|Visualizza una scheda, lo stile di Microsoft OneNote, per ogni pagina delle proprietà. Il framework vengono visualizzate le schede nella parte superiore della finestra delle proprietà e le frecce di scorrimento se sono presenti più schede di spazio sufficiente in una singola riga.|  
|`CMFCPropertySheet::PropSheetLook_List`|Visualizza un elenco a sinistra della finestra delle proprietà. Ogni elemento di elenco corrisponde a una pagina delle proprietà. Il framework Visualizza frecce di scorrimento, se sono presenti più elementi di elenco di quelle presenti l'area visibile dell'elenco.|  
