...
Non è consigliabile creare manualmente `CKeyboardManager` un oggetto. Verrà creato automaticamente dal framework dell'applicazione. Tuttavia, è necessario chiamare [CWinAppEx:: InitKeyboardManager](../../mfc/reference/cwinappex-class.md#initkeyboardmanager) durante il processo di inizializzazione dell'applicazione. Per ottenere un puntatore al gestore della tastiera per l'applicazione, chiamare [CWinAppEx:: GetKeyboardManager](../../mfc/reference/cwinappex-class.md#getkeyboardmanager).
...
|*nKey*|in Chiave da verificare.|
|*fVirt*|in Specifica il comportamento del tasto di scelta rapida. Per un elenco di valori possibili, vedere la pagina relativa alla [struttura di Accel](/windows/win32/api/winuser/ns-winuser-accel).|
|*pWndFrame*|in Una finestra cornice. Questo metodo determina se un tasto di scelta rapida viene gestito in questo frame.|
|*bIsDefaultFrame*|in Parametro booleano che indica se *pWndFrame* è la finestra cornice predefinita.|
...
|-|-|
|Parametro|DESCRIZIONE|
|*nChar*|in Carattere verificato da questo metodo.|
...
*lpszProfileName*<br/>
in Percorso del registro di `CKeyboardManager` sistema in cui vengono salvati i dati.
...
Se il parametro *lpszProfileName* è null, questo metodo controlla il percorso del registro di `CKeyboardManager` sistema predefinito per i dati. Il percorso predefinito del registro di sistema viene specificato dalla [classe CWinAppEx](../../mfc/reference/cwinappex-class.md). I dati devono essere scritti in precedenza con il metodo [CKeyboardManager:: SaveState](#savestate).
...
Questa funzione Cancella i collegamenti archiviati nell' `CKeyboardManager` istanza di. Quindi ricarica lo stato del gestore della tastiera dalla risorsa dell'applicazione.
...
*lpszProfileName*<br/>
in Percorso del registro di sistema per `CKeyboardManager` il salvataggio dello stato.
