...
|[CFrameWnd:: create](#create)|Chiamare per creare e inizializzare la finestra cornice di Windows associata all'oggetto `CFrameWnd`.|
...
|[CFrameWnd:: EndModalState](#endmodalstate)|Termina lo stato modale della finestra cornice. Abilita tutte le finestre disabilitate dal `BeginModalState`.|
...
|[CFrameWnd:: InitialUpdateFrame](#initialupdateframe)|Determina la chiamata della funzione membro `OnInitialUpdate` che appartiene a tutte le visualizzazioni nella finestra cornice.|
...
|[CFrameWnd:: RecalcLayout](#recalclayout)|Riposiziona le barre di controllo dell'oggetto `CFrameWnd`.|
...
|[CFrameWnd:: ShowOwnedWindows](#showownedwindows)|Mostra tutte le finestre che sono discendenti dell'oggetto `CFrameWnd`.|
...
|[CFrameWnd:: rectDefault Windows](#rectdefault)|Passare questo `CRect` statico come parametro durante la creazione di un oggetto `CFrameWnd` per consentire a Windows di scegliere le dimensioni e la posizione iniziali della finestra.|
...
Quando un oggetto `CFrameWnd` contiene viste e documenti, questi vengono creati indirettamente dal Framework anziché direttamente dal programmatore. L'oggetto `CDocTemplate` orchestra la creazione del frame, la creazione delle visualizzazioni contenenti e la connessione delle visualizzazioni al documento appropriato. I parametri del costruttore di `CDocTemplate` specificano il `CRuntimeClass` delle tre classi necessarie (documento, frame e visualizzazione). Un oggetto `CRuntimeClass` viene usato dal Framework per creare in modo dinamico nuovi frame quando specificato dall'utente, ad esempio usando il comando file nuovo o la finestra interfaccia a documenti multipli (MDI).

Una classe della finestra cornice derivata da `CFrameWnd` deve essere dichiarata con DECLARE_DYNCREATE per consentire il corretto funzionamento del meccanismo RUNTIME_CLASS precedente.

Un `CFrameWnd` contiene implementazioni predefinite per eseguire le funzioni seguenti di una finestra principale in un'applicazione tipica per Windows:

- Una finestra cornice `CFrameWnd` tiene traccia di una visualizzazione attualmente attiva indipendente dalla finestra attiva di Windows o dallo stato attivo per l'input corrente. Quando il frame viene riattivato, la visualizzazione attiva riceve una notifica chiamando `CView::OnActivateView`.

- I messaggi di comando e molti messaggi comuni di notifica frame, inclusi quelli gestiti dalle funzioni `OnSetFocus`, `OnHScroll`e `OnVScroll` di `CWnd`, sono delegati da una finestra cornice `CFrameWnd` alla visualizzazione attualmente attiva.
...
- Una finestra cornice `CFrameWnd` dispone di una tabella di tasti di scelta rapida che converte automaticamente gli acceleratori della tastiera.

- Una finestra cornice `CFrameWnd` dispone di un ID della guida facoltativo impostato con `LoadFrame` utilizzato per la Guida sensibile al contesto. Una finestra cornice è l'agente di orchestrazione principale degli Stati semimodali, ad esempio la Guida sensibile al contesto (MAIUSC + F1) e le modalità di anteprima di stampa.

- Una finestra cornice `CFrameWnd` apre un file trascinato da Gestione file e viene rilasciato nella finestra cornice. Se un'estensione di file è registrata e associata all'applicazione, la finestra cornice risponde alla richiesta di apertura di Dynamic Data Exchange (DDE) che si verifica quando l'utente apre un file di dati in gestione file o quando viene chiamata la funzione `ShellExecute` Windows.
...
Costruisce un oggetto `CFrameWnd`, ma non crea la finestra cornice visibile.
...
*lpszClassName*<br/>
Punta a una stringa di caratteri con terminazione null che denomina la classe Windows. Il nome della classe può essere qualsiasi nome registrato con la funzione globale `AfxRegisterWndClass` o con la funzione `RegisterClass` Windows. Se è NULL, utilizza gli attributi predefiniti `CFrameWnd` predefiniti.
...
Costruire un oggetto `CFrameWnd` in due passaggi. Prima di tutto, richiamare il costruttore, che costruisce l'oggetto `CFrameWnd`, quindi chiamare `Create`, che crea la finestra cornice di Windows e la collega all'oggetto `CFrameWnd`. `Create` Inizializza il nome della classe della finestra e il nome della finestra e registra i valori predefiniti per il relativo stile, padre e il menu associato.

Usare `LoadFrame` anziché `Create` per caricare la finestra cornice da una risorsa anziché specificarne gli argomenti.
...
Usare questa funzione membro per creare "viste" che non sono derivate da `CView`all'interno di un frame. Dopo la chiamata di `CreateView`, è necessario impostare manualmente la visualizzazione su attivo e impostarla in modo che sia visibile; Queste attività non vengono eseguite automaticamente da `CreateView`.
...
Chiamare questa funzione membro per ottenere un puntatore al `CDocument` corrente collegato alla visualizzazione attiva corrente.
...
Il parametro *NID* si riferisce all'identificatore univoco passato al metodo `Create` della barra di controllo. Per ulteriori informazioni sulle barre di controllo, vedere l'argomento intitolato [barre di controllo](../../mfc/control-bars.md).
...
Chiamare questa funzione membro per archiviare le informazioni sullo stato sulle barre di controllo della finestra cornice in un oggetto `CDockState`.
...
È quindi possibile scrivere il contenuto di `CDockState` nell'archiviazione utilizzando `CDockState::SaveState` o `Serialize`. Se in un secondo momento si vuole ripristinare lo stato precedente delle barre di controllo, caricare lo stato con `CDockState::LoadState` o `Serialize`, quindi chiamare `SetDockState` per applicare lo stato precedente alle barre di controllo della finestra cornice.
...
In questo modo, tutte le visualizzazioni nella finestra cornice riceveranno le chiamate `OnInitialUpdate`.
...
Costruire un oggetto `CFrameWnd` in due passaggi. Prima di tutto, richiamare il costruttore, che costruisce l'oggetto `CFrameWnd`, quindi chiamare `LoadFrame`, che carica la finestra cornice di Windows e le risorse associate e connette la finestra cornice all'oggetto `CFrameWnd`. Il parametro *nIDResource* specifica il menu, la tabella dei tasti di scelta rapida, l'icona e la risorsa di stringa del titolo per la finestra cornice.

Utilizzare la funzione membro `Create` anziché `LoadFrame` quando si desidera specificare tutti i parametri di creazione della finestra cornice.
...
Questo membro dati semplifica l'implementazione dei comandi facoltativi in base alla selezione corrente e riduce la necessità di scrivere ON_UPDATE_COMMAND_UI gestori per l'abilitazione e la disabilitazione delle voci di menu.
...
*nBorderCmd*<br/>
Contiene uno dei valori seguenti dal `enum BorderCmd`:
...
Questa funzione membro è l'implementazione `CFrameWnd` della negoziazione dello spazio del bordo OLE.
...
per la mappa messaggi della classe `CFrameWnd` e aggiungere anche una voce della tabella di tasti di scelta rapida, in genere SHIFT + F1, per abilitare questa funzione membro.

Se l'applicazione è un contenitore OLE, `OnContextHelp` inserisce tutti gli elementi sul posto contenuti all'interno dell'oggetto finestra cornice in modalità guida. Il cursore assume la forma di una freccia e un punto interrogativo e l'utente può quindi spostare il puntatore del mouse e premere il pulsante sinistro del mouse per selezionare una finestra di dialogo, una finestra, un menu o un pulsante di comando. Questa funzione membro chiama la funzione di Windows `WinHelp` con il contesto della Guida dell'oggetto sotto il cursore.

##  <a name="oncreateclient"></a>CFrameWnd:: OnCreateClient

Chiamata eseguita dal Framework durante l'esecuzione del `OnCreate`.
...
L'implementazione predefinita di questa funzione crea un oggetto `CView` dalle informazioni fornite in *pContext*, se possibile.

Eseguire l'override di questa funzione per eseguire l'override dei valori passati nell'oggetto `CCreateContext` o per modificare la modalità di creazione dei controlli nell'area client principale della finestra cornice. I membri di `CCreateContext` di cui è possibile eseguire l'override sono descritti nella classe [CCreateContext](../../mfc/reference/ccreatecontext-structure.md) .

> [!NOTE]
>  Non sostituire i valori passati nella struttura del `CREATESTRUCT`. Sono destinate esclusivamente all'utilizzo informativo. Se si desidera eseguire l'override del rettangolo della finestra iniziale, ad esempio, eseguire l'override della funzione membro `CWnd` [PreCreateWindow](../../mfc/reference/cwnd-class.md#precreatewindow).
...
*pState*<br/>
Puntatore a una struttura `CPrintPreviewState`.
...
*pCmdUI*<br/>
Puntatore a un oggetto [CCmdUI](../../mfc/reference/ccmdui-class.md) che rappresenta il menu che ha generato il comando di aggiornamento. Il gestore di aggiornamento chiama la funzione membro [Enable](../../mfc/reference/ccmdui-class.md#enable) dell'oggetto `CCmdUI` tramite *pCmdUI* per aggiornare l'interfaccia utente.
...
L'implementazione predefinita di questa funzione membro chiama la funzione membro `CWnd` `RepositionBars` per riposizionare tutte le barre di controllo nel frame e nella finestra del client principale (in genere un `CView` o MDICLIENT).
...
##  <a name="rectdefault"></a>CFrameWnd:: rectDefault Windows

Passare questo `CRect` statico come parametro durante la creazione di una finestra per consentire a Windows di scegliere le dimensioni e la posizione iniziali della finestra.
...
*bNotify*<br/>
Specifica se la visualizzazione deve ricevere notifiche di attivazione. Se TRUE, viene chiamato `OnActivateView` per la nuova visualizzazione. Se FALSE, non lo è.
...
Chiamare questa funzione membro per applicare le informazioni sullo stato archiviate in un oggetto `CDockState` alle barre di controllo della finestra cornice.
...
*nProgressPos*<br/>
Specifica la posizione da impostare. Deve essere compreso nell'intervallo impostato per `SetProgressBarRange`.
...
Chiamare questa funzione membro per mostrare tutte le finestre che sono discendenti dell'oggetto `CFrameWnd`.
