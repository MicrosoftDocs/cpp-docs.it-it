...
### <a name="parameters"></a>Parametri  
 ...
 `nFormat`  
 Specifica il metodo di formattazione del testo. Può essere qualsiasi combinazione dei valori descritti per il `uFormat` parametro [DrawText](http://msdn.microsoft.com/library/windows/desktop/dd162498) in Windows SDK. (Combinare mediante bit per bit `OR` (operatore)):  
...
### <a name="remarks"></a>Note  
 Formatta testo all'espansione tabulazioni in spazi appropriati, l'allineamento del testo a sinistra, destra, oppure il centro del rettangolo specificato e il testo la suddivisione in più righe che rientrano all'interno del rettangolo specificato. Viene specificato il tipo di formattazione da `nFormat` e `lpDTParams`. Per ulteriori informazioni, vedere [CDC](#drawtext) e [DrawTextEx](http://msdn.microsoft.com/library/windows/desktop/dd162499) in Windows SDK.  
...
### <a name="return-value"></a>Valore restituito  
 Specifica l'ultimo valore restituito dal [funzione di callback](callback-functions-used-by-mfc.md#enum_objects). Il significato è definito dall'utente.  
...
### <a name="parameters"></a>Parametri  
 ...
 Per un elenco completo delle funzioni di escape, vedere [Escape](http://msdn.microsoft.com/library/windows/desktop/dd162701) in Windows SDK.  
...
### <a name="remarks"></a>Note  
 Dell'originale escape della stampante, solo **QUERYESCSUPPORT** è supportato per le applicazioni Win32. Tutte le altre sequenze di escape della stampante sono obsoleti e sono supportate solo per compatibilità con le applicazioni a 16 bit.  
...
- [CDC::StartPage](#startpage)  
 ...
 Inoltre, [CDC:: GetDeviceCaps](#getdevicecaps) supporta gli indici di Win32 che sostituiscono altri caratteri di escape della stampante. Vedere [GetDeviceCaps](http://msdn.microsoft.com/library/windows/desktop/dd144877) in Windows SDK per ulteriori informazioni.  
...
### <a name="remarks"></a>Note  
 ...
 Se `nFillType` è impostato su **FLOODFILLSURFACE**, la funzione inizia nel punto specificato da *x* e *y* e continua in tutte le direzioni, la compilazione di tutte le aree adiacenti contenente il colore specificato da `crColor`.  
...
### <a name="remarks"></a>Note  
 ...
 Se il rettangolo **inferiore** coordinata è minore o uguale a **top**, o se **destra** è minore o uguale a **sinistro**, il rettangolo non è stato creato.  
  
 Bordo creato dal `FrameRect` è nella stessa posizione di un bordo disegnato mediante il **rettangolo** funzione membro usando le stesse coordinate (se **rettangolo** utilizza una penna di 1 unità logica wide). L'interno del rettangolo non viene compilato da `FrameRect`.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro emula la funzionalità della funzione [GetCharacterPlacement](http://msdn.microsoft.com/library/windows/desktop/dd144860\(v=vs.85\).aspx), come descritto in Windows SDK.  
...
### <a name="remarks"></a>Note  
 ...
 Per convertire le larghezze ABC in unità di progettazione caratteri, un'applicazione deve creare un tipo di carattere con altezza (come specificato nella **lfHeight** appartenente il [LOGFONT](http://msdn.microsoft.com/library/windows/desktop/dd145037) struttura) è uguale al valore archiviato nel **ntmSizeEM** appartenente il [NEWTEXTMETRIC](http://msdn.microsoft.com/library/windows/desktop/dd162741) struttura. (Il valore di **ntmSizeEM** membro può essere recuperato chiamando il [EnumFontFamilies](http://msdn.microsoft.com/library/windows/desktop/dd162619) funzione Windows.)  
...
### <a name="remarks"></a>Note  
 Questa funzione membro emula la funzionalità della funzione [GetCharABCWidthsI](http://msdn.microsoft.com/library/windows/desktop/dd144859), come descritto in Windows SDK.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro emula la funzionalità della funzione [GetCharWidthI](http://msdn.microsoft.com/library/windows/desktop/dd144864), come descritto in Windows SDK.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro emula la funzionalità della funzione [GetDCBrushColor](http://msdn.microsoft.com/library/windows/desktop/dd144872), come descritto in Windows SDK.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro utilizza la funzione Win32 [GetDCPenColor](http://msdn.microsoft.com/library/windows/desktop/dd144875), come descritto in Windows SDK.  
...
### <a name="parameters"></a>Parametri  
 `nIndex`  
 Specifica il tipo di informazioni da restituire. Vedere [GetDeviceCaps](http://msdn.microsoft.com/library/windows/desktop/dd144877) in Windows SDK per un elenco di valori.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro emula la funzionalità della funzione [GetFontLanguageInfo](http://msdn.microsoft.com/library/windows/desktop/dd144886), come descritto in Windows SDK.  
...
- **PT_LINETO** specifica che il punto precedente e il corrispondente punto in `lpPoints` sono gli endpoint di una riga.  
 ...
 **PT_BEZIERTO** sempre si verificano nei set di tre tipi. Il punto del percorso facendola precedere immediatamente definisce il punto iniziale della curva Bzier. I primi due **PT_BEZIERTO** punti sono i punti di controllo e il terzo **PT_BEZIERTO** punto rappresenta il punto finale (se hardcoded).  
     ...
     Oggetto **PT_LINETO** o **PT_BEZIERTO** tipo può essere combinato con il flag seguente (usando l'operatore OR bit per bit `OR`) per indicare che il punto corrispondente è l'ultimo punto in una figura e che il Figura deve essere chiusa:  
...
### <a name="remarks"></a>Note  
 Questa funzione membro emula la funzionalità della funzione [GetTextExtentExPointI](http://msdn.microsoft.com/library/windows/desktop/dd144936), come descritto in Windows SDK.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro emula la funzionalità della funzione [GetTextExtentPointI](http://msdn.microsoft.com/library/windows/desktop/dd144939), come descritto in Windows SDK.  
...
### <a name="return-value"></a>Valore restituito  
 Il x - e y-extent (in unità dispositivo) come un `CSize` oggetto.  
 ...
 *nMeshElements*  
 ...
 `dwMode`  
 Specifica la modalità di riempimento sfumato. Per un elenco di valori possibili, vedere [GradientFill](http://msdn.microsoft.com/library/windows/desktop/dd144957) in Windows SDK.  
...
### <a name="remarks"></a>Note  
 ...
 Un'applicazione può disegnare in grigio stringhe (grigio) nei dispositivi che supportano un colore grigio a tinta unita senza chiamare il `GrayString` funzione membro. Il colore di sistema **COLOR_GRAYTEXT** è il colore grigio a tinta unita sistema utilizzato per disegnare testo disabilitato. L'applicazione può chiamare il **GetSysColor** funzione di Windows per recuperare il valore del colore **COLOR_GRAYTEXT**. Se il colore è diverso da 0 (nero), l'applicazione può chiamare il `SetTextColor` funzione membro per impostare il colore del testo per il valore di colore e quindi disegnare direttamente la stringa. Se il colore recuperato è nero, l'applicazione deve chiamare `GrayString` a dim (grigio) il testo.  
...
### <a name="remarks"></a>Note  
 ...
 La conversione viene eseguita prima si convertono i **HIMETRIC** unità in pixel e quindi convertire queste unità in unità logiche tramite le unità di mapping corrente del contesto di dispositivo. Si noti che gli extent della finestra del dispositivo e viewport influirà il risultato.  
...
### <a name="return-value"></a>Valore restituito  
 Diverso da zero se la `CDC` oggetto è un controller di dominio; in caso contrario 0.  
 ...
 *y*  
 ...
 `pSrcDC`  
 Identifica il contesto di dispositivo da cui la bitmap viene copiato. Deve essere zero se la *dwRop* parametro specifica un'operazione raster che non include un'origine.  
 ...
 **PT_BEZIERTO** sempre si verificano nei set di tre tipi. La posizione corrente definisce il punto di partenza per la spline Bzier. I primi due **PT_BEZIERTO** punti sono i punti di controllo e il terzo **PT_BEZIERTO** punto è il punto finale. Il punto finale diventa la nuova posizione corrente. Se non vi sono tre consecutivi **PT_BEZIERTO** punti, un errore.  
...
### <a name="remarks"></a>Note  
 Questa funzione può essere utilizzata per disegnare figure non contigui al posto delle chiamate consecutive di `CDC::MoveTo`, `CDC::LineTo`, e `CDC::PolyBezierTo` funzioni membro. Le righe e una spline di tipo vengono disegnati utilizzando la penna corrente e cifre non sono state compilate. Se è un percorso attivo avviato chiamando il `CDC::BeginPath` funzione membro, `PolyDraw` aggiunge al percorso. I punti contenuti nel `lpPoints` matrice e in `lpTypes` indicano se ogni punto fa parte di un `CDC::MoveTo`, `CDC::LineTo`, o un **CDC::BezierTo** operazione. È anche possibile chiudere cifre. Questa funzione Aggiorna la posizione corrente.  
...
### <a name="remarks"></a>Note  
 ...
 Per ulteriori informazioni, vedere [polilinea](http://msdn.microsoft.com/library/windows/desktop/dd162815) in Windows SDK.  
...
### <a name="parameters"></a>Parametri  
 `lpfn`  
 Puntatore alla funzione di interruzione da installare come procedura di interruzione. Per ulteriori informazioni sulla funzione di callback, vedere [funzione di Callback per CDC:: SetAbortProc](callback-functions-used-by-mfc.md#setabortproc).  
...
### <a name="remarks"></a>Note  
 Questo metodo emula la funzionalità della funzione [SetDCBrushColor](http://msdn.microsoft.com/library/windows/desktop/dd162969), come descritto in Windows SDK.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro utilizza la funzione Win32 [SetDCPenColor](http://msdn.microsoft.com/library/windows/desktop/dd162970), come descritto in Windows SDK.  
...
### <a name="remarks"></a>Note  
 In genere, la chiamata non **SetLayout** per una finestra. Piuttosto, controllare il layout da destra a sinistra in una finestra impostando il [stili finestra estesi](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles) , ad esempio **WS_EX_RTLREADING**. Un contesto di dispositivo, ad esempio una stampante o un metafile, non eredita il layout. L'unico modo per impostare il contesto di dispositivo per un layout da destra a sinistra è chiamando **SetLayout**.  
 ...
 *y*  
 ...
 `crColor`  
 Oggetto **COLORREF** valore RGB che specifica il colore usato per disegnare il punto. Vedere [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449) in Windows SDK per una descrizione di questo valore.  
...
### <a name="return-value"></a>Valore restituito  
 ...
 Può essere qualsiasi valore fornito in Windows SDK.  
...
### <a name="remarks"></a>Note  
 ...
 Il **BLACKONWHITE** ( **STRETCH_ANDSCANS**) e **WHITEONBLACK** ( **STRETCH_ORSCANS**) in genere utilizzate per mantenere le modalità pixel di primo piano nella bitmap monocromatica. Il **COLORONCOLOR** ( **STRETCH_DELETESCANS**) modalità viene in genere utilizzata per mantenere colore nella bitmap a colori.  
...
### <a name="remarks"></a>Note  
 La finestra, insieme a riquadro di visualizzazione contesto di dispositivo, definisce come GDI esegue il mapping di punti nel sistema di coordinate logico di punti nel sistema di coordinate del dispositivo.  
...
### <a name="remarks"></a>Note  
 ...
 Per ulteriori informazioni, vedere [TransparentBlt](http://msdn.microsoft.com/library/windows/desktop/dd145141) in Windows SDK.  
...
### <a name="remarks"></a>Note  
 ...
 Per ulteriori informazioni sull'utilizzo di tavolozze dei colori, vedere [UpdateColors](http://msdn.microsoft.com/library/windows/desktop/dd145166) in Windows SDK.  
