...
|[CDHtmlDialog::DDX_DHtml_SelectString](#ddx_dhtml_selectstring)|Ottiene o imposta il testo visualizzato di una voce di elenco casella (in base all'indice corrente) in una pagina HTML.|  
...
|[CDHtmlDialog::FilterDataObject](#filterdataobject)|Consente la finestra di dialogo filtrare gli oggetti di dati negli Appunti creati dal browser ospitato.|  
...
|[CDHtmlDialog::IsExternalDispatchSafe](#isexternaldispatchsafe)|Indica se l'host `IDispatch` interfaccia è sicura per lo scripting.|  
...
|[CDHtmlDialog::OnDocumentComplete](#ondocumentcomplete)|Chiamato dal framework per notificare a un'applicazione quando un documento ha raggiunto la `READYSTATE_COMPLETE` stato.|  
...
|Nome|Descrizione|  
|----------|-----------------|  
|[CDHtmlDialog::m_bUseHtmlTitle](#m_busehtmltitle)|Indica se utilizzare il titolo del documento HTML come la didascalia della finestra di dialogo.|  
...
### <a name="parameters"></a>Parametri  
 `pContainer`  
 Un puntatore al [COleControlContainer](../../mfc/reference/colecontrolcontainer-class.md) oggetto  
...
### <a name="parameters"></a>Parametri  
 ...
 `dispid`  
 ID dispatch della proprietà a cui si desidera scambiare dati.  
  
 `szPropName`  
 Nome della proprietà.  
  
 `var`  
 Tipo del membro dati `VARIANT`, [COleVariant](../../mfc/reference/colevariant-class.md), o [CComVariant](../../atl/reference/ccomvariant-class.md), che contiene il valore scambiato con la proprietà del controllo ActiveX.  
 ...
 *pbCollection*  
 Oggetto **BOOL** che indica se l'oggetto rappresentato dal *ppdisp* è un singolo elemento o una raccolta di elementi.  
...
### <a name="parameters"></a>Parametri  
 ...
 `lpvPostData`  
 Puntatore ai dati da inviare con la transazione HTTP POST. Ad esempio, la transazione POST viene utilizzata per inviare i dati raccolti da un modulo HTML. Se questo parametro non corrisponde a tutti i dati inviati, **Naviga** genera una transazione HTTP GET. Questo parametro viene ignorato se l'URL non è un URL HTTP.  
...
### <a name="remarks"></a>Note  
 Questo messaggio viene inviato alla finestra di dialogo durante il **crea**, `CreateIndirect`, o `DoModal` chiamate, che si verificano subito prima che viene visualizzata la finestra di dialogo.  
  
 Eseguire l'override di questa funzione membro se è necessario eseguire un'elaborazione speciale quando viene inizializzata nella finestra di dialogo. La versione sottoposta a override, chiamare innanzitutto la classe di base `OnInitDialog` ma ignorare il valore restituito. In genere restituirà **TRUE** dalla funzione membro sottoposto a override.  
...
### <a name="parameters"></a>Parametri  
 ...
 `bstrText`  
 Il nuovo valore della **innerHTML** proprietà.  
...
### <a name="parameters"></a>Parametri  
 ...
 `bstrText`  
 Il nuovo valore della **innerText** proprietà.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro è l'implementazione del CDHtmlDialog di [IDocHostUIHandler::UpdateUI](https://msdn.microsoft.com/library/aa753268.aspx), come descritto in Windows SDK.  
