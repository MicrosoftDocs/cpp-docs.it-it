...
|[CSplitterWnd:: Create](#create)|Chiamata per creare una finestra con separatore dinamica e associarlo al `CSplitterWnd` oggetto.|  
...
|[CSplitterWnd::GetColumnCount](#getcolumncount)|Restituisce il numero di colonne corrente del riquadro.|  
|[CSplitterWnd::GetColumnInfo](#getcolumninfo)|Restituisce le informazioni nella colonna specificata.|  
|[CSplitterWnd::GetPane](#getpane)|Restituisce il riquadro in cui la riga specificata e la colonna.|  
|[CSplitterWnd::GetRowCount](#getrowcount)|Restituisce il numero di riga corrente del riquadro.|  
|[CSplitterWnd::GetRowInfo](#getrowinfo)|Restituisce informazioni su una riga specificata.|  
|[CSplitterWnd::GetScrollStyle](#getscrollstyle)|Restituisce lo stile della barra di scorrimento condiviso.|  
|[CSplitterWnd::IdFromRowCol](#idfromrowcol)|Restituisce l'elemento figlio ID della finestra del riquadro in cui la riga specificata e la colonna.|  
...
|[CSplitterWnd::IsTracking](#istracking)|Determina se la barra di divisione viene spostata.|  
|[CSplitterWnd::RecalcLayout](#recalclayout)|Chiamata eseguita per visualizzare nuovamente la finestra con separatore dopo la regolazione delle dimensioni di riga o colonna.|  
|[CSplitterWnd::SetActivePane](#setactivepane)|Imposta un riquadro da quello attivo nel frame.|  
|[CSplitterWnd::SetColumnInfo](#setcolumninfo)|Chiamata eseguita per impostare le informazioni della colonna specificata.|  
|[CSplitterWnd::SetRowInfo](#setrowinfo)|Chiamata eseguita per impostare le informazioni sulla riga specificata.|  
|[CSplitterWnd::SetScrollStyle](#setscrollstyle)|Specifica che il nuovo stile della barra di scorrimento per la finestra con separatore condivisa supporto della barra di scorrimento.|  
...
|[CSplitterWnd::OnInvertTracker](#oninverttracker)|Esegue il rendering di immagini di una finestra divisa per la stessa dimensione e forma della finestra con frame.|  
  
## <a name="remarks"></a>Note  
 Un riquadro è in genere un oggetto specifico dell'applicazione derivato da [CView](../../mfc/reference/cview-class.md), ma può essere qualsiasi [CWnd](../../mfc/reference/cwnd-class.md) oggetto con l'ID di finestra figlio appropriato.  
  
 Oggetto `CSplitterWnd` oggetto è in genere incorporato in un elemento padre [CFrameWnd](../../mfc/reference/cframewnd-class.md) o [CMDIChildWnd](../../mfc/reference/cmdichildwnd-class.md) oggetto. Creare un `CSplitterWnd` oggetto usando la procedura seguente:  
...
3.  All'interno di sottoposto a override `OnCreateClient`, chiamare il [crea](#create) o [CreateStatic](#createstatic) funzione membro di `CSplitterWnd`.  
  
 Chiamare il **crea** funzione membro per creare una finestra con separatore dinamico. Una finestra con separatore dinamico, in genere viene utilizzata per creare e scorrere fino a un numero di singoli riquadri o viste, dello stesso documento. Il framework crea automaticamente un riquadro iniziale per la barra di divisione; quindi il framework crea, ridimensiona ed elimina i riquadri aggiuntivi come l'utente opera controlli della finestra con separatore.  
 ...
 Utilizzare inoltre il `SetColumnInfo` e `SetRowInfo` funzioni membro per impostare una larghezza "ideale" per una colonna e l'altezza "ideale" per una riga. Quando il framework visualizza una finestra con separatore, viene innanzitutto visualizzato il frame padre, quindi finestra con separatore. Il framework viene disposto quindi i riquadri in colonne e righe in base alle dimensioni ideali, utilizzando l'angolo superiore sinistro all'angolo inferiore destro dell'area client della finestra con separatore.  
  
 Tutti i riquadri in una finestra con separatore dinamico devono essere della stessa classe. Applicazioni comuni che supportano le finestre con separatore dinamico includono Microsoft Word e Microsoft Excel.  
 ...
 In particolare, è necessario creare i riquadri del tutti il separatore statico quando si crea il separatore statico. Assicurarsi di creare tutti i riquadri prima del frame padre `OnCreateClient` restituiti da funzioni membro o verrà eseguito il framework non visualizzare correttamente la finestra.  
  
 Il `CreateStatic` funzione membro inizializza automaticamente con un separatore statico con un minimo di righe altezza e larghezza di colonna pari a 0. Dopo aver chiamato **crea**, modificare questi valori minimi chiamando il [SetColumnInfo](#setcolumninfo) e [SetRowInfo](#setrowinfo) funzioni membro. Utilizzare inoltre `SetColumnInfo` e `SetRowInfo` dopo aver chiamato `CreateStatic` per indicare desiderata dimensioni riquadro ideale.  
 ...
 Una finestra con separatore supporta le barre di scorrimento speciale (a parte le barre di scorrimento che possono disporre di riquadri). Queste barre di scorrimento sono figli di `CSplitterWnd` dell'oggetto e vengono condivise con i riquadri.  
  
 Creare le barre di scorrimento speciali quando si crea una finestra con separatore. Ad esempio, un `CSplitterWnd` che ha una riga, due colonne e **WS_VSCROLL** stile verrà visualizzata una barra di scorrimento verticale che viene condiviso da due riquadri. Quando l'utente sposta la barra di scorrimento, `WM_VSCROLL` i messaggi vengono inviati a entrambi i riquadri. Quando i riquadri di impostare la posizione della barra di scorrimento, la barra di scorrimento condiviso.  
...
### <a name="parameters"></a>Parametri  
 `bPrev`  
 Indica quale finestra da attivare. **TRUE** per precedente; **FALSE** per successivo.  
  
### <a name="remarks"></a>Note  
 Questa funzione membro è un comando di alto livello che viene utilizzato il [CView](../../mfc/reference/cview-class.md) classe di delegare il `CSplitterWnd` implementazione.  
 ...
 *nMaxRows*  
 Numero massimo di righe in una finestra con separatore. Questo valore non deve superare i 2.  
  
 *nMaxCols*  
 Numero massimo di colonne nella finestra con separatore. Questo valore non deve superare i 2.  
  
 `sizeMin`  
 Specifica la dimensione minima in cui verrà visualizzato un riquadro.  
...
3.  Chiamare il **crea** funzione membro dall'interno sottoposto a override `OnCreateClient`.  
 ...
 Quando si crea una finestra con separatore all'interno di un frame principale, passare il frame padre `pContext` parametro alla finestra con separatore. In caso contrario, questo parametro può essere **NULL**.  
  
 La larghezza di altezza e la colonna iniziale minime delle righe di una finestra con separatore dinamico impostati dal `sizeMin` parametro. Questi valori minimi, che determinano se un riquadro è troppo piccolo per essere visualizzati nella sua interezza, possono essere modificati con il [SetRowInfo](#setrowinfo) e [SetColumnInfo](#setcolumninfo) funzioni membro.  
...
### <a name="remarks"></a>Note  
 Eseguire l'override `CreateScrollBarCtrl` per includere controlli aggiuntivi accanto a una barra di scorrimento. Il comportamento predefinito consiste nel creare i controlli barra di scorrimento Windows normale.  
...
### <a name="parameters"></a>Parametri  
 ...
 `col`  
 Specifica la colonna della finestra con separatore in cui inserire la nuova vista.  
 ...
 *sizeInit*  
 ...
 `pContext`  
 Un puntatore a un contesto di creazione considerato di creare la vista (in genere il `pContext` passato del frame padre sottoposto a override [CFrameWnd::OnCreateClient](../../mfc/reference/cframewnd-class.md#oncreateclient) funzione membro in cui viene creata la finestra con separatore).  
...
### <a name="remarks"></a>Note  
 Costruire un `CSplitterWnd` oggetto in due passaggi. In primo luogo, chiamare il costruttore, che crea il `CSplitterWnd` e quindi chiamare il [crea](#create) funzione membro, che crea la finestra con separatore e lo collega al `CSplitterWnd` oggetto.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro viene chiamata dal framework per implementare la logica della finestra con separatore dinamico (vale a dire, se la finestra con separatore di **SPLS_DYNAMIC_SPLIT** stile). Può essere personalizzato, insieme alla funzione virtuale [CreateView](#createview), per implementare più separatori dinamici avanzati.  
...
### <a name="parameters"></a>Parametri  
 `row`  
 Specifica la riga della finestra con separatore in cui si desidera eliminare la visualizzazione.  
  
 `col`  
 Specifica la colonna della finestra con separatore in cui si desidera eliminare la vista.  
...
- **SB_TOP** scorre verso l'alto.  
 ...
 `bDoScroll`  
 Determina se l'azione di scorrimento specificata. Se `bDoScroll` è **TRUE** (ovvero, se esiste una finestra figlio, e se la suddivisione di windows è un intervallo di scorrimento), quindi l'azione di scorrimento specificata può avvenire; se `bDoScroll` è **FALSE** (ovvero, se non esiste alcun intervallo figlio o le visualizzazioni suddivise non dispongono di alcun intervallo di scorrimento), quindi non si verifica lo scorrimento.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro viene chiamata dal framework in risposta a un messaggio di scorrimento, per eseguire la sincronizzazione lo scorrimento delle finestre della suddivisione della quantità, in pixel, indicato da `sizeScroll`. I valori positivi indicano lo scorrimento verso il basso e a destra. valori negativi indicano lo scorrimento verticale e a sinistra.  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce il numero corrente delle colonne nella barra di divisione. Per un separatore statico, sarà anche il numero massimo di colonne.  
...
##  <a name="getpane"></a>  CSplitterWnd::GetPane  
 Restituisce il riquadro in cui la riga specificata e la colonna.  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce il riquadro in cui la riga specificata e la colonna. Il riquadro restituito è in genere un [CView](../../mfc/reference/cview-class.md)-classe derivata.  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce il numero corrente di righe in una finestra con separatore. Per una finestra con separatore statico, sarà anche il numero massimo di righe.  
  
##  <a name="getrowinfo"></a>  CSplitterWnd::GetRowInfo  
 Restituisce informazioni su una riga specificata.  
...
### <a name="parameters"></a>Parametri  
 ...
 `cyCur`  
 Riferimento a `int` impostare l'altezza corrente della riga in pixel.  
...
### <a name="return-value"></a>Valore restituito  
 Uno o più delle seguenti finestre, i flag di stile se ha esito positivo:  
  
- **WS_HSCROLL** se la barra di divisione attualmente gestisce le barre di scorrimento orizzontale condivisa.  
  
- **WS_VSCROLL** se la barra di divisione attualmente gestisce le barre di scorrimento verticale condivisa.  
  
 Se è zero, la finestra con separatore attualmente non gestisce le barre di scorrimento condiviso.  
  
##  <a name="idfromrowcol"></a>  CSplitterWnd::IdFromRowCol  
 Ottiene l'elemento figlio ID di finestra per il riquadro in cui la riga specificata e la colonna.  
...
### <a name="return-value"></a>Valore restituito  
 Se diverso da zero, `pWnd` è attualmente un riquadro figlio di questa finestra con separatore, e `pRow` e `pCol` vengono riempiti con la posizione del riquadro nella finestra con separatore. Se `pWnd` non è un riquadro figlio di questa finestra con separatore, viene restituito 0.  
...
- **splitBox** trascinare la barra di divisione.  
  
- `splitBar` La barra che tra le finestre due divise verrà visualizzata.  
  
- **splitIntersection** l'intersezione delle finestre divise. Questo elemento non verrà chiamato quando è in esecuzione in Windows 95/98.  
  
- **splitBorder** i bordi della finestra divisa.  
  
 `rect`  
 Un riferimento a un [CRect](../../atl-mfc-shared/reference/crect-class.md) oggetto che specifica le dimensioni delle finestre della divisione.  
  
### <a name="remarks"></a>Note  
 Questa funzione membro viene chiamata dal framework per disegnare e specificare le caratteristiche esatto di una finestra con separatore. Eseguire l'override `OnDrawSplitter` per la personalizzazione avanzata delle immagini per i vari componenti con interfaccia grafici di una finestra con separatore. L'immagine predefinita è simile alla barra di divisione in Microsoft Works per Windows o Microsoft Windows 95/98, in quanto le intersezioni tra le barre di divisione vengono combinate insieme.  
  
 Per ulteriori informazioni sulle finestre con separatore dinamico, vedere "Finestre con separatore" nell'articolo [tipi di documenti multipli, visualizzazioni e finestre cornice](../../mfc/multiple-document-types-views-and-frame-windows.md), [29 Nota tecnica](../../mfc/tn029-splitter-windows.md)e `CSplitterWnd` Cenni preliminari sulla classe.  
  
##  <a name="oninverttracker"></a>  CSplitterWnd::OnInvertTracker  
 Esegue il rendering di immagini di una finestra divisa per la stessa dimensione e forma della finestra con frame.  
...
##  <a name="recalclayout"></a>  CSplitterWnd::RecalcLayout  
 Chiamata eseguita per visualizzare nuovamente la finestra con separatore dopo la regolazione delle dimensioni di riga o colonna.  
...
### <a name="example"></a>Esempio  
  Per vedere l'esempio [CSplitterWnd::SetColumnInfo](#setcolumninfo).  
  
##  <a name="setactivepane"></a>  CSplitterWnd::SetActivePane  
 Imposta un riquadro da quello attivo nel frame.  
...
### <a name="parameters"></a>Parametri  
 ...
 `pWnd`  
 Un puntatore a un `CWnd` oggetto. Se **NULL**, il riquadro specificato da `row` e `col` è impostata come attiva. In caso contrario **NULL**, specifica il riquadro è impostata come attivo.  
...
##  <a name="setcolumninfo"></a>  CSplitterWnd::SetColumnInfo  
 Chiamata eseguita per impostare le informazioni della colonna specificata.  
 ...
 *cxIdeal*  
 Specifica una larghezza ideale della colonna della finestra con separatore in pixel.  
...
### <a name="remarks"></a>Note  
 ...
 Quando il framework visualizza una finestra con separatore, disposto i riquadri in colonne e righe in base alle dimensioni ideali, utilizzando l'angolo superiore sinistro all'angolo inferiore destro dell'area client della finestra con separatore.  
...
##  <a name="setscrollstyle"></a>  CSplitterWnd::SetScrollStyle  
 Specifica che il nuovo stile di scorrimento per la finestra con separatore condivisa supporto della barra di scorrimento.  
...
### <a name="parameters"></a>Parametri  
 `dwStyle`  
 Il nuovo stile di scorrimento per la finestra con separatore condiviso supporto della barra di scorrimento, che può essere uno dei valori seguenti:  
  
- **WS_HSCROLL** crea/Mostra orizzontale condivise le barre di scorrimento.  
  
- **WS_VSCROLL** crea/Mostra verticale condivise le barre di scorrimento.  
  
### <a name="remarks"></a>Note  
 Dopo aver creata una barra di scorrimento verrà eliminato anche se `SetScrollStyle` viene chiamato senza tale stile; invece vengono nascoste le barre di scorrimento. In questo modo le barre di scorrimento mantenere il proprio stato anche quando sono nascosti. Dopo la chiamata `SetScrollStyle` è necessario chiamare [RecalcLayout](#recalclayout) per tutte le modifiche abbiano effetto.  
