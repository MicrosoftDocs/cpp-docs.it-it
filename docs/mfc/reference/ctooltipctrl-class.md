...
- `TTDT_AUTOPOP` Recuperare l'intervallo di tempo che la finestra di descrizione comandi rimane visibile se il puntatore è fermo su rettangolo di delimitazione dello strumento.  
...
- `TTDT_RESHOW` Recuperare la lunghezza di tempo che necessaria per le successive finestre di descrizione comandi da visualizzare quando il puntatore viene spostato da uno strumento a altro.  
