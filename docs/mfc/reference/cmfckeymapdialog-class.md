...
ms.locfileid: "37037288"
---
# <a name="cmfckeymapdialog-class"></a>Classe CMFCKeyMapDialog
Il `CMFCKeyMapDialog` classe supporta un controllo che associa i comandi ai tasti sulla tastiera.  
...
|Nome|Descrizione|  
|----------|-----------------|  
|[CMFCKeyMapDialog::FormatItem](#formatitem)|Chiamato dal framework per compilare una stringa che descrive un mapping delle chiavi. Per impostazione predefinita, la stringa contiene il nome del comando, i tasti di scelta rapida utilizzate e la descrizione di tasti di scelta rapida.|  
...
|[CMFCKeyMapDialog::OnPrintHeader](#onprintheader)|Chiamato dal framework per stampare l'intestazione per la mappa da tastiera in una nuova pagina.|  
...
## <a name="remarks"></a>Note  
 Utilizzare la `CMFCKeyMapDialog` classe per implementare una finestra di dialogo di mapping della tastiera ridimensionabile. La finestra di dialogo Usa un controllo visualizzazione elenco per visualizzare i tasti di scelta rapida e i relativi comandi.  
  
 Usare la `CMFCKeyMapDialog` classe in un'applicazione, passare un puntatore alla finestra cornice principale come un parametro il `CMFCKeyMapDialog` costruttore. Chiamare quindi il `DoModal` metodo per avviare una finestra di dialogo modale.  
...
### <a name="example"></a>Esempio  
 Nell'esempio seguente viene illustrato come costruire un oggetto del `CMFCKeyMapDialog` classe. In questo esempio fa parte il [esempio dimostrativo di Visual Studio](../../visual-cpp-samples.md).  
...
### <a name="return-value"></a>Valore restituito  
 Un intero con segno, ad esempio `IDOK` o `IDCANCEL`, vale a dire passato per il [CDialog::EndDialog](../../mfc/reference/cdialog-class.md#enddialog) (metodo). Il metodo, a sua volta, chiude la finestra di dialogo. Per altre informazioni, vedere [CDialog::DoModal](../../mfc/reference/cdialog-class.md#domodal).  
  
### <a name="remarks"></a>Note  
 La finestra di dialogo di mappatura della tastiera consente di selezionare e assegnare tasti di scelta rapida per le diverse categorie di comandi. Inoltre, è possibile copiare negli Appunti i tasti di scelta rapida selezionato e la relativa descrizione.  
  
##  <a name="formatitem"></a>  CMFCKeyMapDialog::FormatItem  
 Chiamato dal framework per compilare una stringa che descrive un mapping delle chiavi. Per impostazione predefinita, la stringa contiene il nome del comando, i tasti di scelta rapida utilizzate e la descrizione di tasti di scelta rapida.  
...
### <a name="return-value"></a>Valore restituito  
 Delimitato da punto e virgola (';') elenco dei tasti di scelta rapida che viene associata al comando specificato.  
  
### <a name="remarks"></a>Note  
  
##  <a name="oninsertitem"></a>  CMFCKeyMapDialog::OnInsertItem  
 Chiamato dal framework prima di un nuovo elemento viene inserito un controllo elenco interno che supporta il controllo di mappatura della tastiera.  
...
##  <a name="onprintheader"></a>  CMFCKeyMapDialog::OnPrintHeader  
 Chiamato dal framework per stampare l'intestazione per la mappa da tastiera in una nuova pagina.  
...
### <a name="return-value"></a>Valore restituito  
 Se l'operazione riesce, l'altezza del testo stampato. Per altre informazioni, vedere la sezione di valore restituito del [CDC](../../mfc/reference/cdc-class.md#drawtext).  
...
### <a name="remarks"></a>Note  
 Il framework chiama questo metodo per la stampa di un elemento di finestra di dialogo mappa di chiavi. Per impostazione predefinita, questo metodo visualizza il nome di comando dell'elemento, tasti di scelta rapida e descrizione comando.  
...
### <a name="remarks"></a>Note  
 Per impostazione predefinita, questo metodo ottiene le didascalie per le colonne da tre risorse. La didascalia di colonna comando a IDS_AFXBARRES_COMMAND, la didascalia della colonna chiave è da IDS_AFXBARRES_KEYS, e la didascalia di colonna Descrizione è da IDS_AFXBARRES_DESCRIPTION.  
...
### <a name="remarks"></a>Note  
 Il `PrintKeyMap` metodo stampa la mappa di chiavi. Avvia un nuovo processo di stampato e quindi chiama ripetutamente la [CMFCKeyMapDialog::OnPrintHeader](#onprintheader) e [CMFCKeyMapDialog::OnPrintItem](#onprintitem) metodi fino a quando non vengono stampati tutti i mapping dei tasti.  
