...
|Nome|Descrizione|  
|----------|-----------------|  
|[CDumpContext::DumpAsHex](#dumpashex)|Esegue il dump l'elemento indicato in formato esadecimale.|  
...
## <a name="remarks"></a>Note  
 ...
 È possibile utilizzare [afxDump](diagnostic-services.md#afxdump), un dichiarata in precedenza `CDumpContext` oggetto, per la maggior parte del dump. Il `afxDump` l'oggetto è disponibile solo nella versione di Debug della libreria Microsoft Foundation Class.  
  
 Numerosi la memoria [servizi di diagnostica](../../mfc/reference/diagnostic-services.md) utilizzare `afxDump` per il relativo output.  
  
 Nell'ambiente di Windows, l'output dalla predefiniti `afxDump` oggetto, concettualmente simile per il `cerr` flusso, viene indirizzato al debugger tramite la funzione di Windows **OutputDebugString**.  
 ...
 Se si utilizza il [IMPLEMENT_DYNAMIC](../../mfc/reference/run-time-object-model-services.md#implement_dynamic) oppure [IMPLEMENT_SERIAL](../../mfc/reference/run-time-object-model-services.md#implement_serial) macro nell'implementazione della classe, quindi `CObject::Dump` verrà stampato il nome del `CObject`-classe derivata. In caso contrario, verrà stampata `CObject`.  
  
 Il `CDumpContext` classe è disponibile con le versioni di Debug e di rilascio della libreria, ma la `Dump` funzione membro è definita solo nella versione di Debug. Uso **debug #ifdef**  /  `#endif` istruzioni da racchiudere tra parentesi quadre del codice di diagnostica, inclusi personalizzato `Dump` funzioni membro.  
...
### <a name="remarks"></a>Note  
 ...
 Non scrivere nei sottostante `CFile` mentre il contesto di dump è attivo; in caso contrario, interferisce con il dump. Nell'ambiente di Windows, l'output viene indirizzato al debugger tramite la funzione di Windows **OutputDebugString**.  
...
### <a name="return-value"></a>Valore restituito  
 La profondità del dump come impostato dal `SetDepth`.  
  
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo [SetDepth](#setdepth).  
  
##  <a name="hexdump"></a>  CDumpContext::HexDump  
 Esegue il dump una matrice di byte formattato come numeri esadecimali.  
...
### <a name="remarks"></a>Note  
 Per eseguire il dump un tipo di elemento singolo, specifico come numero esadecimale, chiamare [CDumpContext::DumpAsHex](#dumpashex).  
...
### <a name="return-value"></a>Valore restituito  
 Oggetto `CDumpContext` riferimento. Tramite il valore restituito, è possibile scrivere più inserimenti in una singola riga di codice sorgente.  
  
### <a name="remarks"></a>Note  
 L'operatore di inserimento è sottoposto a overload per `CObject` anche i puntatori a quello dei tipi più primitivi. Un puntatore ai risultati di tipo carattere in un dump del contenuto di stringa; un puntatore a **void** comporta un dump esadecimale dell'indirizzo solo. Un **LongLong con** comporta un dump di un intero con segno a 64 bit; Un **ULONGLONG** comporta un dump di un intero senza segno a 64 bit.  
  
 Se si utilizza il `IMPLEMENT_DYNAMIC` oppure `IMPLEMENT_SERIAL` macro nell'implementazione della classe, l'operatore di inserimento, tramite `CObject::Dump`, verrà stampato il nome del `CObject`-classe derivata. In caso contrario, verrà stampata `CObject`. Se esegue l'override di `Dump` funzione della classe, è possibile fornire un output più significativo del contenuto dell'oggetto anziché un dump esadecimale.  
