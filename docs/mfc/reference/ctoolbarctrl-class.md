...
|[CToolBarCtrl::Create](#create)|Crea un controllo barra degli strumenti e lo collega a `CToolBarCtrl` un oggetto.|
|[CToolBarCtrl::CreateEx](#createex)|Crea un controllo Toolbar con gli stili estesi di Windows specificati e lo collega a `CToolBarCtrl` un oggetto.|
|[CToolBarCtrl::Customize](#customize)|Consente di visualizzare la finestra di dialogo Personalizza barra degli strumenti.|
|[CToolBarCtrl::DeleteButton](#deletebutton)|Elimina un pulsante dal controllo Toolbar.|
...
|[CToolBarCtrl::GetImageList](#getimagelist)|Recupera l'elenco di immagini utilizzato da un controllo Toolbar per visualizzare i pulsanti nello stato predefinito.|
...
|[CToolBarCtrl::SetImageList](#setimagelist)|Imposta l'elenco di immagini che verrà utilizzato dalla barra degli strumenti per visualizzare i pulsanti che si trovano nello stato predefinito.|
...
|[CToolBarCtrl::SetMetrics](#setmetrics)|Imposta la metrica di un controllo della barra degli strumenti.|
|[CToolBarCtrl::SetOwner](#setowner)|Imposta la finestra per la ricezione di messaggi di notifica dal controllo Toolbar.|
|[CToolBarCtrl::SetPadding](#setpadding)|Imposta la spaziatura interna orizzontale e verticale del controllo toolbar corrente.|
...
|[CToolBarCtrl::SetToolTips](#settooltips)|Associa un controllo descrizione comando al controllo Toolbar.|
...
1. Chiamare [create](#create) per creare il controllo comune della barra degli strumenti di Windows e `CToolBarCtrl` collegarlo all'oggetto. Indicare lo stile della barra degli strumenti usando gli stili, ad esempio TBSTYLE_TRANSPARENT per una barra degli strumenti trasparente o TBSTYLE_DROPDOWN per una barra degli strumenti che supporta i pulsanti di stile a discesa.
...
1. Se si desiderano le descrizioni comandi per un pulsante della barra degli strumenti in una finestra `CFrameWnd`proprietaria che non è un, è necessario gestire i messaggi TTN_NEEDTEXT nella finestra proprietaria della barra degli strumenti, come descritto in [gestione delle notifiche delle descrizioni comandi](../../mfc/handling-tool-tip-notifications.md). Se la finestra padre della barra degli strumenti è derivata `CFrameWnd`da, le descrizioni comandi vengono visualizzate senza alcuna ulteriore operazione da `CFrameWnd` parte dell'utente perché fornisce un gestore predefinito.
...
Una barra degli strumenti trasparente consente di visualizzare il client sotto la barra degli strumenti. Per creare una barra degli strumenti trasparente, usare entrambi gli stili TBSTYLE_FLAT e TBSTYLE_TRANSPARENT. Funzionalità di rilevamento a caldo della barra degli strumenti trasparente; ovvero, quando il puntatore del mouse viene spostato su un pulsante attivo sulla barra degli strumenti, l'aspetto del pulsante cambia. Le barre degli strumenti create solo con lo stile TBSTYLE_FLAT conterranno pulsanti non trasparenti.
...
Per altre informazioni sull'uso `CToolBarCtrl`di, vedere [controlli](../../mfc/controls-mfc.md) e [uso di CToolBarCtrl](../../mfc/using-ctoolbarctrl.md).
...
- `idCommand`
   ...
   Identificatore di comando associato al pulsante. Questo identificatore viene inviato in un messaggio WM_COMMAND quando si sceglie il pulsante. Se il `fsStyle` membro ha il valore TBSTYLE_SEP, questo membro deve essere zero.
...
- `fsStyle`
   ...
   - TBSTYLE_CHECK crea un pulsante che alterna gli stati premuti e non stampati ogni volta che l'utente fa clic su di esso. Il pulsante ha un colore di sfondo diverso quando si trova nello stato premuto.
...
|---------------|-----------------|
|*idButton*|in Identificatore di comando del pulsante che deve ricevere una nuova bitmap.|
|*iBitmap*|in Indice in base zero di un'immagine nell'elenco immagini del controllo toolbar corrente.|
...
Questo metodo invia il messaggio [TB_CHANGEBITMAP](/windows/win32/Controls/tb-changebitmap) , descritto nella Windows SDK.
...
Crea un controllo barra degli strumenti e lo collega a `CToolBarCtrl` un oggetto.
...
*rect*<br/>
Facoltativamente specifica le dimensioni e la posizione del controllo Toolbar. Può essere un oggetto [CRect](../../atl-mfc-shared/reference/crect-class.md) o una struttura [Rect](/previous-versions/dd162897\(v=vs.85\)) .
...
Si costruisce `CToolBarCtrl` un in due passaggi. Chiamare innanzitutto il costruttore, quindi chiamare `Create`, che crea il controllo Toolbar e lo collega `CToolBarCtrl` all'oggetto. Applicare gli stili della finestra seguenti a un controllo Toolbar.
...
Per utilizzare gli stili estesi della barra degli strumenti, chiamare [SetExtendedStyle](#setextendedstyle) dopo aver chiamato `Create`. Per creare una barra degli strumenti con stili estesi della `Create`finestra, chiamare [CToolBarCtrl:: CreateEx](#createex) anziché.
...
Crea un controllo (una finestra figlio) e lo associa `CToolBarCtrl` all'oggetto.
...
Usare `CreateEx` anziché [create](#create) per applicare gli stili estesi di Windows, specificato dalla prefazione **WS_EX_** di Windows Extended Style. `CreateEx`Crea il controllo con gli stili Windows estesi specificati da *dwExStyle*. Impostare gli stili estesi specifici di un controllo usando [SetExtendedStyle](#setextendedstyle). Ad esempio, usare `CreateEx` per impostare tali stili come WS_EX_CONTEXTHELP, ma usare `SetExtendedStyle` per impostare tali stili come TBSTYLE_EX_DRAWDDARROWS. Per ulteriori informazioni, vedere gli stili descritti in [stili estesi della barra degli strumenti](/windows/win32/Controls/toolbar-extended-styles) nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETANCHORHIGHLIGHT](/windows/win32/Controls/tb-getanchorhighlight), come descritto nel Windows SDK.
...
Oggetto UINT con il flag TBBF_LARGE impostato se la visualizzazione può supportare bitmap di grandi dimensioni della barra degli strumenti; in caso contrario, deselezionare.
...
*lpButton*<br/>
Indirizzo della `TBBUTTON` struttura che deve ricevere una copia delle informazioni sul pulsante. Per informazioni sulla `TBBUTTON` struttura, vedere [CToolBarCtrl:: AddButtons](#addbuttons) .
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETBUTTONINFO](/windows/win32/Controls/tb-getbuttoninfo), come descritto nel Windows SDK.
...
|Parametro|Descrizione|
|---------------|-----------------|
|*idButton*|in Identificatore del pulsante il cui testo visualizzato viene recuperato.|
...
Questo metodo invia il messaggio [TB_GETBUTTONTEXT](/windows/win32/Controls/tb-getbuttontext) , descritto nella Windows SDK.
...
|Parametro|Descrizione|
|---------------|-----------------|
|*lpColorScheme*|out Puntatore a una struttura [ColorScheme](/windows/win32/api/commctrl/ns-commctrl-colorscheme) che riceve le informazioni sulla combinazione di colori. Quando questo metodo restituisce, la struttura descrive il colore di evidenziazione e il colore dell'ombreggiatura del controllo Toolbar.|
...
Questo metodo invia il messaggio [TB_GETCOLORSCHEME](/windows/win32/Controls/tb-getcolorscheme) , descritto nella Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETDISABLEDIMAGELIST](/windows/win32/Controls/tb-getdisabledimagelist), come descritto nel Windows SDK. L'implementazione MFC di `GetDisabledImageList` usa un `CImageList` oggetto contenente le immagini dei pulsanti del controllo Toolbar, anziché un handle per un elenco di immagini.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETOBJECT](/windows/win32/Controls/tb-getobject), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETEXTENDEDSTYLE](/windows/win32/Controls/tb-getextendedstyle), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETHOTIMAGELIST](/windows/win32/Controls/tb-gethotimagelist), come descritto nel Windows SDK. Un pulsante attivo appare evidenziato quando il puntatore del mouse si trova sopra di esso.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETHOTITEM](/windows/win32/Controls/tb-gethotitem), come descritto nel Windows SDK.

##  <a name="getimagelist"></a>CToolBarCtrl:: GetImages
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETIMAGELIST](/windows/win32/Controls/tb-getimagelist), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETINSERTMARK](/windows/win32/Controls/tb-getinsertmark), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETINSERTMARKCOLOR](/windows/win32/Controls/tb-getinsertmarkcolor), come descritto nel Windows SDK.
...
*lpRect*<br/>
Indirizzo di una struttura [Rect](/previous-versions/dd162897\(v=vs.85\)) o di un oggetto [CRect](../../atl-mfc-shared/reference/crect-class.md) che riceve le coordinate del rettangolo di delimitazione.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETMAXSIZE](/windows/win32/Controls/tb-getmaxsize), come descritto nel Windows SDK.
...
Questa funzione membro emula la funzionalità del messaggio [TB_GETMETRICS](/windows/win32/Controls/tb-getmetrics) , come descritto nel Windows SDK.
...
|Parametro|DESCRIZIONE|
|---------------|-----------------|
|*pnHorzPadding*|out Intero che riceve la spaziatura interna orizzontale del controllo barra degli strumenti, in pixel.|
|*pnVertPadding*|out Intero che riceve la spaziatura interna verticale del controllo della barra degli strumenti, in pixel.|
...
Questo metodo invia il messaggio [TB_GETPADDING](/windows/win32/Controls/tb-getpadding) , descritto nella Windows SDK.
...
Questo metodo invia il messaggio [TB_GETPRESSEDIMAGELIST](/windows/win32/Controls/tb-getpressedimagelist) , descritto nella Windows SDK.
...
*lpRect*<br/>
Puntatore a una struttura [Rect](/previous-versions/dd162897\(v=vs.85\)) per ricevere le informazioni sul rettangolo di delimitazione.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETRECT](/windows/win32/Controls/tb-getrect), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_GETSTRING](/windows/win32/Controls/tb-getstring), come descritto nel Windows SDK.
...
*ppt*<br/>
Puntatore a una struttura di [punti](/previous-versions/dd162805\(v=vs.85\)) che contiene la coordinata x dell'hit test nel `x` membro e la coordinata y dell' `y` hit test nel membro. Le coordinate sono relative all'area client della barra degli strumenti.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_HITTEST](/windows/win32/Controls/tb-hittest), come descritto nel Windows SDK.
...
##  <a name="indeterminate"></a>CToolBarCtrl:: indeterminato
...
*lpButton*<br/>
Indirizzo di una `TBBUTTON` struttura contenente informazioni sul pulsante da inserire. Per una descrizione della `TBBUTTON` struttura, vedere [CToolBarCtrl:: AddButtons](#addbuttons) .
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_INSERTMARKHITTEST](/windows/win32/Controls/tb-insertmarkhittest), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_LOADIMAGES](/windows/win32/Controls/tb-loadimages), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_MAPACCELERATOR](/windows/win32/Controls/tb-mapaccelerator), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_MARKBUTTON](/windows/win32/Controls/tb-markbutton), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_MOVEBUTTON](/windows/win32/Controls/tb-movebutton), come descritto nel Windows SDK.
...
Questo metodo invia il messaggio [TB_PRESSBUTTON](/windows/win32/Controls/tb-pressbutton) , descritto nella Windows SDK.
...
|Parametro|DESCRIZIONE|
|---------------|-----------------|
|*pReplaceBitmap*|in Puntatore a una struttura [TBREPLACEBITMAP](/windows/win32/api/commctrl/ns-commctrl-tbreplacebitmap) che descrive la bitmap da sostituire e la nuova bitmap.|
...
Questo metodo invia il messaggio [TB_REPLACEBITMAP](/windows/win32/Controls/tb-replacebitmap) , descritto nella Windows SDK.
...
Questo metodo implementa il comportamento del messaggio Win32 [TB_SETANCHORHIGHLIGHT](/windows/win32/Controls/tb-setanchorhighlight), come descritto nel Windows SDK.
...
Per ulteriori informazioni sulla `TBBUTTON` struttura, vedere [CToolBarCtrl:: AddButtons](#addbuttons) .
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_SETBUTTONWIDTH](/windows/win32/Controls/tb-setbuttonwidth), come descritto nel Windows SDK.
...
|Parametro|DESCRIZIONE|
|---------------|-----------------|
|*lpColorScheme*|in Puntatore a una struttura [ColorScheme](/windows/win32/api/commctrl/ns-commctrl-colorscheme) che descrive il colore di evidenziazione e il colore dell'ombreggiatura del controllo Toolbar.|

### <a name="remarks"></a>Note

Questo metodo non ha effetto se è impostato un tema visivo di Windows Vista.

Questo metodo invia il messaggio [TB_SETCOLORSCHEME](/windows/win32/Controls/tb-setcolorscheme) , descritto nella Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_SETDISABLEDIMAGELIST](/windows/win32/Controls/tb-setdisabledimagelist), come descritto nel Windows SDK. L'implementazione MFC di `SetDisabledImageList` usa un `CImageList` oggetto contenente le immagini dei pulsanti disabilitati del controllo Toolbar, anziché un handle per un elenco di immagini.
...
*dwMask*<br/>
Combinazione di uno o più flag DT_, specificati nella funzione Win32 [DrawText](/windows/win32/api/winuser/nf-winuser-drawtext), che indica i bit in *dwDTFlags* che verranno utilizzati durante il disegno del testo.

*dwDTFlags*<br/>
Combinazione di uno o più flag DT_, specificati nella funzione `DrawText`Win32, che indicano come verrà disegnato il testo del pulsante. Questo valore viene passato a `DrawText` quando viene disegnato il testo del pulsante.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_SETDRAWTEXTFLAGS](/windows/win32/Controls/tb-setdrawtextflags), come descritto nel Windows SDK. Questa funzione membro imposta i flag nella funzione `DrawText`Win32, che disegna il testo nel rettangolo specificato, formattato in base alla modalità di impostazione dei flag.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_SETEXTENDEDSTYLE](/windows/win32/Controls/tb-setextendedstyle), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_SETHOTIMAGELIST](/windows/win32/Controls/tb-sethotimagelist), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_SETHOTITEM](/windows/win32/Controls/tb-sethotitem), come descritto nel Windows SDK.

##  <a name="setimagelist"></a>CToolBarCtrl:: seimagine
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_SETIMAGELIST](/windows/win32/Controls/tb-setimagelist), come descritto nel Windows SDK.
...
##  <a name="setindent"></a>CToolBarCtrl:: sedent
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_SETINSERTMARK](/windows/win32/Controls/tb-setinsertmark), come descritto nel Windows SDK.
...
Questa funzione membro implementa il comportamento del messaggio Win32 [TB_SETINSERTMARKCOLOR](/windows/win32/Controls/tb-setinsertmarkcolor), come descritto nel Windows SDK.
...
Questa funzione membro emula la funzionalità del messaggio [TB_SETMETRICS](/windows/win32/Controls/tb-setmetrics) , come descritto nel Windows SDK.
...
*pWnd*<br/>
Puntatore all' `CWnd` oggetto derivato `CWnd`da o che sarà la nuova finestra proprietaria per il controllo Toolbar.
...
|Parametro|Descrizione|
|---------------|-----------------|
|*nHorzPadding*|in Specifica la spaziatura interna orizzontale del controllo barra degli strumenti, in pixel.|
|*nVertPadding*|in Specifica la spaziatura verticale del controllo barra degli strumenti, in pixel.|
...
Questo metodo invia il messaggio [TB_SETPADDING](/windows/win32/Controls/tb-setpadding) , descritto nella Windows SDK.
...
|Parametro|DESCRIZIONE|
|---------------|-----------------|
|*iImageID*|in Indice in base zero dell'elenco di immagini. Se si usa un solo elenco di immagini, impostare questo parametro su zero.|
...
Questo metodo invia il messaggio [TB_SETPRESSEDIMAGELIST](/windows/win32/Controls/tb-setpressedimagelist) , descritto nella Windows SDK.
...
*lpRect*<br/>
Punta all'oggetto [CRect](../../atl-mfc-shared/reference/crect-class.md) o alla struttura [Rect](/previous-versions/dd162897\(v=vs.85\)) che riceverà il nuovo rettangolo di delimitazione della barra degli strumenti.
...
##  <a name="settooltips"></a>CToolBarCtrl:: setooltips
...
Questa funzione membro emula la funzionalità del messaggio [TB_SETWINDOWTHEME](/windows/win32/Controls/tb-setwindowtheme) , come descritto nel Windows SDK.
