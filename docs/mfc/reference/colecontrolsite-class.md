...
|Nome|Descrizione|  
|----------|-----------------|  
|[COleControlSite::BindDefaultProperty](#binddefaultproperty)|Associa la proprietà predefinita del controllo ospitato in un'origine dati.|  
|[COleControlSite::BindProperty](#bindproperty)|Associa una proprietà del controllo ospitato in un'origine dati.|  
...
|[COleControlSite::EnableDSC](#enabledsc)|Consente di determinazione dell'origine per un sito di controllo dei dati.|  
...
|[COleControlSite::m_ctlInfo](#m_ctlinfo)|Contiene informazioni sulla tastiera di gestione per il controllo.|  
...
### <a name="parameters"></a>Parametri  
 ...
 `pDSCWnd`  
 Un puntatore al `CWnd`-oggetto derivata che ospita il controllo origine dati a cui verrà associata la proprietà.  
 ...
 *psize*  
 Un puntatore a un **dimensioni** struttura che contiene la dimensione del controllo. Nell'angolo superiore sinistro è determinato dal valore di `ppt`. Il `ppt` e *psize* valori rappresentano un metodo facoltativo che specifica la dimensione e posizione opf il controllo.  
...
|-5|Attiva un controllo sul posto, senza elementi dell'interfaccia utente aggiuntivi.|**OLEIVERB_INPLACEACTIVATE**|  
...
##  <a name="enabledsc"></a>  COleControlSite::EnableDSC  
 Consente di determinazione dell'origine per il sito del controllo dei dati.  
...
> [!NOTE]
>  Il controllo non è necessario interrompere la generazione degli eventi, se richiesto dal sito del controllo. La generazione può continuare, ma tutti gli eventi successivi verranno ignorati dal sito di controllo.  
...
### <a name="return-value"></a>Valore restituito  
 Diverso da zero se ha esito positivo, in caso contrario 0. Se ha esito positivo, `piid` contiene l'ID di interfaccia per l'interfaccia di eventi del controllo predefinito.  
...
### <a name="parameters"></a>Parametri  
 `dwDispID`  
 Identifica l'ID dispatch della proprietà, trovata nel valore predefinito del controllo `IDispatch` interfaccia da recuperare.  
...
### <a name="remarks"></a>Note  
 Per un elenco di valori possibili, vedere [stili Windows](../../mfc/reference/styles-used-by-mfc.md#window-styles). Per recuperare gli stili estesi del sito di controllo, chiamare [COleControlSite::GetExStyle](#getexstyle).  
...
### <a name="remarks"></a>Note  
 Se il controllo supporta la proprietà predefinita didascalia, viene restituito questo valore. Se la proprietà predefinita di didascalia non è supportata, viene restituito il valore della proprietà Text.  
...
### <a name="parameters"></a>Parametri  
 ...
 `wFlags`  
 Flag che descrivono il contesto della chiamata a IDispatch:: Invoke. Possibile `wFlags` valori, vedere `IDispatch::Invoke` in Windows SDK.  
 ...
 `pbParamInfo`  
 Puntatore a una stringa di byte con terminazione Null che specifica i tipi dei parametri che seguono `pbParamInfo`. Per i valori possibili, vedere la sezione Osservazioni per [COleDispatchDriver:: InvokeHelper](../../mfc/reference/coledispatchdriver-class.md#invokehelper).  
...
### <a name="remarks"></a>Note  
 ...
 Questa funzione converte i parametri **VARIANTARG** valori, quindi richiama il **IDispatch:: Invoke** metodo sul controllo. Se la chiamata a **IDispatch:: Invoke** ha esito negativo, questa funzione genera un'eccezione. Se il codice di stato restituito da **IDispatch:: Invoke** è `DISP_E_EXCEPTION`, questa funzione genera un **COleDispatchException** oggetto, in caso contrario viene generata una `COleException`.  
...
### <a name="remarks"></a>Note  
 Il parametro `pbParamInfo` specifica i tipi dei parametri passati al metodo o alla proprietà. Parametri aggiuntivi per il metodo o proprietà richiamata possono essere passati tramite la *va_list* parametro.  
...
### <a name="return-value"></a>Valore restituito  
 Diverso da zero se il controllo pulsante predefinito nella finestra, in caso contrario 0.  
...
### <a name="remarks"></a>Note  
 Il valore viene recuperato dalla proprietà predefinite di abilitato del controllo.  
  
##  <a name="m_biswindowless"></a>  COleControlSite::m_bIsWindowless  
 Determina se l'oggetto è privo di finestra.  
...
### <a name="remarks"></a>Note  
 Diverso da zero se il controllo non ha una finestra, in caso contrario 0.  
...
### <a name="remarks"></a>Note  
 Per ulteriori informazioni, vedere [OLEMISC](http://msdn.microsoft.com/library/windows/desktop/ms678497)in Windows SDK.  
...
### <a name="parameters"></a>Parametri  
 ...
 `nFlags`  
 Flag di posizionamento delle finestre. Per un elenco di valori possibili, vedere il [SetWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms633545) funzione in Windows SDK.  
  
### <a name="return-value"></a>Valore restituito  
 Diverso da zero se gli stili vengono modificati, zero in caso contrario.  
  
### <a name="remarks"></a>Note  
 Verranno modificato per corrispondere alle impostazioni di azioni del controllo proprietà Enabled **WS_DISABLED**. Proprietà di stile del bordo predefinite del controllo verrà modificata in modo che corrisponda all'impostazione richiesta per `WS_BORDER`. Tutti gli altri stili vengono applicate direttamente all'handle di finestra del controllo, se presente.  
  
 Modifica degli stili di finestra di controllo. Stili dovrà essere aggiunto o rimosso possono essere combinati utilizzando l'operatore OR ( &#124; ) (operatore). Vedere il [CreateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632679) funzione in Windows SDK per informazioni sugli stili di finestra disponibili.  
...
### <a name="remarks"></a>Note  
 ...
 Modifica la finestra stili dell'oggetto del sito di controllo estesi. Stili dovrà essere aggiunto o rimosso possono essere combinati utilizzando l'operatore OR ( &#124; ) (operatore). Vedere il [CreateWindowEx](http://msdn.microsoft.com/library/windows/desktop/ms632680) funzione in Windows SDK per informazioni sugli stili di finestra disponibili.  
...
### <a name="return-value"></a>Valore restituito  
 Diverso da zero se il sito del controllo è stata attivata, in caso contrario 0.  
  
### <a name="remarks"></a>Note  
 Questa funzione deve essere chiamata solo se l'utente esegue l'override il processo di creazione del controllo.  
  
 Il `IPersist*::Load` e `IPersist*::InitNew` metodi devono essere chiamati dopo l'attivazione rapida. Il controllo deve stabilire le connessioni a sink del contenitore durante l'attivazione rapida. Tuttavia, queste connessioni non sono in tempo reale fino `IPersist*::Load` o `IPersist*::InitNew` è stato chiamato.  
...
### <a name="parameters"></a>Parametri  
 *lpmsg*  
 Un puntatore a un [struttura MSG](../../mfc/reference/msg-structure1.md). Questa struttura contiene il messaggio di Windows attivazione il `SetFocus` richiesta per il controllo contenuto nel sito del controllo corrente.  
...
### <a name="remarks"></a>Note  
 ...
 Il tipo di eccezione è determinato dal valore restituito del tentativo di impostare la proprietà o metodo. Se il valore restituito è `DISP_E_EXCEPTION`, **COleDispatchExcpetion** generato; in caso contrario un `COleException`.  
...
### <a name="remarks"></a>Note  
 È possibile passeed parametri aggiuntivi per il metodo o proprietà richiamato utilizzando il *arg_list* parametro. Se `SetProperty` incontra un errore, viene generata un'eccezione.  
 ...
 *y*  
 ...
 `nFlags`  
 Specifica la finestra di ridimensionamento e posizionamento flag. Per i valori possibili, vedere la sezione Osservazioni per [SetWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms633545) in Windows SDK.  
...
### <a name="remarks"></a>Note  
 Questa funzione tenta innanzitutto di impostare la proprietà predefinita di didascalia. Se la proprietà predefinita di didascalia non è supportata, è invece la proprietà Text.  
