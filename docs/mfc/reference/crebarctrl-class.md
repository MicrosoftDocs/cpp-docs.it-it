...
|[CReBarCtrl:: create](#create)|Crea il controllo Rebar e lo connette all'oggetto `CReBarCtrl`.|
|[CReBarCtrl:: CreateEx](#createex)|Crea un controllo Rebar con gli stili estesi di Windows specificati e lo associa a un oggetto `CReBarCtrl`.|
...
|[CReBarCtrl:: GetDropTarget](#getdroptarget)|Recupera un puntatore all'interfaccia `IDropTarget` del controllo Rebar.|
...
- La banda 2 contiene una casella combinata e quattro pulsanti standard.
   ...
   ![Esempio di un menu Rebar](../../mfc/reference/media/vc4scc1.gif "Esempio di menu Rebar")
...
Crea il controllo Rebar e lo connette all'oggetto `CReBarCtrl`.
...
1. Chiamare [CReBarCtrl](#crebarctrl) per costruire un oggetto `CReBarCtrl`.

1. Chiamare questa funzione membro, che crea il controllo Rebar di Windows e lo collega all'oggetto `CReBarCtrl`.
...
*PRC*<br/>
Puntatore a una struttura [Rect](/previous-versions/dd162897\(v=vs.85\)) che riceverà i bordi della banda. Se il controllo Rebar dispone dello stile RBS_BANDBORDERS, ogni membro di questa struttura riceverà il numero di pixel, sul lato corrispondente della banda, che costituiscono il bordo. Se il controllo Rebar non ha lo stile RBS_BANDBORDERS, solo il membro sinistro di questa struttura riceve informazioni valide. Per una descrizione degli stili del controllo Rebar, vedere [stili di controllo Rebar](/windows/win32/Controls/rebar-control-styles) nel Windows SDK.
...
*prbbi*<br/>
Puntatore a una struttura [REBARBANDINFO](/windows/win32/api/commctrl/ns-commctrl-rebarbandinfow) per ricevere le informazioni sulla banda. È necessario impostare il membro `cbSize` di questa struttura su `sizeof(REBARBANDINFO)` e impostare il membro `fMask` sugli elementi che si desidera recuperare prima di inviare questo messaggio.
...
La struttura `COLORSCHEME` include il colore di evidenziazione del pulsante e il colore dell'ombreggiatura del pulsante.
...
Si noti che questa funzione membro utilizza un oggetto `CPalette` come valore restituito, anziché un HPALETTE.
...
*prbht*<br/>
Puntatore a una struttura [RBHITTESTINFO](/windows/win32/api/commctrl/ns-commctrl-rbhittestinfo) . Prima di inviare il messaggio, è necessario inizializzare il membro `pt` di questa struttura sul punto che verrà testato nelle coordinate del client.
...
*prbbi*<br/>
Puntatore a una struttura [REBARBANDINFO](/windows/win32/api/commctrl/ns-commctrl-rebarbandinfow) che definisce la banda da inserire. Prima di chiamare questa funzione, è necessario impostare il membro *cbSize* della struttura su `sizeof(REBARBANDINFO)`.
...
*prbbi*<br/>
Puntatore a una struttura [REBARBANDINFO](/windows/win32/api/commctrl/ns-commctrl-rebarbandinfow) che definisce la banda da inserire. È necessario impostare il membro `cbSize` di questa struttura su `sizeof(REBARBANDINFO)` prima di inviare questo messaggio.
...
*pWnd*<br/>
Puntatore a un oggetto `CWnd` da impostare come proprietario del controllo Rebar.
...
Si noti che questa funzione membro utilizza i puntatori per `CWnd` oggetti sia per il proprietario corrente che per quello selezionato del controllo Rebar anziché per le finestre.
...
##  <a name="setpalette"></a>CReBarCtrl:: sepalette
...
Valore [COLORREF](/windows/win32/gdi/colorref) che rappresenta il colore del testo precedente associato all'oggetto `CReBarCtrl`.

### <a name="remarks"></a>Osservazioni

Viene fornito per supportare la flessibilità del colore del testo in un controllo Rebar.

##  <a name="settooltips"></a>CReBarCtrl:: setooltips
...
Al termine dell'operazione, è necessario eliminare definitivamente l'oggetto `CToolTipCtrl`.
...
Si noti che questa funzione membro utilizza un oggetto `CRect` come parametro, anziché una struttura di `RECT`.
