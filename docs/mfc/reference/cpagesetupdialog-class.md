...
|[CPageSetupDialog::OnDrawPage](#ondrawpage)|Chiamato dal framework per eseguire il rendering di un'immagine video di una pagina stampata.|  
|[CPageSetupDialog::PreDrawPage](#predrawpage)|Chiamato dal framework prima del rendering di un'immagine video di una pagina stampata.|  
...
## <a name="remarks"></a>Note  
 ...
 Se `DoModal` restituisce **IDOK**, è possibile usare diversi `CPageSetupDialog`dell'accesso o le funzioni membro il `m_psd` membro dati, per recuperare le informazioni input dall'utente.  
  
> [!NOTE]
>  Quando viene chiusa la finestra di dialogo di impostazione pagina OLE comune, eventuali modifiche apportate dall'utente non verranno salvate dal framework. È compito dell'applicazione per salvare tutti i valori da questa finestra di dialogo in una posizione permanente, ad esempio membro del documento dell'applicazione o classe dell'applicazione.  
 ...
 *dwFlags*  
 Uno o più flag che è possibile utilizzare per personalizzare le impostazioni della finestra di dialogo. I valori possono essere combinati utilizzando l'operatore OR bit per bit. Questi valori non hanno i significati seguenti:  
...
- **PSD_MINMARGINS** , il sistema userà i valori specificati nel **rtMinMargin** membro come gli spessori minimi consentiti per sinistro, superiore, destro e margini inferiore. Il sistema impedisce all'utente di immettere una lunghezza minore rispetto al valore minimo specificato. Se **PSD_MINMARGINS** non viene specificato, il sistema imposta la larghezza minima consentita a quelle consentite dalla stampante.  
...
##  <a name="domodal"></a>  CPageSetupDialog::DoModal  
 Chiamare questa funzione per visualizzare una finestra di dialogo Impostazione pagina OLE comune di Windows e consentire all'utente di selezionare varie opzioni di stampa, ad esempio i margini di stampa, dimensioni e l'orientamento del foglio e stampante di destinazione.  
...
### <a name="return-value"></a>Valore restituito  
 Il nome del dispositivo utilizzato dal `CPageSetupDialog` oggetto.  
...
### <a name="remarks"></a>Note  
 Utilizzare un puntatore ai `CString` oggetto restituito da `GetDriverName` come valore di `lpszDriverName` in una chiamata a [CDC::CreateDC](../../mfc/reference/cdc-class.md#createdc).  
...
### <a name="parameters"></a>Parametri  
 *lpRectMargins*  
 Puntatore a un [RECT](https://www.microsoftonedoc.com/#/organizations/e6f6a65cf14f462597b64ac058dbe1d0/projects/3fedad16-eaf1-41a6-8f96-0c1949c68f32/containers/a3daf831-1c5f-4bbe-964d-503870caf874/tocpaths/18113766-3975-4369-bc07-92e34cba712e/locales/en-us) struttura oppure [CRect](../../atl-mfc-shared/reference/crect-class.md) oggetto che descrive (in pollici 1/1000 o mm 1/100) i margini di stampa per la stampante selezionata. Passare **NULL** per questo parametro, se non si è interessati in questo rettangolo.  
  
 *lpRectMinMargins*  
 Puntatore a un `RECT` struttura o `CRect` oggetto che descrive (in pollici 1/1000 o mm 1/100) i margini di stampa minimi per la stampante selezionata. Passare **NULL** per questo parametro, se non si è interessati in questo rettangolo.  
...
### <a name="return-value"></a>Valore restituito  
 Un [CSize](../../atl-mfc-shared/reference/csize-class.md) oggetto che contiene le dimensioni della carta (1/1000 pollici o mm 1/100) selezionata per la stampa.  
  
##  <a name="getportname"></a>  CPageSetupDialog::GetPortName  
 Chiamare questa funzione dopo la chiamata `DoModal` per recuperare il nome della porta stampante selezionata.  
...
### <a name="remarks"></a>Note  
 Dopo la costruzione di un `CPageSetupDialog` dell'oggetto, è possibile utilizzare `m_psd` per impostare i vari aspetti della finestra di dialogo prima di chiamare il `DoModal` funzione membro.  
...
##  <a name="ondrawpage"></a>  CPageSetupDialog::OnDrawPage  
 Chiamata eseguita dal framework per disegnare un'immagine video di una pagina stampata.  
 ...
 *nmessaggio*  
 Specifica un messaggio che indica l'area della pagina che viene disegnata. Può essere uno dei seguenti:  
 ...
 *lpRect*  
 Puntatore a un [CRect](../../atl-mfc-shared/reference/crect-class.md) oppure [RECT](https://www.microsoftonedoc.com/#/organizations/e6f6a65cf14f462597b64ac058dbe1d0/projects/3fedad16-eaf1-41a6-8f96-0c1949c68f32/containers/a3daf831-1c5f-4bbe-964d-503870caf874/tocpaths/18113766-3975-4369-bc07-92e34cba712e/locales/en-us) oggetto che contiene le coordinate dell'area di disegno.  
...
### <a name="remarks"></a>Note  
 ...
 Eseguire l'override di questa funzione per personalizzare la creazione di un'area specifica dell'immagine o l'intera immagine. È possibile farlo tramite un **switch** dell'istruzione **case** istruzioni controllando il valore del *nmessaggio*. Ad esempio, per personalizzare il rendering del contenuto dell'immagine della pagina, è possibile utilizzare l'esempio di codice seguente:  
  
 [!code-cpp[NVC_MFCDocView#96](../../mfc/codesnippet/cpp/cpagesetupdialog-class_3.cpp)]  
  
 Si noti che non è necessaria gestire tutti i casi di *nmessaggio*. È possibile scegliere di gestire un componente dell'immagine, diversi componenti per l'immagine o l'intera area.  
...
### <a name="parameters"></a>Parametri  
 *wPaper*  
 Specifica un valore che indica il formato carta. Questo valore può essere uno del **DMPAPER_** i valori elencati nella descrizione del [DEVMODE](http://msdn.microsoft.com/library/windows/desktop/dd183565) struttura.  
...
-   0x001 carta in modalità orizzontale (aghi)  
...
-   0x019 busta in modalità orizzontale (aghi)  
  
-   0x01f busta in modalità verticale (ad aghi)  
  
 *pPSD*  
 Puntatore a un **PAGESETUPDLG** struttura. Per ulteriori informazioni sul [PAGESETUPDLG](http://msdn.microsoft.com/library/windows/desktop/ms646842), vedi il Windows SDK.  
