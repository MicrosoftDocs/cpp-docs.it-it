...
|Nome|Descrizione|  
|----------|-----------------|  
|[CCmdUI::ContinueRouting](#continuerouting)|Indica al meccanismo di routing di comandi per continuare il routing del messaggio corrente lungo la catena dei gestori.|  
...
|[CCmdUI::m_pSubMenu](#m_psubmenu)|Punta al contenuto sottomenu rappresentato dal `CCmdUI` oggetto.|  
  
## <a name="remarks"></a>Note  
 `CCmdUI` non dispone di una classe basa.  
  
 Quando un utente dell'applicazione estrae verso il basso un menu, ogni elemento di menu deve sapere se deve essere visualizzato come abilitato o disabilitato. La destinazione di un comando di menu verrà indicata questa informazione implementando un `ON_UPDATE_COMMAND_UI` gestore. Per ognuno degli oggetti dell'interfaccia utente comando nell'applicazione, utilizzare la finestra proprietà per creare un prototipo di immissione e la funzione della mappa messaggi per ogni gestore.  
  
 Quando il menu verrà spostato verso il basso, il framework cerca e chiama ognuno `ON_UPDATE_COMMAND_UI` gestore, ogni gestore chiama `CCmdUI` , ad esempio funzioni membro `Enable` e `Check`, e il framework visualizza quindi in modo appropriato ogni voce di menu.  
 ...
 Nella tabella seguente sono riepilogati gli effetti `CCmdUI`delle funzioni membro hanno sui vari elementi dell'interfaccia utente di comando.  
  
|Elemento dell'interfaccia utente|Abilita|SetCheck|SetRadio|SetText|  
|--------------------------|------------|--------------|--------------|-------------|  
|Voce di menu|Abilita o disabilita|Seleziona o deseleziona|Controlli con un punto|Set di elementi di testo|  
...
|Riquadro barra di stato|Testo rende visibile o invisibile|Bordo pop-out o normale set|Uguale a `SetCheck`|Imposta il testo riquadro|  
...
### <a name="remarks"></a>Note  
 Si tratta di una funzione membro avanzata che deve essere utilizzata in combinazione con un `ON_COMMAND_EX` gestore che restituisce **FALSE**. Per altre informazioni, vedere [6 Nota tecnica](../../mfc/tn006-message-maps.md).  
...
##  <a name="m_nid"></a>  CCmdUI::m_nID  
 L'ID della voce di menu, pulsante della barra degli strumenti o un altro oggetto dell'interfaccia utente rappresentato dal `CCmdUI` oggetto.  
...
##  <a name="m_nindex"></a>  CCmdUI::m_nIndex  
 L'indice della voce di menu, pulsante della barra degli strumenti o un altro oggetto dell'interfaccia utente rappresentato dal `CCmdUI` oggetto.  
...
##  <a name="m_pmenu"></a>  CCmdUI::m_pMenu  
 Puntatore (del `CMenu` tipo) per il menu rappresentato dal `CCmdUI` oggetto.  
...
### <a name="remarks"></a>Note  
 **NULL** se l'elemento non è un menu. Se una finestra popup, il menu di sub *m_nID* contiene l'ID del primo elemento del menu a comparsa. Per altre informazioni, vedere [21 Nota tecnica](../../mfc/tn021-command-and-message-routing.md).  
  
##  <a name="m_pother"></a>  CCmdUI::m_pOther  
 Puntatore (di tipo `CWnd`) per l'oggetto finestra, ad esempio una barra degli strumenti o stato, che ha inviato la notifica.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro funziona per voci di menu e pulsanti della barra degli strumenti. Lo stato indeterminato si applica solo ai pulsanti della barra degli strumenti.  
...
### <a name="remarks"></a>Note  
 Questa funzione membro funziona esattamente come `SetCheck`, ad eccezione del fatto che opera sugli elementi dell'interfaccia utente che agisce come parte di un gruppo di opzione. Se si deseleziona gli altri elementi nel gruppo non è automatica, a meno che gli elementi stessi mantengono il comportamento del gruppo di opzione.  
