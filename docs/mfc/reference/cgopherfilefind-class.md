---
description: 'Altre informazioni su: classe CGopherFileFind'
...
Inoltre, se utilizzato con `CGopherFileFind` , la `CFileFind` funzione membro è [](../../mfc/reference/cfilefind-class.md#isdots) sempre false.

Per ulteriori informazioni sull'utilizzo di `CGopherFileFind` e sulle altre classi WinInet, vedere l'articolo [programmazione Internet con WinInet](../../mfc/win32-internet-extensions-wininet.md).
...
Dopo `FindFile` la chiamata a per recuperare il primo oggetto Gopher, è possibile chiamare [FindNextFile](#findnextfile) per recuperare i file Gopher successivi.
...
Diverso da zero se ha esito positivo; 0 in caso di esito negativo. `GetCreationTime` restituisce 0 solo se [FindNextFile](#findnextfile) non è mai stato chiamato su questo `CGopherFileFind` oggetto.
...
Diverso da zero se ha esito positivo; 0 in caso di esito negativo. `GetLastAccessTime` restituisce 0 solo se [FindNextFile](#findnextfile) non è mai stato chiamato su questo `CGopherFileFind` oggetto.
...
Diverso da zero se ha esito positivo; 0 in caso di esito negativo. `GetLastWriteTime` restituisce 0 solo se [FindNextFile](#findnextfile) non è mai stato chiamato su questo `CGopherFileFind` oggetto.
