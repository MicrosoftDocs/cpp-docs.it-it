...
|[CSliderCtrl::GetChannelRect](#getchannelrect)|Recupera le dimensioni del canale del controllo dispositivo di scorrimento.|  
...
|[CSliderCtrl::GetThumbRect](#getthumbrect)|Recupera la dimensione della casella di scorrimento del controllo dispositivo di scorrimento.|  
...
## <a name="remarks"></a>Note  
 ...
 Questo controllo (e pertanto il `CSliderCtrl` classe) è disponibile solo per i programmi in esecuzione in Windows 95/98 e Windows NT versione 3.51 e successive.  
 ...
 *bRedraw*  
 Ridisegna flag. Se questo parametro è **TRUE**, il dispositivo di scorrimento viene ridisegnato dopo la selezione è deselezionata; in caso contrario, il dispositivo di scorrimento non venga aggiornato.  
...
### <a name="parameters"></a>Parametri  
 *bRedraw*  
 Ridisegna flag. Se questo parametro è **TRUE**, il dispositivo di scorrimento viene ridisegnato dopo che sono stati cancellati i segni di graduazione; in caso contrario, il dispositivo di scorrimento non venga aggiornato.  
 ...
 *Rect*  
 Specifica dimensioni e la posizione del controllo dispositivo di scorrimento. Può essere un [CRect](../../atl-mfc-shared/reference/crect-class.md) oggetto o una [RECT](http://msdn.microsoft.com/library/windows/desktop/dd162897) struttura.  
...
### <a name="remarks"></a>Note  
 ...
 A seconda dei valori impostati per `dwStyle`, controllo dispositivo di scorrimento può avere un orientamento verticale o orizzontale. Può avere i segni di graduazione su entrambi i lati, entrambi i lati o nessuno dei due. E può essere utilizzato anche per specificare un intervallo di valori consecutivi.  
  
 Per applicare stili finestra estesi per il controllo dispositivo di scorrimento, chiamare [CreateEx](#createex) anziché `Create`.  
...
- **TRUE** recupera l'handle per il controllo correlato a sinistra del dispositivo di scorrimento. Se il controllo dispositivo di scorrimento Usa il `TBS_VERT` stile, il messaggio verrà recuperato il contatto sopra il cursore.  
  
- **FALSE** recupera l'handle per il controllo correlato a destra del dispositivo di scorrimento. Se il controllo dispositivo di scorrimento Usa il `TBS_VERT` stile, il messaggio recupererà buddy sotto il cursore.  
  
### <a name="return-value"></a>Valore restituito  
 Un puntatore a un [CWnd](../../mfc/reference/cwnd-class.md) finestra buddy nella posizione specificata dall'oggetto `fLocation`, oppure **NULL** se nessuna finestra buddy presente in tale percorso.  
...
### <a name="remarks"></a>Note  
 Le dimensioni di riga influisce sulla quantità il dispositivo di scorrimento viene spostato per il **TB_LINEUP** e **TB_LINEDOWN** le notifiche. L'impostazione predefinita per le dimensioni di riga è 1.  
...
### <a name="remarks"></a>Note  
 Le dimensioni della pagina influisce sulla quantità il dispositivo di scorrimento viene spostato per il **TB_PAGEUP** e **TB_PAGEDOWN** le notifiche.  
...
### <a name="remarks"></a>Note  
 Questa funzione copia i valori in interi fa *nMin* e *nMax*.  
...
### <a name="return-value"></a>Valore restituito  
 Un puntatore a un [CToolTipCtrl](../../mfc/reference/ctooltipctrl-class.md) oggetto, o **NULL** se le descrizioni comandi non sono in uso. Se il controllo dispositivo di scorrimento non usa il **TBS_TOOLTIPS** stile, il valore restituito è **NULL**.  
...
- **TRUE** il contatto verrà visualizzato a sinistra del controllo trackbar se il controllo trackbar Usa il `TBS_HORZ` stile. Se il controllo trackbar viene utilizzato il `TBS_VERT` applicare uno stile, il contatto viene visualizzato sopra il controllo trackbar.  
  
- **FALSE** il contatto verrà visualizzato a destra del controllo trackbar se il controllo trackbar Usa il `TBS_HORZ` stile. Se il controllo trackbar viene utilizzato il `TBS_VERT` applicare uno stile, il contatto viene visualizzato sotto il controllo trackbar.  
  
### <a name="return-value"></a>Valore restituito  
 Un puntatore a un [CWnd](../../mfc/reference/cwnd-class.md) oggetto che in precedenza è stato assegnato al controllo dispositivo di scorrimento in tale posizione.  
  
### <a name="remarks"></a>Note  
 Questa funzione membro implementa il comportamento del messaggio Win32 [TBM_SETBUDDY](http://msdn.microsoft.com/library/windows/desktop/bb760213), come descritto in Windows SDK. Si noti che questa funzione membro utilizza i puntatori a `CWnd` oggetti invece di handle di finestra per il valore restituito e parametri.  
...
### <a name="remarks"></a>Note  
 Le dimensioni di riga influisce sulla quantità il dispositivo di scorrimento viene spostato per il **TB_LINEUP** e **TB_LINEDOWN** le notifiche.  
 ...
 *bRedraw*  
 Il flag di aggiornamento. Se questo parametro è **TRUE**, il dispositivo di scorrimento viene ridisegnato dopo l'intervallo viene impostato; in caso contrario, il dispositivo di scorrimento non venga aggiornato.  
...
### <a name="remarks"></a>Note  
 Questo metodo richiede che il controllo trackbar sia impostato su [TBS_FIXEDLENGTH](http://msdn.microsoft.com/library/windows/desktop/bb760147) stile.  
...
### <a name="example"></a>Esempio  
 Esempio di codice seguente definisce la variabile, `m_sliderCtrl`, che viene utilizzato per accedere al controllo trackbar corrente. Nell'esempio viene definita anche una variabile, `thumbLength`, che viene utilizzata per archiviare la lunghezza predefinita del componente casella di scorrimento del controllo trackbar. Nell'esempio seguente vengono utilizzate queste variabili.  
  
 [!code-cpp[NVC_MFC_CSliderCtrl_s1#1](../../mfc/reference/codesnippet/cpp/csliderctrl-class_1.h)]  
  
### <a name="example"></a>Esempio  
 Esempio di codice seguente imposta componente casella di scorrimento del controllo trackbar due volte la lunghezza predefinita.  
...
### <a name="remarks"></a>Note  
 Ad esempio, se la frequenza è impostata su 2, un segno di graduazione viene visualizzato per ogni altro incremento nell'intervallo del dispositivo di scorrimento. L'impostazione predefinita per la frequenza è 1 (ovvero, ogni incremento nell'intervallo è associato a un segno di graduazione).  
  
 È necessario creare il controllo con il `TBS_AUTOTICKS` stile da utilizzare per questa funzione. Per altre informazioni, vedere [CSliderCtrl::Create](#create).  
...
### <a name="parameters"></a>Parametri  
 *Nposizione*  
 Valore che rappresenta la posizione in cui si desidera visualizzare il controllo descrizione comando. Per un elenco di valori possibili, vedere il messaggio Win32 [TBM_SETTIPSIDE](http://msdn.microsoft.com/library/windows/desktop/bb760240), come descritto in Windows SDK.  
  
### <a name="return-value"></a>Valore restituito  
 Un valore che rappresenta una posizione precedente del controllo tooltip. Il valore restituito è uguale a uno dei possibili valori per *Nposizione*.  
  
### <a name="remarks"></a>Note  
 Questa funzione membro implementa il comportamento del messaggio Win32 **TBM_SETTIPSIDE**, come descritto in Windows SDK. Dispositivo di scorrimento controlli che utilizzano le **TBS_TOOLTIPS** stile descrizioni comando visualizzato. Per una descrizione degli stili del controllo dispositivo di scorrimento, vedere [stili del controllo Trackbar](http://msdn.microsoft.com/library/windows/desktop/bb760147) in Windows SDK.  
