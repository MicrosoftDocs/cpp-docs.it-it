...
|[CDaoDatabase:: CanUpdate](#canupdate)|Restituisce un valore diverso da zero se l' `CDaoDatabase` oggetto è aggiornabile (non di sola lettura).|
|[CDaoDatabase:: Close](#close)|Chiude la connessione al database.|
|[CDaoDatabase:: create](#create)|Crea l'oggetto di database DAO sottostante e Inizializza l' `CDaoDatabase` oggetto.|
...
|[CDaoDatabase:: Execute](#execute)|Esegue una query di azione. Se si chiama `Execute` per una query che restituisce risultati, viene generata un'eccezione.|
|[CDaoDatabase:: GetConnect](#getconnect)|Restituisce la stringa di connessione utilizzata per connettere l' `CDaoDatabase` oggetto a un database. Utilizzato per ODBC.|
...
Se la rete supporta la convenzione di denominazione uniforme (UNC), è anche possibile specificare un percorso di rete, ad esempio " \\ \\ \\ \MYSERVER \\ \MYSHARE \\ \mydir \\ \MYDB. MDB ". (Le barre rovesciate doppie sono obbligatorie nei valori letterali stringa perché " \\ " è il carattere di escape C++).
