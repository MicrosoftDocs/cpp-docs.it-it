...
- **Esito negativo**: chiamato dopo la reimpostazione di una finestra di messaggio che avverte l'utente all'errore di input. Questa routine ripristinerà lo stato attivo per l'ultimo controllo (l'ultima chiamata a `PrepareCtrl` / `PrepareEditCtrl`) e genera un'eccezione. Questa funzione membro può essere chiamata da routine sia _ e DDV _.  
...
```  
 ...
 \<promptX >  
 stringa da inserire sopra la voce di modifica (& per tasti di scelta rapida)  
  
 \<fmtX >  
 formato carattere per il tipo di argomento, uno dei  
  
 1!d = int  
  
 u = senza segno  
  
 1!d = long int (vale a dire, long)  
