...
```  
 ...
 In MFC/OLE1 `COleClientItem::GetBounds` e `SetBounds` utilizzati per eseguire una query e modificare l'estensione di un elemento (il **sinistro** e **top** membri sono stati sempre zero). In MFC/OLE 2, in questo modo più diretto è supportato in `COleClientItem::GetExtent` e `SetExtent`, che gestiscono un **dimensioni** o `CSize` invece.  
