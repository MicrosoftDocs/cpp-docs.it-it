---
title: 'Contenitori di controlli ActiveX: Gestione degli eventi da un controllo ActiveX | Microsoft Docs'
ms.custom: ''
ms.date: 09/12/2018
ms.technology:
- cpp-mfc
...
3.  Nella finestra Proprietà, selezionare l'ID di controllo del controllo ActiveX incorporato. In questo esempio Usa `IDC_CIRCCTRL1`.  
...
4.  Selezionare l'evento che si vuole che la classe di finestra di dialogo per la gestione. Per questo esempio, selezionare **fare clic su**.  
  
5.  Dalla casella di riepilogo a destra, selezionare  **\<Aggiungi > ClickCircctrl1**.  
  
6.  Fare doppio clic sulla nuova funzione di gestione dalla visualizzazione di classi a cui passare il codice del gestore eventi nell'implementazione (. File CPP) di `CContainerDlg`.  
