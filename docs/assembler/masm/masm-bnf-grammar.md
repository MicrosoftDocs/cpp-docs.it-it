---
title: Microsoft Macro Assembler BNF Grammatica
...
ms.locfileid: "81317971"
---
# <a name="microsoft-macro-assembler-bnf-grammar"></a>Microsoft Macro Assembler BNF Grammatica

Questa pagina contiene una descrizione BNF della grammatica MASM. È fornito come supplemento al riferimento e non è garantito per essere completo. Consultare il riferimento per informazioni complete su parole chiave, parametri, operazioni e così via.

Per illustrare l'utilizzo del BNF, nel diagramma seguente viene illustrata *`typedefDir`* la definizione della direttiva TYPEDEF, a partire dalla direttiva non terminale .

![Esempio MASM BNF](media/bnf.png)

Le voci sotto ogni parentesi graffa **`NEAR16`** orizzontale sono terminali, ad esempio , **`NEAR32`**, **`FAR16`** e **`FAR32`**. In alternativa, sono non terminali *`qualifiedType`* *`distance`* come *`qualifier`* *`protoSpec`* , , e che possono essere ulteriormente definiti. Ogni non terminale *`typedefDir`* in corsivo nella definizione è anche una voce nel BNF. Tre punti verticali indicano una definizione di ramificazione per un non terminale che, per semplicità, questa figura non illustra.

La grammatica BNF consente definizioni ricorsive. Ad esempio, la grammatica utilizza qualifiedType come possibile definizione per qualifiedType, che è anche un componente della definizione per il qualificatore. Il simbolo "- " specifica una scelta *`endOfLine`*  |  *`comment`* tra espressioni alternative, ad esempio . Le parentesi graffe doppie specificano un parametro facoltativo, ad esempio . *`macroParmList`* Le parentesi non vengono effettivamente visualizzate nel codice sorgente.
...
*`character`*\
&nbsp;&nbsp;&nbsp;&nbsp;Qualsiasi carattere con ordinale compreso nell'intervallo da 0 a 255 ad eccezione dell'avanzamento riga (10).
...
*`delimiter`*\
&nbsp;&nbsp;&nbsp;&nbsp;Qualsiasi carattere tranne*`whiteSpaceCharacter`*
...
*`id`*\
&nbsp;&nbsp;&nbsp;&nbsp;Il primo carattere dell'identificatore può essere un carattere alfabetico maiuscolo o minuscolo (`[A–Za-z]`) o uno di questi quattro caratteri: `@ _ $ ?` i caratteri rimanenti possono essere uno qualsiasi degli stessi caratteri o una cifra decimale (`[0–9]`). La lunghezza massima è 247 caratteri.
...
*`stringChar`*\
&nbsp;&nbsp;&nbsp;&nbsp;*`quote`**`quote`* | Qualsiasi carattere tranne virgolette.
...
*`text`*\
&nbsp;&nbsp;&nbsp;&nbsp;*`textLiteral`* | *`text`* proprietà character **`!`** *`character`* *`text`* | *`character`* | **`!`***`character`*
