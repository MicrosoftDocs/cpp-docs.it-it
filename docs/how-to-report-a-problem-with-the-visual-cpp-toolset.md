...
2.  Dall'elenco delle corrispondenze scegliere la versione del **prompt dei comandi per gli sviluppatori** che corrisponde alla versione di Visual Studio usata.  
  
3.  Nella console **Prompt dei comandi per gli sviluppatori** immettere il comando `cl /Bv /CLR`.  
...
2.  Individuare la riga che contiene il nome del file del codice sorgente in cui si verifica il problema. La riga sottostante contiene il comando cl.exe corrispondente e i relativi argomenti.  
...
#### <a name="frontend-parser-crash"></a>Arresto anomalo del front-end (parser)  
 ...
 Per questo tipo di arresto anomalo, inviare una [procedura di riproduzione pre-elaborata](#preprocessed_repro).  
...
####  <a name="backend_crash"></a> Arresto anomalo del back-end (generazione del codice)  
 ...
 Di seguito è riportato un esempio di output del compilatore per questo tipo di arresto anomalo in cui la Generazione codice in fase di collegamento **non** è in uso. Se l'output del compilatore è simile al seguente è necessario inviare una [procedura di riproduzione pre-elaborata](#preprocessed_repro).  
...
```  
 ...
 Se la riga che inizia con **INTERNAL COMPILER ERROR** specifica link.exe anziché cl.exe, significa che la Generazione codice in fase di collegamento è stata abilitata ed è necessario inviare una [procedura di riproduzione del collegamento](#link_repro). Se dal messaggio di errore del compilatore non è chiaro se la Generazione codice in fase di collegamento è stata abilitata, è possibile esaminare gli argomenti della riga di comando copiata dal log di compilazione nel passaggio precedente per l'argomento della riga di comando `/GL`.  
...
>  Se invece l'output specifica C1001 o include la Generazione codice in fase di collegamento, fare riferimento a [Arresto anomalo del back-end (generazione del codice)](#backend_crash) per altre informazioni.  
 ...
 Per questo tipo di arresto anomalo, inviare una [procedura di riproduzione del collegamento](#link_repro).  
...
### <a name="file-a-bug-on-microsoft-connect"></a>Segnalare un bug in Microsoft Connect  
 Microsoft Connect ([connect.microsoft.com](http://connect.microsoft.com)) è uno strumento a disposizione della community degli utenti che consente di mettersi direttamente in contatto con i team che compilano i prodotti Microsoft. In Connect è possibile segnalare nuovi bug e inoltrare richieste di funzionalità, visualizzare altri bug e richieste inoltrati dalla community e segnalare i più rilevanti.  
  
 Se si vuole condividere il report con la community di Visual Studio e monitorarne l'avanzamento dello stesso, la segnalazione del problema tramite Connect è la scelta migliore. Condividendo infatti il report, succede a volte che altri utenti suggeriscano soluzioni alternative o forniscano informazioni aggiuntive che favoriscono la diagnosi del problema. È comunque possibile usare Connect anche se si vuole mantenere privato il report, ad esempio quando non si può condividere il codice nel report. In questo caso è importante però controllare che il report sia visibile privatamente prima di inviare il modulo.  
  
-   [Microsoft Connect: segnalare un problema con Visual Studio 2015 Update 3](https://connect.microsoft.com/VisualStudio/Feedback/LoadSubmitFeedbackForm?FormID=6493)  
  
-   [Microsoft Connect: segnalare un problema con Visual Studio 2015 Update 2](https://connect.microsoft.com/VisualStudio/Feedback/LoadSubmitFeedbackForm?FormID=6406)  
  
-   [Microsoft Connect: segnalare un problema con Visual Studio 2015 Update 1](https://connect.microsoft.com/VisualStudio/Feedback/LoadSubmitFeedbackForm?FormID=6326)  
  
-   [Microsoft Connect: segnalare un problema con Visual Studio 2015](https://connect.microsoft.com/VisualStudio/Feedback/LoadSubmitFeedbackForm?FormID=6240)  
  
 È possibile segnalare un problema con altri prodotti Visual Studio e .Net Framework scegliendo il prodotto nell'elenco a discesa della pagina dei [commenti e dei suggerimenti su Visual Studio e .NET Framework](https://connect.microsoft.com/VisualStudio/feedback/LoadSubmitFeedbackForm) di Connect.  
...
### <a name="send-an-email"></a>Inviare un messaggio di posta elettronica  
 ...
 Segnalando il problema tramite posta elettronica non si coinvolge la vasta community di Microsoft Connect, ma a volte può essere la soluzione preferibile per inviare procedure di riproduzione estese. Può anche essere l'opzione migliore o l'unica se l'ambiente di lavoro non è connesso a Internet o non si ha la possibilità di usare Microsoft Connect.  
...
3.  Nella finestra della console **Prompt dei comandi per gli sviluppatori** immettere il comando `cl /P argumentsfilename.cpp`.  
...
3.  Nella finestra della console **Prompt dei comandi per gli sviluppatori** immettere il comando `cl arguments /TP filename.i`.  
  
4.  Verificare che il problema viene riprodotto.  
...
2.  Impostare la variabile di ambiente link_repro nella directory appena creata. Immettere il comando `set link_repro=directory`.  
  
3.  Se si vuole eseguire la compilazione da Visual Studio, avviarlo dal prompt dei comandi immettendo il comando `devenv`. In questo modo il valore della variabile di ambiente link_repro sarà visibile in Visual Studio.  
  
4.  Compilare l'applicazione e confermare che il problema previsto si è verificato.  
  
5.  Chiudere ora Visual Studio se è stato avviato nel passaggio 3.  
  
6.  Cancellare la variabile di ambiente link_repro. Immettere il comando `set link_repro=`  
