...
Vettore minore o uguale confronto.  
...
## Note  
 La funzione di operatore restituisce `!(``right` `<` `left``)`.  È possibile utilizzarlo per verificare se `left` non è stato ordinato dopo `right` quando i due vettori sono elemento confrontata dall'elemento.  
