...
ms.locfileid: "46428369"
---
# <a name="how-to-iterate-over-arrays-with-for-each"></a>Procedura: scorrere le matrici con for each

Questo argomento viene illustrato come usare il [per ognuna, in](../dotnet/for-each-in.md) (parola chiave) su diversi tipi di matrici.

## <a name="example"></a>Esempio

In questo esempio viene illustrato come utilizzare `for each` nella matrice di tipi di riferimento.  Si noti che se qualsiasi dimensione di una matrice dimensionale multipla è uguale a zero, il `for each` ciclo non scorre la matrice.
...
In questo esempio mostra per ogni iterazione su un <xref:System.Collections.ArrayList>, che implementa l'interfaccia <xref:System.Collections.IEnumerable>.
