...
Confronto di tipo uguale o maggiore di.  
...
## Note  
 La funzione operatore restituisce `!(``left` `<` `right``)`.  Utilizzarla per verificare se `left` non viene ordinato prima `right` quando le due coppie vengono confrontate elemento per elemento.  
