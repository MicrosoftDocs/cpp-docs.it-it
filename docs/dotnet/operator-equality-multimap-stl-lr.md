...
## Note  
 La funzione operatore restituisce true solo se le sequenze controllate da `left` e `right` hanno la stessa lunghezza e per ogni posizione `i`, `left``[i] ==` `right``[i]`.  Utilizzarla per verificare se `left` viene ordinato allo stesso modo di `right` quando i due multimaps vengono confrontati elemento per elemento.  
