...
## Note  
 La funzione di operatore restituisce `!(``left` `<` `right``)`.  Utilizzarla per verificare se `left` non è stato ordinato prima di `right` quando le due coppie sono elemento confrontata dall'elemento.  
