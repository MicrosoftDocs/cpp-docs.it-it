...
## Note  
 La funzione membro restituisce `(float)`[hash\_multiset::size](../dotnet/hash-multiset-size-stl-clr.md)`() /` [hash\_multiset::bucket\_count](../dotnet/hash-multiset-bucket-count-stl-clr.md)`()`.  Utilizzarla per determinare la dimensione media del bucket.  
