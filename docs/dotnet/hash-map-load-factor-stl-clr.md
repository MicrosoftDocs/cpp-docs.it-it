...
## Note  
 La funzione membro restituisce `(float)`[hash\_map::size](../dotnet/hash-map-size-stl-clr.md)`() /` [hash\_map::bucket\_count](../dotnet/hash-map-bucket-count-stl-clr.md)`()`.  Utilizzarla per determinare la dimensione media del bucket.  
