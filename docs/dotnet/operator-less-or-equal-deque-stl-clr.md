...
## Note  
 La funzione di operatore restituisce `!(``right` `<` `left``)`.  È possibile utilizzarlo per verificare se `left` non è stato ordinato dopo `right` quando i due deques sono elemento confrontata dall'elemento.  
