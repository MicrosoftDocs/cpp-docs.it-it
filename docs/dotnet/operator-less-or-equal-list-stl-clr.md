...
## Note  
 La funzione di operatore restituisce `!(``right` `<` `left``)`.  Utilizzarla per verificare se `left` non è stato ordinato dopo `right` quando i due elenchi sono elemento confrontata dall'elemento.  
