...
Accodare maggiore del confronto.  
...
## Note  
 La funzione di operatore restituisce `right` `<` `left`.  È possibile utilizzarlo per verificare se `left` viene ordinato dopo `right` quando le due code sono elemento confrontata dall'elemento.  
