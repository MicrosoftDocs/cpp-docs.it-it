...
## Note  
 La funzione di operatore restituisce `!(``right` `<` `left``)`.  Utilizzarla per verificare se `left` non è stato ordinato dopo `right` quando le due mappe vengono elemento confrontata dall'elemento.  
