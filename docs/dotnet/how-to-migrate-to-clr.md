...
ms.locfileid: "70311712"
---
# <a name="how-to-migrate-to-clr"></a>Procedura: Esegui la migrazione a/CLR

In questo argomento vengono illustrati i problemi che si verificano durante la compilazione di codice nativo con **/CLR** (vedere [/CLR (compilazione Common Language Runtime)](../build/reference/clr-common-language-runtime-compilation.md) per ulteriori informazioni. **/CLR** consente a C++ codice nativo di richiamare e richiamare da assembly .NET oltre ad altro codice nativo C++ . Per ulteriori informazioni sui vantaggi della compilazione con **/CLR**, vedere [assembly misti (nativi e gestiti)](../dotnet/mixed-native-and-managed-assemblies.md) e [interoperabilità .NET e nativa](../dotnet/native-and-dotnet-interoperability.md) .
...
## <a name="compile-with-visual-c"></a>Compila con VisualC++
...
Se si esegue l'aggiornamento a Visual Studio 2010 da una versione precedente a Visual Studio 2003, è possibile che vengano visualizzati errori del compilatore C++ correlati alla conformità standard avanzata in visual Studio 2003
...
### <a name="upgrading-from-managed-extensions-for-c"></a>Aggiornamento da estensioni gestite perC++

A partire da Visual Studio 2005, il codice scritto con le C++ estensioni gestite per non verrà compilato in **/CLR**.

## <a name="convert-c-code-to-c"></a>Convertire il codice C inC++

Sebbene Visual Studio compili i file C, è necessario convertirli in C++ per una compilazione **/CLR** . Non è necessario modificare il nome file effettivo; è possibile usare **/TP** (vedere [/TC,/TP,/TC,/TP (specifica il tipo di file di origine)](../build/reference/tc-tp-tc-tp-specify-source-file-type.md). Si noti che C++ anche se i file di codice sorgente sono necessari per **/CLR**, non è necessario eseguire il refactoring del codice per usare i paradigmi orientati a oggetti.

È molto probabile che il codice C richieda modifiche quando viene C++ compilato come file. Le C++ regole di indipendenza dai tipi sono rigorose, pertanto le conversioni di tipi devono essere rese esplicite con i cast. Ad esempio, malloc restituisce un puntatore void, ma può essere assegnato a un puntatore a qualsiasi tipo in C con un cast:
...
Anche i puntatori a funzione sono rigorosamente indipendenti C++dai tipi in, quindi il codice C seguente richiede la modifica. In C++ è preferibile creare un `typedef` oggetto che definisce il tipo di puntatore a funzione e quindi usare tale tipo per eseguire il cast dei puntatori a funzione:
...
C++richiede anche che le funzioni siano prototipi o completamente definite prima che sia possibile farvi riferimento o richiamarle.

Gli identificatori usati nel codice C che si verificano come C++ parole chiave (ad esempio, **Virtual**, **New**, **Delete**, **bool**, **true**, **false**e così via) devono essere rinominati. Questa operazione può essere eseguita in genere con semplici operazioni di ricerca e sostituzione.
...
`__controlfp`non è supportato nella Common Language Runtime (per altre informazioni, vedere [_control87 \_, _controlfp, _control87_2](../c-runtime-library/reference/control87-controlfp-control87-2.md) ). CLR non rispetta inoltre l' [allineamento](../cpp/align-cpp.md).
...
È possibile che si verifichi una riduzione delle C++ prestazioni quando i metodi nativi generati per MSIL vengono chiamati indirettamente (chiamate di funzioni virtuali o con puntatori a funzione). Per ulteriori informazioni, vedere [doppio thunk](../dotnet/double-thunking-cpp.md).
...
In alcuni casi, CLR può essere arrestato prima del completamento dell'esecuzione del codice gestito. L' `std::set_terminate` uso `SIGTERM` di e può causare questa operazione. Per ulteriori informazioni, vedere [costanti Signal](../c-runtime-library/signal-constants.md) e [set_terminate](../c-runtime-library/abnormal-termination.md) .

## <a name="using-new-visual-c-features"></a>Uso di nuove C++ funzionalità visive
...
Per informazioni sulla programmazione .NET in visuale C++ , vedere:
