...
## Note  
 La funzione di operatore restituisce `!(``left` `<` `right``)`.  È possibile utilizzarlo per verificare se `left` non è stato ordinato prima di `right` quando i due multi\-insieme sono elemento confrontata dall'elemento.  
