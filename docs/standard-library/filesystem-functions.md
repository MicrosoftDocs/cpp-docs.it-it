...
Se `!exists(f) || equivalent(f, t) || is_other(f) || is_other(t) || is_directory(f)&& is_regular_file(t)`, viene quindi segnalato un errore (e non viene eseguita alcuna operazione).

In caso contrario, se `is_symlink(f)`:

- Se `options & copy_options::skip_symlinks`, non eseguire alcuna operazione.

- In caso contrario, se `!exists(t)&& options & copy_options::copy_symlinks`, `copy_symlink(from, to, opts)`.

- In caso contrario, segnalare un errore.

In caso contrario, se `is_regular_file(f)`:

- Se `opts & copy_options::directories_only`, non eseguire alcuna operazione.

- In caso contrario, se `opts & copy_options::create_symlinks`, `create_symlink(to, from)`.

- In caso contrario, se `opts & copy_options::create_hard_links`, `create_hard_link(to, from)`.
...
Tutte le funzioni eventualmente copiano il file in da *a sotto il controllo di* *opz*, che viene utilizzato come `copy_options::none` per gli overload senza parametro *opz* . gli *opz* devono contenere al massimo uno dei `skip_existing`, `overwrite_existing`o `update_existing`.

Se `exists(to) && !(opts & (copy_options::skip_existing | copy_options::overwrite_existing | copy_options::update_existing))`, segnalare come un errore che il file esiste già.
...
Per un percorso, ad esempio un\/b\/c, la funzione crea le directory a e\/b in base alle esigenze, in modo da poter creare la directory a\/b\/c in base alle esigenze. Restituisce **true** solo se crea effettivamente la directory *pval*.
...
Le funzioni impostano le autorizzazioni per il percorso scelto da *pval* per `mask & perms::mask` sotto il controllo della `perms & (perms::add_perms | perms::remove_perms)`. *mask* deve contenere al massimo uno dei `perms::add_perms` e `perms::remove_perms`.
...
Le funzioni restituiscono **true** solo se `exists(symlink_status(pval))` e il file viene rimosso correttamente. Viene rimosso un collegamento simbolico, non il file scelto.
...
Le funzioni restituiscono lo stato del collegamento simbolico del percorso, il tipo di file e le autorizzazioni associate a *pval*. Le funzioni si comportano come `status(pval)` ad eccezione del fatto che viene testato un collegamento simbolico, non il file scelto.
...
La prima funzione si comporta come `path(source)` e la seconda funzione si comporta come `path(first, last)` ad eccezione del fatto che l'origine scelta in ogni caso viene considerata come una sequenza di elementi char codificata come UTF-8, indipendentemente dal file System.
