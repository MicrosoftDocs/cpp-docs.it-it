...
In caso contrario, se `!exists(to) || opts & copy_options::overwrite_existing || opts & copy_options::update_existing&& last_write_time(to) < last_write_time(from) || !(opts & (copy_options::skip_existing | copy_options::overwrite_existing | copy_options:update_existing))`, quindi tentare di copiare il contenuto e gli attributi del file *dalla* nel file *a*. Segnalare come errore se il tentativo di copia non riesce.
...
Per un percorso, ad esempio un\/b\/c, la funzione crea le directory e una\/b in base alle esigenze, in modo da poter creare la directory di una\/b\/c come richiesto. Viene restituito **true** solo se effettivamente crea la directory *pval*.
...
Le funzioni restituiscono **true** solo se *a sinistra* e *destro* scegliere la stessa entità di file System.
...
Le funzioni restituiscono le dimensioni in byte del file scelto dal *pval*, se `exists(pval) && is_regular_file(pval)` ed è possono determinare le dimensioni del file. In caso contrario si segnala un errore e restituiscono `uintmax_t(-1)`.
...
Le funzioni impostano le autorizzazioni per il percorso scelto dal *pval* al `mask & perms::mask` sotto il controllo di `perms & (perms::add_perms | perms::remove_perms)`. *maschera* deve contenere al massimo uno tra `perms::add_perms` e `perms::remove_perms`.
...
## <a name="proximate"></a> nelle vicinanze
...
Le funzioni restituiscono **true** solo se `exists(symlink_status(pval))` e il file è stato rimosso correttamente. Un collegamento simbolico viene rimosso, non il file viene scelto.
...
Le funzioni rinominano *dal* al *a*. Un collegamento simbolico viene rinominato, non il file viene scelto.
...
La funzione restituisce informazioni sul volume scelto dal *pval*, in una struttura di tipo `space_info`. La struttura contiene `uintmax_t(-1)` per qualsiasi valore che non può essere determinato.
...
Le funzioni restituiscono lo stato del percorso, il tipo di file e le autorizzazioni, associati *pval*. Un collegamento simbolico non viene testato, ma il file viene scelto.
...
Le funzioni restituiscono lo stato del collegamento simbolico percorso, il tipo di file e le autorizzazioni, associati *pval*. Le funzioni si comportano allo stesso come `status(pval)` ad eccezione del fatto che viene testato un collegamento simbolico, il file non viene scelto.
...
La prima funzione si comporta come `path(source)` e la seconda funzione si comporta come `path(first, last)` ad eccezione del fatto che l'origine selezionata in ogni caso viene eseguita come una sequenza di elementi char codificata come UTF-8, qualunque sia il file System.
