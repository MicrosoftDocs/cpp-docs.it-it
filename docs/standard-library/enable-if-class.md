...
```  
 ...
 Lo scenario 2 funziona anche per i costruttori normali.  Non è invece adatto agli operatori di conversione, che non accettano parametri supplementari.  Non funziona neanche per i costruttori [variadic](../cpp/ellipses-and-variadic-templates.md) perché con l'aggiunta di parametri supplementari il pacchetto di parametri di funzione diventa un contesto non dedotto e quindi lo scopo di `enable_if` è vanificato.  
...
-   Non usare `enable_if` per applicare requisiti.  Se si vogliono convalidare i parametri modello e, in caso di esito negativo della convalida, fare in modo che venga generato un errore anziché selezionare un'altra implementazione, usare [static\_assert](../cpp/static-assert.md).  
...
## Esempio  
 In questo esempio viene spiegato come la funzione modello STL [std::make\_pair\(\)](../Topic/make_pair.md) sfrutta `enable_if`.  
