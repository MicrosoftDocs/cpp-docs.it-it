...
`right` Oggetto `basic_istream` oggetto da copiare.
...
`count` Il numero di caratteri da leggere dalla `strbuf`.

`Delim` Il carattere che deve terminare la lettura se viene raggiunta prima `count`.

`str` Una stringa in cui scrivere.

`Ch` Carattere da ottenere.
...
`count` Il numero di elementi da ignorare dalla posizione di lettura corrente.

`Delim` L'elemento che, se rilevati prima di conteggio, causa **ignorare** per restituire e consentendo di tutti gli elementi dopo `Delim` da leggere.
...
`val` Il valore per la lettura dal flusso.
...
Il \<istream > intestazione definisce anche diversi operatori di estrazione globale. Per altre informazioni, vedere [operator>> (\<istream>)](../standard-library/istream-operators.md#op_gt_gt).
...
`right` Un' `rvalue` fanno riferimento a un `basic_ifstream` oggetto.
...
`Ch` Carattere da riportato indietro nel flusso.
...
`str` Matrice in cui leggere i caratteri.

`count` Il numero di caratteri da leggere.
...
`str` La matrice in cui `readsome` archivia i caratteri letti.
...
`pos` La posizione assoluta in cui si desidera spostare il puntatore di lettura.

`off` Un offset per spostare il puntatore relativo alla lettura `way`.

`way` Uno del [ios_base:: seekdir](../standard-library/ios-base-class.md#seekdir) enumerazioni.
...
sentry classe {pubblico: sentry esplicita (basic_istream\<Elem, Tr > & _Istr, bool _Noskip = false); bool() operator const;};
...
`right` Riferimento lvalue a un `basic_istream` oggetto.
