...
- Il metodo `try_lock` tenta di ottenere la proprietà del tipo mutex senza blocco. Il tipo restituito è convertibile in **bool** ed è **true** se il metodo ottiene la proprietà; in caso contrario, è **false**.
...
Un *tipo mutex programmato* soddisfa i requisiti per un tipo mutex. Include inoltre i `try_lock_for` metodi e `try_lock_until` che devono essere richiamabili tramite un argomento e devono restituire un tipo convertibile in **bool**. Un tipo di mutex programmato consente di definire queste funzioni con argomenti aggiuntivi, purché tali argomenti aggiuntivi dispongano tutti di valori predefiniti.
...
|[Struttura once_flag](../standard-library/once-flag-structure.md)|Rappresenta uno **struct** utilizzato con la funzione `call_once` di modello per garantire che il codice di inizializzazione venga chiamato una sola volta, anche in presenza di più thread di esecuzione.|
|[Struttura try_to_lock_t](../standard-library/try-to-lock-t-structure.md)|Rappresenta uno **struct** che definisce un `try_to_lock` oggetto e viene utilizzato per selezionare uno dei costruttori di overload di. `unique_lock`|
