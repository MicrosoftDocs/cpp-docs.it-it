...
-   Una classe modello, poiché le funzionalità che fornisce sono generiche e indipendenti dal tipo di elemento o di chiave. I tipi di dati utilizzati per gli elementi e le chiavi vengono specificati come parametri nella classe modello insieme alla funzione di confronto e all'allocatore.  
 ...
 L'oggetto map ordina gli elementi che controlla chiamando un oggetto funzione archiviato di tipo [key_compare](#map__key_compare). Questo oggetto archiviato è una funzione di confronto a cui si accede chiamando il metodo [key_comp](#map__key_comp). In genere, due elementi qualsiasi vengono confrontati per determinare se uno è minore dell'altro o se sono equivalenti. Una volta che tutti gli elementi sono stati confrontati, viene creata una sequenza ordinata di elementi non equivalenti.  
  
> [!NOTE]
>  La funzione di confronto è un predicato binario che provoca un ordinamento di tipo "strict weak" nel senso matematico standard. Un predicato binario                     f(x,y) è un oggetto funzione che dispone di due oggetti argomento                     x e                     y e di un valore restituito `true` o `false`. Un ordinamento imposto a un set è un ordinamento di tipo "strict weak" se il predicato binario è irriflessivo, antisimmetrico e transitivo e se l'equivalenza è transitiva, in cui due oggetti                     x and                     y vengono definiti equivalenti quando sia                     f(x,y) che                     f(y,x) sono `false`. Se la condizione di uguaglianza più forte tra le chiavi sostituisce quella di equivalenza, l'ordinamento diventa totale (nel senso che tutti gli elementi vengono ordinati l'uno rispetto all'altro) e le chiavi accoppiate saranno indistinguibili l'una dall'altra.  
>   
...
|||  
|-|-|  
|[map](#map__map)|Costruisce una lista di dimensioni specifiche con elementi di un valore specifico, con un `allocator` specifico o come copia di un'altra mappa.|  
  
### <a name="typedefs"></a>Typedef  
  
|||  
|-|-|  
|[allocator_type](#map__allocator_type)|typedef per la classe `allocator` dell'oggetto mappa.|  
|[const_iterator](#map__const_iterator)|typedef per un iteratore bidirezionale in grado di leggere un elemento `const` della mappa.|  
|[const_pointer](#map__const_pointer)|typedef per un puntatore a un elemento `const` di una mappa.|  
|[const_reference](#map__const_reference)|typedef per un riferimento a un elemento `const` archiviato in una mappa per la lettura e l'esecuzione di operazioni `const`.|  
|[const_reverse_iterator](#map__const_reverse_iterator)|Tipo che fornisce un iteratore bidirezionale in grado di leggere qualsiasi elemento `const` della mappa.|  
|[difference_type](#map__difference_type)|typedef Signed Integer per il numero di elementi di una mappa in un intervallo compreso tra gli elementi a cui puntano gli iteratori.|  
|[iterator](#map__iterator)|typedef per un iteratore bidirezionale in grado di leggere o di modificare qualsiasi elemento di una mappa.|  
|[key_compare](#map__key_compare)|typedef per un oggetto funzione in grado di confrontare due chiavi di ordinamento per determinare l'ordine relativo di due elementi della mappa.|  
|[key_type](#map__key_type)|typedef per la chiave di ordinamento archiviata in ogni elemento della mappa.|  
|[mapped_type](#map__mapped_type)|typedef per i dati archiviati in ogni elemento di una mappa.|  
|[pointer](#map__pointer)|typedef per un puntatore a un elemento `const` di una mappa.|  
|[reference](#map__reference)|typedef per un riferimento a un elemento archiviato in una mappa.|  
|[reverse_iterator](#map__reverse_iterator)|typedef per un iteratore bidirezionale in grado di leggere o di modificare un elemento di una mappa invertita.|  
|[size_type](#map__size_type)|typedef Unsigned Integer per il numero di elementi di una mappa|  
|[value_type](#map__value_type)|typedef per il tipo di oggetto archiviato come elemento di una mappa.|  
  
### <a name="member-functions"></a>Funzioni membro  
  
|||  
|-|-|  
|[at](#map__at)|Trova un elemento con un valore della chiave specificata.|  
|[begin](#map__begin)|Restituisce un iteratore che punta al primo elemento della mappa.|  
|[cbegin](#map__cbegin)|Restituisce un iteratore const che punta al primo elemento della mappa.|  
|[cend](#map__cend)|Restituisce un iteratore const successivo all'ultimo valore.|  
|[clear](#map__clear)|Cancella tutti gli elementi di una mappa.|  
|[count](#map__count)|Restituisce il numero di elementi di una mappa la cui chiave corrisponde alla chiave specificata in un parametro.|  
|[crbegin](#map__crbegin)|Restituisce un iteratore const che punta al primo elemento di una mappa invertita.|  
|[crend](#map__crend)|Restituisce un iteratore const che punta alla posizione successiva all'ultimo elemento di una mappa invertita.|  
|[emplace](#map__emplace)|Inserisce un elemento costruito sul posto nella mappa.|  
|[emplace_hint](#map__emplace_hint)|Inserisce un elemento costruito sul posto nella mappa, con un suggerimento sulla posizione.|  
|[empty](#map__empty)|Restituisce `true` se una mappa è vuota.|  
|[end](#map__end)|Restituisce l'iteratore successivo all'ultimo valore.|  
|[equal_range](#map__equal_range)|Restituisce una coppia di iteratori. Il primo iteratore della coppia fa riferimento al primo elemento di un `map` con una chiave maggiore di una chiave specificata. Il secondo iteratore della coppia fa riferimento al primo elemento della `map` con una chiave uguale o maggiore della chiave specificata.|  
|[erase](#map__erase)|Rimuove un elemento o un intervallo di elementi di una mappa dalle posizioni specificate.|  
|[find](#map__find)|Restituisce un iteratore che punta alla posizione di un elemento di una mappa con una chiave uguale a una chiave specificata.|  
|[get_allocator](#map__get_allocator)|Restituisce una copia dell'oggetto `allocator` utilizzato per costruire la mappa.|  
|[insert](#map__insert)|Incolla un elemento o un intervallo di elementi nella mappa in una posizione specificata.|  
|[key_comp](#map__key_comp)|Restituisce una copia dell'oggetto di confronto utilizzato per ordinare le chiavi di una mappa.|  
|[lower_bound](#map__lower_bound)|Restituisce un iteratore al primo elemento di una mappa con un valore della chiave uguale o maggiore di quello di una chiave specificata.|  
|[max_size](#map__max_size)|Restituisce la lunghezza massima della mappa.|  
|[rbegin](#map__rbegin)|Restituisce un iteratore che punta al primo elemento di una mappa invertita.|  
|[rend](#map__rend)|Restituisce un iteratore che punta alla posizione successiva all'ultimo elemento di una mappa invertita.|  
|[size](#map__size)|Restituisce il numero di elementi della mappa.|  
|[swap](#map__swap)|Scambia gli elementi di due oggetti map.|  
|[upper_bound](#map__upper_bound)|Restituisce un iteratore al primo elemento di una mappa con un valore della chiave uguale o maggiore di quello di una chiave specificata.|  
|[value_comp](#map__value_comp)|Recupera una copia dell'oggetto di confronto utilizzato per ordinare i valori di una mappa.|  
  
### <a name="operators"></a>Operatori  
  
|||  
|-|-|  
|[operator&#91;&#93;](#map__operator_at)|Inserisce un elemento in una mappa con un valore di chiave specificato.|  
|[operator=](#map__operator_eq)|Sostituisce gli elementi di una mappa con una copia di un'altra mappa.|  
...
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo a [get_allocator](#map__get_allocator) per indicazioni su come usare `allocator_type`.  
...
##  <a name="a-namemapbegina--mapbegin"></a><a name="map__begin"></a>  map::begin  
 Restituisce un iteratore che punta al primo elemento dell'oggetto map.  
...
### <a name="return-value"></a>Valore restituito  
 Iteratore bidirezionale che punta al primo elemento dell'oggetto map o alla posizione successiva a un oggetto map vuoto.  
...
##  <a name="a-namemapconstiteratora--mapconstiterator"></a><a name="map__const_iterator"></a>  map::const_iterator  
 Tipo che fornisce un iteratore bidirezionale in grado di leggere un elemento **const** dell'oggetto map.  
...
### <a name="remarks"></a>Note  
 ...
 Il tipo `const_iterator` definito da map punta a elementi che sono oggetti di [value_type](#map__value_type), ovvero di tipo `pair`\< **constKey**, **Type**>, in cui il primo membro è la chiave dell'elemento e il secondo membro è il punto di riferimento mappato incluso nell'elemento.  
  
 Per dereferenziare un tipo `const_iterator``cIter` che punta a un elemento in un oggetto map, usare l'operatore **->**.  
...
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo a [begin](#map__begin) per indicazioni su come usare `const_iterator`.  
  
##  <a name="a-namemapconstpointera--mapconstpointer"></a><a name="map__const_pointer"></a>  map::const_pointer  
 Tipo che fornisce un puntatore a un elemento **const** di un oggetto map.  
...
### <a name="remarks"></a>Note  
 ...
 Nella maggior parte dei casi, è consigliabile usare un tipo [iterator](#map__iterator) per accedere agli elementi di un oggetto map.  
  
##  <a name="a-namemapconstreferencea--mapconstreference"></a><a name="map__const_reference"></a>  map::const_reference  
 Tipo che fornisce un riferimento a un elemento **const** archiviato in un oggetto map per la lettura e l'esecuzione di operazioni **const**.  
...
##  <a name="a-namemapconstreverseiteratora--mapconstreverseiterator"></a><a name="map__const_reverse_iterator"></a>  map::const_reverse_iterator  
 Tipo che fornisce un iteratore bidirezionale in grado di leggere qualsiasi elemento **const** dell'oggetto map.  
...
### <a name="remarks"></a>Note  
 ...
 Il tipo `const_reverse_iterator` definito da map punta a elementi che sono oggetti di [value_type](#map__value_type), ovvero di tipo `pair`*\<***constKey***,* **Type***>*, dove il primo membro è la chiave dell'elemento e il secondo membro è il punto di riferimento mappato incluso nell'elemento.  
  
 Per dereferenziare un tipo `const_reverse_iterator``crIter` che punta a un elemento in un oggetto map, usare l'operatore **->**.  
  
 Per accedere al valore della chiave per l'elemento, usare `crIter` -> **first**, che equivale a (\* `crIter`). **first**.  
  
 Per accedere al valore del punto di riferimento mappato per l'elemento, usare `crIter` -> **second**, che equivale a (\* `crIter`). **first**.  
  
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo a [rend](#map__rend) per indicazioni su come dichiarare e usare `const_reverse_iterator`.  
...
### <a name="remarks"></a>Note  
 `crbegin` viene usato con un oggetto `map` invertito proprio come [begin](#map__begin) viene usato con un oggetto `map`.  
...
### <a name="remarks"></a>Note  
 `crend` viene usato con un oggetto map invertito proprio come [end](#map__end) viene usato con un oggetto `map`.  
...
|-|-|  
|Parametro|Descrizione|  
|`args`|Argomenti inoltrati per costruire un elemento da inserire nell'oggetto map a meno che quest'ultimo non contenga già un elemento il cui valore è ordinato in modo equivalente.|  
  
### <a name="return-value"></a>Valore restituito  
 Atruttura [pair](../standard-library/pair-structure.md) il cui componente `bool` è true se è stato eseguito un inserimento e false se l'oggetto map contiene già un elemento di valore equivalente nell'ordinamento. Il componente dell'iteratore della coppia di valori restituita punta all'elemento appena inserito se il componente `bool` è true oppure all'elemento esistente se il componente `bool` è false.  
  
 Per accedere al componente iterator di una struttura `pair``pr`, usare `pr.first` e per dereferenziarlo, usare `*pr.first`. Per accedere al componente `bool`, usare `pr.second`. Per un esempio, vedere il codice di esempio più avanti in questo articolo.  
...
### <a name="remarks"></a>Note  
 ...
 L'oggetto [value_type](#map__value_type) di un elemento è una coppia, pertanto il valore di un elemento sarà una coppia ordinata in cui il primo componente equivale al valore della chiave e il secondo componente equivale al valore dei dati dell'elemento.  
...
|-|-|  
|Parametro|Descrizione|  
|`args`|Argomenti inoltrati per costruire un elemento da inserire nell'oggetto map a meno che quest'ultimo non contenga già tale elemento o, più in generale, un elemento la cui la chiave è ordinata in modo equivalente.|  
|`where`|Posizione in cui iniziare a cercare il punto di inserimento corretto. Se tale punto è immediatamente seguito da `where`, l'inserimento può avvenire in tempo costante ammortizzato anziché in tempo logaritmico.|  
...
### <a name="return-value"></a>Valore restituito  
 **true** se l'oggetto map è vuoto; in caso contrario, **false**.  
...
### <a name="remarks"></a>Note  
 **end** viene usato per verificare se un iteratore ha superato la fine del relativo oggetto map.  
  
 Non è consigliabile dereferenziare il valore restituito da **end**.  
  
 Per un esempio di codice, vedere [map::find](#map__find).  
  
##  <a name="a-namemapequalrangea--mapequalrange"></a><a name="map__equal_range"></a>  map::equal_range  
 Restituisce una coppia di iteratori che rappresentano rispettivamente l'elemento [lower_bound](#map__lower_bound) e l'elemento [upper_bound](#map__upper_bound) della chiave.  
...
### <a name="return-value"></a>Valore restituito  
 Iteratore che fa riferimento alla posizione di un elemento con una chiave specificata o alla posizione successiva all'ultimo elemento dell'oggetto map ( `map::end()`) se non viene trovata alcuna corrispondenza per la chiave.  
...
|-|-|  
|Parametro|Descrizione|  
|`Val`|Valore di un elemento da inserire nella mappa a meno che non vi sia già contenuto un elemento la cui chiave è ordinata in modo equivalente.|  
|`Where`|Posizione in cui iniziare a cercare il punto di inserimento corretto. Se tale punto è immediatamente seguito da `Where`, l'inserimento può avvenire in tempo costante ammortizzato anziché in tempo logaritmico.|  
|`ValTy`|Parametro di modello che specifica il tipo di argomento che può essere usato dall'oggetto map per costruire un elemento di [value_type](#map__value_type) e che esegue l'inoltro perfetto di `Val` come argomento.|  
|`First`|Posizione del primo elemento da copiare.|  
|`Last`|Posizione immediatamente dopo l'ultimo elemento da copiare.|  
|`InputIterator`|Argomento della funzione modello che soddisfa i requisiti di un [iteratore di input](../standard-library/input-iterator-tag-struct.md) che punta agli elementi di un tipo utilizzabili per costruire oggetti [value_type](#map__value_type).|  
|`IList`|Oggetto [initializer_list](../standard-library/initializer-list.md) da cui copiare gli elementi.|  
  
### <a name="return-value"></a>Valore restituito  
 Le funzioni membro a elemento singolo (1) e (2) restituiscono una struttura [pair](../standard-library/pair-structure.md) il cui componente `bool` è true se è stato eseguito un inserimento e false se l'oggetto map contiene già un elemento la cui chiave ha un valore equivalente nell'ordinamento. Il componente dell'iteratore della coppia di valori restituita punta all'elemento appena inserito se il componente `bool` è true oppure all'elemento esistente se il componente `bool` è false.  
...
### <a name="remarks"></a>Note  
 ...
 Per accedere al componente iteratore di un oggetto `pair``pr` restituito dalle funzioni membro a elemento singolo, usare `pr.first`. Per dereferenziare l'iteratore all'interno della coppia restituita, usare `*pr.first`, che fornisce un elemento. Per accedere al componente `bool`, usare `pr.second`. Per un esempio, vedere il codice di esempio più avanti in questo articolo.  
  
 L'oggetto [value_type](#map__value_type) di un contenitore è un typedef appartenente al contenitore e, per map, `map<K, V>::value_type` è `pair<const K, V>`. Il valore di un elemento è una coppia ordinata in cui il primo componente equivale al valore della chiave e il secondo componente equivale al valore dati dell'elemento.  
 ...
 Per l'inserimento di un elemento costruito sul posto, ovvero senza operazioni di copia o spostamento, vedere [map::emplace](#map__emplace) e [map::emplace_hint](#map__emplace_hint).  
...
### <a name="remarks"></a>Note  
 Il tipo **iterator** definito da map punta a elementi che sono oggetti di [value_type](#map__value_type), ovvero di tipo `pair`*\<***constKey**, **Type***>*, dove il primo membro è la chiave dell'elemento e il secondo membro è il punto di riferimento mappato incluso nell'elemento.  
  
 Per dereferenziare un tipo **iterator**`Iter` che punta a un elemento in un oggetto map, usare l'operatore **->**.  
...
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo a [begin](#map__begin) per indicazioni su come dichiarare e usare **iterator**.  
...
### <a name="remarks"></a>Note  
 ...
 Per altre informazioni su `Traits`, vedere l'argomento [Classe map](../standard-library/map-class.md).  
  
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo a [key_comp](#map__key_comp) per indicazioni su come dichiarare e usare `key_compare`.  
...
### <a name="remarks"></a>Note  
 ...
 Per altre informazioni su `Key`, vedere la sezione Note dell'argomento [Classe map](../standard-library/map-class.md).  
  
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo a [value_type](#map__value_type) per indicazioni su come dichiarare e usare `key_type`.  
...
### <a name="return-value"></a>Valore restituito  
 Tipo **iterator** o `const_iterator` che punta alla posizione di un elemento di un oggetto map con una chiave uguale o maggiore della chiave dell'argomento o che punta alla posizione successiva all'ultimo elemento dell'oggetto map se non viene trovata alcuna corrispondenza per la chiave.  
...
|-|-|  
|Parametro|Descrizione|  
|`Al`|Classe dell'allocatore di archiviazione da usare per l'oggetto map. Per impostazione predefinita è `Allocator`.|  
|`Comp`|Funzione di confronto di tipo `const Traits` usata per ordinare gli elementi nell'oggetto map. Per impostazione predefinita è `hash_compare`.|  
...
### <a name="remarks"></a>Note  
 ...
 Il sesto, il settimo e l'ottavo costruttore usano un oggetto initializer_list da cui copiare i membri.  
...
### <a name="remarks"></a>Note  
 Il tipo `mapped_type` è un sinonimo per il parametro di modello `Type` della classe.  
  
 Per altre informazioni su `Type`, vedere l'argomento [Classe map](../standard-library/map-class.md).  
  
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo a [value_type](#map__value_type) per indicazioni su come dichiarare e usare `mapped_type`.  
...
|-|-|  
|Parametro|Descrizione|  
|` right`|Oggetto [map](../standard-library/map-class.md) da copiare nell'oggetto `map`.|  
...
### <a name="remarks"></a>Note  
 `rbegin` viene usato con un oggetto map invertito proprio come [begin](#map__begin) viene usato con un oggetto map.  
...
### <a name="remarks"></a>Note  
 `rend` viene usato con un oggetto map invertito proprio come [end](#map__end) viene usato con un oggetto map.  
...
### <a name="remarks"></a>Note  
 ...
 Il tipo `reverse_iterator` definito da map punta a elementi che sono oggetti di [value_type](#map__value_type), ovvero di tipo `pair`*\<***constKey**, **Type***>*, dove il primo membro è la chiave dell'elemento e il secondo membro è il punto di riferimento mappato incluso nell'elemento.  
  
 Per dereferenziare un tipo `reverse_iterator``rIter` che punta a un elemento in un oggetto map, usare l'operatore **->**.  
...
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo a [rbegin](#map__rbegin) per indicazioni su come dichiarare e usare `reverse_iterator`.  
...
### <a name="example"></a>Esempio  
  Vedere l'esempio relativo a [size](#map__size) per indicazioni su come dichiarare e usare `size_type`.  
...
### <a name="return-value"></a>Valore restituito  
 Tipo **iterator** o `const_iterator` che punta alla posizione di un elemento di un oggetto map con una chiave maggiore della chiave dell'argomento o che punta alla posizione successiva all'ultimo elemento dell'oggetto map se non viene trovata alcuna corrispondenza per la chiave.  
...
### <a name="remarks"></a>Note  
 Per un oggetto map *m*, se due elementi *e*1( *k*1, *d*1) e *e*2( *k*2, `d`2) sono oggetti di tipo `value_type`, dove *k*1 e *k*2 sono le rispettive chiavi di tipo `key_type` e `d`1 e `d`2 sono i rispettivi dati di tipo `mapped_type`, *m.*`value_comp`( *e*1, *e*2) è equivalente a *m.*`key_comp`*(k*1, *k*2). Un oggetto archiviato definisce la funzione membro  
...
## <a name="see-also"></a>Vedere anche  
 [\<map> Members](http://msdn.microsoft.com/en-us/7e8f0bc2-6034-40f6-9d14-76d4cef86308) (Membri di <map>)   
