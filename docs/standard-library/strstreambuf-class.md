...
ms.locfileid: "68459073"
---
# <a name="strstreambuf-class"></a>Classe strstreambuf

Descrive un buffer del flusso che controlla la trasmissione di elementi da e verso una sequenza di elementi archiviati in un oggetto matrice di **caratteri** .
...
Se la funzione non può essere eseguita correttamente, restituisce `EOF`. In caso contrario, se  *\_meta* == `EOF`, restituisce un valore diverso `EOF`da. In caso contrario, restituisce  *\_meta*.

### <a name="remarks"></a>Note

`EOF`Se  *\_meta* ! =, la funzione membro virtuale protetta tenta di inserire l'elemento `(char)_Meta` nel buffer di output. Questa operazione può essere eseguita in vari modi:
...
*Se\_meta*,l'elemento`EOF`di cui eseguire il push è effettivamente quello già presente nel flusso prima dell'elemento corrente. ==  In caso contrario, l'elemento viene `ch = (char)_Meta`sostituito da. La funzione può reinserire un elemento in vari modi:
...
- Se è disponibile una posizione reinserimento e se la modalità strstreambuf indica che la sequenza controllata è modificabile, la funzione può `ch` archiviare nella posizione reinserimento e decrementare il puntatore successivo per il buffer di input.
...
In particolare, se [pptr](../standard-library/basic-streambuf-class.md#pptr) è un puntatore null, la funzione restituisce zero. In caso contrario, `pptr`restituisce  -  [Pbase](../standard-library/basic-streambuf-class.md#pbase).
...
*conteggio*\
Determina la lunghezza del buffer a cui punta *_Getptr*. Se *_Getptr* non è un argomento (primo form del costruttore), dimensione di allocazione consigliata per i buffer.
...
- Se (`count` = = 0), *N* `strlen`è (( **const** `char` *) `_Getptr` ).
...
La funzione membro virtuale protetta tenta di estrarre l' `ch` elemento corrente dal buffer di input, quindi avanza la posizione del flusso corrente e restituisce l'elemento come (`int`) (`unsigned char`) **ch**. Questa operazione può essere eseguita solo in un modo: se è disponibile una posizione di lettura, `ch` accetta come elemento archiviato nella posizione di lettura e sposta in avanti il puntatore successivo per il buffer di input.
