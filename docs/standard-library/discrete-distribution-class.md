...
- Se *count* > 0, **n** = *count*. Fornito **d** = (*High* - *low*)/ **n** è maggiore di zero **, usando le** unità secondarie uniformi, ogni peso viene assegnato come `weight[k] = weightfunc(x)`segue:, dove **x** = *low*  +  **k**    d/2, per k = 0,..., n-1. +  * 
