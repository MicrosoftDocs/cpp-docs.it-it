...
## Valore restituito  
 ...
 Il quarto costruttore inizializza il primo elemento della coppia su `Val1` e il secondo su *Val2* usando i [Dichiaratore di riferimento rvalue: &&](../cpp/rvalue-reference-declarator-amp-amp.md).  
  
## Note  
 Lo struct del modello archivia una coppia di oggetti, rispettivamente di tipo **T1** e **T2**.  Il tipo **first\_type** è uguale al parametro di modello **T1**, mentre il tipo **second\_type** è uguale al parametro di modello **T2**.  Sia **T1** che **T2** devono fornire solo un costruttore predefinito, un costruttore ad argomento singolo e un distruttore.  Tutti i membri del tipo `pair` sono pubblici perché il tipo è dichiarato come `struct` anziché come **class**.  Una coppia viene usata in genere in due modi: come tipo restituito per le funzioni che restituiscono due valori e come elemento per le classi di contenitori associativi [classe map](../standard-library/map-class.md) e [classe multimap](../standard-library/multimap-class.md), entrambe con una chiave e un tipo di valore associati a ogni elemento.  L'ultimo soddisfa il requisito relativo ai contenitori associativi di coppie e ha un tipo di valore con formato `pair`\<**const** `key_type`, `mapped_type`\>.  
...
## Vedere anche  
 [Operatore logico pair](../misc/pair-logical-operator.md)   
