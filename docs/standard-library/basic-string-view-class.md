...
In string_view viene descritta l'interfaccia comune minima necessaria per leggere i dati di stringa. Fornisce l'accesso const ai dati sottostanti. non esegue alcuna copia (ad eccezione della `copy` funzione). È possibile che i dati non contengano valori null (' \ 0') in qualsiasi posizione. Un string_view non ha alcun controllo sulla durata dell'oggetto. È responsabilità del chiamante assicurarsi che i dati stringa sottostanti siano validi.
...
Un string_view costruito con un valore letterale stringa C non include la terminazione null e pertanto nell'esempio seguente `back` restituisce "p" e non "\ 0".
...
`rend`viene usato con un string_view invertito proprio come [end](#end) viene usato con una string_view. `rend`può essere usato per verificare se un iteratore inverso ha raggiunto la fine del string_view. Non è consigliabile dereferenziare il valore restituito da `rend`.
