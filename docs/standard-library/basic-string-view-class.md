...
In string_view viene descritta l'interfaccia comune minima necessaria per leggere i dati di stringa. Fornisce l'accesso const ai dati sottostanti. non esegue alcuna copia, ad eccezione della funzione `copy`. È possibile che i dati non contengano valori null (' \ 0') in qualsiasi posizione. Un string_view non ha alcun controllo sulla durata dell'oggetto. È responsabilità del chiamante assicurarsi che i dati stringa sottostanti siano validi.

È possibile eseguire una funzione che accetta un parametro di tipo string_view per funzionare con qualsiasi tipo stringa, senza rendere la funzione in un modello o vincolare la funzione a un subset specifico di tipi di stringa. L'unico requisito è che esiste una conversione implicita dal tipo stringa a string_view. Tutti i tipi di stringa standard sono convertibili in modo implicito in un string_view che contiene lo stesso tipo di elemento. In altre parole, un `std::string` è convertibile in un `string_view` ma non in un `wstring_view`.
...
\ *CharType*
Tipo di caratteri archiviati nel string_view. La C++ libreria standard fornisce i typedef seguenti per le specializzazioni di questo modello.
...
|??|Descrizione|
|-|-|
|[operator=](#op_eq)|Assegna un string_view o un oggetto stringa convertibile a un altro string_view.|
...
Un string_view costruito con un valore letterale stringa C non include il valore null di terminazione e pertanto nell'esempio seguente `back` restituisce "p" e non "\ 0".
...
`operator[]` non controlla se l'indice passato come argomento è valido. Un indice non valido passato a `operator[]` comporta un comportamento non definito.
