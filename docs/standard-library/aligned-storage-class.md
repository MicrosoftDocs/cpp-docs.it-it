...
Il typedef `type` del membro del modello è un sinonimo di un tipo POD con allineamento *allineamento e dimensione* *Len*. *Align* deve essere uguale a `alignment_of<T>::value` per un determinato `T`tipo o all'allineamento predefinito.
