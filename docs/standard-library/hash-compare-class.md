...
- Per tutti i `key` valori di `Key`tipo, lachiamata hash_comp`key`() funge da funzione hash, che produce una distribuzione di valori di tipo. `size_t` La funzione fornita da hash_compare restituisce `key`.
