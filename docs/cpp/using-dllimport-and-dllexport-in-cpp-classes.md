...
## <a name="inheritance-and-exportable-classes"></a><a name="_pluslang_using_dllimport_and_dllexport_in_c2b2binheritanceandexportableclasses"></a>Ereditarietà e classi esportabili
...
Poiché le funzioni membro e i dati statici all'interno di una classe dispongono implicitamente di collegamento esterno, è possibile dichiararli con l'attributo **dllimport** o **dllexport,** a meno che non venga esportata l'intera classe. Se l'intera classe viene importata o esportata, la dichiarazione esplicita di funzioni membro e dati come **dllimport** o **dllexport** non è consentita. Se si dichiara un membro dati statici all'interno di una definizione di classe come **dllexport**, una definizione deve trovarsi in un punto qualsiasi all'interno dello stesso programma (come per il collegamento esterno non di classe).

Analogamente, è possibile dichiarare le funzioni membro con gli attributi **dllimport** o **dllexport.** In questo caso, è necessario fornire una definizione **dllexport** da qualche parte all'interno dello stesso programma.
...
- Sebbene la definizione dei membri di classe come **dllimport** o **dllexport** sia consentita, non è possibile eseguire l'override dell'interfaccia specificata nella definizione della classe.
