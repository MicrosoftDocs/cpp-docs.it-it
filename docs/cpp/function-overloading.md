...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "dichiarazione di funzioni, overload"
  - "overload di funzioni"
  - "overload di funzioni, informazioni su overload di funzioni"
...
|**const** o `volatile` \(vedere di seguito\)|Sì|  
  
 Anche se è possibile distinguere le funzioni in base al tipo restituito, non è possibile eseguirne l'overload su questa base.  `Const` o `volatile` vengono usati come base per l'overload solo se vengono usati in una classe per l'applicazione al puntatore **this** per la classe, non al tipo restituito della funzione.  In altre parole, l'overload si applica solo se la parola chiave **const** o `volatile` segue l'elenco di argomenti della funzione nella dichiarazione.  
...
```  
 ...
 Per le limitazioni relative all'overload e per informazioni su l'overload influisce su altri elementi di C\+\+, vedere [Overload](../misc/overloading-cpp.md).  
...
## Differenze tra tipi di argomenti  
 ...
 Per lo stesso motivo, gli argomenti della funzione di un tipo modificato da **const** o `volatile` non vengono gestiti in modo diverso rispetto al tipo base per eseguire l'overload.  
  
 Tuttavia, il meccanismo di overload della funzione è in grado di distinguere tra i riferimenti che sono qualificati da **const** e `volatile` e i riferimenti al tipo di base.  Ciò rende possibile un codice del tipo riportato di seguito:  
...
```  
 ...
 I puntatori a **const** e agli oggetti `volatile` vengono inoltre considerati diversi dai puntatori al tipo base allo scopo di eseguire l'overload.  
...
-   Le sequenze di conversioni che possono essere abbreviate rimuovendo le conversioni intermedie vengono ignorate.  
 ...
 La sequenza di conversioni risultante, se disponibile, è nota come sequenza di corrispondenza migliore.  Esistono diversi modi per convertire un oggetto di tipo `int` in tipo `unsigned long`usando le conversioni standard \(descritte in [Conversioni standard](../cpp/standard-conversions.md)\):  
...
1.  Corrispondenza esatta.  Una corrispondenza esatta tra i tipi con cui viene chiamata la funzione e i tipi dichiarati nel prototipo di funzione è sempre la corrispondenza ottimale.  Le sequenze di conversioni semplici vengono classificate come corrispondenze esatte.  Tuttavia, le sequenze che non eseguono queste conversioni vengono considerate migliori rispetto alle sequenze che eseguono la conversione:  
    ...
    -   Dal puntatore, al puntatore a **const** \(`type` **\*** a **cost** `type` **\***\).  
...
3.  Corrispondenza che usa le conversioni standard.  Qualsiasi sequenza non classificata come una corrispondenza esatta o una corrispondenza che usa promozioni solo con conversioni standard e semplici viene classificata come corrispondenza che usa le conversioni standard.  All'interno di questa categoria, sono necessarie le seguenti regole:  
    ...
    -   La conversione da un puntatore a una classe derivata, a un puntatore a una classe base diretta o indiretta è preferibile alla conversione in **void \*** o in **const void \***.  
 ...
 ![Conversioni preferite](../cpp/media/vc391t1.png "vc391T1")  
Grafico indicante le conversioni preferite  
 ...
 ![Ereditarietà multipla con conversioni preferite](../cpp/media/vc391t2.png "vc391T2")  
...
```  
 ...
 Le conversioni definite dall'utente disponibili per la classe `UDC` sono dal tipo `int` e dal tipo **long**.  Di conseguenza, il compilatore considera le conversioni per il tipo di oggetto di cui si sta eseguendo la corrispondenza: `UDC`.  Esiste una conversione in `int` ed è selezionata.  
...
```  
 ...
 Nell'esempio precedente, la conversione definita dall'utente, **operator long**, viene richiamata per convertire `udc` nel tipo **long**.  Se non fosse stata definita alcuna conversione definita dall'utente nel tipo **long**, la conversione sarebbe proseguita nel modo seguente: il tipo `UDC` sarebbe stato convertito nel tipo `int` mediante la conversione definita dall'utente.  La conversione standard dal tipo `int` al tipo **long** sarebbe stata applicata per eseguire la corrispondenza dell'argomento nella dichiarazione.  
...
-   Conversione dal tipo `int` al tipo **long**; quindi conversione nel tipo `UDC2` \(conversione in due fasi\).  
...
## Corrispondenza di argomenti e puntatore this  
 ...
 Tali funzioni membro non statiche richiedono che il puntatore `this` corrisponda al tipo di oggetto tramite cui la funzione viene chiamata oppure, per gli operatori di overload, richiedono che il primo argomento corrisponda all'oggetto sul quale viene applicato l'operatore.  Per altre informazioni sulla definizione di operatori di overload, vedere [Operatori di overload](../cpp/operator-overloading.md).  
 ...
 Quando l'operatore di selezione dei membri `– >` viene usato per accedere a una funzione membro, l'argomento del puntatore `this` è di tipo `class-name` `* const`.  Se i membri dichiarati come `const` o `volatile`, i tipi sono `const` `class-name``* const` e `volatile` `class-name` `* const`, rispettivamente.  
...
## Corrispondenza di dichiarazione  
