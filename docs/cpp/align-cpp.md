...
Senza `__declspec(align(#))`, il compilatore in genere Allinea i dati sui limiti naturali secondo il processore di destinazione e le dimensioni dei dati, fino a limiti di 4 byte su processori a 32 bit, in base e i limiti di 8 byte su processori a 64 bit. Dati nelle classi o strutture vengono allineati nella classe o struttura almeno di allineamento naturale e l'impostazione di compressione corrente (da #pragma `pack` o **/Zp** l'opzione del compilatore).
