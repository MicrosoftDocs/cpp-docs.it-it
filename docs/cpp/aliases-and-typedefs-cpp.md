---
title: Alias e typedef (C++) | Documenti Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
```  
 ...
 Anche gli alias di lavorare con i puntatori a funzione, ma sono molto più leggibili equivalente typedef:  
...
## <a name="typedefs"></a>Definizioni typedef  
 ...
 Al contrario di **classe**, `struct`, **unione**, e `enum` dichiarazioni, `typedef` non introducono nuovi tipi: introducono nuovi nomi per i tipi esistenti.  
...
```  
 ...
 In C++, la differenza tra `typedef` nomi e tipi reali (dichiarati con la **classe**, `struct`, **unione**, e `enum` parole chiave) è più distinta. Sebbene la procedura C di dichiarazione di una struttura senza nome in un'istruzione `typedef` funzioni ancora, non fornisce gli stessi vantaggi di notazione del linguaggio C.  
...
-   Il nome (il sinonimo) non può comparire dopo un **classe**, `struct`, o **unione** prefisso.  
