...
-   **variant_t (classe variant_t &***var_t_Src***)** costruisce una `_variant_t` da un altro oggetto `_variant_t` oggetto.     Il tipo variant viene mantenuto.  
...
-   **variant_t (short***sSrc* **, VARTYPE**`vtSrc`**= VT_I2)** costruisce una `_variant_t` oggetto di tipo `VT_I2` o `VT_BOOL` da un **breve** valore intero.       Qualsiasi altro **VARTYPE** comporta un `E_INVALIDARG` errore.  
  
-   **variant_t (long** `lSrc` **, VARTYPE**`vtSrc`**= VT_I4)** costruisce una `_variant_t` oggetto di tipo `VT_I4`, `VT_BOOL`, oppure `VT_ERROR`da un **lungo** valore intero.       Qualsiasi altro **VARTYPE** comporta un `E_INVALIDARG` errore.  
  
-   **variant_t (float**`fltSrc`**)** costrutti un `_variant_t` oggetto di tipo `VT_R4` da un **float** valore numerico.      
...
-   **variant_t (CY &**`cySrc`**)** costrutti un `_variant_t` oggetto di tipo `VT_CY` da un **CY** oggetto.      
  
-   **variant_t ( bstr_t &**`bstrSrc`**)** costrutti un `_variant_t` oggetto di tipo `VT_BSTR` da un `_bstr_t` oggetto.     Un nuovo `BSTR` viene allocato.  
...
-   **variant_t (bool**`bSrc`**)** costrutti un `_variant_t` oggetto di tipo `VT_BOOL` da un `bool` valore.      
...
-   **variant_t (decimale &**`decSrc`**)** costrutti un `_variant_t` oggetto di tipo **VT_DECIMAL** da un **decimale** valore.      
  
-   **variant_t (BYTE**`bSrc`**)** costrutti un `_variant_t` oggetto di tipo `VT_UI1` da un **BYTE** valore.      
