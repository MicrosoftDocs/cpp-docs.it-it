...
1. char _pad0[3] per allineare il membro int b in base a un limite di 4 byte

1. char _pad1[1] per allineare gli elementi della matrice della struttura struct _x bar[3];

1. Grazie alla spaziatura interna gli elementi di bar[3] vengono allineati in modo da consentire l'accesso naturale.
...
[Allineamento di strutture di dati](http://en.wikipedia.org/wiki/Data_structure_alignment)