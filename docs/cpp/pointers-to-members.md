...
1. Identificatore di dichiarazione:
   ...
   - Identificatori facoltativi **const** e/o **volatile** .
...
Nell'esempio precedente, `pwCaption` è un puntatore a qualsiasi membro della classe `Window` con tipo `char*`. Il tipo di `pwCaption` è `char * Window::* `. Il frammento di codice seguente dichiara i puntatori alle funzioni membro `SetCaption` e `GetCaption`.
...
Differenza tra **.** gli operatori <strong>\*</strong> e **->** <strong>\*</strong> (gli operatori puntatore a membro) sono che **.** <strong>\*</strong> operatore seleziona i membri in base a un riferimento a un oggetto o a un oggetto, mentre l'operatore **->** <strong>\*</strong> seleziona i membri tramite un puntatore. Per ulteriori informazioni su questi operatori, vedere [espressioni con gli operatori puntatore a membro](../cpp/pointer-to-member-operators-dot-star-and-star.md).
...
L'indirizzo di un membro statico non è un puntatore a un membro. È un normale puntatore all'istanza del membro statico. Poiché per tutti gli oggetti di una determinata classe esiste una sola istanza di un membro statico, è possibile utilizzare gli operatori di indirizzo ( **&** ) e dereferenziazione ordinaria (<strong>\*</strong>).
