---
title: "Conversioni | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
helpviewer_keywords: 
  - "coercizione"
  - "costruttori [C++], e costanti"
  - "costruttori [C++], conversione"
  - "costruttori [C++], svantaggi"
  - "costruttori di conversione"
  - "funzioni di conversione"
  - "funzioni di conversione, regole per la dichiarazione"
  - "conversioni [C++], costruttori"
  - "conversioni [C++], esplicita"
  - "conversione di oggetti"
  - "conversione di tipi di dati [C++], esplicita"
  - "dichiarazione di funzioni, funzioni di conversione"
  - "explicit (parola chiave)"
  - "funzioni [C++], conversione"
  - "oggetti [C++], conversione"
  - "tipo (conversione)"
  - "conversione di tipi [C++], conversione esplicita"
...
Una *conversione* produce un nuovo valore di un certo tipo da un valore di tipo differente.  Le *conversioni standard* sono incorporate nel linguaggio C\+\+ e supportano i relativi tipi incorporati; è anche possibile creare *conversioni definite dall'utente* in modo da eseguire conversioni a, da o tra tipi definiti dall'utente.  
  
 Le conversioni standard avvengono tra tipi incorporati, tra puntatori o riferimenti ai tipi correlati per ereditarietà, a e da puntatori void e al puntatore null.  Per altre informazioni, vedere [Conversioni standard](../cpp/standard-conversions.md).  Le conversioni definite dall'utente avvengono tra tipi definiti dall'utente oppure tra tipi definiti dall'utente e tipi incorporati.  È possibile implementarle come [Costruttori di conversione](#ConvCTOR) o come [Funzioni di conversione.](#ConvFunc).  
...
-   Il tipo di operando fornito a un operatore non è uguale a quello del parametro di operando corrispondente.  Per gli operatori incorporati, entrambi gli operandi devono essere dello stesso tipo e vengono convertiti in un tipo comune che rappresenta entrambi.  Per altre informazioni, vedere [Conversioni aritmetiche](../misc/arithmetic-conversions.md).  Per gli operatori definiti dall'utente, il tipo di ogni operando deve essere lo stesso del parametro di operando corrispondente.  
...
-   Ereditarietà multipla.  La conversione viene definita in più classi di base.  Per altre informazioni, vedere [\(NOTINBUILD\) Ambiguity](http://msdn.microsoft.com/it-it/0b399cab-40a7-4e79-9278-05f40139a0e1).  
...
## La parola chiave explicit e i problemi insiti nella conversione implicita  
...
```  
 ...
 Notare che la prima chiamata alla funzione `display_balance`, che accetta un argomento di tipo `Money`, non richiede una conversione perché il relativo argomento è del tipo corretto.  Tuttavia, nella seconda chiamata `display_balance`, è necessaria una conversione perché il tipo di argomento, un `double` con valore pari a `49.95`, non è quello previsto dalla funzione.  La funzione non può usare direttamente questo valore, ma poiché esiste una conversione dal tipo di argomento \(`double`\) al tipo di parametro corrispondente \(`Money`\), viene costruito un tipo di valore di tipo `Money` dall'argomento, usato per completare la chiamata di funzione.  Nella terza chiamata a `display_balance`, notare che l'argomento non è un `double`, ma è invece un `float` con un valore di `9,99`, eppure la chiamata di funzione può ancora essere completata perché il compilatore può eseguire una conversione standard \(in questo caso, da `float` a `double`\) ed eseguire la conversione definita dall'utente da `double` a `Money` per completare la conversione necessaria.  
  
### Dichiarazione di costruttori di conversione  
...
### Costruttori di conversione espliciti  
...
### Dichiarazione delle funzioni di conversione  
...
### Funzioni esplicite di conversione  
