...
2.  **Visual C++ 2010 e versioni successive:** il `auto` (parola chiave) non è più un identificatore di classe di archiviazione, C++ e `register` parola chiave è deprecata. **Visual Studio 2017 15.3 e versioni successive:** (disponibile con [/std:c + + 17](../build/reference/std-specify-language-standard-version.md)): il `register` (parola chiave) non è più una classe di archiviazione supportati. La parola chiave è ancora riservata nello standard per un utilizzo futuro. 
```cpp
...
Dichiarazione di `const` variabili con la `extern` classe di archiviazione forza la variabile con collegamento esterno. L'inizializzazione di un `extern const` variabile è consentita nell'unità di conversione definizione. Inizializzazioni in unità di conversione diverse dalle unità di conversione definizione producono risultati non definiti. Per ulteriori informazioni, vedere [utilizzo di extern per specificare un collegamento](../cpp/using-extern-to-specify-linkage.md)  
...
-  L'utilizzo di `thread_local` può interferire con [caricamento ritardato](../build/reference/linker-support-for-delay-loaded-dlls.md) delle importazioni delle DLL. 
  
-  Nei sistemi XP `thread_local` potrebbe non funzionare correttamente se una DLL Usa `thread_local` dati e viene caricata dinamicamente tramite `LoadLibrary`.  
