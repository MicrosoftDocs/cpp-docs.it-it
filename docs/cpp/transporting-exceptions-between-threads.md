...
ms.locfileid: "34704646"
---
# <a name="transporting-exceptions-between-threads"></a>Il trasferimento delle eccezioni tra thread
...
|---------------|-----------------|
|*Non specificato*|Classe interna non specificata utilizzata per implementare il tipo `exception_ptr`.|
...
Il `make_exception_ptr` restituito dalla funzione un' `exception_ptr` oggetto cui fa riferimento all'eccezione specificata dal *e* parametro.
...
Malgrado il nome, un oggetto `exception_ptr` non è un puntatore. Non rispetta la semantica di puntatore e non può essere utilizzato con l'accesso al membro puntatore (`->`) o un riferimento indiretto (`*`) gli operatori. L'oggetto `exception_ptr` non dispone di membri dati pubblici o di funzioni membro.
