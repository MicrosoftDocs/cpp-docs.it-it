---
title: 'Procedura dettagliata: Creazione di una rete di elaborazione di immagini | Microsoft Docs'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-concrt
...
2.  Per le immagini che hanno il rosso come colore dominante, rimuovere i componenti di colore verde e blu e quindi rende più scura.  
  
3.  Per qualsiasi altra immagine, si applicano seppia.  
...
2.  Nel `ProcessImages` funzionamento, creare un `countdown_event` variabile. Il `countdown_event` classe è illustrata più avanti in questa procedura dettagliata.  
...
6.  Aggiungere il codice seguente per inviare all'inizio della rete il percorso completo di ogni file JPEG nella directory.  
