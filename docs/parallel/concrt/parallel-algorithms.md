...
Gli algoritmi paralleli sono costituiti da funzionalità esistenti nel runtime di concorrenza. Ad esempio, l'algoritmo [Concurrency::p arallel_for](reference/concurrency-namespace-functions.md#parallel_for) utilizza un oggetto Concurrency [:: structured_task_group](../../parallel/concrt/reference/structured-task-group-class.md) per eseguire le iterazioni del ciclo parallelo. Le `parallel_for` partizioni dell'algoritmo funzionano in modo ottimale in base al numero di risorse di calcolo disponibili.

##  <a name="top"></a> Sezioni

- [Algoritmo parallel_for](#parallel_for)

- [Algoritmo parallel_for_each](#parallel_for_each)

- [Algoritmo parallel_invoke](#parallel_invoke)

- [Algoritmi parallel_transform e parallel_reduce](#parallel_transform_reduce)

    - [Algoritmo parallel_transform](#parallel_transform)

    - [Algoritmo parallel_reduce](#parallel_reduce)
...
##  <a name="parallel_for"></a>Algoritmo parallel_for
...
È possibile convertire molti `for` cicli da usare `parallel_for`. Tuttavia, l' `parallel_for` algoritmo differisce `for` dall'istruzione nei modi seguenti:

- L' `parallel_for` algoritmo`parallel_for` non esegue le attività in un ordine predeterminato.

- L' `parallel_for` algoritmo non supporta le condizioni di chiusura arbitrarie. L' `parallel_for` algoritmo si interrompe quando il valore corrente della variabile di iterazione è uno `last`minore di.
...
Nell'esempio seguente viene illustrata la struttura di `parallel_for` base dell'algoritmo. In questo esempio viene stampato nella console ogni valore compreso nell'intervallo [1, 5] in parallelo.
...
Per un esempio completo in cui viene `parallel_for` usato l'algoritmo [, vedere Procedura: Scrivere un ciclo](../../parallel/concrt/how-to-write-a-parallel-for-loop.md)parallel_for.

[[Torna all'inizio](#top)]

##  <a name="parallel_for_each"></a>Algoritmo parallel_for_each

L'algoritmo [Concurrency::p arallel_for_each](reference/concurrency-namespace-functions.md#parallel_for_each) esegue attività in un contenitore iterativo, ad esempio quelli forniti dalla C++ libreria standard, in parallelo. Usa la stessa logica di partizionamento usata dall' `parallel_for` algoritmo.

L' `parallel_for_each` algoritmo è simile all' C++ algoritmo [std:: for_each](../../standard-library/algorithm-functions.md#for_each) della libreria standard, ad eccezione del `parallel_for_each` fatto che l'algoritmo esegue le attività contemporaneamente. Analogamente ad altri algoritmi paralleli, `parallel_for_each` non esegue le attività in un ordine specifico.
...
Nell'esempio seguente viene illustrata la struttura di `parallel_for_each` base dell'algoritmo. In questo esempio viene stampato nella console ogni valore in un oggetto [std:: Array](../../standard-library/array-class-stl.md) in parallelo.
...
Per un esempio completo in cui viene `parallel_for_each` usato l'algoritmo [, vedere Procedura: Scrivere un ciclo](../../parallel/concrt/how-to-write-a-parallel-for-each-loop.md)parallel_for_each.

[[Torna all'inizio](#top)]

##  <a name="parallel_invoke"></a>Algoritmo parallel_invoke
...
Analogamente ad altri algoritmi paralleli, `parallel_invoke` non esegue le attività in un ordine specifico. Nell'argomento [parallelismo delle attività](../../parallel/concrt/task-parallelism-concurrency-runtime.md) viene illustrato `parallel_invoke` il modo in cui l'algoritmo è correlato a attività e gruppi di attività.

### <a name="example"></a>Esempio

Nell'esempio seguente viene illustrata la struttura di `parallel_invoke` base dell'algoritmo. Questo esempio chiama simultaneamente la `twice` funzione su tre variabili locali e stampa il risultato nella console.
...
Per esempi completi che usano l' `parallel_invoke` algoritmo, vedere [How to: Usare parallel_invoke per scrivere una routine](../../parallel/concrt/how-to-use-parallel-invoke-to-write-a-parallel-sort-routine.md) di ordinamento in parallelo e come eseguire le [operazioni seguenti: Usare parallel_invoke per eseguire operazioni](../../parallel/concrt/how-to-use-parallel-invoke-to-execute-parallel-operations.md)parallele.

[[Torna all'inizio](#top)]

##  <a name="parallel_transform_reduce"></a>Algoritmi parallel_transform e parallel_reduce

Gli algoritmi [Concurrency::p arallel_transform](reference/concurrency-namespace-functions.md#parallel_transform) e Concurrency [::p arallel_reduce](reference/concurrency-namespace-functions.md#parallel_reduce) sono versioni parallele degli algoritmi C++ di libreria standard [std:: Transform](../../standard-library/algorithm-functions.md#transform) e [std:: accumulate](../../standard-library/numeric-functions.md#accumulate), rispettivamente. Le versioni runtime di concorrenza si comportano come le versioni della C++ libreria standard, ad eccezione del fatto che l'ordine delle operazioni non è determinato perché vengono eseguite in parallelo. Sfruttare questi algoritmi quando si utilizza un set sufficientemente grande da ottenere vantaggi di scalabilità e di prestazioni dall'esecuzione in parallelo.
...
###  <a name="parallel_transform"></a>Algoritmo parallel_transform
...
###  <a name="parallel_reduce"></a>Algoritmo parallel_reduce
...
In molti casi, è possibile pensare `parallel_reduce` a come a sintassi abbreviata per l'utilizzo `parallel_for_each` dell'algoritmo insieme alla classe Concurrency [:: combinable](../../parallel/concrt/reference/combinable-class.md) .
...
Per un altro esempio in cui viene eseguita un'operazione di mapping e riduzione [in parallelo, vedere Procedura: Eseguire operazioni di mapping e riduzione in](../../parallel/concrt/how-to-perform-map-and-reduce-operations-in-parallel.md)parallelo.
...
> [!WARNING]
>  Gli `parallel_for_each` algoritmi `parallel_transform` e supportano solo i contenitori che usano iteratori ad accesso casuale (ad esempio std::[vector](../../standard-library/vector-class.md)) per i partitioner statici, semplici e affinity. Se si utilizzano contenitori con iteratori bidirezionali e di inoltro viene generato un errore in fase di compilazione. Il partitioner predefinito, `auto_partitioner`, supporta tutti e tre i tipi di iteratore.
...
Gli algoritmi `parallel_sort` e `parallel_buffered_sort` sono entrambi basati sul confronto, cioè gli elementi vengono confrontati in base al valore. L'algoritmo `parallel_sort` non presenta requisiti di memoria aggiuntivi ed è appropriato per un ordinamento di utilizzo generale. L' `parallel_buffered_sort` algoritmo può ottenere prestazioni migliori `parallel_sort`rispetto a, ma richiede uno spazio O (N).
...
|---------------|-----------------|-----------------------|--------------------|-------------------------|---------------------|---------------------|
|`parallel_sort`|Ordinamento basato sul confronto per utilizzo generale.|Basato sul confronto (crescente)|Instabile|Nessuna|O ((N/P) log (N/P) + 2N ((P-1)/P))|Random|
|`parallel_buffered_sort`|Ordinamento più veloce basato sul confronto per utilizzo generale per cui è richiesto lo spazio O(N).|Basato sul confronto (crescente)|Instabile|Richiede spazio aggiuntivo O (N)|O ((N/P) log (N))|Random|
|`parallel_radixsort`|Ordinamento basato su chiave di interi per cui è richiesto lo spazio O(N).|Basato su hash|Stable|Richiede spazio aggiuntivo O (N)|O(N/P)|Random|
...
In questo esempio viene illustrato come fornire una funzione di confronto personalizzata. Usa il metodo [std:: Complex:: Real](../../standard-library/complex-class.md#real) per ordinare i valori della [doppia >\<std:: Complex](../../standard-library/complex-double.md) in ordine crescente.
...
In questo esempio, in cui si presuppone che sia accettabile allocare O (N) spazio durante l'ordinamento `parallel_radixsort` , viene eseguito il meglio in questo set di dati in questa configurazione del computer.
...
|Titolo|Descrizione|
|-----------|-----------------|
|[Procedura: Scrivere un ciclo parallel_for](../../parallel/concrt/how-to-write-a-parallel-for-loop.md)|Viene illustrato come utilizzare l' `parallel_for` algoritmo per eseguire la moltiplicazione di matrici.|
|[Procedura: Scrivere un ciclo parallel_for_each](../../parallel/concrt/how-to-write-a-parallel-for-each-loop.md)|Viene illustrato come utilizzare l' `parallel_for_each` algoritmo per calcolare il numero di numeri primi in un oggetto [std:: Array](../../standard-library/array-class-stl.md) in parallelo.|
...
[parallel_sort (funzione)](reference/concurrency-namespace-functions.md#parallel_sort)

[parallel_buffered_sort (funzione)](reference/concurrency-namespace-functions.md#parallel_buffered_sort)

[parallel_radixsort (funzione)](reference/concurrency-namespace-functions.md#parallel_radixsort)
