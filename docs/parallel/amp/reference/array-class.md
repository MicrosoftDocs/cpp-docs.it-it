...
|[distruttore ~ Array](#dtor)|Elimina definitivamente l'oggetto `array`.|
...
|[get_associated_accelerator_view](#get_associated_accelerator_view)|Ottiene il secondo [accelerator_view](accelerator-view-class.md) oggetto passato come parametro quando viene chiamato un costruttore di gestione temporanea per creare un'istanza dell'oggetto `array`.|
...
|[reinterpret_as](#reinterpret_as)|Restituisce una matrice unidimensionale che contiene tutti gli elementi nell'oggetto `array`.|
|[section](#section)|Restituisce una sottosezione dell'oggetto `array` che si trova all'origine specificata e, facoltativamente, con l'extent specificato.|
|[view_as](#view_as)|Restituisce un oggetto [array_view](array-view-class.md) creato dall'oggetto `array`.|
...
|[operator=](#operator_eq)|Copia il contenuto dell'oggetto `array` specificato in questo oggetto.|
...
|[associated_accelerator_view](#associated_accelerator_view)|Ottiene il secondo [accelerator_view](accelerator-view-class.md) oggetto passato come parametro quando viene chiamato un costruttore di gestione temporanea per creare un'istanza dell'oggetto `array`.|
...
Il tipo di `array<T,N>` viene usato in diversi scenari:
...
*_Cpu_access_type*<br/>
[Access_type](concurrency-namespace-enums-amp.md#access_type) desiderato per la matrice sulla CPU. Il valore predefinito di questo parametro è `access_type_auto` lasciando la CPU `access_type` determinazione al runtime. È possibile eseguire query sul `access_type` della CPU effettivo per la matrice utilizzando il metodo `get_cpu_access_type`.
...
Ottiene il secondo [accelerator_view](accelerator-view-class.md) oggetto passato come parametro quando viene chiamato un costruttore di gestione temporanea per creare un'istanza dell'oggetto `array`.
...
Copia il contenuto del `array` in un altro `array`.
...
Restituisce un puntatore ai dati non elaborati della `array`.
...
Ottiene l'oggetto [extent](extent-class.md) che definisce la forma dell'`array`.
...
Restituisce il [accelerator_view](accelerator-view-class.md) oggetto che rappresenta la posizione in cui è allocato l'oggetto `array`. È possibile accedere a questa proprietà solo sulla CPU.
...
Oggetto `accelerator_view` che rappresenta la posizione in cui è allocato l'oggetto `array`.
...
Oggetto di tipo `vector<T>` contenente una copia dei dati contenuti nella matrice.
...
Copia il contenuto dell'oggetto `array` specificato.
...
*_Other*<br/>
Oggetto `array` da cui eseguire la copia.
...
Riferimento a questo oggetto `array`.

## <a name="rank"></a>Rank

Archivia il rango del `array`.
...
Restituisce una sottosezione dell'oggetto `array` che si trova all'origine specificata e, facoltativamente, con l'extent specificato. Quando viene specificato solo l'oggetto `index`, la sottosezione contiene tutti gli elementi della griglia associata che hanno indici maggiori degli indici degli elementi nell'oggetto `index`.
...
*_New_rank*<br/>
Rango del `extent` oggetto passato come parametro.

*_View_extent*<br/>
Extent utilizzato per costruire il nuovo oggetto [array_view](array-view-class.md) .

*value_type*<br/>
Il tipo di dati degli elementi nell'oggetto `array` originale e nell'oggetto `array_view` restituito.
