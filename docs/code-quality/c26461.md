...
ms.locfileid: "89385404"
---
# <a name="c26461-use_const_pointer_arguments"></a>USE_CONST_POINTER_ARGUMENTS C26461:

L'argomento '% argument%' del puntatore per la funzione '% Function%' può essere contrassegnato come puntatore a `const` (con. 3). 

Una funzione con un `T*` argomento può modificare il valore dell'oggetto. Se questo non è lo scopo della funzione, è preferibile rendere invece il puntatore a `const T*` .
