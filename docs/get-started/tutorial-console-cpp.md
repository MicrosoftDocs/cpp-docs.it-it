...
In genere il punto di partenza per un programmatore C++ è un'applicazione "Hello, world!" eseguita sulla riga di comando. Questo articolo illustra la procedura di creazione dell'applicazione in Visual Studio.

## <a name="prerequisites"></a>Prerequisiti

- Visual Studio con il carico di lavoro Sviluppo di applicazioni desktop con C++ deve essere installato e in esecuzione nel computer. Se non è ancora installato, vedere l'articolo relativo all'[installazione del supporto per C++ in Visual Studio 2017](../build/vscpp-step-0-installation.md).
...
Se si compila e si esegue di nuovo il codice a questo punto, viene comunque visualizzato "Hello World" nella console. Successivamente, si modificherà la funzione `main` per eseguire alcuni calcoli.
...
![Applicazione console Calculator](./media/calculator-app.gif "Applicazione console Calculator")
