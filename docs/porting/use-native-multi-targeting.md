...
È anche possibile sviluppare progetti di Visual Studio 2008, ma non possono essere usati se non vengono modificati. Per informazioni dettagliate, vedere la sezione **istruzioni per Visual Studio 2008** .
...
Visual Studio 2008 dispone di un proprio sistema di compilazione dedicato per C++ denominato **VCBuild**. A partire da Visual Studio 2010, i progetti di Visual Studio C++ sono stati modificati in modo da usare **MSBuild**. Ciò significa che se si esegue l'aggiornamento in modo permanente o multitargeting, è necessario eseguire un passaggio di aggiornamento per compilare i progetti di Visual Studio 2008 nella versione più recente di Visual Studio. Il progetto aggiornato genera ancora i file binari completamente compatibili con i file binari creati usando l'IDE di Visual Studio 2008.

Per prima cosa, oltre alla versione corrente di Visual Studio, è necessario installare Visual Studio 2010 nello stesso computer di Visual Studio 2008. Solo Visual Studio 2010 installa gli script **MSBuild** necessari per la destinazione dei progetti di Visual Studio 2008.
