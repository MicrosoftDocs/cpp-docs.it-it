...
description: Il C++ riferimento alle costanti TRACING_SESSION_MSVC_EVENT_FLAGS di build Insights SDK.
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
Le costanti `TRACING_SESSION_MSVC_EVENT_FLAGS` vengono utilizzate per descrivere gli eventi MSVC da raccogliere durante una traccia. Utilizzarli per inizializzare il campo `MsvcEventFlags` della struttura [TRACING_SESSION_OPTIONS](tracing-session-options-struct.md) .
