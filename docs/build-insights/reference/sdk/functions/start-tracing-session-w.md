...
description: Il riferimento alla funzione StartTracingSessionW dell'SDK di Build Insights in C.
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
La `StartTracingSessionW` funzione avvia una sessione di traccia. Gli eseguibili che chiamano questa funzione devono disporre di privilegi di amministratore.
...
*nomesessione*\
Nome della sessione di traccia da avviare. Utilizzare lo stesso nome quando si chiama [StopTracingSessionW](stop-tracing-session-w.md)o qualsiasi altra funzione di traccia di arresto.
