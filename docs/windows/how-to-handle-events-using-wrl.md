...
Per un esempio più semplice che crea un'istanza del componente e recupera un valore della proprietà, vedere [procedura: attivare e utilizzare un componente di Runtime Windows](../windows/how-to-activate-and-use-a-windows-runtime-component-using-wrl.md).

## <a name="subscribing-to-and-handling-events"></a>Sottoscrizione e la gestione degli eventi

La procedura seguente avvia un `ABI::Windows::System::Threading::IDeviceWatcher` dell'oggetto e usare i gestori eventi per monitorare lo stato di avanzamento. Il `IDeviceWatcher` interfaccia consente di enumerare i dispositivi in modo asincrono o in background e ricevere una notifica quando i dispositivi sono aggiunto, rimosso o modificati. Il [Callback](../windows/callback-function-windows-runtime-cpp-template-library.md) funzione è una parte importante di questo esempio perché consente di specificare i gestori di eventi che elaborano i risultati dell'operazione in background. Segue un esempio completo.
