---
title: Eliminazione di simboli non assegnati | Documenti Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-windows
ms.topic: conceptual
dev_langs:
- C++
helpviewer_keywords:
- symbols, deleting
- symbols, unassigned
- unassigned symbols
ms.assetid: 47641c46-1bad-44fb-8f85-79ae36919f13
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
- uwp
ms.openlocfilehash: 947249b4a0a9bd2233d76671b1b5bf05c70337db
ms.sourcegitcommit: d55ac596ba8f908f5d91d228dc070dad31cb8360
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 05/08/2018
---
# <a name="deleting-unassigned-symbols"></a>Eliminazione di simboli non assegnati
### <a name="to-delete-an-unassigned-unused-symbol"></a>Per eliminare un simbolo non assegnato (non usato)  
  
1.  Nel [la finestra di dialogo Simboli risorsa](../windows/resource-symbols-dialog-box.md), selezionare il simbolo che si desidera eliminare e fare clic su **eliminare**.  
  
    > [!NOTE]
    >  Prima di eliminare un simbolo non usato in un file di risorse, verificare che non venga usato in un punto del programma o da file di risorse inclusi in fase di compilazione.  
  

  
 **Requisiti**  
  
 Win32  
  
## <a name="see-also"></a>Vedere anche  
 [Modifica di simboli non assegnati](../windows/changing-unassigned-symbols.md)   
 [Restrizioni relative ai nomi di simbolo](../windows/symbol-name-restrictions.md)   
 [Restrizioni relative ai valori di simboli](../windows/symbol-value-restrictions.md)   
 [ID simbolo predefiniti](../windows/predefined-symbol-ids.md)