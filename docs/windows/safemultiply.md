...
Moltiplica raccolta due numeri in modo che protegge dall'overflow.  
...
#### Parametri  
 \[in\] `t`  
 Primo numero da moltiplicare.  L'espressione deve essere di tipo T.  
  
 \[in\] `u`  
 Secondo numero da moltiplicare.  L'espressione deve essere di tipo U.  
  
 \[out\] `result`  
 Il parametro `SafeMultiply` dove archiviare il risultato.  
  
## Valore restituito  
 `true` se non si verificano errori; `false` se si verifica un errore.  
  
## Note  
 Questo metodo fa parte di [Libreria SafeInt](../windows/safeint-library.md) ed è progettata per una singola operazione di moltiplicazione senza creare un'istanza di [Classe SafeInt](../windows/safeint-class.md).  
