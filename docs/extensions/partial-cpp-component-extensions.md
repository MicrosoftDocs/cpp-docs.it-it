...
Contenuto: Con due eccezioni, una definizione di classe parziale può contenere qualsiasi elemento che può essere incluso in una definizione di classe completa se la parola chiave **partial** viene omessa. Non è tuttavia possibile specificare l'accessibilità della classe (ad esempio `public partial class X { ... };`) o un oggetto **declspec**.
...
Dichiarazione: una definizione parziale di un oggetto *identifier* di una classe introduce solo il nome *identifier*, ma non è possibile usare *identifier* in un modo che richiede una definizione di classe. Il nome *identifier* non può essere usato per conoscere la dimensione di *identifier* o per usare una classe di base o un membro di *identifier* fino a quando il compilatore non raggiunge la definizione completa di *identifier*.

Numero e ordinamento: per *identifier* possono esserci zero o più definizioni di classi parziali. Ogni definizione di classe parziale di *identifier* deve precedere lessicalmente la definizione completa di *identifier* (se la definizione completa è presente; in caso contrario, la classe non può essere usata se non come dichiarazione con prototipo), ma non è necessario che preceda le dichiarazioni con prototipo di *identifier*. Tutti gli elementi class-key devono corrispondere.

Definizione completa: al momento della definizione completa dell'oggetto *identifier* della classe, il comportamento equivale a quello che si verifica quando la definizione di *identifier* dichiara tutte le classi di base, i membri e così via, nell'ordine in cui sono stati rilevati e definiti nelle classi parziali.
