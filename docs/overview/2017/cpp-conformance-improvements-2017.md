...
In Visual Studio 2017 versione 15.9 il codice genera questo errore: *error C3615: constexpr function 'S<int>::f' cannot result in a constant expression. note: failure was caused by call of undefined function or one not declared 'constexpr' note: see usage of 'g'*. Per evitare l'errore, rimuovere il qualificatore `constexpr` dalla creazione esplicita dell'istanza della funzione f(). 
