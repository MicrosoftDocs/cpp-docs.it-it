...
|[CStringt:: SetSysString](#setsysstring)|Imposta un oggetto BSTR esistente con i dati di un `CStringT` oggetto.|
...
`CString`, `CStringA` e `CStringW` vengono esportati dalla DLL MFC (MFC90.DLL), mai dalle DLL utente. Questa operazione viene eseguita per impedire `CStringT` che venga definita una moltiplicazione.
...
Zero se le stringhe sono identiche, ovvero ignorando la distinzione tra maiuscole e minuscole, < 0 se l' `CStringT` oggetto è minore di *PSZ* (maiuscole/minuscole) o > 0 se l' `CStringT` oggetto è maggiore di *PSZ* (caso ignorante).
...
A seconda del contenuto dell' `CStringT` oggetto, il valore di BSTR a cui fa riferimento *pbstr* può cambiare. La funzione genera un'eccezione `CMemoryException` se esiste una memoria insufficiente.
...
La `Tokenize` funzione trova il token successivo nella stringa di destinazione. Il set di caratteri in *pszTokens* specifica i possibili delimitatori del token da trovare. A ogni chiamata alla `Tokenize` funzione inizia in corrispondenza *di*, ignora i delimitatori iniziali e restituisce un `CStringT` oggetto contenente il token corrente, ovvero la stringa di caratteri fino al carattere delimitatore successivo. Il valore di *Tart* viene aggiornato in modo che sia la posizione successiva al carattere delimitatore finale, oppure-1 se è stata raggiunta la fine della stringa. Un numero maggiore di token può essere suddiviso dal resto della stringa di destinazione da una serie di chiamate a `Tokenize` , mediante l'utilizzo *di un* oggetto per tenere traccia della posizione della stringa in cui deve essere letto il token successivo. Quando non sono presenti altri token, la funzione restituirà una stringa vuota e l'oggetto *Tart* verrà impostato su-1.

Diversamente dalle funzioni CRT tokenize come [strtok_s, _strtok_s_l, wcstok_s, _wcstok_s_l, _mbstok_s](../../c-runtime-library/reference/strtok-s-strtok-s-l-wcstok-s-wcstok-s-l-mbstok-s-mbstok-s-l.md)_mbstok_s_l, non `Tokenize` modifica la stringa di destinazione.
