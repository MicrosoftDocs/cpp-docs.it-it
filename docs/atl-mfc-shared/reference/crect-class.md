...
|[CRect::P tInRect](#ptinrect)|Determina se il punto specificato si trova `CRect`all'interno di.|
...
Un `CRect` oggetto può essere passato come parametro di funzione ogni volta `RECT` che è `LPCRECT`possibile passare `LPRECT` una struttura, o.
...
Prestare attenzione quando si modifica un `CRect` oggetto con le funzioni membro [CDC::D ptolp](../../mfc/reference/cdc-class.md#dptolp) e [CDC:: DPtoLP](../../mfc/reference/cdc-class.md#lptodp) . Se la modalità di mapping di un contesto di visualizzazione è tale che l'extent y è negativo, come `MM_LOENGLISH`in, `CDC::DPtoLP` trasformerà `CRect` il in modo che la sua parte superiore sia maggiore della parte inferiore. Funzioni come `Height` e `Size` restituiranno quindi valori negativi per l'altezza dell'oggetto trasformato `CRect`e il rettangolo non sarà normalizzato.

Quando si usano `CRect` gli operatori di overload, il primo operando deve `CRect`essere un; il secondo può essere una struttura [Rect](/windows/win32/api/windef/ns-windef-rect) o un `CRect` oggetto.
...
A tale scopo, `DeflateRect` aggiunge unità a sinistra e in alto e sottrae le unità a destra e in basso. I parametri di `DeflateRect` sono valori con segno. i valori positivi deflate `CRect` e i valori negativi lo inflatano.
...
*lpRect*<br/>
Punta a una [RECT](/windows/win32/api/windef/ns-windef-rect) struttura o `CRect` a un oggetto Rect che contiene le coordinate dell'angolo superiore sinistro e inferiore destro di un rettangolo.
...
A tale scopo, `InflateRect` sottrae le unità dal lato sinistro e superiore e aggiunge le unità a destra e in basso. I parametri di `InflateRect` sono valori con segno. i valori positivi sono inflat `CRect` e i valori negativi lo deflatano.
...
*lpRect1*<br/>
Punta a una [RECT](/windows/win32/api/windef/ns-windef-rect) struttura o `CRect` a un oggetto Rect contenente un rettangolo di origine.
...
Diverso da zero `CRect` se è vuoto; 0 se `CRect` non è vuoto.
...
## <a name="crectoperator-lpcrect-converts-a-crect-to-an-lpcrect"></a><a name="operator_lpcrect"></a>CRect:: operator LPCRECT converte un `CRect` oggetto in un [LPCRECT](../../mfc/reference/data-types-mfc.md).
...
*srcRect*<br/>
Fa riferimento a un rettangolo di origine. Può essere un [RECT](/windows/win32/api/windef/ns-windef-rect) oggetto Rect `CRect`o.
...
*lpRect*<br/>
Punta a una [RECT](/windows/win32/api/windef/ns-windef-rect) struttura o `CRect` a un oggetto Rect che contiene il numero di unità per cui si desidera gonfiare ogni lato `CRect`.
...
*lpRect*<br/>
Punta a una [RECT](/windows/win32/api/windef/ns-windef-rect) struttura o `CRect` a un oggetto Rect che contiene il numero di unità per cui deflatare `CRect`ogni lato.

### <a name="remarks"></a>Osservazioni

I valori *x* e *y* del parametro ( `cx` o `cy`e) vengono sottratti da `CRect`.

Il terzo overload viene deflatto in `CRect` base al numero di unità specificato in ogni membro del parametro. Si noti che questo overload funziona come [DeflateRect](#deflaterect).
...
*Rect*<br/>
Contiene un [RECT](/windows/win32/api/windef/ns-windef-rect) oggetto Rect `CRect`o.
...
*lpRect*<br/>
Punta a una [RECT](/windows/win32/api/windef/ns-windef-rect) struttura o `CRect` a un oggetto Rect che contiene il numero di unità per ingrandire ogni lato del valore restituito.
...
*lpRect*<br/>
Punta a una [RECT](/windows/win32/api/windef/ns-windef-rect) struttura o `CRect` a un oggetto Rect che contiene il numero di unità per deflatare ogni lato del valore restituito.
...
*lpRectSrc1*<br/>
Punta alla struttura [RECT](/windows/win32/api/windef/ns-windef-rect) o `CRect` all'oggetto Rect da cui deve essere sottratto un rettangolo.
...
*lpRect1*<br/>
Punta a un [RECT](/windows/win32/api/windef/ns-windef-rect) oggetto Rect `CRect` o che contiene un rettangolo di origine.
