...
*BaseType*<br/>
Tipo di carattere della classe String. Può essere uno dei collegamenti seguenti:
...
|Name|Descrizione|
|----------|-----------------|
|[CSimpleStringT:: Append](#append)|Accoda un oggetto `CSimpleStringT` a un oggetto `CSimpleStringT` esistente.|
|[CSimpleStringT:: AppendChar](#appendchar)|Aggiunge un carattere a un oggetto `CSimpleStringT` esistente.|
...
|[CSimpleStringT:: GetAllocLength](#getalloclength)|Recupera la lunghezza allocata di un oggetto `CSimpleStringT`.|
...
|[CSimpleStringT:: GetBufferSetLength](#getbuffersetlength)|Restituisce un puntatore ai caratteri in una `CSimpleStringT`, troncando alla lunghezza specificata.|
|[CSimpleStringT:: GetLength](#getlength)|Restituisce il numero di caratteri in un oggetto `CSimpleStringT`.|
|[CSimpleStringT:: GetManager](#getmanager)|Recupera il gestore della memoria dell'oggetto `CSimpleStringT`.|
|[CSimpleStringT:: GetString](#getstring)|Recupera la stringa di caratteri|
|[CSimpleStringT:: IsEmpty](#isempty)|Verifica se un oggetto `CSimpleStringT` non contiene caratteri.|
...
|[CSimpleStringT:: ReleaseBuffer](#releasebuffer)|Rilascia il controllo del buffer restituito dal `GetBuffer`.|
|[CSimpleStringT:: ReleaseBufferSetLength](#releasebuffersetlength)|Rilascia il controllo del buffer restituito dal `GetBuffer`.|
|[CSimpleStringT:: SetAt](#setat)|Imposta un carattere in una posizione specificata.|
|[CSimpleStringT:: semanager](#setmanager)|Imposta il gestore della memoria di un oggetto `CSimpleStringT`.|
|[CSimpleStringT:: sestring](#setstring)|Imposta la stringa di un oggetto `CSimpleStringT`.|
...
|Name|Descrizione|
|----------|-----------------|
|[CSimpleStringT:: operator PCXSTR](#operator_pcxstr)|Accede direttamente ai caratteri archiviati in un oggetto `CSimpleStringT` come una stringa di tipo C.|
...
|[CSimpleStringT:: operator =](#operator_eq)|Assegna un nuovo valore a un oggetto `CSimpleStringT`.|
...
Accoda un oggetto `CSimpleStringT` a un oggetto `CSimpleStringT` esistente.
...
Chiamare questo metodo per aggiungere un oggetto `CSimpleStringT` esistente a un altro oggetto `CSimpleStringT`.
...
Aggiunge un carattere a un oggetto `CSimpleStringT` esistente.
...
Chiamare questa funzione per aggiungere il carattere specificato alla fine di un oggetto `CSimpleStringT` esistente.

##  <a name="copychars"></a>CSimpleStringT:: CopyChars

Copia un carattere o caratteri in un oggetto `CSimpleStringT`.
...
Chiamare questo metodo per copiare i caratteri da *pchSrc* alla stringa *pchDest* . A differenza di `CopyChars`, `CopyCharsOverlapped` fornisce un metodo sicuro per la copia da buffer di caratteri che potrebbero essere sovrapposti.
...
*strSrc*<br/>
Oggetto `CSimpleStringT` esistente da copiare in questo oggetto `CSimpleStringT`.

*pchSrc*<br/>
Puntatore a una matrice di caratteri di lunghezza *nLength*, not null terminato.

*pszSrc*<br/>
Stringa con terminazione null da copiare in questo oggetto `CSimpleStringT`.

*nLength*<br/>
Conteggio del numero di caratteri in `pch`.

*pStringMgr*<br/>
Puntatore al gestore della memoria dell'oggetto `CSimpleStringT`. Per ulteriori informazioni su `IAtlStringMgr` e sulla gestione della memoria per `CSimpleStringT`, vedere [gestione della memoria e CStringT](../memory-management-with-cstringt.md).
...
Nell'esempio seguente viene illustrato l'uso di `CSimpleStringT::CSimpleStringT` usando l'`CSimpleString`**typedef** di ATL. `CSimpleString` è una specializzazione comunemente utilizzata del modello di classe `CSimpleStringT`.
...
Rende questa `CSimpleStringT` oggetto una stringa vuota e libera la memoria nel modo appropriato.
...
Recupera la lunghezza allocata di un oggetto `CSimpleStringT`.
...
Chiamare questo metodo per determinare il numero di caratteri allocati per questo oggetto `CSimpleStringT`. Vedere [FreeExtra](#freeextra) per un esempio di chiamata a questa funzione.

##  <a name="getat"></a>CSimpleStringT:: GetAt

Restituisce un carattere da un oggetto `CSimpleStringT`.
...
*iChar*<br/>
Indice in base zero del carattere nell'oggetto `CSimpleStringT`. Il parametro *ICHAR* deve essere maggiore o uguale a 0 e minore del valore restituito da [GetLength](#getlength). In caso contrario, `GetAt` genererà un'eccezione.

### <a name="return-value"></a>Valore restituito

`XCHAR` contenente il carattere in corrispondenza della posizione specificata nella stringa.

### <a name="remarks"></a>Note

Chiamare questo metodo per restituire il carattere specificato da *ICHAR*. L'operatore di indice ( **[]** ) di overload è un alias pratico per `GetAt`. Il terminatore null è indirizzabile senza generare un'eccezione usando `GetAt`. Tuttavia, non viene conteggiato da `GetLength`e il valore restituito è 0.
...
Chiamare questo metodo per restituire il contenuto del buffer dell'oggetto `CSimpleStringT`. Il `PXSTR` restituito non è una costante e pertanto consente la modifica diretta del contenuto del `CSimpleStringT`.
...
L'indirizzo restituito da `GetBuffer` potrebbe non essere valido dopo la chiamata a `ReleaseBuffer` perché altre operazioni di `CSimpleStringT` possono causare la riallocazione del buffer `CSimpleStringT`. Il buffer non viene riallocato se non si modifica la lunghezza del `CSimpleStringT`.
...
Se la memoria disponibile non è sufficiente per soddisfare la richiesta di `GetBuffer`, questo metodo genera un'eccezione CMemoryException *.
...
Restituisce un puntatore al buffer di caratteri interno per l'oggetto `CSimpleStringT`, troncando o aumentando la lunghezza, se necessario, per corrispondere esattamente alla lunghezza specificata in *nLength*.
...
*nLength*<br/>
Dimensioni esatte del buffer di caratteri `CSimpleStringT` in caratteri.
...
Chiamare questo metodo per recuperare una lunghezza specificata del buffer interno dell'oggetto `CSimpleStringT`. Il puntatore `PXSTR` restituito non è **const** e pertanto consente la modifica diretta del contenuto del `CSimpleStringT`.

Se si usa il puntatore restituito da [GetBufferSetLength](#getbuffersetlength) per modificare il contenuto della stringa, chiamare `ReleaseBuffer` per aggiornare lo stato interno di `CsimpleStringT` prima di usare altri metodi di `CSimpleStringT`.

L'indirizzo restituito da `GetBufferSetLength` potrebbe non essere valido dopo la chiamata a `ReleaseBuffer` perché altre operazioni di `CSimpleStringT` possono causare la riallocazione del buffer `CSimpleStringT`. Il buffer non viene riassegnato se non si modifica la lunghezza del `CSimpleStringT`.

La memoria del buffer viene liberata automaticamente quando l'oggetto `CSimpleStringT` viene eliminato definitivamente.

Se si tiene traccia della lunghezza della stringa manualmente, non aggiungere il carattere null di terminazione. È necessario specificare la lunghezza finale della stringa quando si rilascia il buffer utilizzando `ReleaseBuffer`. Se si aggiunge un carattere di terminazione null quando si chiama `ReleaseBuffer`, Pass-1 (impostazione predefinita) per la lunghezza da `ReleaseBuffer`e `ReleaseBuffer` eseguirà un `strlen` sul buffer per determinarne la lunghezza.
...
Per i set di caratteri multibyte (MBCS), `GetLength` conta ogni carattere a 8 bit; ovvero un byte di apertura e di chiusura in un carattere multibyte viene conteggiato come due byte. Vedere [FreeExtra](#freeextra) per un esempio di chiamata a questa funzione.
...
Chiamare questo metodo per recuperare il gestore della memoria usato dall'oggetto `CSimpleStringT`. Per ulteriori informazioni sui gestori della memoria e sugli oggetti stringa, vedere [gestione della memoria e CStringT](../memory-management-with-cstringt.md).
...
> [!NOTE]
>  Il puntatore `PCXSTR` restituito è **const** e non consente la modifica diretta del contenuto del `CSimpleStringT`.
...
Verifica un oggetto `CSimpleStringT` per la condizione vuota.
...
Restituisce TRUE se la lunghezza dell'oggetto `CSimpleStringT` è 0; in caso contrario, FALSE.
...
Puntatore a un oggetto `CSimpleStringT` o a una stringa con terminazione null.
...
Al termine della `LockBuffer`, chiamare [UnlockBuffer](#unlockbuffer) per reimpostare il conteggio dei riferimenti su 1.

> [!NOTE]
>  Se si chiama [GetBuffer](#getbuffer) in un buffer bloccato e si imposta il parametro `GetBuffer` `nMinBufferLength` su un valore maggiore della lunghezza del buffer corrente, si perderà il blocco del buffer. Tale chiamata a `GetBuffer` Elimina il buffer corrente, lo sostituisce con un buffer della dimensione richiesta e reimposta il conteggio dei riferimenti su zero.
...
> [!NOTE]
>  È possibile utilizzare l'operatore di indice ( **[]** ) per ottenere il valore di un carattere in un `CSimpleStringT`, ma non è possibile utilizzarlo per modificare il valore di un carattere in una `CSimpleStringT`.
...
*strSrc*<br/>
Puntatore a un oggetto `CSimpleStringT` esistente.
...
L'operatore accetta un altro `CSimpleStringT` oggetto o un carattere. Si noti che le eccezioni di memoria possono verificarsi quando si usa questo operatore di concatenazione perché è possibile allocare una nuova risorsa di archiviazione per i caratteri aggiunti a questo oggetto `CSimpleStringT`.
...
Assegna un nuovo valore a un oggetto `CSimpleStringT`.
...
Accede direttamente ai caratteri archiviati in un oggetto `CSimpleStringT` come una stringa di tipo C.
...
Nessun carattere copiato; viene restituito solo un puntatore. Prestare attenzione a questo operatore. Se si modifica un oggetto `CString` dopo avere ottenuto il puntatore a caratteri, è possibile che si verifichi una riallocazione della memoria che invalida il puntatore.
...
##  <a name="preallocate"></a>CSimpleStringT::P riallocare
...
*nNewLength*<br/>
Nuova lunghezza della stringa in caratteri, senza contare un carattere di terminazione null. Se la stringa è con terminazione null, il valore predefinito-1 imposta la dimensione `CSimpleStringT` sulla lunghezza corrente della stringa.

### <a name="remarks"></a>Note

Chiamare questo metodo per riallocare o liberare il buffer dell'oggetto stringa. Se si è certi che la stringa nel buffer è con terminazione null, è possibile omettere l'argomento *nNewLength* . Se la stringa non è con terminazione null, usare *nNewLength* per specificarne la lunghezza. L'indirizzo restituito da [GetBuffer](#getbuffer) non è valido dopo la chiamata a `ReleaseBuffer` o qualsiasi altra operazione di `CSimpleStringT`.
...
Imposta un singolo carattere da un oggetto `CSimpleStringT`.
...
*iChar*<br/>
Indice in base zero del carattere nell'oggetto `CSimpleStringT`. Il parametro *ICHAR* deve essere maggiore o uguale a 0 e minore del valore restituito da [GetLength](#getlength).
...
##  <a name="setmanager"></a>CSimpleStringT:: semanager
...
Chiamare questo metodo per specificare un nuovo gestore della memoria usato dall'oggetto `CSimpleStringT`. Per ulteriori informazioni sui gestori della memoria e sugli oggetti stringa, vedere [gestione della memoria e CStringT](../memory-management-with-cstringt.md).
...
##  <a name="setstring"></a>CSimpleStringT:: sestring
...
Entrambe le versioni di `SetString` verificano se *pszSrc* è un puntatore null e, in caso contrario, generano un errore E_INVALIDARG.

La versione a un parametro di `SetString` prevede che *pszSrc* punti a una stringa con terminazione null.
...
La versione a due parametri di `SetString` controlla anche se *pszSrc* punta a una posizione nel buffer corrente nel `CSimpleStringT`. In questo caso speciale, `SetString` usa una funzione di copia della memoria che non sovrascrive i dati stringa durante la copia dei dati di stringa nel buffer.
...
Il distruttore `CSimpleStringT` chiama automaticamente `UnlockBuffer` per assicurarsi che il buffer non venga bloccato quando viene chiamato il distruttore. Per un esempio di questo metodo, vedere [LockBuffer](#lockbuffer).
