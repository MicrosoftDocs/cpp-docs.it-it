---
title: Classe CFileTime | Documenti Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
|[CFileTime::LocalToUTC](#localtoutc)|Chiamare questo metodo per convertire l'ora di un file basata su Coordinated Universal Time (UTC) ora di un file locale.|  
...
## <a name="remarks"></a>Note  
 Questa classe fornisce metodi per gestire i valori di data e ora associati con la creazione, l'accesso e modifica dei file. I metodi e i dati di questa classe vengono spesso utilizzati in combinazione con `CFileTimeSpan` oggetti, trattano i valori di tempo relativo.  
 ...
 Sono disponibili le seguenti variabili membro const statici di semplificare i calcoli:  
 ...
 **Nota** non tutti i file System in grado di registrare la creazione e l'ora dell'ultimo accesso e non tutti i sistemi di file registrano allo stesso modo. Per esempio, nel file system FAT di Windows NT, creare ora ha una risoluzione di 10 millisecondi, ora di scrittura con una risoluzione di 2 secondi e tempo di accesso con una risoluzione di 1 giorno (Data access). Su NTFS, tempo di accesso con una risoluzione di 1 ora. Inoltre, FAT registra volte su disco nell'ora locale, ma NTFS registra volte sul disco in formato UTC. Per ulteriori informazioni, vedere [volte File](http://msdn.microsoft.com/library/windows/desktop/ms724290).  
...
### <a name="remarks"></a>Note  
 Il `CFileTime` oggetto può essere creato utilizzando un'esistente di data e ora da un `FILETIME` struttura o espressa come valore a 64 bit (in locale o formati di ora (UTC) Coordinated Universal Time). Il costruttore predefinito imposta l'ora su 0.  
...
### <a name="example"></a>Esempio  
 Per vedere l'esempio [CFileTime::Millisecond](#millisecond).  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce la data e l'ora come numero a 64 bit, che può essere nel formato Coordinated Universal Time (UTC) o locale.  
...
##  <a name="localtoutc"></a>  CFileTime::LocalToUTC  
 Chiamare questo metodo per convertire l'ora di un file basata su Coordinated Universal Time (UTC) ora di un file locale.  
...
### <a name="example"></a>Esempio  
 Per vedere l'esempio [CFileTime::UTCToLocal](#utctolocal).  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce **true** se l'elemento da confrontare non è uguale al `CFileTime` oggetto, in caso contrario **false**.  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce l'aggiornamento `CFileTime` oggetto che rappresenta il risultato dell'ora originale più tempo relativo.  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce **true** se il primo oggetto è minore di (precedente nel tempo), il secondo **false** in caso contrario.  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce l'aggiornamento `CFileTime` oggetto.  
...
### <a name="parameters"></a>Parametri  
 `nTime`  
 Il valore a 64 bit che rappresenta la data e l'ora, nel formato Coordinated Universal Time (UTC) o locale.  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce un `CFileTime` oggetto contenente l'ora in formato ora file locale.  
