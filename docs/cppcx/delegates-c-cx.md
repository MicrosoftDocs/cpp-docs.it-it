...
ms.locfileid: "70740274"
---
# <a name="delegates-ccx"></a>Delegati (C++/CX)

La `delegate` parola chiave viene usata per dichiarare un tipo di riferimento che è l'equivalente Windows Runtime di un oggetto funzione C++in standard. Dichiarazione di un delegato simile a una firma della funzione. Specifica il tipo restituito e i tipi di parametro per la relativa funzione di cui è stato eseguito il wrapping. Si tratta di una dichiarazione di un delegato definita dall'utente:
...
Quando si dichiarano delegati che verranno esposti ai client attraverso l'interfaccia binaria dell'applicazione Windows Runtime, utilizzare [Windows:\<: Foundation:: TypedEventHandler TSender, TResult >](/uwp/api/windows.foundation.typedeventhandler). I binari proxy e stub usati come prefisso per questo delegato ne permettono l'uso da parte di client Javascript.
...
Nell'esempio seguente un'app client passa un delegato personalizzato a un metodo pubblico in un componente Windows Runtime che esegue il delegato rispetto a ogni elemento in un `Vector`oggetto:
