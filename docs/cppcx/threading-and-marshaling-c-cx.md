---
description: 'Altre informazioni su: Threading e marshalling (C++/CX)'
...
Se nessuna di queste condizioni è applicabile, puoi contrassegnare la classe contenitore come non Agile. In altre parole, è consigliabile tenere direttamente gli oggetti non Agile solo nelle classi non agile e mantenere gli oggetti non agile tramite Platform:: agile \<T> nelle classi agile.
...
Quando si definisce una classe di riferimento in C++/CX, è agile per impostazione predefinita, ovvero presenta `ThreadingModel` = Both e `MarshallingType` = agile.  Se si usa la libreria di modelli di Windows Runtime C++, è possibile rendere agile la classe tramite la derivazione da `FtmBase` , che usa `FreeThreadedMarshaller` .  Se crei una classe con l'attributo `ThreadingModel`impostato su Both o l'attributo `ThreadingModel`impostato su MTA, assicurati che la classe sia thread-safe.
...
Nell'esempio seguente viene illustrato come applicare `MarshalingBehavior` `ThreadingModel` gli attributi e a una classe di runtime in una libreria di classi Windows Runtime. Quando un'applicazione utilizza la DLL e utilizza la parola chiave `ref new` per attivare un oggetto classe `MySTAClass` , l'oggetto viene attivato in un apartment a thread singolo e non supporta il marshalling.
