...
-   [Creare cinque typedef](#vcconstorage_and_exposure_typedefs) che descrive come vengono archiviati gli elementi della raccolta e modo in cui vengono esposti ai client attraverso interfacce COM.  
...
##  <a name="vcconstorage_and_exposure_typedefs"></a> Creazione di TypeDef per l'archiviazione e l'esposizione  
 ...
 In questo caso, verranno archiviati i dati come un **std:: Vector** di **std:: String**s. **std:: Vector** è una classe di contenitori della libreria Standard C++ che si comporta come una matrice gestita. **std:: String** classe stringa della libreria Standard C++. Queste classi rendono più facile da usare con una raccolta di stringhe.  
