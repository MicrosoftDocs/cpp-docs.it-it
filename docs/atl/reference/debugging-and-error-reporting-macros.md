...
|||
|-|-|
|[_ATL_DEBUG_INTERFACES](#_atl_debug_interfaces)|Scrive, nella finestra di output, qualsiasi perdita di interfaccia che viene rilevata quando viene chiamato `_Module.Term`.|
...
Definire questa macro prima di includere tutti i file di intestazione ATL per tracciare tutte le chiamate `AddRef` e `Release` sulle interfacce dei componenti nella finestra di output.
...
La prima parte di ogni traccia sarà sempre `ATL: QIThunk`. Next è un valore che identifica il *thunk di interfaccia* particolare usato. Un thunk di interfaccia è un oggetto usato per mantenere un conteggio dei riferimenti e fornire la funzionalità di traccia usata qui. Un nuovo thunk di interfaccia viene creato a ogni chiamata a `QueryInterface` ad eccezione delle richieste per l'interfaccia `IUnknown` (in questo caso, viene restituito lo stesso thunk ogni volta per conformarsi alle regole di identità di COM).

Verrà quindi visualizzato `AddRef` o `Release` che indica il metodo chiamato. In seguito, verrà visualizzato un valore che identifica l'oggetto di cui è stato modificato il conteggio dei riferimenti dell'interfaccia. Il valore tracciato è il puntatore **this** dell'oggetto.

Il conteggio dei riferimenti tracciato è il conteggio dei riferimenti in tale thunk dopo la chiamata di `AddRef` o `Release`. Si noti che questo conteggio dei riferimenti potrebbe non corrispondere al conteggio dei riferimenti per l'oggetto. Ogni thunk mantiene un conteggio dei riferimenti per consentire una conformità completa alle regole di conteggio dei riferimenti di COM.

L'informazione finale tracciata è il nome dell'oggetto e l'interfaccia interessata dalla chiamata `AddRef` o `Release`.

Eventuali perdite di interfaccia rilevate quando il server si arresta e viene chiamato `_Module.Term` viene registrato come segue:
...
Se una chiamata a `QueryInterface` non è riuscita, nella finestra di output viene visualizzato:
...
In ATLENSURE_THROW caso, viene chiamato `AtlThrow` con il valore HRESULT specificato.
...
*lpszFormat*<br/>
in Stringa di formato in stile `printf`da usare per creare una stringa da inviare al dispositivo di dump.
...
Per dichiarare una categoria di traccia personalizzata, dichiarare un'istanza globale della classe `CTraceCategory` come indicato di seguito:
...
Per specificare che si desidera filtrare i messaggi di traccia, inserire le definizioni per queste macro in stdafx. h prima dell'istruzione `#include <atlbase.h>`.
