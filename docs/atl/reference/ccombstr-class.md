...
|[CComBSTR::Detach](#detach)|`m_str` Disconnette`CComBSTR` dall'oggetto.|
...
|NOME|Descrizione|
|----------|-----------------|
|[CComBSTR::m_str](#m_str)|Contiene il BSTR associato `CComBSTR` all'oggetto.|
...
> [!NOTE]
>  A causa del miglioramento del comportamento di ricerca implementato in Visual Studio .NET, il `bstr = L"String2" + bstr;`codice, ad esempio, che potrebbe essere stato compilato nelle versioni precedenti, `bstr = CStringW(L"String2") + bstr`deve invece essere implementato come.

Per un elenco di attenzioni quando si `CComBSTR`USA, vedere [programmazione con CComBSTR](../../atl/programming-with-ccombstr-atl.md).
...
Libera qualsiasi stringa esistente contenuta nell' `CComBSTR` oggetto, quindi crea un BSTR dal primo carattere di ogni elemento di SAFEARRAY e lo collega `CComBSTR` all'oggetto.
...
##  <a name="attach"></a>CComBSTR:: Connetti

Connette un BSTR all' `CComBSTR` oggetto impostando il membro [m_str](#m_str) su *src*.
...
> [!NOTE]
>  Questo metodo asserirà `m_str` se è diverso da null.
...
Il costruttore di copia `m_str` imposta su una copia del membro BSTR di *src*. Il `REFGUID` Costruttore converte il GUID in una stringa utilizzando `StringFromGUID2` e archivia il risultato.
...
Dopo la chiamata a questo metodo, la variante a cui punta *pvarDest* sarà di tipo VT_BSTR.
...
BSTR associato `CComBSTR` all'oggetto.
...
Contiene il BSTR associato `CComBSTR` all'oggetto.
...
##  <a name="operator_amp"></a>Operatore CComBSTR::&amp;
...
`CComBstr operator &`dispone di un'asserzione speciale associata per facilitare l'identificazione delle perdite di memoria. Il programma verrà asserito `m_str` quando il membro viene inizializzato. Questa asserzione è stata creata per identificare le `& operator` situazioni in cui un programmatore usa per assegnare un nuovo valore al `m_str` membro senza liberare la prima `m_str`allocazione di. Se `m_str` è uguale a null, il programma presuppone che m_str non sia stato ancora allocato. In questo caso, il programma non verrà asserita.
...
##  <a name="operator_lt"></a>Operatore CComBSTR::&lt;
...
Imposta il membro [m_str](#m_str) su una copia di *pSrc* o su una copia del membro BSTR di *src*. L'operatore di assegnazione di `src` spostamento viene spostato senza copiarlo.
...
##  <a name="operator_gt"></a>Operatore CComBSTR::&gt;
...
Per `CharLowerBuff` ulteriori informazioni sulla modalità di esecuzione della conversione, vedere.
...
Per `CharUpperBuff` ulteriori informazioni sulla modalità di esecuzione della conversione, vedere.
