---
title: Classe CComCurrency | Documenti Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-atl
...
ms.locfileid: "32366247"
---
# <a name="ccomcurrency-class"></a>Classe di CComCurrency
...
## <a name="remarks"></a>Note  
 `CComCurrency` è un wrapper per il **valuta** tipo di dati. **VALUTA** viene implementato come un valore integer a 8 byte in complemento a due ridimensionato di 10.000. In questo modo si ottiene un numero a virgola fissa con 15 cifre a sinistra del separatore decimale e 4 cifre a destra. Il **valuta** tipo di dati è estremamente utile per i calcoli che includono denaro o per i calcoli a virgola fissa in cui la precisione è importante.  
  
 Il **CComCurrency** wrapper implementa le operazioni aritmetiche, di assegnazione e confronto per questo tipo a virgola fissa. Le applicazioni supportate sono state selezionate per controllare gli errori di arrotondamento che possono verificarsi durante i calcoli a virgola fissa.  
  
 L'oggetto `CComCurrency` fornisce accesso ai numeri su entrambi i lati del separatore decimale sotto forma di due componenti: un componente Integer che archivia il valore a sinistra del separatore decimale e un componente frazionario che archivia il valore a destra del separatore decimale. Il componente frazionario viene archiviato internamente come valore intero compreso tra -9999 ( **CY_MIN_FRACTION**) e + 9999 ( **CY_MAX_FRACTION**). Il metodo [CComCurrency:: Getfraction](#getfraction) restituisce un valore scalato da un fattore 10.000 ( **CY_SCALE**).  
...
|$10.05|CComCurrency (10.500) *o* CComCurrency (10,05)|  
 ...
 I valori **CY_MIN_FRACTION**, **CY_MAX_FRACTION**, e **CY_SCALE** definiti in atlcur. h.  
...
### <a name="parameters"></a>Parametri  
 ...
 `nInteger`, *nFraction*  
 I componenti integer e frazionario del valore di valuta iniziale. Vedere il [CComCurrency](../../atl/reference/ccomcurrency-class.md) overview per ulteriori informazioni.  
...
### <a name="remarks"></a>Note  
 Il costruttore imposta il valore iniziale di [CComCurrency::m_currency](#m_currency)e accetta un'ampia gamma di tipi di dati, inclusi i valori integer, stringhe, numeri a virgola mobile, **valuta** variabili e altri `CComCurrency` oggetti. Se non viene fornito alcun valore, `m_currency` è impostato su 0.  
  
 In caso di errore, ad esempio un overflow, i costruttori privi di una specifica di eccezione vuota ( **throw ()**) chiamare `AtlThrow` con un valore HRESULT che descrive l'errore.  
  
 Quando si utilizzano i valori a virgola mobile o doppie per assegnare un valore, si noti che **CComCurrency (10,50)** equivale a **CComCurrency (10.5000)** e non **CComCurrency(10,50)**.  
...
### <a name="remarks"></a>Note  
 Il componente frazionario è un valore integer a 4 cifre compreso tra -9999 ( **CY_MIN_FRACTION**) e + 9999 ( **CY_MAX_FRACTION**). `GetFraction` Restituisce il valore ridimensionato di 10000 ( **CY_SCALE**). I valori di **CY_MIN_FRACTION**, **CY_MAX_FRACTION**, e **CY_SCALE** definiti in atlcur. h.  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce **true** se l'elemento da confrontare non è uguale al `CComCurrency` oggetto; in caso contrario, **false**.  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce l'aggiornamento `CComCurrency` oggetto. In caso di errore, ad esempio un overflow, questo operatore chiama `AtlThrow` con un valore HRESULT che descrive l'errore.  
...
### <a name="return-value"></a>Valore restituito  
 Restituisce l'aggiornamento `CComCurrency` oggetto. Se il divisore è 0, si verificherà un errore di asserzione.  
...
##  <a name="operator_currency"></a>  CComCurrency::operator valuta  
 Questi operatori vengono utilizzati per eseguire il cast un `CComCurrency` l'oggetto in un **valuta** tipo di dati.  
...
### <a name="parameters"></a>Parametri  
 *nFraction*  
 Il valore da assegnare al componente frazionario del `m_currency` (membro dati). Il segno del componente frazionario necessario lo stesso come il componente integer e il valore deve essere nell'intervallo da -9999 ( **CY_MIN_FRACTION**) a + 9999 ( **CY_MAX_FRACTION**).  
...
### <a name="parameters"></a>Parametri  
 ...
 `nInteger` deve essere compreso nell'intervallo **CY_MIN_INTEGER** alla **CY_MAX_INTEGER** inclusivo. Questi valori sono definiti in atlcur. h.  
