...
|Avviso del compilatore (livello 1) C4807|'*operazione*': combinazione non affidabile del tipo '*type1*'e firmati campo di bit di tipo'*type2*'|
|Avviso del compilatore (livello 1) C4808|case '*valore*' non è un valore valido per la condizione switch di tipo 'bool'|
|Avviso del compilatore (livello 1) C4809|istruzione switch è ridondante 'default' etichetta; vengono fornite tutti i possibili etichette 'case'|
...
|[Avviso del compilatore (livello 1) C4819](compiler-warning-level-1-c4819.md)|Il file contiene un carattere che non può essere rappresentato nella tabella codici corrente (numero). Salvare il file in formato Unicode per evitare perdite di dati|
...
|[Avviso del compilatore (livello 3) C4823](compiler-warning-level-3-c4823.md)|'*funzione*': utilizza puntatori di blocco di rimozione, ma la semantica non è abilitata. Si consiglia di utilizzare /EHa|
...
|[Avviso del compilatore (livello 1) C4835](compiler-warning-level-1-c4835.md)|'*variabile*': l'inizializzatore per i dati esportati non verrà eseguita fino al codice gestito viene eseguito per prima nell'assembly su host|
|Avviso del compilatore (livello 4) C4837|rilevato trigramma: '? *carattere*'sostituito'*carattere*'|
...
|[Avviso del compilatore (livello 3) C4839](compiler-warning-level-3-c4839.md)|utilizzo standard della classe*tipo*' come argomento a una funzione variadic|
...
|Avviso C4844 del compilatore|' esportare modulo *nome_modulo*;' è ora la sintassi preferita per la dichiarazione di un'interfaccia di modulo|
...
|[Compilatore (livello 4) Avviso C4868](compiler-warning-c4868.md)|'_file_(*line_number*)' compilatore non può imporre l'ordine di valutazione da sinistra a destra nell'elenco di inizializzazione tra parentesi graffe|
...
|Avviso del compilatore (livello 1) C4880|esegue il cast da ' const *type_1*'a'*type_2*': cast di constness da un puntatore o riferimento può comportare un comportamento non definito in una funzione con restrizioni amp|
...
|Avviso del compilatore (livello 1) C4910|'\<identificatore >: 'dllexport' ed 'extern' sono incompatibili in un'istanza esplicita|
...
|[Avviso del compilatore (livello 1) C4930](compiler-warning-level-1-c4930.md)|'*prototipo*': funzione con prototipo non chiamata (invece di una definizione di variabile destinata)?|
...
|[Avviso del compilatore (livello 2) C4948](compiler-warning-level-2-c4948.md)|tipo restituito di '*della funzione di accesso*' non corrisponde al tipo dell'ultimo parametro del metodo set corrispondente|
...
|Avviso del compilatore (livello 1) C4951|'*funzione*' è stato modificato dopo profilo sono stati raccolti dati, dati di profilo di funzione non utilizzati|
|Avviso del compilatore (livello 1) C4952|'*funzione*': dati di profilo non trovato nel database di programma '*pgd_file*'|
|Avviso del compilatore (livello 1) C4953|Entità incorporata '*funzione*' è stato modificato dopo profilo sono stati raccolti dati, i dati di profilo non utilizzati|
|Avviso C4954 del compilatore|'*funzione*': non eseguire il profiling (contiene un'espressione switch __int64)|
...
|Avviso del compilatore (livello 1, errore) l'avviso C4958|'*operazione*': aritmetica dei puntatori non verificabile|
...
|Avviso del compilatore (livello 4) C4962|'*funzione*': ottimizzazioni PGO disabilitate perché le ottimizzazioni provocano l'incoerenza dei dati di profilo|
|Avviso del compilatore (livello 1) C4963|'*descrizione*': dati di profilo non trovati; nella compilazione instrumentata sono state usate opzioni di compilatore differente|
|[Avviso del compilatore (livello 1) C4964](compiler-warning-level-1-c4964.md)|Nessuna opzione di ottimizzazione specificata; le informazioni sul profilo non verranno raccolti|
...
|Avviso del compilatore (livello 1) C4966|'*funzione*' è disponibile un'annotazione code_seg con nome segmento non supportato, annotazione ignorato|
...
|[Avviso del compilatore C4986](compiler-warning-c4986.md)|'*dichiarazione*': la specifica di eccezione non corrisponde alla dichiarazione precedente|
...
|Avviso del compilatore (livello 1) C4998|PREVISIONE DISATTESA: *expectation*(*valore*)|
|Avviso C4999 del compilatore|Avviso sconosciuto scegliere supporto tecnico dal menu della Guida di Visual C++ o aprire il file della Guida di supporto tecnico per ulteriori informazioni|
...
|Avviso del compilatore (livello 4) C5031|#pragma warning (POP): probabile che non esista una corrispondenza uno stato di avviso inserito nel file diverso|
|Avviso del compilatore (livello 4) C5032|rilevato #pragma warning alcun warning (POP) #pragma corrispondente|
|Avviso del compilatore (livello 1) C5033|'*della classe di archiviazione*' non è più una classe di archiviazione supportati|
|Avviso C5034 del compilatore|utilizzo della funzione intrinseca '*intrinseco*', funzione *funzione* da compilare come codice guest|
|Avviso C5035 del compilatore|utilizzo della funzionalità '*feature*', funzione *funzione* da compilare come codice guest|
...
|[Avviso del compilatore (livello 4) C5038](c5038.md)|membro dati '*membro1*'verrà inizializzata dopo il membro dati'*membro2*'|
...
|Avviso del compilatore (livello 3) C5040|le specifiche di eccezione dinamiche sono valide solo in c++14 e versioni precedenti; considerato noexcept (false)|
|Avviso del compilatore (livello 1) C5041|'*definizione*': definizione out-of-line per un membro dati statici constexpr non è necessaria ed è deprecato in c++17|
|Avviso del compilatore (livello 3) C5042|'*dichiarazione*': le dichiarazioni di funzione in ambito blocco non possono essere specificato 'inline' in C++ standard, rimuovere l'identificatore 'inline'|
...
|Avviso del compilatore (livello 4) C5044|Un argomento all'opzione della riga di comando *opzione* punta a un percorso di '*percorso*' non esiste|
|[Avviso C5045 del compilatore](c5045.md)|Compilatore inserirà attenuazione Spectre per carico di memoria se /Qspectre passa specificato|
