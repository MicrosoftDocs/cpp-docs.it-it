...
|[Avviso del compilatore (livello 1) C4806](compiler-warning-level-1-c4806.md)|«*operazione*': operazione non affidabile: nessun valore di tipo '*type1*'promosso al tipo'*type2*' può eguagliare la costante data|
|[Avviso del compilatore (livello 1) C4807](compiler-warning-level-1-c4807.md)|«*operazione*': combinazione non affidabile del tipo '*type1*'e firmati campo di bit del tipo'*type2*»|
...
|[Avviso del compilatore (livello 1) C4812](compiler-warning-level-1-c4812.md)|tipo di dichiarazione obsoleto: usare '*new_syntax*' invece|
|[Avviso del compilatore (livello 1) C4813](compiler-warning-level-1-c4813.md)|«*funzione*': una funzione friend delle classi locali debba essere dichiarata in precedenza|
|[Avviso del compilatore (livello 4) C4816](compiler-warning-level-4-c4816.md)|«*param*': parametro contiene una matrice con dimensione zero che verrà troncata (a meno che l'oggetto viene passato per riferimento)|
|[Avviso del compilatore (livello 1) C4817](compiler-warning-level-1-c4817.md)|'*membro*': utilizzo non valido di '.' per accedere a questo membro; sostituito con '->' dal compilatore|
...
|[Avviso del compilatore (livello 1) C4822](compiler-warning-level-1-c4822.md)|'member function': funzione membro della classe locale non ha un corpo|
...
| Avviso del compilatore (livello 4) C4845 | «\_\_declspec (no\_init\_all)' viene ignorato se ' / d1initall\[0\|1\|2\|3]' non è stato specificato nella riga di comando |
| Avviso del compilatore (livello 4) C4846 | «*valore*' non è un argomento valido per ' / d1initall': il flag della riga di comando ignorato |
...
|[Genera un avviso C4900](compiler-warning-level-1-c4900.md)|Errata corrispondenza tra '*tool1*'version'*version1*"e"*tool2*'version'*version2*'|
...
|[Avviso del compilatore (livello 1) C4910](compiler-warning-level-1-c4910.md)|'\<identificatore >: 'dllexport' e 'extern' sono incompatibili in una creazione esplicita di istanza|
|[Avviso del compilatore (livello 1) C4912](compiler-warning-level-1-c4912.md)|«*attributo*': attributo ha un comportamento indefinito su un tipo annidato definito dall'utente|
...
|[Avviso del compilatore (livello 4) C4918](compiler-warning-level-4-c4918.md)|«*carattere*': carattere non valido nell'elenco di ottimizzazione pragma|
|[Avviso del compilatore (livello 1) C4920](compiler-warning-level-1-c4920.md)|Enumerazione enum membro member_1 = value_1 già presente in enum'enum ' come member_2 = value_2|
|Avviso del compilatore (livello 3) C4921|«*description*': valore dell'attributo '*attributo*' non deve essere specificato più volte|
|[Avviso del compilatore (livello 1) C4925](compiler-warning-level-1-c4925.md)|«*metodo*': Impossibile chiamare il metodo di interfaccia dispatch da script|
|[Avviso del compilatore (livello 1) C4926](compiler-warning-level-1-c4926.md)|«*identificatore*': simbolo già definito: attributi ignorati|
...
|[Avviso del compilatore (livello 4) C4932](compiler-warning-level-4-c4932.md)|Identifier (*identifier*) e \_identificatori (*identificatore*) non sono distinguibili|
|Avviso del compilatore (livello 1) C4934|'__delegate(multicast)' è deprecata, usare '\_delegate ' invece|
|[Avviso del compilatore (livello 1) C4935](compiler-warning-level-1-c4935.md)|Identificatore di accesso assembly modificato da '*accesso*'|
...
|[Avviso del compilatore (livello 4) C4937](compiler-warning-level-4-c4937.md)|«*text1*'e'*text2*'sono indistinguibili come argomenti per'*direttiva*»|
|[Avviso del compilatore (livello 4) C4938](compiler-warning-level-4-c4938.md)|'*var*': Variabile di riduzione a virgola mobile può causare risultati incoerenti in /fp: strict o #pragma fenv_access|
|[Genera un avviso C4939](compiler-warning-level-1-c4939.md)|#pragma vtordisp è deprecato e verrà rimosso in una futura versione di Visual C++|
|[Avviso del compilatore (livello 1) C4944](compiler-warning-level-1-c4944.md)|«*simbolo*': non è possibile importare il simbolo da '*assembly1*': come*simbolo*' esiste già nell'ambito corrente|
...
|[Avviso del compilatore (livello 1) C4951](compiler-warning-level-1-c4951.md)|«*funzione*' è stato modificato dopo la raccolta dei dati, profilo dati di profilo di funzione non utilizzati|
|[Avviso del compilatore (livello 1) C4952](compiler-warning-level-1-c4952.md)|«*funzione*': nessun dato di profilo trovati nel database di programma '*pgd_file*»|
|[Avviso del compilatore (livello 1) C4953](compiler-warning-level-1-c4953.md)|Entità incorporata '*funzione*' è stato modificato dopo la raccolta dei dati, profilo dati di profilo non utilizzati|
...
|[Avviso del compilatore (livello 1, errore) C4956](compiler-warning-c4956.md)|«*tipo*': questo tipo non verificabile|
|[Avviso del compilatore (livello 1, errore) C4957](compiler-warning-c4957.md)|«*cast*': cast esplicito da '*cast_from*'a'*cast_to*' non è verificabile|
|[Avviso del compilatore (livello 1, errore) C4958](compiler-warning-c4958.md)|«*operazione*': aritmetica dei puntatori non verificabile|
|[Avviso del compilatore (livello 1, errore) C4959](compiler-warning-c4959.md)|non è possibile definire il tipo non gestito '*tipo*' in /CLR: safe perché l'accesso ai membri produce codice non verificabile|
|[Avviso del compilatore (livello 4) C4960](compiler-warning-level-4-c4960.md)|«*funzione*' è troppo grande per la profilatura|
...
|[Avviso del compilatore (livello 4) C4962](compiler-warning-c4962.md)|«*funzione*': Ottimizzazioni PGO disabilitate perché le ottimizzazioni provocano l'incoerenza dei dati del profilo|
...
|[Avviso del compilatore (livello 1) C4997](compiler-warning-level-1-c4997.md)|«*classe*': la coclasse non implementa un'interfaccia COM o una pseudo-interfaccia|
|Avviso del compilatore (livello 1) C4998|PREVISIONE DISATTESA: *aspettativa*(*valore*)|
|[Genera un avviso C4999](compiler-warning-level-1-c4999.md)|Avviso sconosciuto scegliere supporto tecnico dal menu di Visual C++ o aprire il file della Guida di supporto tecnico per altre informazioni|
...
| Avviso del compilatore (livello 1) C5047 | usare conforme allo standard \_ \_se\_esistente con i moduli non è supportato |
| Avviso del compilatore (livello 1) C5048 | Utilizzo della macro '*nomemacro*' può comportare l'output non deterministico |
| Avviso del compilatore (livello 1) C5049 | '*string*': Incorporamento di un percorso completo può comportare output dipende dal computer |
| Avviso del compilatore (livello 1) C5050 | Ambiente incompatibili possibili durante l'importazione del modulo '*module_name*': *problema* |
| Avviso del compilatore (livello 1) C5100 | \_\_Valutazione della vulnerabilità\_ARGS\_ \_ è riservato per l'uso nelle macro variadic |
| Avviso del compilatore (livello 1) C5101 | utilizzo della direttiva del preprocessore nell'elenco di argomenti di macro di funzione è un comportamento non definito |
| Avviso del compilatore (livello 1) C5102 | verrà ignorata una definizione di macro della riga di comando non valido '*valore*' |
| Avviso del compilatore (livello 1) C5103 | operazione Incolla '*token1*'e'*token2*' non restituisce un token di pre-elaborazione valido |
| Avviso del compilatore (livello 1) C5104 | trovato '*string1*#*string2*'nell'elenco di sostituzione di macro, si intendeva'*string1*"" &*string2*'? |
