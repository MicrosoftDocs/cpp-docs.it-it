...
|Avviso|Messaggio|  
|-------------|-------------|  
|[Avviso del compilatore (livello 1) C4600](../../error-messages/compiler-warnings/compiler-warning-level-1-c4600.md)|#pragma '*nome macro*': prevista una stringa vuota non valida|  
...
|[Avviso del compilatore (livello 1) C4407](../../error-messages/compiler-warnings/compiler-warning-level-1-c4407.md)|cast tra rappresentazioni di puntatori a membri differenti; il compilatore potrebbe generare codice non corretto|  
...
|Avviso del compilatore (livello 1) C4417|una creazione di istanza di modello esplicita non può avere __declspec(code_seg(...)): ignorata|  
|Avviso del compilatore (livello 1) C4418|__declspec(code_seg(...)) ignorato per una enumerazione|  
...
|Avviso del compilatore (livello 1) C4425|Impossibile applicare l'annotazione SAL a '...'|  
|Avviso del compilatore C4426|flag di ottimizzazione modificati dopo l'inclusione dell'intestazione, potrebbe essere dovuto a optimize () #pragma|  
|Avviso del compilatore (livello 1) C4427|'*operatore*': overflow nella divisione delle costanti, un comportamento non definito|  
|[Avviso del compilatore (livello 4) C4429](../../error-messages/compiler-warnings/compiler-warning-level-4-c4429.md)|uno dei nomi di caratteri universali probabilmente incompleto o in formato non valido|  
|[Avviso del compilatore C4430](../../error-messages/compiler-warnings/compiler-warning-c4430.md)|identificatore di tipo mancante, verrà utilizzato int. Nota: In C++ non supportati int predefinito|  
...
|[Avviso del compilatore C4439](../../error-messages/compiler-warnings/compiler-warning-c4439.md)|'*funzione*': definizione di funzione con un tipo gestito nella firma deve avere un clrcall convenzione di chiamata|  
...
|Avviso del compilatore (livello 1) C4483|errore di sintassi: prevista parola chiave di C++|  
|[Avviso del compilatore C4484](../../error-messages/compiler-warnings/compiler-warning-c4484.md)|'*funzione_override*': il metodo di classe di riferimento di base di corrispondenze '*funzione_classe_base*', ma non è contrassegnato 'virtual', 'new' o 'override'; verrà utilizzato 'new' (e non 'virtual')|  
|[Avviso del compilatore C4485](../../error-messages/compiler-warnings/compiler-warning-c4485.md)|'*funzione_override*': il metodo di classe di riferimento di base di corrispondenze '*funzione_classe_base*', ma non è contrassegnato 'new' o 'override'; 'new' (e 'virtual') equivale a|  
...
|Avviso del compilatore (livello 1) C4492|'*function1*': il metodo di classe di riferimento di base di corrispondenze '*function2*', ma non è contrassegnato 'override'|  
|Avviso del compilatore (livello 3) C4493|Espressione DELETE non ha alcun effetto poiché il distruttore di '*tipo*' non dispone di accessibilità 'public'|  
...
|[Avviso del compilatore (livello 4) C4517](../../error-messages/compiler-warnings/compiler-warning-level-4-c4517.md)|le dichiarazioni di accesso sono deprecate. Le dichiarazioni using di membri rappresentano una più valida alternativa.|  
...
|Avviso C4519 del compilatore|argomenti di modello predefiniti consentiti solo nei modelli di classe|  
...
|[Avviso del compilatore (livello 1) C4550](../../error-messages/compiler-warnings/compiler-warning-level-1-c4550.md)|l'espressione restituisce una funzione senza elenco di argomenti|  
|[Avviso del compilatore (livello 1) C4551](../../error-messages/compiler-warnings/compiler-warning-level-1-c4551.md)|chiamata di funzione senza elenco di argomenti|  
...
|[Avviso del compilatore (livello 1) C4561](../../error-messages/compiler-warnings/compiler-warning-level-1-c4561.md)|'\_\_fastcall' incompatibile con il ' o clr' opzione: la conversione in '\_stdcall '|  
|Avviso del compilatore (livello 4) C4562|necessaria l'opzione '/clr' per le funzioni con prototipo completo. Conversione di '()' in '(void)'.|  
...
|[Avviso del compilatore (livello 4) C4571](../../error-messages/compiler-warnings/compiler-warning-level-4-c4571.md)|Messaggio informativo: la semantica di catch(...) è cambiata dopo Visual C++ 7.1; le eccezioni strutturate (SEH) non vengono più individuate|  
|[Avviso del compilatore (livello 1) C4572](../../error-messages/compiler-warnings/compiler-warning-level-1-c4572.md)|L'attributo [ParamArray] è deprecato in /clr, utilizzare '...' In alternativa|  
...
|Avviso del compilatore (livello 1) C4575|'\_\_vectorcall' incompatibile con il ' o clr' opzione: la conversione in '\_stdcall '|  
...
|Avviso del compilatore (livello 1) C4585|'*classe*': classe non sealed A WinRT 'public ref class' deve essere sealed o derivare da un oggetto esistente|  
|Avviso del compilatore (livello 1) C4586|'*tipo*': Impossibile dichiarare un tipo pubblico in uno spazio dei nomi principale chiamato 'Windows'|  
