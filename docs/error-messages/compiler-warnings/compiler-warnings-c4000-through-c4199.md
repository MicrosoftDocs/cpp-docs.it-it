...
|Avviso del compilatore (livello 1) C4002|troppi parametri effettivi per la macro 'identifier'|
...
|Avviso del compilatore (livello 1) C4006|previsto un identificatore dopo #undef|
...
|Avviso del compilatore (livello 3) C4008|'function': attributo 'l'attributo' ignorato|
...
|Avviso del compilatore (livello 1) C4025|'function': puntatore based passato a funzione con argomenti variabili: parametro 'numero_parametro'|
...
|Avviso del compilatore (livello 1) C4038|'modifier': modificatore di classe non valido|
|Avviso del compilatore (livello 1) C4041|limite del compilatore: termine output per il browser|
...
|Avviso del compilatore (livello 1) C4112|#line richiede un numero intero compreso tra 1 e 'line_count'|
...
|Avviso del compilatore (livello 1 e livello 4) C4115|'type': definizione di tipo racchiuso tra parentesi denominato|
...
|Avviso del compilatore (livello 1) C4122|'function': alloc_text utilizzabile solo per le funzioni con collegamento C|
...
|Avviso del compilatore (livello 1) C4145|'espressione1': espressione relazionale come espressione switch. possibile confusione con 'Espressione2'|
...
|Avviso del compilatore (livello 4) C4152|utilizzata estensione non standard, conversione di puntatore funzione/dati nell'espressione|
...
|Avviso del compilatore (livello 1) C4158|assuming #pragma pointers_to_members(full_generality, 'inheritance_type')|
...
|Avviso del compilatore (livello 1) C4160|#pragma ' pragma'(pop,...): non è stato trovato l'identificatore 'identifier'|
...
|Avviso del compilatore (livello 1) C4174|'name': non disponibile come componente #pragma|
...
|Avviso del compilatore (livello 1) C4182|#include a livello di annidamento è 'nest_count'; possibile ricorsione infinita possibile ricorsione infinita|
...
|Avviso del compilatore (livello 1) C4186|#import attributo 'attribute' richiede 'argument_count' argomenti. ignorato|
...
|Avviso del compilatore (livello 4) C4189|'identifier': variabile locale inizializzata ma non si fa riferimento|
...
|Avviso del compilatore (livello 3) C4191|' operator/operation': conversione non affidabile da 'type_of_expression' a 'type_required' \nCalling questa funzione tramite il puntatore risultante può causare l'arresto del programma|
