...
'Class': diretta di base 'Base2' non è accessibile. già base di 'Base1'  
 ...
 `Class` già deriva direttamente dalla `Base1`. `Base2` deriva anche da `Base1`. `Class` non può derivare da `Base2` perché che comporterebbe l'eredità (indirettamente) da `Base1` nuovamente, che non è valido perché `Base1` è già una classe base diretta.  
